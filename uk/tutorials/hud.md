---
brief: У цьому прикладі проєкту ви дізнаєтеся, як створювати ефекти для підрахунку очок.
layout: tutorial
locale: uk
title: Приклад коду HUD
---

# HUD — приклад проєкту {#hud-sample-project}

<iframe width="560" height="315" src="https://www.youtube.com/embed/NoPHHG2kbOk" frameborder="0" allowfullscreen></iframe>

У цьому прикладі проєкту, який можна [відкрити з редактора](/uk/manuals/project-setup/) або [завантажити з GitHub](https://github.com/defold/sample-hud), ми демонструємо ефекти для підрахунку очок. Очки з’являються у випадкових місцях на екрані, імітуючи гру, у якій гравець отримує очки в різних позиціях.

Після появи очки деякий час плавно підіймаються. Для цього ми спочатку робимо очки прозорими, а потім плавно проявляємо їхній колір. Також анімуємо їхній рух угору. Це реалізовано у функції `on_message()` нижче.

Потім вони рухаються вгору до загального рахунку у верхній частині екрана, де додаються до нього.
Під час руху вгору вони також стають трохи прозорішими. Це реалізовано у функції `float_done()`.

Коли вони досягають рахунку вгорі, їхні значення додаються до цільового рахунку, до якого поступово зростає загальний рахунок. Це реалізовано у функції `swoosh_done()`.

Під час оновлення скрипт перевіряє, чи збільшився цільовий рахунок і чи потрібно нарощувати загальний рахунок. Якщо так, загальний рахунок збільшується невеликим кроком.
Після цього анімується масштаб загального рахунку, створюючи ефект підстрибування. Це реалізовано у функції `update()`.

Щоразу, коли загальний рахунок збільшується, ми створюємо кілька маленьких зірок і анімуємо їхній рух від загального рахунку. Зірки створюються, анімуються й видаляються у функціях `spawn_stars()`, `fade_out_star()` і `delete_star()`.
...
У main.script ми отримуємо введення від дотиків або миші, а потім надсилаємо повідомлення скрипту GUI, який створює нові очки в позиції дотику.
