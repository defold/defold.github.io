---
brief: У цьому прикладі проєкту ви ознайомитеся з ефектами показу підрахунку очок після завершення рівня.
layout: tutorial
locale: uk
title: Зразок коду завершення рівня
---

# Завершення рівня — приклад проєкту {#level-complete-sample-project}

<iframe width="560" height="315" src="https://www.youtube.com/embed/tSdTSvku1o8" frameborder="0" allowfullscreen></iframe>

У цьому прикладі проєкту, який можна [відкрити з редактора](/uk/manuals/project-setup/) або [завантажити з GitHub](https://github.com/defold/sample-levelcomplete), ми демонструємо ефекти показу підрахунку очок після завершення рівня. Лічильник очок поступово збільшується до підсумкового результату, а коли кількість очок досягає певних порогових значень, з’являються три зірки. У прикладі також використовується перезавантаження, щоб швидко перевіряти результат під час підбирання значень.

Сцена запускається повідомленням із гри.
Повідомлення містить загальну кількість набраних очок і порогові значення, за яких мають з’явитися три зірки.
Коли це відбувається, текст заголовка («Level completed!») плавно з’являється, одночасно зменшуючись до звичайного розміру (100%). Це виконується в `on_message()` нижче.

Після завершення анімації тексту заголовка починається підрахунок очок. На кожному кроці поточна кількість очок трохи збільшується. Потім ми перевіряємо, чи подолано один із порогів появи зірок, і якщо так, запускаємо анімацію зірки (див. нижче). Доки цільової кількості очок не досягнуто, лічильник очок анімується з ефектом підстрибування.
Його масштаб також зростає до максимального значення в міру наближення до підсумкового результату. Так само його колір поступово змінюється з білого на зелений. Це виконується в `inc_score()`.

Кожна зірка плавно з’являється й зменшується до звичайного розміру. Це виконується в `animate_star()`.

Після завершення анімації зірки навколо неї по колу створюються менші зірки. Це виконується в `spawn_small_stars()`.

Потім вони анімовано розлітаються від великої зірки з випадковими параметрами. Під час розльоту їхня швидкість і масштаб задаються випадково. Далі вони поступово стають прозорими й зрештою видаляються. Це виконується в `animate_small_star()` і `delete_small_star()`.

Коли лічильник досягає загальної кількості очок, позначка рекорду плавно з’являється й зменшується до потрібного розміру на своєму місці. Цей процес запускається наприкінці `inc_score()` і виконується в `animate_imprint()`.

Функція `setup()` забезпечує правильні початкові значення вузлів. Викликаючи `setup()` з `on_reload()`, ми забезпечуємо правильне налаштування всіх елементів щоразу, коли скрипт перезавантажується з редактора Defold.
