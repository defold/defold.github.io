...
- anchor: movie-clips-and-game-objects
  title: Кліпи та ігрові об’єкти
- anchor: flashmanually-creating-movie-clips
  title: Flash — створення кліпів вручну
- anchor: defoldmanually-creating-game-objects
  title: Defold — створення ігрових об’єктів вручну
- anchor: flashreferencing-manually-created-movie-clips
  title: Flash — звертання до кліпів, створених вручну
- anchor: defoldgame-object-id
  title: Defold — ідентифікатор ігрового об’єкта
- anchor: flashdynamically-creating-movie-clips
  title: Flash — динамічне створення кліпів
- anchor: defoldcreating-game-objects-using-factories
  title: Defold — створення ігрових об’єктів за допомогою фабрик
- anchor: flashstage
  title: Flash — сцена
- anchor: defoldcollections
  title: Defold — колекції
- anchor: flashtimeline
  title: Flash — часова шкала
- anchor: defoldproperty-animation
  title: Defold — анімація властивостей
- anchor: flashdepth-index
  title: Flash — індекс глибини
- anchor: defoldz-position
  title: Defold — позиція z
- anchor: flash-hittestobject-and-hittestpoint-collision-detection
  title: Виявлення колізій у Flash за допомогою hitTestObject і hitTestPoint
- anchor: defoldcollision-objects
  title: Defold — об’єкти колізій
- anchor: flashevent-handling
  title: Flash — обробка подій
- anchor: defoldcall-back-functions-and-messaging
  title: Defold — функції зворотного виклику та передавання повідомлень
- anchor: flashbutton-symbols
  title: Flash — символи кнопок
- anchor: defoldgui-scenes-and-scripts
  title: Defold — сцени та скрипти GUI
- anchor: debugging
  title: Налагодження
...
![ігровий об’єкт і кліп](/manuals/images/flash/go_movieclip.png)

На відміну від кліпів Flash, ігрові об’єкти Defold не мають часових шкал. Натомість ігровий об’єкт складається з кількох компонентів (component). До компонентів належать, зокрема, спрайти, звуки й скрипти (докладніше про доступні компоненти дивіться в [документації про будівельні блоки](/uk/manuals/building-blocks) та пов’язаних статтях). Ігровий об’єкт на знімку екрана нижче складається зі спрайта та скрипту. Компонент-скрипт керує поведінкою й виглядом ігрових об’єктів протягом їхнього життєвого циклу:

![компонент-скрипт](/manuals/images/flash/script_component.png)
...
![ігрові об’єкти, створені вручну](/manuals/images/flash/manual_game_objects.png)
...
![ім’я екземпляра у Flash](/manuals/images/flash/flash_instance_name.png)
...
![ідентифікатор ігрового об’єкта](/manuals/images/flash/game_object_id.png)
...
Модель адресації та передавання повідомлень — ключові поняття розробки ігор на Defold. Їх докладно пояснено в [посібнику з адресації](/uk/manuals/addressing) та [посібнику з передавання повідомлень](/uk/manuals/message-passing).
...
![зв’язування ActionScript](/manuals/images/flash/actionscript_linkage.png)
...
![фабрика logo](/manuals/images/flash/logo_factory.png)
...
![компонент-фабрика](/manuals/images/flash/factory_component.png)
...
![часова шкала й сцена](/manuals/images/flash/stage.png)
...
Колекції — це контейнери, які використовуються в редакторі для організації ігрових об’єктів та інших колекцій. Вміст колекції також можна створювати скриптом під час виконання за допомогою [фабрики колекцій](/uk/manuals/collection-factory/#spawning-a-collection), яка працює так само, як звичайна фабрика ігрових об’єктів. Це корисно, наприклад, для створення груп ворогів або певної схеми розташування монет для збирання. На знімку екрана нижче ми вручну розмістили два екземпляри колекції `logos` у колекції `main`.

![колекція](/manuals/images/flash/collection.png)

Іноді потрібно завантажити цілком новий ігровий світ. Компонент [проксі колекції](/uk/manuals/collection-proxy/) дає змогу створити новий ігровий світ на основі вмісту файлу колекції. Це стане в пригоді, наприклад, для завантаження нових ігрових рівнів, мініігор або сюжетних вставок.
...
![часова шкала](/manuals/images/flash/timeline.png)
...
![покадрова анімація](/manuals/images/flash/flipbook.png)
...
![індекс глибини](/manuals/images/flash/depth_index.png)
...
![індекс глибини](/manuals/images/flash/depth_index_2.png)

## Defold — позиція z {#defoldz-position}

Позиції ігрових об’єктів у Defold представлені векторами з трьох змінних: x, y та z. Позиція z визначає глибину ігрового об’єкта. У стандартному [скрипті рендерингу](/uk/manuals/render) доступні позиції z лежать у діапазоні від -1 до 1.
...
![порядок за віссю z](/manuals/images/flash/z_order.png)
...
![порядок за віссю z](/manuals/images/flash/z_order_outline.png)
...
![перевірка колізії](/manuals/images/flash/hittest.png)
...
![перевірка колізії за обмежувальними рамками](/manuals/images/flash/hitboundingbox.png)
...
Усі перелічені вище функції зворотного виклику необов’язкові, і їх можна видалити, якщо вони не використовуються. Докладніше про налаштування введення дивіться в [посібнику з введення](/uk/manuals/input). Під час роботи з проксі колекцій часто виникає одна проблема — докладніше про неї дивіться в [цьому розділі](/uk/manuals/input/#input-dispatch-and-on_input) посібника з введення.
...
![кнопка](/manuals/images/flash/button.png)

## Defold — сцени та скрипти GUI {#defoldgui-scenes-and-scripts}

Defold не має вбудованого компонента кнопки, а також не дає змоги легко виявляти клацання за формою певного ігрового об’єкта так, як це працює з кнопками у Flash. Найпоширеніше рішення — використовувати компонент [GUI](/uk/manuals/gui), зокрема тому, що на позиції компонентів GUI в Defold не впливає ігрова камера (якщо вона використовується). API GUI також містить функції для визначення того, чи введення користувача, як-от клацання й дотики, відбувається в межах елемента GUI.
...
![без налагодження](/manuals/images/flash/no_debug.png)

Увімкнення налагодження фізики показує об’єкти колізій, додані до наших ігрових об’єктів:

![з налагодженням](/manuals/images/flash/with_debug.png)

Коли виникають колізії, відповідні об’єкти колізій підсвічуються. Також відображається вектор колізії:

![колізія](/manuals/images/flash/collision.png)

Насамкінець, інформацію про відстеження використання процесора й пам’яті дивіться в [документації профайлера](/ref/profiler/). Докладніше про складніші методи налагодження читайте в [розділі про налагодження](/uk/manuals/debugging) посібника Defold.
