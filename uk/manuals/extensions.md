...
- anchor: native-extensions
  title: Нативні розширення
- anchor: the-build-server
  title: Сервер збирання
- anchor: project-layout
  title: Структура проєкту
- anchor: manifest-files
  title: Файли маніфестів
- anchor: r8-keep-rules-for-android
  title: Правила збереження R8 для Android
- anchor: custom-resources
  title: Користувацькі ресурси
- anchor: sharing-an-extension
  title: Поширення розширення
- anchor: a-simple-example-extension
  title: Простий приклад розширення
- anchor: extension-lifecycle
  title: Життєвий цикл розширення
- anchor: defined-platform-identifiers
  title: Визначені ідентифікатори платформ
- anchor: build-server-logs
  title: Журнали сервера збирання
- anchor: example-extensions
  title: Приклади розширень
---
...
Defold дає змогу почати роботу з нативними розширеннями без налаштування завдяки хмарному збиранню. Будь-яке нативне розширення, розроблене й додане до ігрового проєкту безпосередньо або через [бібліотечний проєкт](/uk/manuals/libraries/), стає частиною звичайного вмісту проєкту. Не потрібно збирати спеціальні версії рушія й розповсюджувати їх серед учасників команди: це відбувається автоматично — кожен учасник команди, який збирає та запускає проєкт, отримає виконуваний файл рушія для цього проєкту з усіма вбудованими нативними розширеннями.

![Хмарне збирання](/manuals/images/extensions/cloud_build.png)

Defold надає хмарний сервер збирання безкоштовно й без обмежень використання. Сервер розміщено в Європі, а URL, на який надсилається нативний код, налаштовується у [вікні Editor Preferences](/uk/manuals/editor-preferences/#extensions) або за допомогою параметра командного рядка `--build-server` для [bob](/uk/manuals/bob/#usage). Якщо ви хочете налаштувати власний сервер, [дотримуйтеся цих інструкцій](/uk/manuals/extender-local-setup).
...
```
*ext.manifest*
: Папка розширення _обов’язково_ має містити файл *ext.manifest*. Це файл конфігурації з прапорцями й макровизначеннями, які використовуються під час збирання окремого розширення. Опис формату файлу наведено в [посібнику з маніфесту розширення](https://defold.com/uk/manuals/extensions-ext-manifests/).
...
: Ця необов’язкова папка містить скомпільовані бібліотеки, від яких залежить розширення. Файли бібліотек слід розміщувати в підпапках із назвами у форматі `platform` або `architecture-platform`, залежно від того, які архітектури підтримують ваші бібліотеки.
  ...
  {% include shared/uk/platforms.md %}
...
* `android` — у цій папці можна розмістити файл фрагмента маніфесту, який буде об’єднано з маніфестом основного застосунку ([як описано тут](/uk/manuals/extensions-manifest-merge-tool)).
  * Папка також може містити файл `build.gradle` із залежностями, які [оброблятиме Gradle](/uk/manuals/extensions-gradle).
...
* `ios` — у цій папці можна розмістити файл фрагмента маніфесту, який буде об’єднано з маніфестом основного застосунку ([як описано тут](/uk/manuals/extensions-manifest-merge-tool)).
  * Папка також може містити файл `Podfile` із залежностями, які [оброблятиме Cocoapods](/uk/manuals/extensions-cocoapods).
* `osx` — у цій папці можна розмістити файл фрагмента маніфесту, який буде об’єднано з маніфестом основного застосунку ([як описано тут](/uk/manuals/extensions-manifest-merge-tool)).
* `web` — у цій папці можна розмістити файл фрагмента маніфесту, який буде об’єднано з маніфестом основного застосунку ([як описано тут](/uk/manuals/extensions-manifest-merge-tool)).
...
Ці правила об’єднуються з вибраним файлом збереження проєкту, коли [ввімкнено R8](/uk/manuals/android/#enabling-r8).
...
![Маніфест](/manuals/images/extensions/manifest.png)
...
![Файл C++](/manuals/images/extensions/cppfile.png)
