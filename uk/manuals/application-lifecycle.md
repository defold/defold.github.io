...
- anchor: preinitialization
  title: Попередня ініціалізація
- anchor: collection-init
  title: Ініціалізація колекції
- anchor: post-update-in-initialization
  title: Дії після оновлення під час ініціалізації
- anchor: update-loop
  title: Цикл оновлення
- anchor: input-phase
  title: Етап введення
- anchor: update-phase
  title: Етап оновлення
- anchor: render-update-phase
  title: Етап оновлення рендерингу
- anchor: post-update-phase
  title: Етап дій після оновлення
- anchor: render-phase
  title: Етап рендерингу
- anchor: finalization
  title: Фіналізація
- anchor: dispatching-messages
  title: Доставлення повідомлень
---
...
![Огляд життєвого циклу](/manuals/images/application_lifecycle/application_lifecycle.png)
...
![Ініціалізація](/manuals/images/application_lifecycle/initialization.png)
...
![Попередня ініціалізація](/manuals/images/application_lifecycle/pre_init.png)
...
![Ініціалізація колекції](/manuals/images/application_lifecycle/collection_init.png)
...
![Дії після оновлення](/manuals/images/application_lifecycle/post_init.png)
...
Цілком можливо завантажити [проксі колекції](/uk/manuals/collection-proxy) під час `init()`, забезпечити ініціалізацію всіх об’єктів, які вона містить, а потім вивантажити колекцію через проксі — усе це до першого виклику `update()` компонента, тобто перш ніж рушій завершить етап ініціалізації та ввійде в цикл оновлення:
...
![Цикл оновлення](/manuals/images/application_lifecycle/update_loop.png)
...
![Етап оновлення](/manuals/images/application_lifecycle/update_phase.png)
...
Якщо для симуляції фізики використовується [фіксований крок часу](/uk/manuals/physics/#physics-updates), у всіх компонентах-скриптах також може викликатися функція `fixed_update()`. Ця функція корисна в іграх, що ґрунтуються на фізиці, коли ви хочете змінювати фізичні об’єкти через регулярні проміжки часу, щоб забезпечити стабільну симуляцію фізики.
...
![Етап оновлення рендерингу](/manuals/images/application_lifecycle/render_update_phase.png)
...
![Етап дій після оновлення](/manuals/images/application_lifecycle/post_update_phase.png)
...
![Етап рендерингу](/manuals/images/application_lifecycle/render_phase.png)

Потім виконується рендеринг графіки, а також візуального профайлера (див. [документацію з налагодження](/uk/manuals/debugging)). Після рендерингу графіки виконується захоплення відео.
...
(Докладніше див. у [посібнику з проксі колекцій](/uk/manuals/collection-proxy) та описі [`set_time_step`](/ref/collectionproxy#set-time-step))
...
![Фіналізація](/manuals/images/application_lifecycle/finalization.png)
...
![Доставлення повідомлень](/manuals/images/application_lifecycle/dispatch_messages.png)
...
Оскільки код `on_message()` будь-якого компонента-отримувача може надсилати додаткові повідомлення, диспетчер повідомлень продовжує рекурсивно доставляти надіслані повідомлення, доки черга не спорожніє. Однак кількість проходів диспетчера через чергу повідомлень обмежена. Докладніше див. у розділі [Ланцюги повідомлень](/uk/manuals/message-passing).