...
За замовчуванням Bob шукає проєкт у поточному каталозі та збирає ресурси, досяжні з `game.project`, у *build/default*. Ресурси, на які немає посилань, не компілюються. Якщо код під час виконання завантажує необроблені файли за шляхом, додайте їх через [налаштування проєкту Custom Resources](/uk/manuals/project-settings/#custom-resources); для інших ресурсів Defold потрібне посилання, досяжне з кореня збирання.
