---
brief: Цей посібник пояснює, як підключати незалежних від моделі агентів програмування до інтерфейсів автоматизації Defold із чітко визначеними вимогами до перевірки, дозволів і безпеки.
...
* [Скрипти редактора](/uk/manuals/editor-scripts) можуть надавати специфічні для проєкту операції з ресурсами та інструменти.
* [HTTP API редактора](/uk/manuals/editor-http-api) надає команди редактора, результати збирання, виведення консолі, пошук у довіднику, попередні перегляди, налаштування й маршрути скриптів редактора.
* [Сервіс рушія та API автоматизації середовища виконання](/uk/manuals/engine-service) надають поточний стан налагоджувального рушія, введення, знімки екрана й визначені розширеннями операції.
* [Bob](/uk/manuals/bob) забезпечує збирання з командного рядка, звіти, архіви й пакети.

Модель, доступна лише через інтерфейс чату, може пропонувати зміни коду, але не може самостійно інспектувати локальний проєкт або перевіряти запущений результат. Те, що агент насправді може спостерігати й робити, визначає додаткова інтеграція з його середовищем.
...
Для операцій редактора виявляйте поточний інтерфейс через `/openapi.json`, а не надавайте агенту постійно жорстко закодовану копію API. Для розширень середовища виконання перевіряйте їхню працездатність, версію API та можливості.
...
Defold Foundation не розробляє ці проєкти, не проводить їх аудит, не супроводжує й офіційно не підтримує їх. Перед установленням будь-якої інтеграції спільноти перевірте її поточний вихідний код, залежності, дозволи, мережеву поведінку й сумісність із використовуваною версією Defold.
...
Агенти повинні виконувати той самий [цикл інспектування, змінення, перевірки й оцінювання](/uk/manuals/automation/#the-automation-loop), що й будь-яка інша автоматизація.
...
Корисні рекомендації щодо автоматизованого тестування й перевірки описано в [цьому посібнику](/uk/manuals/automated-testing).

## Мультимодальне оцінювання {#multimodal-evaluation}

Агент із можливістю обробляти зображення може інспектувати [попередні перегляди редактора](/uk/manuals/editor-http-api/#rendering-scene-previews), знімки екрана середовища виконання, візуальні відмінності та знімки браузера.
...
Докладніше про попередні перегляди редактора, знімки екрана середовища виконання та візуальне інспектування читайте в [цьому посібнику](/uk/manuals/automated-testing).
...
* Виконуйте широку автономну роботу в окремій гілці, робочому дереві, тимчасовій копії, контейнері, пісочниці або під обліковим записом з обмеженими правами.
...
* Зберігайте придатну для перевірки різницю між версіями та детерміновані докази тестування перед прийняттям змін.
