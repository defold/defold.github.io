...
- anchor: receiving-messages
  title: Отримання повідомлень
- anchor: messaging-between-game-worlds
  title: Передавання повідомлень між ігровими світами
- anchor: message-chains
  title: Ланцюги повідомлень
---

# Передавання повідомлень {#message-passing}

Передавання повідомлень — це механізм у Defold, який дає ігровим об’єктам (game object) змогу спілкуватися один з одним. Цей посібник передбачає, що ви знайомі з [механізмом адресації](/uk/manuals/addressing) у Defold та [базовими будівельними блоками](/uk/manuals/building-blocks).

У Defold не використовується об’єктно-орієнтований підхід у тому сенсі, що ваш застосунок складається з ієрархій класів з успадкуванням та функціями-членами в об’єктах (як, наприклад, у Java, C++ або C#). Натомість Defold доповнює Lua простим і потужним об’єктно-орієнтованим дизайном: стан об’єкта зберігається всередині компонентів-скриптів (script component) і доступний через посилання `self`. Крім того, об’єкти можна повністю відокремити один від одного, використовуючи асинхронне передавання повідомлень як засіб комунікації.


## Приклади використання {#usage-examples}

Спочатку розгляньмо декілька простих прикладів використання. Уявіть, що ви створюєте гру з такою структурою:

1. Головна стартова колекція (bootstrap collection) містить ігровий об’єкт з компонентом GUI (GUI складається з мінімапи та лічильника рахунку), а також колекцію (collection) з ідентифікатором "level".
2. Колекція "level" містить два ігрові об’єкти: героя, яким керує гравець, і ворога.

![Структура передавання повідомлень](/manuals/images/message_passing/message_passing_structure.png)

<div class='sidenote' markdown='1'>
Вміст цього прикладу міститься у двох окремих файлах: один призначений для головної стартової колекції, а другий — для колекції з ідентифікатором "level". Однак імена файлів _не мають значення_ у Defold. Значення мають ідентифікатори, які ви призначаєте екземплярам (instance).
</div>

Гра складається з декількох простих механік, які потребують комунікації між об’єктами:

![Передавання повідомлень](/manuals/images/message_passing/message_passing.png)

① Герой б’є ворога
: У межах цієї механіки повідомлення `"punch"` надсилається від компонента-скрипту об’єкта "hero" до компонента-скрипту об’єкта "enemy". Віддамо перевагу відносній адресації, тому що обидва об’єкти перебувають в одному місці ієрархії колекції:
  ...
  У компоненті-скрипті ворога створіть функцію, щоб отримати повідомлення:
...
: Щоразу, коли гравець долає ворога, його рахунок збільшується. Повідомлення `"update_score"` також надсилається від компонента-скрипту ігрового об’єкта "hero" до компонента "gui", що належить ігровому об’єкту "interface".
...
## Надсилання повідомлень {#sending-messages}
...
Щоб отримувати повідомлення, цільовий компонент-скрипт повинен містити функцію `on_message()`. Вона приймає чотири параметри:

`function on_message(self, message_id, message, sender)`

`self`
: Посилання на сам компонент-скрипт.
...
## Передавання повідомлень між ігровими світами {#messaging-between-game-worlds}

Якщо ви використовуєте компонент проксі колекції (collection proxy) для завантаження нового ігрового світу в середовище виконання, вам знадобиться надсилати повідомлення між ігровими світами. Припустімо, ви завантажили колекцію через проксі, а її властивість *Name* дорівнює "level":

![Ім’я колекції](/manuals/images/message_passing/collection_name.png)
...
```
Докладніший опис роботи проксі наведено в [посібнику з проксі колекцій](/uk/manuals/collection-proxy).

## Ланцюги повідомлень {#message-chains}
...
Бачимо, що ця версія рушія Defold виконує 10 проходів обробки черги повідомлень між викликом `init()` і першим викликом `update()`. Потім рушій виконує 75 проходів під час кожного наступного циклу оновлення.