...
- anchor: script-execution-callbacks-and-self
  title: Виконання скриптів, зворотні виклики та self
- anchor: reactive-logic
  title: Реактивна логіка
- anchor: preprocessing
  title: Попередня обробка
- anchor: editor-support
  title: Підтримка в редакторі
---

# Скрипти {#scripts}

Компоненти-скрипти (script components) дають змогу створювати ігрову логіку за допомогою [мови програмування Lua](/uk/manuals/lua).
...
: Розширення _.script_. Ці скрипти додаються до ігрових об’єктів (game objects) так само, як будь-який інший [компонент (component)](/uk/manuals/components), а Defold виконує код Lua в межах функцій життєвого циклу рушія. Скрипти ігрових об’єктів зазвичай використовують для керування ігровими об’єктами та логікою, що об’єднує гру в єдине ціле: завантаженням рівнів, правилами гри тощо. Скрипти ігрових об’єктів мають доступ до функцій [GO](/ref/go) та всіх функцій бібліотек Defold, крім функцій [GUI](/ref/gui) і [Render](/ref/render).
...
Коли компоненту-скрипту надсилають повідомлення через [`msg.post()`](/ref/msg#msg.post), рушій викликає цю функцію компонента-отримувача. Дізнайтеся [більше про передавання повідомлень](/uk/manuals/message-passing).
...
Якщо цей компонент отримав фокус введення (див. [`acquire_input_focus`](/ref/go/#acquire_input_focus)), рушій викликає цю функцію, коли реєструється введення. Дізнайтеся [більше про обробку введення](/uk/manuals/input).
...
#### `on_reload(self)`
Ця функція викликається, коли скрипт перезавантажується за допомогою функції гарячого перезавантаження редактора (<kbd>Edit ▸ Reload Resource</kbd>). Це дуже корисно для налагодження, тестування й підлаштування. Дізнайтеся [більше про гаряче перезавантаження](/uk/manuals/hot-reload).
...
![Автодоповнення](/manuals/images/script/completion.png)