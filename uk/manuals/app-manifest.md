...
- anchor: exclude-gui
  title: Виключення GUI (Exclude GUI)
- anchor: exclude-particle-fx
  title: Виключення ефектів частинок (Exclude Particle FX)
- anchor: exclude-tilemaps
  title: Виключення карт плиток (Exclude Tilemaps)
...
- anchor: use-rich-text
  title: Використання форматованого тексту (Use Rich Text)
...
## Виключення GUI (Exclude GUI) {#exclude-gui}

Вилучіть із рушія ресурси, компоненти й підтримку Lua для GUI. Увімкніть цей параметр лише тоді, коли проєкт не використовує сцени або скрипти GUI. Компоненти Label залишаються доступними. За замовчуванням цей параметр вимкнено.


## Виключення ефектів частинок (Exclude Particle FX) {#exclude-particle-fx}

Вилучіть ресурси й компоненти ефектів частинок та модуль Lua `particlefx`. Це також вилучає підтримку вузлів частинок у сценах GUI; сцени GUI без вузлів частинок залишаються підтримуваними. Перед увімкненням цього параметра вилучіть посилання на ефекти частинок і виклики їхніх API. За замовчуванням параметр вимкнено.


## Виключення карт плиток (Exclude Tilemaps) {#exclude-tilemaps}

Вилучіть ресурси й компоненти карт плиток та модуль Lua `tilemap`. Увімкніть цей параметр лише тоді, коли проєкт не використовує компоненти карт плиток або їхні API. Джерела плиток, які використовують інші компоненти, залишаються доступними. За замовчуванням цей параметр вимкнено.
...
Якщо ввімкнено (`true`), до рушія включається повна система компонування тексту з підтримкою формування гліфів, зокрема для мов із письмом справа наліво. Увімкніть цей параметр разом із `font.runtime_generation` у *game.project*, щоб генерувати шрифти SDF під час виконання з ресурсів TrueType (`.ttf`) або OpenType (`.otf`). Генерування під час виконання з ресурсів `.otf` підтримується починаючи з Defold 1.13.2. Докладніше читайте в [посібнику зі шрифтів](/uk/manuals/font/#enabling-runtime-fonts).


## Використання форматованого тексту (Use Rich Text) {#use-rich-text}

Включіть розбір форматованого тексту й ефекти стилів для написів і тексту GUI. За замовчуванням цей параметр увімкнено. Вимкніть його, щоб зменшити розмір рушія, якщо проєкту потрібен лише звичайний текст. Написи й текст GUI залишаються підтримуваними, але розмітка відображатиметься як звичайний текст без застосування форматування або ефектів.
