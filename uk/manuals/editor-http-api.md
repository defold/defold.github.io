...
- anchor: automating-the-defold-editor
  title: Автоматизація редактора Defold
- anchor: starting-the-editor-from-an-external-tool
  title: Запуск редактора із зовнішнього інструмента
- anchor: locating-the-editor-server
  title: Виявлення сервера редактора
- anchor: discovering-operations-through-openapi
  title: Виявлення операцій через OpenAPI
- anchor: executing-editor-commands
  title: Виконання команд редактора
- anchor: command-responses-and-asynchronous-work
  title: Відповіді команд і асинхронна робота
- anchor: building-html5
  title: Збирання HTML5
- anchor: searching-api-documentation
  title: Пошук документації API
- anchor: reading-console-output
  title: Читання виведення консолі
- anchor: rendering-scene-previews
  title: Рендеринг попередніх переглядів сцени
- anchor: executing-editor-lua
  title: Виконання Lua редактора
- anchor: modifying-resources-and-files
  title: Змінення ресурсів і файлів
- anchor: preferences
  title: Налаштування
- anchor: project-defined-routes
  title: Визначені проєктом маршрути
...
Установлені версії Defold можна знайти за допомогою `installations.json`, як описано в [посібнику редактора](/uk/manuals/editor/#editor-installation-metadata). Його поле `launcherPath` містить виконуваний файл для запуску. Передайте шлях до `game.project` як перший позиційний аргумент, щоб безпосередньо відкрити цей проєкт.
...
Редактор є графічним настільним застосунком. Запускайте його в інтерактивному сеансі користувача з доступом до дисплея. Використовуйте [Bob](/uk/manuals/bob), коли графічний сеанс недоступний, наприклад у CI без графічного інтерфейсу, або для автоматизації лише компіляції та створення автономних пакетів.
...
![Домашня сторінка локального сервера редактора](/manuals/images/automation/editor_server.png)
...
Зачекайте, доки URL-адреса стане доступною, перш ніж запускати браузерні тести. Докладніше дивіться в розділі [Браузерні тести для HTML5](/uk/manuals/automated-testing/#browser-tests-for-html5).
...
Про оформлення результатів тестування й класифікацію помилок читайте в розділі [Автоматизоване тестування й перевірка](/uk/manuals/automated-testing/#structured-test-results).
...
![Відрендерений редактором попередній перегляд головної колекції](/manuals/images/automation/main-preview.png)
...
![Відрендерений редактором попередній перегляд моделі куба](/manuals/images/automation/cube-preview.png)
...
Скрипти редактора можуть визначати додаткові маршрути за допомогою [`get_http_server_routes()`](/uk/manuals/editor-scripts/#http-server). Необов’язкова таблиця операції OpenAPI відкриває маршрут через той самий документ `/openapi.json`, що й вбудовані операції.
...
Сервер редактора належить процесу редактора. Запущена гра має інший порт та інші обов’язки, описані в [посібнику із сервісу рушія та HTTP API середовища виконання](/uk/manuals/engine-service).