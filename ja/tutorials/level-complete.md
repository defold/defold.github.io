---
brief: このサンプルプロジェクトでは、レベルをクリアしたときに使える、スコアをカウントアップして表示する演出を学びます。
layout: tutorial
locale: ja
title: レベルクリアのコードサンプル
---

# レベルクリア - サンプルプロジェクト {#level-complete-sample-project}

<iframe width="560" height="315" src="https://www.youtube.com/embed/tSdTSvku1o8" frameborder="0" allowfullscreen></iframe>

このサンプルプロジェクトでは、レベルをクリアしたときに使える、スコアをカウントアップして表示する演出を紹介します。プロジェクトは[エディターから開く](/ja/manuals/project-setup/)か、[GitHub からダウンロード](https://github.com/defold/sample-levelcomplete)できます。合計スコアをカウントアップし、異なる基準スコアに達するたびに、3つの星を1つずつ表示します。また、このサンプルではリロード（reload）機能を使い、数値の調整結果をすばやく確認できるようにしています。

このシーンは、ゲームからのメッセージ（message）をきっかけに開始します。
メッセージには、獲得した合計スコアと、3つの星を表示する基準スコアが含まれています。
メッセージを受信すると、見出しのテキスト（「Level completed!」）が通常のサイズ（100%）に縮小しながらフェードインします。この処理は、以下の `on_message()` で行います。

見出しのテキストのアニメーションが完了すると、合計スコアのカウントアップが始まります。カウントアップのたびに、現在のスコアに少しずつ加算します。続いて、いずれかの星の基準スコアに到達したかどうかを確認し、到達していれば星のアニメーションを開始します（後述）。目標スコアに達するまでは、合計スコアの表示に弾むようなアニメーションを付けます。
合計スコアに近づくほど、表示のスケールも最大値に向けて大きくなります。同じように、色も白から緑へ徐々に変化します。この処理は `inc_score()` で行います。

星が表示されるたびに、通常のサイズに縮小しながらフェードインします。この処理は `animate_star()` で行います。

星のアニメーションが終わると、その大きな星の周囲に小さな星を円状に生成します。この処理は `spawn_small_stars()` で行います。

続いて、小さな星が大きな星からランダムに飛び出すアニメーションを付けます。外側へ広がるときの速度とスケールは、どちらもランダムに設定します。その後、小さな星はフェードアウトし、最終的に削除されます。この処理は `animate_small_star()` と `delete_small_star()` で行います。

スコアが合計スコアに達すると、ハイスコアのスタンプがフェードインしながら縮小し、所定の位置に収まります。この処理は `inc_score()` の最後で開始し、`animate_imprint()` で実行します。

`setup()` 関数は、ノード（node）に正しい初期値を設定します。`on_reload()` から `setup()` を呼び出すことで、Defold エディターからスクリプトをリロードするたびに、すべてが正しく設定されるようにします。
