---
brief: このサンプルプロジェクトでは、スコアを加算するときのエフェクトを学びます。
layout: tutorial
locale: ja
title: HUD コードサンプル
---

# HUD - サンプルプロジェクト {#hud-sample-project}

<iframe width="560" height="315" src="https://www.youtube.com/embed/NoPHHG2kbOk" frameborder="0" allowfullscreen></iframe>

このサンプルプロジェクトは、[エディターから開く](/ja/manuals/project-setup/)か、[GitHub からダウンロードする](https://github.com/defold/sample-hud)ことができます。ここでは、HUD（ゲーム中の情報表示）でスコアを加算するときのエフェクトを紹介します。スコアは画面上のランダムな位置に表示され、プレイヤーがさまざまな位置でスコアを獲得するゲームを再現します。

スコアは表示された後、しばらく浮かび上がります。これを実現するため、スコアを透明に設定し、その色をフェードインさせます。また、上方向へ移動するアニメーションも設定します。この処理は、以下の `on_message()` で行います。

その後、スコアは画面上部の合計スコアまで移動し、そこで合算されます。
上へ移動する間、少しフェードアウトもします。この処理は `float_done()` で行います。

スコアが上部の合計スコアに到達すると、その値を目標スコアに加算します。合計スコアは、この目標スコアに向けて少しずつ増えていきます。この処理は `swoosh_done()` で行います。

スクリプトの更新時に、目標スコアが増え、合計スコアを増やす必要があるかどうかを確認します。この条件を満たす場合は、合計スコアを小刻みに増やします。
続いて、合計スコアのスケールをアニメーションさせ、弾むようなエフェクトを付けます。この処理は `update()` で行います。

合計スコアが増えるたびに、小さな星をいくつか生成し、合計スコアから外へ広がるアニメーションを設定します。星の生成、アニメーション、削除は、`spawn_stars()`、`fade_out_star()`、`delete_star()` で行います。
...
main.script では、タッチ入力やマウス入力を受け取り、GUI スクリプト（GUI script）へメッセージを送信して、タッチ位置に新しいスコアを作成します。
