...
Defold エディターの操作に慣れており、GL シェーダー（shader）と Defold のレンダリングパイプライン（rendering pipeline）の基礎を理解していることを前提とします。これらについて学ぶ必要がある場合は、[シェーダーマニュアル](/ja/manuals/shader/)と[レンダリングマニュアル](/ja/manuals/render/)を参照してください。
...
コンポーネントのシェーダープログラムは、ピクセルデータを変更するものや、プログラムでまったく新しいピクセルの色を作り出すものに置き換えられます。その方法は [Shadertoy チュートリアル](/ja/tutorials/shadertoy)で説明しています。
...
![レンダーターゲット](/tutorials/images/grading/render_target.png)
...
7. *`main.collection`* を開き、ゲームオブジェクト「`grade`」のモデルコンポーネントを選択して、*Material* プロパティを「`/main/grade.material`」に設定します。
   ...
   ![モデルのプロパティ](/tutorials/images/grading/model_properties.png)
...
![レンダーターゲットへのカラーグレーディング](/tutorials/images/grading/render_target_grading.png)
...
![ルックアップテクスチャ](/tutorials/images/grading/lut.png)
...
![ルックアップテーブル](/tutorials/images/grading/lut_lookup.png)
...
![ルックアップテーブルのフィルタリング](/tutorials/images/grading/lut_filtering.png)
...
3. *`Filter min`* プロパティを `FILTER_MODE_MIN_LINEAR` に、*`Filter mag`* プロパティを `FILTER_MODE_MAG_LINEAR` に設定します。
    ...
    ![ルックアップテーブルのサンプラー](/tutorials/images/grading/material_lut_sampler.png)

4. 次のルックアップテーブルのテクスチャ（*`lut16.png`*）をダウンロードし、プロジェクトに追加します。

    ![16色のルックアップテーブル](/tutorials/images/grading/lut16.png)

5. *`main.collection`* を開き、*`lut`* テクスチャプロパティを、ダウンロードしたルックアップテクスチャに設定します。

    ![クワッドモデルのルックアップテーブル](/tutorials/images/grading/quad_lut.png)
...
![ゲームワールドの元の外観](/tutorials/images/grading/world_original.png)

ここまではすべて正しくできているように見えますが、表面には見えない問題が潜んでいます。グラデーションのテスト用テクスチャを使ったスプライトを追加するとどうなるか、見てみましょう。

![青のバンディング](/tutorials/images/grading/blue_banding.png)
...
![バンディングのない青](/tutorials/images/grading/blue_no_banding.png)
...
![Affinity で表示したゲームワールド](/tutorials/images/grading/world_graded_affinity.png)
...
![グレーディングしたゲームワールド](/tutorials/images/grading/world_graded.png)
