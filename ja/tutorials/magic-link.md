...
* 「Empty Project」テンプレートから[新しいプロジェクト](/ja/manuals/project-setup/#creating-a-new-project)を作成します。
...
![ゲームのルールの図解](/tutorials/images/magic-link/linker_rules.png)
...
![フォルダー構造](/tutorials/images/magic-link/linker_folders.png)
...
![プロジェクト設定](/tutorials/images/magic-link/linker_project_settings.png)

描画するスプライトの最大数も増やす必要があります。必要であれば、次のセクションに進み、スプライト数が上限に達したという通知がコンソールに表示された時点で、ここに戻ってもかまいません。

![ゲームの寸法とレイアウト](/tutorials/images/magic-link/linker_layout.png)
...
![スプライトの最大数](/tutorials/images/magic-link/linker_sprite_max_count.png)
...
![画像の追加](/tutorials/images/magic-link/linker_add_images.png)
...
![アトラスへの画像の追加](/tutorials/images/magic-link/linker_add_to_atlas.png)
...
![盤面のコレクション](/tutorials/images/magic-link/linker_board_collection.png)

![盤面のコレクションを起動時に読み込む設定](/tutorials/images/magic-link/linker_bootstrap_board.png)
...
![ブロックのゲームオブジェクト](/tutorials/images/magic-link/linker_block.png)
...
![ブロックのファクトリー](/tutorials/images/magic-link/linker_blockfactory.png)

これでゲームを実行すると、ランダムな色のブロックで埋まった盤面が表示されるはずです。

![最初のスクリーンショット](/tutorials/images/magic-link/linker_first_screenshot.png)
...
![入力バインディング](/tutorials/images/magic-link/linker_input_bindings.png)
...
![コネクターのゲームオブジェクト](/tutorials/images/magic-link/linker_connector.png)

![コネクターのファクトリー](/tutorials/images/magic-link/linker_connector_factory.png)
...
![ゲーム内のコネクター](/tutorials/images/magic-link/linker_connector_screen.png)
...
![ブロックを下に移動](/tutorials/images/magic-link/linker_blocks_slide.png)
...
![カバーのスプライト](/tutorials/images/magic-link/linker_cover.png)
...
![消灯時と点灯時のマジックブロック](/tutorials/images/magic-link/linker_magic_blocks.png)
...
![領域のマーク](/tutorials/images/magic-link/linker_regions.png)
...
![初めてのクリア](/tutorials/images/magic-link/linker_first_win.png)
...
![ドロップ](/tutorials/images/magic-link/linker_drop.png)
...
![メインメニューの GUI](/tutorials/images/magic-link/linker_main_menu.png)
...
コレクションプロキシ（collection proxy）を使うと、実行中のゲームでコレクションを動的に読み込んだり、アンロードしたりできます。コレクションプロキシは指定したコレクションファイルの代理として機能し、プロキシにメッセージを送信することで、動的なコレクションの読み込み、初期化、有効化、無効化、アンロードを行います。使い方の詳しい説明は、[コレクションプロキシのドキュメント](/ja/manuals/collection-proxy)を参照してください。

ここでは、コレクションプロキシコンポーネントの *Collection* プロパティに、「レベル」を含む *board.collection* を設定します。

![メインコレクション](/tutorials/images/magic-link/linker_main_collection.png)

ここで *game.project* を開き、起動時に読み込むコレクションを指定する *main_collection* を `/main/main.collectionc` に変更します。

![起動時に読み込むメインコレクション](/tutorials/images/magic-link/linker_bootstrap_main.png)
...
![盤面の GUI](/tutorials/images/magic-link/linker_board_gui.png)
...
![やり直し用の GUI](/tutorials/images/magic-link/linker_restart_gui.png)
...
![レベルクリアのダイアログ](/tutorials/images/magic-link/linker_level_complete_gui.png)
...
![現在のレベルを表示する GUI](/tutorials/images/magic-link/linker_present_level_gui.png)
...
![ドロップの空きがないことを示す GUI](/tutorials/images/magic-link/linker_no_drop_room_gui.png)
...
![完成した盤面のコレクション](/tutorials/images/magic-link/linker_board_collection_final.png)
...
![完成したゲーム](/tutorials/images/magic-link/linker_game_finished.png)
