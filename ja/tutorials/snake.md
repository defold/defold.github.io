...
![サムネイル](/tutorials/images/snake/thumbnail.png)
...
このチュートリアルは初心者向けですが、Defold もゲーム開発もまったく初めての方には、まず入門マニュアル、特に [Defold の基本構成要素](/ja/manuals/building-blocks/)と[用語集](/ja/manuals/glossary/)を読むことをお勧めします。まだ Defold をダウンロードしていない場合は、[インストールマニュアル](/ja/manuals/install/)を確認してください。エディター自体にすぐに慣れるために、[エディターの概要](/ja/manuals/editor/)も確認することをお勧めします。このチュートリアルでも、各手順のスクリーンショットを掲載しています。
...
![起動画面](/tutorials/images/snake/1.png)
...
![画面設定](/tutorials/images/snake/2.png)

この設定にするのは、各マスが 16x16 ピクセルのグリッド上にゲームを描画するためです。こうすることで、ゲーム画面の端でマスの一部が切れるのを防げます。`game.project` ファイルには、プロジェクトの重要な設定がすべて含まれています。各設定については、[プロジェクト設定マニュアル](/ja/manuals/project-settings/)で確認できます。
...
![新しいフォルダー](/tutorials/images/snake/3.png)
...
![スネークのスプライト](/tutorials/images/snake/snake.png)

1. 上の画像を<kbd>右クリック</kbd>して、ローカルディスクに保存します。次に、ダウンロードした画像を、先ほどプロジェクトフォルダー内に作成した場所へドラッグ＆ドロップ（またはコピー＆ペースト）します。

![新しいフォルダー](/tutorials/images/snake/4.png)

アセットのインポートについて詳しくは、[こちら](/ja/manuals/importing-graphics/)も参照してください。

<input type="checkbox"/> 完了！

## タイルソースの追加 {#adding-a-tile-source}

Defold には組み込みの[タイルマップ](/ja/manuals/tilemap/)コンポーネント（component）があり、これを使って、*タイル*をグリッド状に並べたプレイフィールドを作成します。タイルマップでは個々のタイルを設定したり読み取ったりできるため、このゲームにぴったりです。タイルマップのグラフィックスは[タイルソース（tile source）](/ja/manuals/tilesource/)から取得するので、タイルソースを作成する必要があります。
...
![新しいタイルソース](/tutorials/images/snake/5.png)
...
![タイルマップ](/tutorials/images/snake/10.png)

3. 画面の端を囲む枠と、いくつかの障害物を描きます。
![完成したタイルマップ](/tutorials/images/snake/11.png)
...
![ゲームの実行](/tutorials/images/snake/14.png)
...
![アルゴリズム](/tutorials/images/snake/17.png)
...
![ゲームの実行](/tutorials/images/snake/snake_run_1.png)
...
![入力](/tutorials/images/snake/18.png)
