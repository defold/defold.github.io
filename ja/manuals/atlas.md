...
- anchor: runtime-texture-and-atlas-creation
  title: 実行時のテクスチャとアトラスの作成
- anchor: creating-a-texture-resource-at-runtime
  title: 実行時のテクスチャリソースの作成
...
: *Asset* ペインからエディタービューに画像をドラッグ＆ドロップします。
  ...
  ![アトラスの作成と画像の追加](/manuals/images/atlas/add.png)
  ...
  ![追加された画像](/manuals/images/atlas/single_images.png)
...
Size
: 生成されるテクスチャ（texture）リソースの合計サイズを、計算結果として表示します。幅と高さは、最も近い2の累乗に設定されます。テクスチャ圧縮を有効にすると、一部の形式では正方形のテクスチャが必要になります。その場合、正方形ではないテクスチャはサイズを変更し、空の領域を埋めて正方形にします。詳細は [テクスチャプロファイルマニュアル](/ja/manuals/texture-profiles/) を参照してください。
...
![複数ページのアトラス](/manuals/images/atlas/multipage_atlas.png)
...
![アトラスのプロパティ](/manuals/images/atlas/atlas_properties.png)
...
![画像のプロパティ](/manuals/images/atlas/image_properties.png)
...
テクスチャは、`go.set()` を使って [モデルコンポーネント](/ja/manuals/model/) に直接使用できます。
...
テクスチャを [スプライトコンポーネント](/ja/manuals/sprite/) で使用するには、先にアトラスでそのテクスチャを使う必要があります。[`resource.create_atlas(path, params)`](https://defold.com/ref/stable/resource/#resource.create_atlas:path-table) を使ってアトラスを作成します。
