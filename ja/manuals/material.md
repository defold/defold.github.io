...
`CONSTANT_TYPE_USER`
: シェーダープログラムに渡す任意のカスタムデータに使える vector4 定数です。定数の定義で初期値を設定でき、[go.set()](/ref/go/#go.set) / [go.animate()](/ref/go/#go.animate) 関数で変更できます。[go.get()](/ref/go/#go.get) で値を取得することもできます。単一のコンポーネントインスタンスのマテリアル定数を変更すると、[バッチ描画が分割され、追加のドローコールが発生します](/ja/manuals/render/#draw-calls-and-batching)。
...
: シェーダープログラムに渡す任意のカスタムデータに使える matrix4 定数です。定数の定義で初期値を設定でき、[go.set()](/ref/go/#go.set) / [go.animate()](/ref/go/#go.animate) 関数で変更できます。[go.get()](/ref/go/#go.get) で値を取得することもできます。単一のコンポーネントインスタンスのマテリアル定数を変更すると、[バッチ描画が分割され、追加のドローコールが発生します](/ja/manuals/render/#draw-calls-and-batching)。
