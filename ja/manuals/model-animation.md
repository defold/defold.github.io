...
アニメーション用の 3D データをモデルにインポートする方法の詳細は、[モデルのドキュメント](/ja/manuals/model)を参照してください。

  ![Blender のアニメーション](/manuals/images/animation/blender_animation.png)
  ![揺れる動きのループ](/manuals/images/animation/suzanne.gif)
...
`model.play_anim()` を使ってモデルアニメーションを進める方法に加えて、*モデル* コンポーネントは `cursor` プロパティ（property）を公開しており、`go.animate()` で操作できます（詳細は[プロパティアニメーション](/ja/manuals/property-animation)を参照してください）。
