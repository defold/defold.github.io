...
スプライトコンポーネントのグラフィックスには、[アトラス（atlas）](/ja/manuals/atlas) または [タイルソース（tile source）](/ja/manuals/tilesource) を使用できます。
...
{% include shared/ja/slice-9-texturing.md %}
...
{% include shared/ja/material-constants.md component='sprite' variable='tint' %}
...
スプライトでは、現在割り当てられているマテリアルの頂点属性（vertex attribute）をオーバーライドできます。これらの属性はコンポーネントから頂点シェーダーへ渡されます（詳しくは [マテリアルのマニュアル](/ja/manuals/material/#attributes) を参照してください）。
...
![スプライトの属性](/manuals/images/graphics/sprite-attributes.png)

## プロジェクトの設定 {#project-configuration}

*game.project* ファイルには、スプライトに関する [プロジェクト設定](/ja/manuals/project-settings#sprite) がいくつかあります。
...
そこで、[アトラス](/ja/manuals/material/) の `Rename patterns` を使って名前を変更します。
