...
デフォルトでは、Bob は現在のディレクトリでプロジェクトを探し、`game.project` から到達可能なリソース（resource）を *build/default* にビルドします。参照されていないリソースはコンパイルされません。コードが実行時にパスを指定して未加工のファイルを読み込む場合は、[プロジェクト設定の Custom Resources](/ja/manuals/project-settings/#custom-resources) を使って、それらを含めてください。それ以外の Defold リソースには、ビルドルートから到達可能な参照が必要です。
