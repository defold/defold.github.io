...
ネイティブコードをデバッグする最も簡単な方法は、[プリントデバッグ](http://en.wikipedia.org/wiki/Debugging#Techniques)を使うことです。[`dmLog` 名前空間](/ref/dmLog/)の関数を使って、変数を監視したり、実行の流れを示したりできます。いずれのログ関数を使っても、エディターの *Console* ビューと[ゲームログ](/ja/manuals/debugging-game-and-system-logs)に出力されます。
