...
レイキャスト（ray cast）は、直線状のレイに沿って物理ワールド（physics world）を調べるために使います。物理ワールド内でレイキャストを行うには、始点と終点の位置に加え、判定対象の[コリジョングループ（collision group）の集合](/ja/manuals/physics-groups)を指定します。
