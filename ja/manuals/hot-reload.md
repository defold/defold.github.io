...
![リソースのリロード](/manuals/images/hot-reload/menu.png)

## デバイス上でのホットリロード {#hot-reloading-on-device}

ホットリロードは、デスクトップだけでなくデバイス上でも動作します。デバイス上で使用するには、モバイルデバイスでゲームのデバッグビルドまたは[開発用アプリ](/ja/manuals/dev-app)を実行し、エディターでそのデバイスをターゲットとして選択します。

![ターゲットデバイス](/manuals/images/hot-reload/target.png)
...
![GUI のリロード](/manuals/images/hot-reload/gui.png)

新しいボタンを追加し、GUI ファイルを保存してホットリロードします。これでスマートフォンの画面に新しいボタンが表示されます。

![リロード後の GUI](/manuals/images/hot-reload/gui-reloaded.png)
...
`my_module.lua` を変更してリロードしても、`user.script` の動作は _変わりません_。その理由と、この落とし穴を避ける方法については、[モジュールのマニュアル](/ja/manuals/modules)を参照してください。
