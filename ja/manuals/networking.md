...
* [HTTP リクエスト](/ja/manuals/http-requests)
* [ソケット接続](/ja/manuals/socket-connections)
* [WebSocket 接続](/ja/manuals/websocket-connections)
* [オンラインサービス](/ja/manuals/online-services)
...
Defold は、オプションでセキュアな接続の SSL 証明書を検証することもできます。*game.project* の Network セクションにある [SSL Certificates 設定](/ja/manuals/project-settings/#network)) のフィールドに、CA ルート証明書の公開鍵または自己署名証明書の公開鍵を含む PEM ファイルを指定すると、SSL 検証が有効になります。CA ルート証明書の一覧は `builtins/ca-certificates` に含まれていますが、新しい PEM ファイルを作成し、ゲームの接続先サーバーに応じて必要な CA ルート証明書をコピーして貼り付けることをお勧めします。