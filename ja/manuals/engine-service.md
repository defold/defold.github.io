...
- anchor: extension-defined-runtime-routes
  title: 拡張機能で定義するランタイムルート
- anchor: automation-bridge-extension
  title: Automation Bridge 拡張
- anchor: editor-and-runtime-clients
  title: エディターとランタイムのクライアント
...
Defold エディターに属し、開いているプロジェクトを制御する[エディターサーバー（editor server）](/ja/manuals/editor-http-api)とは別のサービスです。
...
![Defold のデバッグビルドに表示されるエンジンサービスのポート情報](/manuals/images/automation/engine-service.png)
...
![エンジンサーバーへのアクセス](/manuals/images/automation/engine-server.png)
...
実行時のスクリプト、物理演算、入力、動的に作成されたオブジェクト、プラットフォームのレンダリングには、実行中のエンジンが必要です。これらは[ランタイムの自動テスト](/ja/manuals/automated-testing)で検証することをお勧めします。
