...
エディターはグラフィカルなデスクトップアプリケーションです。ディスプレイにアクセスできる対話型ユーザーセッションで起動してください。画面表示を伴わずに実行するヘッドレス（headless）CI など、グラフィカルなセッションが利用できない場合や、コンパイルのみの自動化、スタンドアロンのバンドル（bundle）の作成には、[Bob](/ja/manuals/bob) を使用してください。
...
ブラウザーテストを開始する前に、この URL が利用可能になるまで待ってください。詳細は、[HTML5 のブラウザーテスト](/ja/manuals/automated-testing/#browser-tests-for-html5) を参照してください。
