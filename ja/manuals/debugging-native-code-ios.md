...
* bob を使用し、`--with-symbols` オプションを付けてアプリのバンドル（bundle）を作成します（[詳細](/ja/manuals/debugging-native-code/#symbolicate-a-callstack)）：
...
* `.dSYM` フォルダー（デバッグシンボル）を取得します。
	...
	* ネイティブ拡張を使用している場合は、[bob.jar](https://www.defold.com/ja/manuals/bob/) でビルドすると `.dSYM` フォルダーが生成されます。ビルドだけで十分です（アーカイブやバンドルの作成は不要です）：
...
* 新しい Xcode プロジェクトを作成し、`Game` テンプレートを選択します。
	...
	![プロジェクトテンプレート](/manuals/images/extensions/debugging/ios/project_template.png)
...
* アプリにコードを追加します。
	...
	![ファイルの追加](/manuals/images/extensions/debugging/ios/add_files.png)

* 「Copy items if needed」のチェックが外れていることを確認します。

	![ソースの追加](/manuals/images/extensions/debugging/ios/add_source.png)

* 最終的に次のようになります。

	![追加されたソース](/manuals/images/extensions/debugging/ios/added_source.png)


* `Build` ステップを無効にします。

	![スキームの編集](/manuals/images/extensions/debugging/ios/edit_scheme.png)

	![ビルドの無効化](/manuals/images/extensions/debugging/ios/disable_build.png)

* `Deployment target` のバージョンを、デバイスの iOS バージョンより大きくなるように設定します。

	![デプロイの対象バージョン](/manuals/images/extensions/debugging/ios/deployment_version.png)

* 対象デバイスを選択します。

	![デバイスの選択](/manuals/images/extensions/debugging/ios/select_device.png)
...
2. または、`Attach to process by PID or Process name` を選択します。
	...
	![デバイスの選択](/manuals/images/extensions/debugging/ios/attach_to_process_name.png)
...
```
	...
	![dSYM の追加](/manuals/images/extensions/debugging/ios/add_dsym.png)
...
* プロジェクトビューでファイルを開き、ブレークポイントを設定します。
	...
	![ブレークポイント](/manuals/images/extensions/debugging/ios/breakpoint.png)
