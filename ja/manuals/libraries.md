...
共有するスプライト（sprite）とタイルソース（tile source）を含むライブラリを作成するとします。まず、[新しいプロジェクトを設定します](/ja/manuals/project-setup/)。プロジェクト内で共有するフォルダーを決め、そのフォルダー名をプロジェクト設定の *`include_dirs`* プロパティに追加します。複数のフォルダーを指定する場合は、名前をスペースで区切ります:

![共有するディレクトリ](/manuals/images/libraries/libraries_include_dirs.png)
...
![GitHub のライブラリ URL](/manuals/images/libraries/libraries_library_url_github.png)
...
<div class='important' markdown='1'>
サードパーティー製のライブラリは、使用前に必ず確認することを推奨します。詳しくは、[サードパーティー製ソフトウェアの利用を安全にする方法](https://defold.com/ja/manuals/application-security/#securing-your-use-of-third-party-software)を参照してください。
</div>
...
![依存関係](/manuals/images/libraries/libraries_dependencies.png)
...
![ライブラリの取得](/manuals/images/libraries/libraries_fetch_libraries.png)

これで、共有したフォルダーが *Assets ペイン* に表示され、共有したすべてのものを使用できます。ライブラリプロジェクトに加えられて同期された変更は、使用するプロジェクトで利用できます。

![ライブラリの設定完了](/manuals/images/libraries/libraries_done.png)
...
マテリアル（material）をコピーして貼り付け、特定の種類のすべてのコンポーネント（component）で使用したい場合は、[プロジェクトごとのテンプレート](/ja/manuals/editor/#creating-new-project-files)を使用すると便利な場合があります。
