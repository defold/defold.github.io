---
brief: このマニュアルでは、アプリケーションマニフェストのマージの仕組みを説明します。
github: https://github.com/defold/doc
layout: manual
locale: ja
title: ネイティブ拡張 - マニフェストマージツール
toc:
...
# アプリケーションマニフェスト {#application-manifests}

一部のプラットフォームでは、ネイティブ拡張（native extension）からアプリケーションマニフェスト（application manifest）の断片（スニペット、またはスタブ）を提供できます。
`AndroidManifest.xml`、`Info.plist`、または `engine_template.html` の一部を提供できます。

アプリケーションのベースマニフェストを起点に、各拡張のマニフェストスタブを順に適用します。
ベースマニフェストは、既定のもの（`builtins\manifests\<platforms>\...` 内にあります）か、ユーザーが提供するカスタムマニフェストのいずれかです。

## 命名と構造 {#naming-and-structure}

拡張が意図どおりに動作するには、拡張のマニフェストを所定のディレクトリ構造で配置する必要があります。
...
Android プラットフォームには既存のマニフェストマージツール（`ManifestMerger2` を基にしたもの）があり、Defold ではこれを `bob.jar` 内で使用してマニフェストをマージします。
Android マニフェストの変更方法について詳しくは、[Android のドキュメント](https://developer.android.com/studio/build/manifest-merge)を参照してください。

<div class='important' markdown='1'>
拡張のマニフェストでアプリの `android:targetSdkVersion` を設定しない場合、次の権限が自動的に追加されます:  `WRITE_EXTERNAL_STORAGE`、`READ_PHONE_STATE`、`READ_EXTERNAL_STORAGE`。詳しくは、公式ドキュメントの[こちらの説明](https://developer.android.com/studio/build/manifest-merge#implicit_system_permissions)を参照してください。
`<uses-sdk android:targetSdkVersion="{% raw %}{{android.target_sdk_version}}{% endraw %}" />` の使用を推奨します。
</div>
### 例 {#example}
...
`Info.plist` では、リストと辞書をマージするために独自の実装を使用しています。キーには、マージを制御するマーカー属性として `merge`、`keep`、または `replace` を指定できます。既定値は `merge` です。
...
HTML テンプレートでは、対応するセクションを照合できるように、各セクションに名前（たとえば「engine-start」）を付けています。
そのうえで、属性 `merge` または `keep` を指定できます。既定値は `merge` です。
