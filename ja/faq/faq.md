...
A: Defold プロジェクトのゲームロジックは、主に Lua 言語で記述します（具体的には Lua 5.1/LuaJIT です。詳しくは [Lua マニュアル](/ja/manuals/lua) を参照してください）。Lua は軽量な動的言語で、高速かつ非常に強力です。Defold は、Lua コードを出力するトランスパイラーをサポートしています。トランスパイラー拡張をインストールすると、[Teal](https://github.com/defold/extension-teal) などの別の言語を使って、静的な検査を受ける Lua コードを記述できます。また、ネイティブコード（プラットフォームに応じて C/C++、Objective-C、Java、JavaScript）を使って、[Defold エンジンを拡張して新しい機能を追加する](/ja/manuals/extensions/) こともできます。[カスタムマテリアル（material）](/ja/manuals/material/) を作成する場合は、OpenGL ES SL シェーダー言語を使って頂点シェーダーとフラグメントシェーダーを記述します。
...
A: 次のプラットフォームをサポートしています。
  ...
  **エンジンのランタイム**
  ...
  | iOS                | 15.0               | `arm-64`           |
  | Android            | 5.0 (API レベル 21) | `arm-32`, `arm-64`, `x86-64` |
...
A: ワンクリックで、PS4™、PS5™、Nintendo Switch、iOS（64-bit）、Android（32-bit と 64-bit）、HTML5 に加え、macOS（x86-64 と arm64）、Windows（64-bit）、Linux（x86-64 と arm64）向けに公開できます。1つのコードベースで複数のプラットフォームをサポートできます。


#### Q: Defold はどのレンダリング API を使っていますか？ {#q-what-rendering-api-does-defold-rely-on}

A: 開発者は、[すべてをスクリプトで制御できるレンダリングパイプライン](/ja/manuals/render/) を使う、単一のレンダリング API だけを扱えば十分です。Defold のレンダースクリプト（render script）API は、描画操作を次のグラフィックス API に変換します。

{% include shared/ja/graphics-api.md %}
...
{% include shared/ja/linux-faq.md %}


## Android に関する質問 {#android-questions}
{% include shared/ja/android-faq.md %}


## HTML5 に関する質問 {#html5-questions}
{% include shared/ja/html5-faq.md %}


## iOS に関する質問 {#ios-questions}
{% include shared/ja/ios-faq.md %}


## Windows に関する質問 {#windows-questions}
{% include shared/ja/windows-faq.md %}


## ゲーム機に関する質問 {#console-questions}
{% include shared/ja/consoles-faq.md %}
...
A: はい、サポートしています。Defold では [コレクション（collection）](/ja/manuals/building-blocks/#collections) と呼びます。複雑なゲームオブジェクト（game object）の階層を作成し、個別の基本構成要素として保存できます。これらは、エディター内や実行時（コレクションの生成による）にインスタンス化できます。GUI ノード（GUI node）については、GUI テンプレートをサポートしています。
...
A: ゲームオブジェクトの Z 位置は、描画順序を制御します。小さい値のものが、大きい値のものより先に描画されます。デフォルトのレンダースクリプトでは、深度が -1 から1の範囲にあるゲームオブジェクトが描画され、それより小さいものや大きいものは描画されません。レンダースクリプトについて詳しくは、公式の [レンダリングドキュメント](/ja/manuals/render) をご覧ください。GUI ノードでは Z 値は無視され、描画順序にまったく影響しません。代わりに、ノードはリストに並んでいる順序と子の階層（およびレイヤー構成）に従って描画されます。GUI の描画と、レイヤーを使ったドローコールの最適化について詳しくは、公式の [GUI ドキュメント](/ja/manuals/gui) をご覧ください。
...
A: 一般に、すべてのリソースは静的に宣言されているため、リソース管理を自分で行う必要がないという利点があります。[リソースプロパティ](/ja/manuals/script-properties/#resource-properties) を使って、コンポーネントに割り当てるリソースを変更できます。
...
A: はい、*physics.debug* フラグを *game.project* で設定します。（公式の [プロジェクト設定ドキュメント](/ja/manuals/project-settings/#debug) を参照してください。）


#### Q: 接触や衝突が多い場合、パフォーマンス上の負荷はどの程度ですか？ {#q-what-are-the-performance-costs-of-having-many-contactscollisions}

A: Defold は内部で Box2D の修正版を実行しており、パフォーマンス上の負荷はかなり近いはずです。[プロファイラー](/ja/manuals/debugging) を開けば、エンジンが物理処理に費やしている時間をいつでも確認できます。使用するコリジョンオブジェクト（collision object）の種類も考慮する必要があります。たとえば、静的オブジェクトはパフォーマンス上の負荷が小さくなります。詳しくは、Defold の公式 [物理ドキュメント](/ja/manuals/physics) を参照してください。
...
A: コレクションプロキシ（collection proxy）で読み込んだコレクションは、それぞれ独自の入力スタックを持ちます。入力は、メインコレクションの入力スタックからプロキシコンポーネントを経由して、コレクション内のオブジェクトへ送られます。つまり、読み込んだコレクション内のゲームオブジェクトが入力フォーカスを取得するだけでは不十分で、プロキシコンポーネントを _持つ_ ゲームオブジェクトも入力フォーカスを取得する必要があります。詳しくは、[入力ドキュメント](/ja/manuals/input) をご覧ください。


#### Q: 文字列型のスクリプトプロパティを使えますか？ {#q-can-i-use-string-type-script-properties}

A: いいえ。Defold は [hash](/ref/builtins#hash) 型のプロパティをサポートしています。これらは、種類、状態の識別子、あらゆる種類のキーを表すために使えます。ハッシュはゲームオブジェクトの ID（パス）の格納にも使えますが、エディターが関連する URL をドロップダウンに自動で列挙するため、[url](/ref/msg#msg.url) プロパティのほうが適していることがよくあります。詳しくは、[スクリプトプロパティのドキュメント](/ja/manuals/script-properties) をご覧ください。
