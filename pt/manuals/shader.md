...
* Veja o [manual de Render](/pt/manuals/render) para detalhes sobre o pipeline de renderização.
* Veja o [manual de Material](/pt/manuals/material) para detalhes sobre materiais.
* Veja o [manual de Compute](/pt/manuals/compute) para detalhes sobre programas de compute.
...
: Constantes de shader permanecem constantes durante a chamada de desenho de renderização. Constantes são adicionadas às seções *Constants* do arquivo de material e depois declaradas como `uniform` no programa de shader. Uniforms de sampler são adicionados à seção *Samplers* do material e depois declarados como `uniform` no programa de shader. As matrizes necessárias para realizar transformações de vértices em um vertex shader ficam disponíveis como constantes:
  ...
  - `CONSTANT_TYPE_WORLDVIEW`, `CONSTANT_TYPE_VIEWPROJ` e `CONSTANT_TYPE_WORLDVIEWPROJ` fornecem as matrizes combinadas correspondentes.
  - `CONSTANT_TYPE_WORLD_INVERSE`, `CONSTANT_TYPE_VIEW_INVERSE`, `CONSTANT_TYPE_PROJECTION_INVERSE`, `CONSTANT_TYPE_VIEWPROJ_INVERSE`, `CONSTANT_TYPE_WORLDVIEW_INVERSE` e `CONSTANT_TYPE_WORLDVIEWPROJ_INVERSE` fornecem matrizes inversas sem exigir que o shader as calcule.
  - `CONSTANT_TYPE_TIME` é um `vec4` fornecido pela engine: o tempo decorrido desde o início da engine em `.x`, o delta time do frame em `.y` e zero em `.z` e `.w`.
  - `CONSTANT_TYPE_USER` é uma constante do tipo `vec4` que você pode usar como quiser.

  O [manual de Material](/pt/manuals/material) explica como especificar constantes.
...
: Shaders podem declarar variáveis uniform do tipo *sampler*. Samplers são usados para ler valores de uma fonte de imagem:
  ...
  Você só pode usar um sampler nas funções de consulta de textura da biblioteca padrão GLSL. O [manual de Material](/pt/manuals/material) explica como especificar configurações de sampler.
...
O Defold gera qualificadores globais de precisão padrão ao compilar shaders para GLSL ES. Os padrões são `mediump` para valores de ponto flutuante e `highp` para inteiros. Eles podem ser alterados com as [configurações de projeto](/pt/manuals/project-settings/#shader) **GLSL ES Default Precision Float** (`shader.glsl_es_default_precision_float`) e **GLSL ES Default Precision Int** (`shader.glsl_es_default_precision_int`); ambas aceitam `mediump` ou `highp`.

Um qualificador explícito em uma variável, input ou output tem precedência sobre o padrão global gerado. Em fragment shaders OpenGL ES 2.0 e WebGL 1.0, nem todos os dispositivos oferecem suporte a `highp`. Quando `highp` é selecionado como padrão global, o Defold o protege com `GL_FRAGMENT_PRECISION_HIGH` e usa `mediump` como fallback nos dispositivos que não oferecem suporte.
