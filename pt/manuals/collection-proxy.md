...
Proxies de coleção diferem de [componentes de fábrica de coleção](/pt/manuals/collection-factory/). Uma fábrica de coleção instancia o conteúdo de uma coleção no mundo de jogo atual. Proxies de coleção criam um novo mundo de jogo em runtime e, portanto, têm casos de uso diferentes.
...
2. Defina a propriedade *Collection* para referenciar uma coleção que você deseja carregar dinamicamente no runtime em um momento posterior. Esta é uma dependência estática em tempo de build: a coleção referenciada e suas dependências são compiladas. A menos que *Exclude* esteja marcada, elas são incluídas no pacote principal. Quando *Exclude* está marcada, os recursos referenciados apenas por proxies excluídos podem ser omitidos do pacote principal para Live Update, e o proxy descarregado pode ser redirecionado em tempo de execução para outra coleção compilada, conforme descrito abaixo.

![adicionar componente proxy](/manuals/images/collection-proxy/create_proxy.png)

(Você pode excluir o conteúdo do build e baixá-lo com código em vez disso marcando a caixa *Exclude* e usando o [recurso Live Update](/pt/manuals/live-update/).)
...
![carregar](/manuals/images/collection-proxy/proxy_load.png)
...
[`collectionproxy.set_collection()`](/ref/collectionproxy/#collectionproxy.set_collection) pode redirecionar um proxy excluído e descarregado para uma coleção compilada, o que é útil após montar um pacote de Live Update. O proxy precisa estar com *Exclude* marcada e não pode estar carregado nem em processo de carregamento. O caminho precisa terminar em `.collectionc`. A coleção e todas as suas dependências precisam estar disponíveis para o sistema de recursos quando o proxy for carregado.

Verifique o valor de retorno antes de carregar o proxy. Inicialize e ative o novo mundo somente após receber `proxy_loaded`:
...
Chame `collectionproxy.set_collection("#level_proxy", nil)` enquanto o proxy não estiver carregado nem em processo de carregamento para restaurar a coleção atribuída no editor. Consulte o [manual de programação com Live Update](/pt/manuals/live-update-scripting/) para saber como baixar e montar conteúdo e a referência da API para conhecer os códigos de falha `collectionproxy.RESULT_*`.
...
![carregar](/manuals/images/collection-proxy/message_passing.png)

E, se você precisar se comunicar com um objeto de jogo na coleção carregada a partir do loader, pode enviar uma mensagem usando a [URL completa para o objeto](/pt/manuals/addressing/#urls):
...
* [Animações de propriedades de objetos de jogo e gui](https://defold.com/pt/manuals/animation/#property-animation-1)
* [Animações flipbook](https://defold.com/pt/manuals/animation/#flip-book-animation)
* [Simulações de Particle FX](https://defold.com/pt/manuals/particlefx/)
* Velocidade de timer
