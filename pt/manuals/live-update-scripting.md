...
Depois que um pacote for montado, um proxy excluído e descarregado também pode ser redirecionado para outra coleção compilada com `collectionproxy.set_collection()`. Consulte [Alterando a coleção de um proxy excluído](/pt/manuals/collection-proxy/#changing-an-excluded-proxys-collection) para conhecer as restrições e a sequência de carregamento.
