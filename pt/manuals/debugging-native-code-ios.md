...
* Empacote o app usando Bob, com a opção `--with-symbols` ([mais informações](/pt/manuals/debugging-native-code/#symbolicate-a-callstack)):
...
* Obtenha a pasta `.dSYM` (isto é, os símbolos de debug)
	...
	* Se você estiver usando uma extensão nativa, a pasta `.dSYM` é gerada quando você compila com [bob.jar](https://www.defold.com/pt/manuals/bob/). Apenas o build é necessário (sem archive ou empacotamento):
