...
* Reduzir overdraw
* Reduzir a complexidade dos shaders - Leia sobre otimizações GLSL [neste artigo da Khronos](https://www.khronos.org/opengl/wiki/GLSL_Optimizations). Você também pode modificar os shaders padrão usados pelo Defold (encontrados em `builtins/materials`) e escolher uma precisão menor onde o shader não precisar de `highp`. Shaders GLSL ES gerados por compilação cruzada usam por padrão `mediump` para valores de ponto flutuante e `highp` para inteiros, e esses padrões podem ser alterados nas configurações de projeto de Shader. Qualificadores explícitos por variável têm precedência. Consulte a [documentação de precisão de shaders](/pt/manuals/shader/#precision).
...
## Frustum culling
O script de renderização pode ignorar automaticamente a renderização de componentes de objetos de jogo que estejam fora de uma caixa delimitadora definida (frustum). Saiba mais sobre Frustum Culling no [manual do Pipeline de Renderização](/pt/manuals/render/#frustum-culling).
