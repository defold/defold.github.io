...
- anchor: heap-size
  title: Tamanho do heap
- Testando uma build HTML5
- anchor: creating-html5-bundle
  title: Criando um pacote HTML5
- anchor: webgl-context-version
  title: Versão do contexto WebGL
...
Você pode incluir uma das arquiteturas ou ambas. Quando as duas são incluídas, o loader seleciona `wasm_pthread-web` se houver suporte no navegador e no ambiente de hospedagem; caso contrário, usa `wasm-web` como fallback. Consulte o [manual do Bob](/pt/manuals/bob/#usage) para ver os nomes canônicos dos alvos.
...
Selecione o contexto gráfico solicitado por meio de [`graphics.webgl_version_hint`](/pt/manuals/project-settings/#webgl-version-hint). O padrão é WebGL 2; solicite WebGL 1 para testar ou usar esse contexto em navegadores compatíveis com as duas versões.
...
Desativar **Verify Downloaded File Size** mantém ativa qualquer verificação SHA-1 incluída no pacote. Consulte as [configurações de projeto HTML5](/pt/manuals/project-settings/#verify-downloaded-file-size).
...
* Suporte a gamepad - [Consulte a documentação de Gamepad](/pt/manuals/input-gamepads/#gamepads-in-html5) para considerações especiais e passos que talvez você precise seguir no HTML5.
...
Mais informações sobre cada opção estão disponíveis no [manual de configurações do projeto](/pt/manuals/project-settings/#html5).
...
Abra *game.project* como texto e confira `[section_name]` e o nome do campo que deseja usar. Então você pode usá-lo como um token: `{% raw %}{{section_name.field}}{% endraw %}` ou `{% raw %}{{{section_name.field}}}{% endraw %}`.
...
<div class='important' markdown='1'>
É importante que este bloco inline apareça antes de o script principal da aplicação ser carregado. Como ele inclui tags HTML, essa macro deve aparecer entre chaves triplas `{% raw %}{{{TOKEN}}}{% endraw %}` para impedir que sequências de caracteres sejam escapadas.
</div>
...
* [Uso de memória](/pt/manuals/optimization-memory)
* [Tamanho da engine](/pt/manuals/optimization-size)
* [Tamanho do jogo](/pt/manuals/optimization-size)
