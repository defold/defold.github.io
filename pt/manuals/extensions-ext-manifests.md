...
O manifesto da aplicação (extensão de arquivo `.appmanifest`) é uma configuração no nível da aplicação que define como compilar seu jogo nos servidores de build. O manifesto da aplicação permite remover partes da engine que você não usa. Se você não precisa de uma engine de física, pode removê-la do executável para reduzir seu tamanho. Saiba como excluir recursos não usados [no manual de manifesto da aplicação](/pt/manuals/app-manifest).
