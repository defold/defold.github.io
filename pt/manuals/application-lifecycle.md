...
![Lifecycle overview](/manuals/images/application_lifecycle/application_lifecycle.png)
...
![Initizalization](/manuals/images/application_lifecycle/initialization.png)
...
![Preinitialization](/manuals/images/application_lifecycle/pre_init.png)
...
![Collection Init](/manuals/images/application_lifecycle/collection_init.png)
...
É totalmente possível carregar um [proxy de coleção](/pt/manuals/collection-proxy) durante `init()`, garantir que todos os objetos contidos nele sejam inicializados e então descarregar a coleção pelo proxy, tudo isso antes que o primeiro `update()` de componente seja chamado, isto é, antes que a engine deixe a etapa de inicialização e entre no loop de atualização:
...
![Update Loop](/manuals/images/application_lifecycle/update_loop.png)
...
A entrada é lida dos dispositivos disponíveis, mapeada pelos [mapeamentos de entrada](/pt/manuals/input) e então despachada. Qualquer objeto de jogo que tenha adquirido foco de entrada recebe entrada em todas as funções `on_input()` dos seus componentes. Um objeto de jogo com um componente de script e um componente GUI com script de GUI receberá entrada nas funções `on_input()` de ambos os componentes, desde que elas estejam definidas e que os componentes tenham adquirido foco de entrada.

![Input Phase](/manuals/images/application_lifecycle/input_phase.png)
...
![Update Phase](/manuals/images/application_lifecycle/update_phase.png)
...
Se um [passo de tempo fixo](/pt/manuals/physics/#physics-updates) for usado para simulação de física, também pode haver uma chamada à função `fixed_update()` em todos os componentes de script. Essa função é útil em jogos baseados em física quando você quer manipular objetos físicos em intervalos regulares para obter uma simulação física estável.
...
![Render Update Phase](/manuals/images/application_lifecycle/render_update_phase.png)
...
![Post Update Phase](/manuals/images/application_lifecycle/post_update_phase.png)
...
![Render Phase](/manuals/images/application_lifecycle/render_phase.png)

Depois, os gráficos são renderizados, assim como qualquer renderização do perfilador visual (veja a [documentação de depuração](/pt/manuals/debugging)). Após a renderização dos gráficos, a captura de vídeo é feita.
...
(Consulte o [manual de proxy de coleção](/pt/manuals/collection-proxy) e [`set_time_step`](/ref/collectionproxy#set-time-step) para detalhes)
...
![Finalization](/manuals/images/application_lifecycle/finalization.png)
...
![Dispatch Messages](/manuals/images/application_lifecycle/dispatch_messages.png)
