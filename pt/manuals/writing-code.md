...
O Defold usa Lua 5.1 e LuaJIT (dependendo da plataforma-alvo), e você precisa seguir a especificação da linguagem para essas versões específicas de Lua ao escrever a lógica do jogo. Para mais detalhes sobre como trabalhar com Lua no Defold, veja nosso [manual Lua no Defold](/pt/manuals/lua).
...
O servidor de linguagem Lua incluído contém anotações de tipos para as APIs do Defold. O preenchimento automático, as informações ao passar o mouse e os diagnósticos reconhecem tipos do Defold como hashes, URLs, vetores e quatérnios, além de argumentos e valores de retorno de funções. O editor fornece anotações para scripts do jogo e para as APIs `editor.*` usadas em arquivos `.editor_script`. Não é necessária uma biblioteca de anotações separada para as APIs integradas ao usar o editor de código do Defold.

APIs de extensões de terceiros podem precisar de suas próprias anotações.

### Formatação de código {#formatting-code}

Selecione <kbd>Edit ▸ Format Document/Selection</kbd> ou pressione <kbd>Alt</kbd> + <kbd>Shift</kbd> + <kbd>F</kbd> para executar o formatador do servidor de linguagem. Com uma seleção, o editor formata as linhas selecionadas; sem seleção, formata o documento. A formatação exige um servidor de linguagem compatível com a operação de formatação correspondente.

Para formatar arquivos abertos e modificados ao salvar, ative **Format on save** em <kbd>Preferences ▸ Code</kbd>. Essa preferência vem desativada por padrão e exige um servidor de linguagem compatível com a formatação de documentos. Consulte [preferências de Code](/pt/manuals/editor-preferences/#code).
...
O editor de código do Defold fornece a funcionalidade básica necessária para escrever código, mas para casos de uso mais avançados ou para usuários experientes com um editor de código favorito, é possível fazer o Defold abrir arquivos usando um editor externo. Na [janela Preferences na aba Code](/pt/manuals/editor-preferences/#code), é possível definir um editor externo que deve ser usado ao editar código.
