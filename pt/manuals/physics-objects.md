...
Objetos dinâmicos
: Objetos dinâmicos são simulados pela engine de física. A engine resolve todas as colisões e aplica as forças resultantes. Objetos dinâmicos são bons para objetos que devem se comportar de forma realista. A forma mais comum de afetá-los é indiretamente, [aplicando forças](/ref/physics/#apply_force) ou alterando o [amortecimento](/ref/physics/#angular_damping) e a [velocidade](/ref/physics/#linear_velocity) angular, além do [amortecimento](/ref/physics/#linear_damping) e da [velocidade](/ref/physics/#angular_velocity) linear. Também é possível manipular diretamente a posição e a orientação de um objeto dinâmico quando a [configuração Allow Dynamic Transforms](/pt/manuals/project-settings/#allow-dynamic-transforms) está habilitada em *game.project*.

Objetos cinemáticos
: Objetos cinemáticos registram colisões com outros objetos de física, mas a engine de física não executa nenhuma simulação automática. O trabalho de resolver colisões, ou ignorá-las, fica por sua conta ([saiba mais](/pt/manuals/physics-resolving-collisions)). Objetos cinemáticos são muito bons para objetos controlados pelo jogador ou por script que exigem controle fino das reações físicas, como um personagem do jogador.

Gatilhos
: Gatilhos são objetos que registram colisões simples. Gatilhos são objetos de colisão leves. Eles são semelhantes a [ray casts](/pt/manuals/physics-ray-casts), no sentido de que leem o mundo de física em vez de interagir com ele. Eles são bons para objetos que só precisam registrar um acerto (como uma bala) ou como parte da lógica do jogo quando você quer disparar determinadas ações quando um objeto atinge um ponto específico. Gatilhos são computacionalmente mais baratos do que objetos cinemáticos e devem ser usados no lugar deles quando possível.
...
2. <kbd>Clique com o botão direito</kbd> no novo componente e selecione <kbd>Add Shape</kbd>. Em seguida, escolha uma forma: <kbd>Box</kbd>, <kbd>Capsule</kbd>, <kbd>Sphere</kbd>, <kbd>Hull</kbd> ou <kbd>Mesh</kbd> em projetos com física 3D, ou <kbd>Box</kbd> ou <kbd>Circle</kbd> em projetos com física 2D. As formas Hull e Mesh estão disponíveis desde o Defold 1.13.2 e usam uma malha nomeada de uma cena glTF ou GLB. Você pode adicionar várias formas ao componente. Também pode usar um tilemap ou um recurso `.convexshape` pela propriedade *Collision Shape*.
...
Um componente de colisão pode conter várias formas incorporadas, incluindo cascos convexos e malhas de triângulos na física 3D, ou usar um recurso de tilemap ou forma convexa. Saiba mais sobre as várias formas e como adicioná-las a um componente de colisão no [manual de Formas de Colisão](/pt/manuals/physics-shapes).
...
Collision Shape
: Um recurso de tilemap ou `.convexshape`. Para usar uma malha glTF ou GLB, adicione uma forma Hull ou Mesh ao componente e defina as propriedades *Scene* e *Mesh* dessa forma. Veja [Formas de Colisão para mais informações](/pt/manuals/physics-shapes).
...
Group
: O nome do grupo de colisão ao qual o objeto deve pertencer. Você pode ter 16 grupos diferentes e nomeá-los como achar adequado para o seu jogo. Por exemplo, `players`, `bullets`, `enemies` e `world`. Se *Collision Shape* estiver definido como um tile map, este campo não é usado; os nomes dos grupos são obtidos do tile source. [Saiba mais sobre grupos de colisão](/pt/manuals/physics-groups).

Mask
: Os outros _grupos_ com os quais este objeto deve colidir. Você pode nomear um grupo ou especificar vários grupos em uma lista separada por vírgulas. Se deixar o campo *Mask* vazio, o objeto não colidirá com nada. [Saiba mais sobre grupos de colisão](/pt/manuals/physics-groups).
