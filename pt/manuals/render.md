...
- anchor: multisampled-render-targets
  title: Alvos de renderização com múltiplas amostras
- Handles de textura
...
No centro do pipeline de renderização está o _script de renderização_. Esse é um script Lua com as funções `init()`, `update()` e `on_message()`, usado principalmente para interagir com a API gráfica subjacente. O script de renderização tem um lugar especial no ciclo de vida do seu jogo. Detalhes podem ser encontrados na [documentação do ciclo de vida da aplicação](/pt/manuals/application-lifecycle).
...
Uma descrição detalhada de como os materiais funcionam pode ser encontrada na [documentação de Material](/pt/manuals/material).
...
Ao usar o script de renderização padrão, se houver [componentes Camera](/pt/manuals/camera) ativados disponíveis no projeto, eles terão precedência sobre qualquer outra visualização/projeção definida no script de renderização. Para ler mais sobre como trabalhar com componentes de câmera em scripts de renderização, consulte a [documentação de Camera](/pt/manuals/camera).
...
1 = A caixa delimitadora de Mesh precisa ser definida pelo desenvolvedor. [Saiba mais](/pt/manuals/mesh/#frustum-culling).
