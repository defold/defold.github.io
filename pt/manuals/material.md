...
`CONSTANT_TYPE_USER`
: Uma constante vector4 que você pode usar para qualquer dado personalizado que queira passar aos seus programas de shader. Você pode definir o valor inicial da constante na definição da constante, mas ela é mutável pelas funções [go.set()](/ref/go/#go.set) / [go.animate()](/ref/go/#go.animate). Você também pode recuperar o valor com [go.get()](/ref/go/#go.get). Alterar uma constante de material de uma única instância de componente [quebra o batching de renderização e resultará em draw calls adicionais](/pt/manuals/render/#draw-calls-and-batching).
