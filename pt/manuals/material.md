...
* Para mais informações sobre o pipeline de renderização, consulte a [documentação de Render](/pt/manuals/render).
* Para uma explicação aprofundada dos programas de shader, consulte a [documentação de Shader](/pt/manuals/shader).
...
Tags
: As tags associadas ao material. Tags são representadas na engine como uma _bitmask_ usada por [`render.predicate()`](/ref/render#render.predicate) para coletar componentes que devem ser desenhados juntos. Veja a [documentação de Render](/pt/manuals/render) para saber como fazer isso. O número máximo de tags que você pode usar em um projeto é 32.
...
`CONSTANT_TYPE_USER`
: Uma constante vector4 que você pode usar para qualquer dado personalizado que queira passar aos seus programas de shader. Você pode definir o valor inicial da constante na definição da constante, mas ela é mutável pelas funções [go.set()](/ref/stable/go/#go.set) / [go.animate()](/ref/stable/go/#go.animate). Você também pode recuperar o valor com [go.get()](/ref/stable/go/#go.get). Alterar uma constante de material de uma única instância de componente [quebra o batching de renderização e resultará em draw calls adicionais](/pt/manuals/render/#draw-calls-and-batching).
...
Dentro de um script de GUI, leia e escreva as constantes de material de um node com `gui.get()` e `gui.set()`, em vez de usar as funções `go`. Há suporte a componentes de vetores, constantes de matriz e arrays de constantes. Os índices de array na tabela de opções começam em 1:
...
<div class='sidenote' markdown='1'>
Para que uma constante de material do tipo `CONSTANT_TYPE_USER` ou `CONSTANT_TYPE_USER_MATRIX4` esteja disponível usando `go.get()` e `go.set()`, ou `gui.get()` e `gui.set()`, ela precisa ser usada no programa de shader. Se a constante for definida no material, mas não usada no programa, ela será removida do material e não ficará disponível em tempo de execução.
</div>
...
Componentes sprite, tilemap, GUI e efeito de partículas vinculam automaticamente sua textura de imagem ao primeiro `sampler2D` declarado. Componentes sprite também oferecem suporte a várias texturas: cada sampler declarado no material se torna um slot de imagem nomeado no componente Sprite. A primeira textura fornece os dados de animação do sprite e controla a sequência de frames. Para cada frame, seu id de imagem é usado para localizar a imagem correspondente em cada textura adicional, que fornece suas próprias coordenadas UV. Portanto, os atlas ou tile sources atribuídos devem conter ids de frame correspondentes e imagens com formatos semelhantes; diferenças nas formas com compactação poligonal podem causar vazamento de textura. Consulte [Sprites com múltiplas texturas](/pt/manuals/sprite/#multi-textured-sprites) para obter detalhes.

Em um componente ou fluxo de renderização que não expõe um slot de textura adicional, use [`render.enable_texture()`](/ref/render/#render.enable_texture) para vincular samplers de textura adicionais a partir do script de renderização.
