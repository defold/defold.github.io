...
As configurações do solver do Box2D são específicas de cada versão. Consulte as [configurações de projeto do Box2D](/pt/manuals/project-settings/#box2d) para obter detalhes.
...
Controla a funcionalidade de rig e modelo, ou selecione None para excluir modelo e rig completamente. (Veja a documentação de [`Model`](https://defold.com/pt/manuals/model/#model-component)).
...
A configuração do manifesto do aplicativo controla se o código do profiler é vinculado a um build. As configurações em `profiler` no *game.project* controlam o comportamento do profiler em tempo de execução. Aprenda a usar os recursos disponíveis no [manual de Profiling](/pt/manuals/profiling/).
...
Inclui o suporte a recursos de som Ogg Opus. O decodificador Opus é excluído por padrão; portanto, esta opção deve ser ativada antes que recursos `.opus` possam ser reproduzidos. Consulte o [manual de Som](/pt/manuals/sound/) para saber quais formatos são compatíveis.
...
Exclui a funcionalidade [Live Update](/pt/manuals/live-update) da engine.
...
Exclui a biblioteca de compressão de textura [Basis Universal](/pt/manuals/texture-profiles) da engine.


## Usar Android Support Lib

Usa a biblioteca Android Support Library obsoleta em vez do AndroidX. [Mais informações](https://defold.com/pt/manuals/android/#using-androidx).
...
Se ativado (`true`), permite usar geração em tempo de execução para fontes do tipo SDF ao usar fontes True Type (`.ttf`) no projeto. Leia mais detalhes no [Manual de fontes](https://defold.com/pt/manuals/font/#enabling-runtime-fonts).
...
Esta opção fornece o valor padrão durante a compilação. O valor de [`html5.heap_size`](/pt/manuals/html5/#heap-size) no *game.project* o substitui em tempo de execução.
