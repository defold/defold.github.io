...
Se encontrar um problema enquanto estiver offline, você pode relatá-lo manualmente mais tarde usando a [opção Report Issue no menu Help](/pt/manuals/getting-help/#report-a-problem-from-the-editor) do editor.


## Buscando dependências

O Defold suporta um sistema em que desenvolvedores podem compartilhar código e assets por meio de algo chamado [Library Projects](/pt/manuals/libraries/). Bibliotecas são arquivos zip que podem ser hospedados em qualquer lugar online. Normalmente você encontra projetos de biblioteca do Defold no GitHub e em outros repositórios de código-fonte online.

Um projeto pode adicionar uma biblioteca como uma [dependência de projeto nas configurações do projeto](/pt/manuals/project-settings/#dependencies). Dependências são baixadas/atualizadas quando o projeto é aberto ou sempre que a opção *Fetch Libraries* é selecionada no menu *Project*.
...
O Defold suporta um sistema em que desenvolvedores podem adicionar código nativo para estender a funcionalidade da engine por meio de um sistema chamado [Native Extensions](/pt/manuals/extensions/). O Defold fornece um ponto de entrada sem configuração para extensões nativas com uma solução de build baseada em nuvem.
