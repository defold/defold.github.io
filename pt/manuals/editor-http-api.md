...
- anchor: automating-the-defold-editor
  title: Automação do editor Defold
- anchor: starting-the-editor-from-an-external-tool
  title: Inicialização do editor por uma ferramenta externa
- anchor: locating-the-editor-server
  title: Localização do servidor do editor
- anchor: discovering-operations-through-openapi
  title: Descoberta de operações por meio de OpenAPI
- anchor: executing-editor-commands
  title: Execução de comandos do editor
- anchor: command-responses-and-asynchronous-work
  title: Respostas de comandos e trabalho assíncrono
- anchor: building-html5
  title: Build para HTML5
- anchor: searching-api-documentation
  title: Busca na documentação de API
- anchor: reading-console-output
  title: Leitura da saída do console
- anchor: rendering-scene-previews
  title: Renderização de pré-visualizações de cenas
- anchor: executing-editor-lua
  title: Execução de Lua no editor
- anchor: modifying-resources-and-files
  title: Modificação de recursos e arquivos
- anchor: preferences
  title: Preferências
- anchor: project-defined-routes
  title: Rotas definidas pelo projeto
...
- anchor: engine-server
  title: Servidor do engine
---

# Automação do editor Defold {#automating-the-defold-editor}

O editor Defold abre um servidor especial para ações automatizadas. A API HTTP controla o projeto aberto. Use-a para comandos do editor, builds, recursos do projeto, pré-visualizações, preferências, saída do console, busca na documentação ou integrações de scripts do editor. Para inspecionar ou controlar o jogo em execução, use o [serviço do engine ou uma API de automação em tempo de execução](/pt/manuals/engine-service).
...
O editor é uma aplicação gráfica para desktop. Inicie-o em uma sessão interativa do usuário com acesso ao display. Use o [Bob](/pt/manuals/bob) quando uma sessão gráfica não estiver disponível, como em CI headless, ou para automação apenas de compilação e criação de pacotes autônomos.
...
![A página inicial do servidor local do editor](/manuals/images/automation/editor_server.png)
...
Aguarde até que a URL esteja disponível antes de iniciar os testes de navegador. Consulte [Testes em navegador para HTML5](/pt/manuals/automated-testing/#browser-tests-for-html5) para obter mais detalhes.
...
Para enquadramento de resultados de teste e classificação de falhas, consulte [Testes automatizados e verificação](/pt/manuals/automated-testing/#structured-test-results).
...
![Uma pré-visualização da coleção principal renderizada pelo editor](/manuals/images/automation/main-preview.png)
...
![Uma pré-visualização do modelo de cubo renderizada pelo editor](/manuals/images/automation/cube-preview.png)
...
<div class='important' markdown='1'>
Uma pré-visualização do editor não é uma captura de tela do jogo em execução. Ela não verifica objetos criados dinamicamente, pós-processamento em tempo de execução nem renderização específica da plataforma. Use uma [captura de tela em tempo de execução](/pt/manuals/automated-testing/#editor-previews-and-runtime-screenshots) quando esses elementos forem necessários.
</div>
...
O código avaliado pode usar a [API do editor](https://defold.com/ref/editor-lua/) e o ambiente de scripts do editor. Ele não pode usar APIs de tempo de execução do jogo, como `go.*`, para manipular um jogo em execução. Use um teste de tempo de execução, depurador, teste em navegador ou uma [API de automação em tempo de execução](/pt/manuals/engine-service/#automation-bridge-extension) para a jogabilidade.
...
Os scripts do editor podem definir rotas adicionais com [`get_http_server_routes()`](/pt/manuals/editor-scripts/#http-server). Uma tabela opcional de operações OpenAPI expõe uma rota por meio do mesmo documento `/openapi.json` que as operações integradas.
...
O servidor do editor pertence ao processo do editor. Um jogo em execução tem uma porta diferente e responsabilidades distintas, descritas no [manual do serviço do engine e da API HTTP de tempo de execução](/pt/manuals/engine-service).