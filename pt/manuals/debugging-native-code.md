...
* Xcode - macOS, iOS ([saiba mais](/pt/manuals/debugging-native-code-ios))
* Android Studio - Android ([saiba mais](/pt/manuals/debugging-native-code-android))
* WinDBG - Windows
...
Se o app for [debuggable](/pt/manuals/project-settings/#android), você pode obter o log de travamento usando a [ferramenta Android Debug Bridge (ADB)](https://developer.android.com/studio/command-line/adb.html) e o comando `adb shell`:
...
É importante que você combine a engine correta com a callstack; caso contrário, é muito provável que você acabe depurando coisas incorretas. Use a flag [`--with-symbols`](https://www.defold.com/pt/manuals/bob/) ao empacotar com [Bob](https://www.defold.com/pt/manuals/bob/) ou marque a checkbox "Generate debug symbols" na caixa de diálogo de empacotamento do editor:
...
* HTML5 - o diretório `<project_name>_symbols` ao lado do pacote HTML5 contém `<project_name>_wasm.js.symbols` e, quando a arquitetura `wasm_pthread-web` está selecionada, `<project_name>_pthread_wasm.js.symbols`.
...
Observação: se você obtiver um stack trace dos [logs do Android](/pt/manuals/debugging-game-and-system-logs), talvez consiga simbolicá-lo usando [ndk-stack](https://developer.android.com/ndk/guides/ndk-stack.html)
