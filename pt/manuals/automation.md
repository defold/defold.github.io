...
- anchor: deterministic-automation-or-ai-agents
  title: Automação determinística ou agentes de IA
- anchor: the-automation-loop
  title: O loop de automação
- anchor: next-steps
...
| Camada | Finalidade |
| --- | --- |
| [Scripts do editor](/pt/manuals/editor-scripts) | Comandos personalizados e fluxos de trabalho ou integrações do editor para acelerar testes e desenvolvimento, como a criação de níveis e assets |
| [Scripts da interface do editor](/pt/manuals/editor-scripts-ui/) | Ferramentas visuais, popups, configuradores ou interfaces de usuário personalizadas que utilizam scripts do editor |
| [API HTTP do editor](/pt/manuals/editor-http-api) | Controlar o projeto de jogo aberto no editor Defold por meio de operações OpenAPI, recursos do projeto, builds, comandos do editor, pré-visualizações, preferências, saída do console ou scripts do editor para operações personalizadas, ferramentas externas, integrações com IDEs e controladores de testes |
| [CLI Bob](/pt/manuals/bob) | Compilar um projeto, criar arquivos de dados ou pacotes autônomos pela linha de comando, relatórios, CI |
| [Hooks de ciclo de vida](/pt/manuals/editor-http-api#lifecycle-hooks) | Validação ou geração antes e depois de builds ou empacotamentos no editor |
| [Serviço HTTP do engine](/pt/manuals/engine-service) | Inspeção do engine de jogo Defold (`dmengine`) em execução, serviços de desenvolvimento, profiling, mensagens de tempo de execução ou APIs de automação em tempo de execução definidas por extensões; ferramentas externas consultando e enviando comandos a um build de depuração em execução |
...
| [Testes automatizados](/pt/manuals/automated-testing) | Testar lógica de jogo, mensagens, componentes, entrada, física e comportamento do engine, inspeção de cenas, feedback visual, por exemplo, por meio de [pré-visualização do editor](/pt/manuals/editor-http-api/#rendering-scene-previews), entrada injetada, estado ativo da aplicação e [coleções de teste em execução](/pt/manuals/automated-testing/#tests-in-a-running-collection) |
...
![O loop de automação de inspecionar, alterar, verificar e avaliar](/manuals/images/automation/automation_loop.png)
...
* [Automação das tarefas do editor Defold com a API HTTP](/pt/manuals/editor-http-api)
* [O serviço do engine e a API HTTP de tempo de execução](/pt/manuals/engine-service)
* [Testes automatizados e verificação](/pt/manuals/automated-testing)
* [Uso de agentes de programação com IA no Defold](/pt/manuals/ai-agents)