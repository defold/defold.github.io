...
Consulte o [manual de escrita de código](/pt/manuals/writing-code) para obter mais detalhes.
...
Leia mais no [manual da API HTTP do editor](/pt/manuals/editor-http-api/#reading-console-output).

### Logs persistidos {#persisted-logs}

O Defold também pode persistir o log do jogo ativando `Write Log File` em `game.project`. Consulte [Logs do jogo e do sistema](/pt/manuals/debugging-game-and-system-logs/). O registro em arquivo é útil para aplicações empacotadas e para testar dispositivos-alvo nos quais o console do editor não está disponível.
...
Leia o [manual do serviço do engine](/pt/manuals/engine-service/#automation-bridge-extension) para obter mais detalhes.
...
O editor pode criar e servir um build HTML5 por meio de seu comando atual `build-html5`, conforme descrito no [manual da API HTTP do editor](/pt/manuals/editor-http-api/#building-html5). O Bob também pode criar um pacote HTML5 sem o editor.
...
| Método | Finalidade |
| --- | --- |
| [Pré-visualização do editor](/pt/manuals/editor-http-api/#rendering-scene-previews) | Layout de um recurso carregado, como nível ou GUI, composição de atlas, inspeção de tile map, composição estática da cena, correção da renderização e dos shaders no editor ou criação de miniaturas para a documentação |
| [Captura de tela em tempo de execução](/pt/manuals/engine-service) | O estado renderizado de um build em execução em um cenário controlado |
...
O [manual do Bob](/pt/manuals/bob) descreve plataformas, arquivos de configurações, pacotes, caches, extensões nativas e relatórios de build.
...
O mesmo formato deve poder ser usado por um desenvolvedor, script local, serviço de CI ou [agente de programação com IA](/pt/manuals/ai-agents). Isso mantém a verificação determinística mesmo quando o diagnóstico ou a correção são delegados.