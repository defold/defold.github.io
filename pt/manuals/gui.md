...
![Novo arquivo GUI](/manuals/images/gui/new_gui_file.png)

O Defold agora abre automaticamente o arquivo no editor de cenas GUI.

![Nova GUI](/manuals/images/gui/new_gui.png)

O *Outline* lista todo o conteúdo da GUI: sua lista de nodes e quaisquer dependências (veja abaixo).

A área central de edição mostra a GUI. A barra de ferramentas no canto superior direito da área de edição contém as ferramentas *Move*, *Rotate* e *Scale*, além de um seletor de [layout](/pt/manuals/gui-layouts).

![barra de ferramentas](/manuals/images/gui/toolbar.png)
...
: O número máximo de texturas dinâmicas rastreadas por este componente GUI, `128` por padrão. Isso inclui texturas criadas com [`gui.new_texture()`](/ref/gui/#gui.new_texture:texture_id-width-height-type-buffer-flip) e texturas externas atribuídas à GUI com `go.set(..., "textures", ...)` ou `gui.set(msg.url(), "textures", ...)`. Projetos que substituem muitas texturas externas talvez precisem aumentar esse limite.
...
![dependências](/manuals/images/gui/dependencies.png)
...
: ![box node](/manuals/images/icons/gui-box-node.png)
  Node retangular com uma única cor, textura ou animação flip-book. Consulte a [documentação de Box node](/pt/manuals/gui-box) para detalhes.

<div style="clear: both;"></div>

Text node
: ![text node](/manuals/images/icons/gui-text-node.png)
  Exibe texto. Consulte a [documentação de Text node](/pt/manuals/gui-text) para detalhes.

<div style="clear: both;"></div>

Pie node
: ![pie node](/manuals/images/icons/gui-pie-node.png)
  Um node circular ou elipsoidal que pode ser parcialmente preenchido ou invertido. Consulte a [documentação de Pie node](/pt/manuals/gui-pie) para detalhes.

<div style="clear: both;"></div>

Template node
: ![template node](/manuals/images/icons/gui.png)
  Templates são usados para criar instâncias baseadas em outros arquivos de cena GUI. Consulte a [documentação de Template node](/pt/manuals/gui-template) para detalhes.

<div style="clear: both;"></div>

ParticleFX node
: ![particlefx node](/manuals/images/icons/particlefx.png)
  Reproduz um efeito de partículas. Consulte a [documentação de ParticleFX node](/pt/manuals/gui-particlefx) para detalhes.
...
![Adicionar nodes](/manuals/images/gui/add_node.png)
...
: Controlam a localização, orientação e alongamento do node. Você pode usar as ferramentas *Move*, *Rotate* e *Scale* para alterar esses valores. Os valores podem ser animados por script ([saiba mais](/pt/manuals/property-animation)).

Size (box, text and pie nodes)
: O tamanho do node é automático por padrão, mas ao definir *Size Mode* como `Manual`, você pode alterar o valor. O tamanho define os limites do node e é usado ao fazer picking de entrada. Esse valor pode ser animado por script ([saiba mais](/pt/manuals/property-animation)).
...
Slice 9 (box nodes)
: Defina para preservar o tamanho em pixels da textura do node ao redor das bordas quando o node for redimensionado. Consulte a [documentação de Box node](/pt/manuals/gui-box) para detalhes.

Inner Radius (pie nodes)
: O raio interno do node, expresso ao longo do eixo X. Consulte a [documentação de Pie node](/pt/manuals/gui-pie) para detalhes.

Outer Bounds (pie nodes)
: Controla o comportamento dos limites externos. Consulte a [documentação de Pie node](/pt/manuals/gui-pie) para detalhes.

Perimeter Vertices (pie nodes)
: O número de segmentos que será usado para construir a forma. Consulte a [documentação de Pie node](/pt/manuals/gui-pie) para detalhes.

Pie Fill Angle (pie nodes)
: Quanto da fatia deve ser preenchido. Consulte a [documentação de Pie node](/pt/manuals/gui-pie) para detalhes.

Template (template nodes)
: O arquivo de cena GUI a usar como template para o node. Consulte a [documentação de Template node](/pt/manuals/gui-template) para detalhes.

ParticleFX (particlefx nodes)
: O efeito de partículas a usar neste node. Consulte a [documentação de ParticleFX node](/pt/manuals/gui-particlefx) para detalhes.

Color
: A cor do node. Se o node tiver textura, a cor tinge a textura. A cor pode ser animada por script ([saiba mais](/pt/manuals/property-animation)).

Alpha
: A translucidez do node. O valor alpha pode ser animado por script ([saiba mais](/pt/manuals/property-animation)).
...
: Define o ponto de pivô do node. Isso pode ser visto como o "ponto central" do node. Qualquer rotação, escala ou mudança de tamanho acontecerá ao redor desse ponto.
  ...
  ![ponto de pivô](/manuals/images/gui/pivot.png)
...
: Ancoragem controla como a posição vertical e horizontal do node é alterada quando os limites da cena, ou os limites do node pai, são esticados para caber no tamanho físico da tela.
  ...
  ![Âncora sem ajuste](/manuals/images/gui/anchoring_unadjusted.png)
  ...
  ![Ancoragem](/manuals/images/gui/anchoring.png)
...
: Define o modo de ajuste do node. A configuração de modo de ajuste controla o que acontece com um node quando os limites da cena, ou os limites do node pai, são ajustados para caber no tamanho físico da tela.
  ...
  ![Sem ajuste](/manuals/images/gui/unadjusted.png)
...
: Define o modo de clipping no node:
  ...
  Consulte o [manual de clipping de GUI](/pt/manuals/gui-clipping) para detalhes.

Clipping Visible (box and pie nodes)
: Defina para renderizar o conteúdo do node na área de stencil. Consulte o [manual de clipping de GUI](/pt/manuals/gui-clipping) para detalhes.

Clipping Inverted (box and pie nodes)
: Inverte a máscara stencil. Consulte o [manual de clipping de GUI](/pt/manuals/gui-clipping) para detalhes.
...
![Ordem de desenho](/manuals/images/gui/draw_order.png)
...
![Pai filho](/manuals/images/gui/parent_child.png)
...
![Quebra de lote na hierarquia](/manuals/images/gui/break_batch.png)
