...
#### Dependencies
Uma lista de URLs para os *Library URL*s do projeto. Consulte o [manual de Bibliotecas](/pt/manuals/libraries/) para mais informações.
...
O carregamento de recursos personalizados é abordado em mais detalhes no [manual de Acesso a Arquivos](/pt/manuals/file-access/#how-to-access-files-bundled-with-the-application).

Caminhos contribuídos por extensões por meio de `custom_resources.default` em `ext.properties` são combinados com esta configuração. Consulte [recursos personalizados de extensões](/pt/manuals/extensions/#custom-resources) para ver um exemplo.

#### Bundle Resources
`bundle_resources`
{% include shared/pt/bundle-resources.md %}

O carregamento de recursos de pacote é abordado em mais detalhes no [manual de Acesso a Arquivos](/pt/manuals/file-access/#how-to-access-files-bundled-with-the-application).
...
#### Include Dirs
Uma lista de diretórios separados por espaço que devem ser compartilhados a partir do seu projeto via compartilhamento de biblioteca. Consulte o [manual de Bibliotecas](/pt/manuals/libraries/) para mais informações.
...
Esta configuração controla a janela. [Alvos de renderização com múltiplas amostras](/pt/manuals/render/#multisampled-render-targets) fora da tela têm sua própria contagem de amostras.
...
#### Display Profiles
Especifica qual arquivo de perfis de exibição usar, `/builtins/render/default.display_profilesc` por padrão. Saiba mais no [manual de Layouts de GUI](/pt/manuals/gui-layouts/#creating-display-profiles).
...
#### WebGL Version Hint
`graphics.webgl_version_hint` seleciona a versão do contexto WebGL a solicitar para HTML5. Os valores válidos são `1` (WebGL 1) e `2` (WebGL 2, o padrão). Defina como `1` para usar ou testar WebGL 1 mesmo em um navegador compatível com WebGL 2. Mantenha [Exclude GLES 2.0](#exclude-gles-20) desativado ao usar WebGL 1 para que os shaders necessários sejam incluídos.
...
Um script de GUI pode sobrescrever o modo de sua cena com [`gui.set_safe_area_mode()`](/ref/gui/#gui.set_safe_area_mode). Consulte as [orientações sobre área segura](/pt/manuals/porting-guidelines/#mobile-phones-and-notch-and-hole-punch-cameras) para informações sobre suporte nas plataformas e layouts personalizados.
...
#### Launch Screen
Arquivo Storyboard (.storyboard). Saiba mais sobre como criar um no [manual de iOS](/pt/manuals/ios/#creating-a-storyboard).
...
Selecione `/builtins/manifests/android/dmengine.keep` para usar diretamente as regras padrão do Defold. As extensões fornecem suas próprias [regras de preservação](/pt/manuals/extensions/#r8-keep-rules-for-android), que são combinadas com esse arquivo.
...
Consulte o [manual de Android](/pt/manuals/android/#shrinking-java-code-with-r8) para ativar o R8 e preservar seu mapeamento de ofuscação junto com um pacote de lançamento.
...
#### App Icon
Arquivo de imagem (.ico) a usar como ícone da aplicação no Windows. Leia mais sobre como criar um arquivo .ico no [manual de Windows](/pt/manuals/windows).

---

### HTML5

Consulte o [manual da plataforma HTML5](/pt/manuals/html5/) para mais informações sobre muitas dessas opções.
...
#### Retry Count
O número de novas tentativas após um download com falha durante a inicialização, incluindo erros de rede, status HTTP de falha e divergências de tamanho no arquivo JavaScript ou WebAssembly da engine. A requisição inicial é separada. A verificação dos arquivos do arquivo de jogo tem seu próprio limite de novas tentativas; consulte [verificação de downloads](/pt/manuals/html5/#download-verification) e `Retry Time`.

#### Retry Time
O número de segundos a aguardar entre tentativas de baixar um arquivo quando o download falha (veja `Retry Count`).

#### Verify Downloaded File Size
`html5.verify_downloaded_file_size` verifica se os arquivos baixados da engine e do arquivo de jogo têm os tamanhos esperados. Vem ativado por padrão (`true`). Defina como `false` somente se um servidor, proxy ou CDN reescrever intencionalmente os arquivos e alterar seus tamanhos. Uma falha na verificação provoca novas tentativas de download antes de a inicialização falhar. Os limites de novas tentativas diferem entre os downloads da engine e a verificação dos arquivos do arquivo de jogo; consulte [verificação de downloads](/pt/manuals/html5/#download-verification).
...
`liveupdate.enabled` ativa o sistema Live update em tempo de execução. Ativado por padrão. Consulte o [manual de Live update](/pt/manuals/live-update/) para saber como excluir, baixar e montar recursos.
...
#### _App Manifest_
Se definido, usa o app manifest para personalizar a build da engine. Isso permite remover partes não usadas da engine para diminuir o tamanho final do binário. Aprenda como excluir recursos não usados [no manual de manifesto da aplicação](/pt/manuals/app-manifest).

---

### Profiler

A configuração **Profiler** do Manifesto do aplicativo controla se o código do profiler é vinculado a builds debug e release. As configurações abaixo controlam o comportamento em tempo de execução do código do profiler presente no build selecionado. Consulte o [manual de Profiling](/pt/manuals/profiling/) para obter detalhes.
