...
: Aqui ficam seus projetos abertos recentemente, para que você possa acessá-los rapidamente. Esta é a visualização padrão da tela inicial.
  ...
  ![meus projetos](/manuals/images/editor/start_no_projects.png)


  Se você já abriu projetos antes, ela mostrará uma lista dos seus projetos, como na imagem abaixo:

  ![meus projetos](/manuals/images/editor/start_my_projects.png)
...
: Contém projetos preparados para demonstrar certos casos de uso.
  ...
  ![Novo projeto](/manuals/images/editor/start_templates.png)

Quando você cria um novo projeto, ele é armazenado no seu disco local, e todas as edições feitas são salvas localmente.

Você pode saber mais sobre as diferentes opções no [manual de configuração de projeto](https://www.defold.com/pt/manuals/project-setup/).
...
- [Navegação de cena 2D e 3D](/pt/manuals/scene-editing/#2d-and-3d-scene-orientation) com modos de câmera ortográfica e perspectiva
- [Ferramentas de transformação](/pt/manuals/scene-editing/#manipulating-objects) para mover, rotacionar e escalar objetos
- [Free Camera Mode](/pt/manuals/scene-editing/#free-camera-mode) para navegação 3D em primeira pessoa
- [Configurações de grid](/pt/manuals/scene-editing/#grid-settings) com tamanho, plano e aparência configuráveis
- [Filtros de visibilidade](/pt/manuals/scene-editing/#visibility-filters) para alternar tipos de componentes e guias

Leia mais no [manual do Scene Editor](/pt/manuals/scene-editing/).
...
*Curve Editor*: usada ao editar curvas no [Particle Editor](/pt/manuals/particlefx/).

O painel Tools também é usado para interagir com o depurador integrado. Leia mais sobre isso no [manual de depuração](/pt/manuals/debugging/).

### 6. Painel Changed Files

Se seu projeto usa o sistema de controle de versão distribuído Git, esta visualização lista quaisquer arquivos que foram alterados, adicionados ou excluídos no seu projeto. Ao sincronizar o projeto regularmente, você pode alinhar sua cópia local com o que está armazenado no repositório Git do projeto; dessa forma, pode colaborar em uma equipe e não perderá seu trabalho se algo der errado. Você pode aprender mais sobre Git no nosso [manual de controle de versão](/pt/manuals/version-control/). Algumas operações orientadas a arquivos podem ser executadas nesta visualização:
...
![Visibilidade dos painéis](/manuals/images/editor/editor_panes.png)
...
- Selecionar `➝| Open As` para usar um editor diferente do padrão ou uma ferramenta externa associada definida em `File ▸ Preferences ▸ Code ▸ Custom Editor`. Veja mais no [manual de Preferences](/pt/manuals/editor-preferences).

![Abas](/manuals/images/editor/tabs_custom.png)
...
![2 painéis](/manuals/images/editor/2-panes.png)
...
![criar arquivo](/manuals/images/editor/create_file.png)
...
![nome do arquivo a criar](/manuals/images/editor/create_file_name.png)

## Templates

É possível especificar templates personalizados para cada projeto. Para isso, crie uma nova pasta chamada `templates` no diretório raiz do projeto e adicione novos arquivos chamados `default.*` com as extensões desejadas, como `/templates/default.gui` ou `/templates/default.script`. Além disso, se o token `{% raw %}{{NAME}}{% endraw %}` for usado nesses arquivos, ele será substituído pelo nome de arquivo especificado na janela de criação de arquivo.
...
![Atualização na seleção de projeto](/manuals/images/editor/update_start.png)
![Atualização no editor](/manuals/images/editor/update_available.png)
...
![Popup de atualização do editor](/manuals/images/editor/update.png)

Você verá o progresso do download na barra de status inferior:

![Progresso do download](/manuals/images/editor/download_status.png)
...
![Reiniciar para atualizar](/manuals/images/editor/restart_to_update.png)
...
Leia mais detalhes no [manual de Preferences](/pt/manuals/editor-preferences)
...
![Mostrar logs](/manuals/images/editor/show_logs.png)
...
A aparência do editor pode ser alterada com estilização personalizada. Leia mais no [manual de estilização do editor](/pt/manuals/editor-styling.md).
