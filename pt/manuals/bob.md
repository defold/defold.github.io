...
`arm64-linux`
: Linux ARM64 para Raspberry Pi e dispositivos portáteis baseados em Linux.

`arm64_sim-ios`
: iOS Simulator em Macs com Apple Silicon. Bundles para o simulador são sempre assinados ad-hoc, portanto `--identity` e `--mobileprovisioning` são ignorados.
...
Por padrão, Bob procura um projeto no diretório atual e compila em *build/default* os recursos alcançáveis a partir de `game.project`. Recursos não referenciados não são compilados. Se o código carregar arquivos brutos por caminho em tempo de execução, inclua-os pela [configuração de projeto Custom Resources](/pt/manuals/project-settings/#custom-resources); outros recursos Defold precisam de uma referência alcançável a partir de uma raiz de build.
