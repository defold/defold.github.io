...
* Para mais informações sobre o pipeline de renderização, veja a [documentação de Render](/pt/manuals/render).
* Para uma explicação aprofundada de programas de shader, veja a [documentação de Shader](/pt/manuals/shader).
...
![Arquivo compute](/manuals/images/compute/compute_file.png)

O novo compute será aberto no *Compute Editor*.

![Editor compute](/manuals/images/compute/compute.png)
...
![Arquivo render compute](/manuals/images/compute/compute_render_file.png)
...
Use `graphics.get_adapter_info()` para verificar se o adaptador gráfico ativo oferece suporte a compute shaders. O campo `features` contém um array com as constantes de recursos de contexto compatíveis com o adaptador:
...
Apenas os recursos compatíveis são incluídos no array; ele não é uma tabela indexada pelas constantes de recursos. Sempre faça essa verificação antes de usar compute shaders quando o jogo puder ser executado com diferentes adaptadores gráficos ou em dispositivos com diferentes níveis de suporte do driver. O suporte a OpenGL e OpenGL ES depende da versão da API e do driver. Vulkan e Metal via MoltenVK oferecem suporte a compute shaders desde a versão 1.0. Use um [manifesto de aplicativo](/pt/manuals/app-manifest) para selecionar Vulkan em plataformas nas quais ele ainda não seja o backend gráfico padrão.