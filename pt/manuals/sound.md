...
*Sound*
: Deve ser definido para um arquivo de som no seu projeto. O arquivo deve estar no formato _Wave_, _Ogg Vorbis_ ou _Ogg Opus_. O Defold oferece suporte a arquivos Wave PCM de 8 e 16 bits. A reprodução de Ogg Opus é opcional e exige **Include Sound Decoder: Opus** no [Manifesto do aplicativo](/pt/manuals/app-manifest/#sound); o decodificador Opus não é incluído por padrão.
...
Quando **Use Linear Gain** está ativado nas [configurações de projeto de Sound](/pt/manuals/project-settings/#sound) (o padrão), o ganho de saída é o resultado desses quatro ganhos multiplicados. Um ganho de `1.0` é o ganho unitário (0 dB). Quando o ganho linear está desativado, o Defold aplica uma curva não linear durante a mixagem; portanto, a multiplicação direta dos quatro valores e a conversão para decibéis abaixo não descrevem o nível de saída resultante.
...
O arquivo *game.project* tem algumas [configurações do projeto](/pt/manuals/project-settings#sound) relacionadas a componentes de som.

## Streaming de som

Também é possível oferecer suporte a [streaming de sons](/pt/manuals/sound-streaming)