...
- anchor: creating-an-ios-application-bundle
  title: Criando um pacote de aplicação iOS
- Info.plist personalizado e descoberta de alvos locais
- Instalando e iniciando o pacote em um iPhone conectado
- anchor: creating-a-storyboard
  title: Criando um storyboard
...
O iOS exige que _qualquer_ app que você compile e queira executar no seu telefone ou tablet _seja_ assinado com um certificado emitido pela Apple e um perfil de provisionamento. Este manual explica as etapas envolvidas em empacotar seu jogo para iOS. Durante o desenvolvimento, executar seu jogo pelo [aplicativo de desenvolvimento](/pt/manuals/dev-app) costuma ser preferível, pois permite fazer hot reload de conteúdo e código diretamente para o dispositivo.
...
Selecione sua identidade de assinatura de código e procure seu arquivo de provisionamento mobile, bem como a variante (Debug ou Release). Opcionalmente, você pode desmarcar a caixa de seleção `Sign application` para pular o processo de assinatura e assinar manualmente em uma etapa posterior. Marque `Simulator` para criar um bundle `arm64_sim-ios` para o iOS Simulator em vez de um bundle para dispositivo.

<div class='important' markdown='1'>
Bundles para o simulador só rodam no iOS Simulator em Macs com Apple Silicon. Eles são sempre assinados ad-hoc, portanto as opções de assinatura, instalação e execução ficam desativadas quando `Simulator` está marcado. Instale o bundle com `xcrun simctl` como descrito abaixo.
</div>
...
Você especifica qual ícone usar para o app, o storyboard da tela de lançamento e assim por diante no arquivo de configurações do projeto *game.project*, na [seção iOS](/pt/manuals/project-settings/#ios).

### Info.plist personalizado e descoberta de alvos locais

O `Info.plist` integrado do iOS contém o serviço Bonjour e a descrição de uso da rede local necessários para a descoberta automática de alvos pelo editor em builds que não sejam release. Um `Info.plist` personalizado substitui esse manifesto base integrado. Se você usar um manifesto personalizado em uma build debug e precisar da descoberta de alvos, profiling, hot reload ou streaming de logs pela rede local, inclua estas entradas:
...
A condição Mustache mantém as entradas de descoberta fora dos pacotes release. A string da descrição de uso é exibida ao usuário pelo iOS e pode ser ajustada ou localizada. Remova a condição somente se a própria aplicação release usar o mesmo serviço Bonjour e a funcionalidade de rede local.

{% include shared/pt/build-variants.md %}
...
Se o editor não conseguir detectar o local de instalação da ferramenta ios-deploy, você precisará especificá-lo em [Preferências](/pt/manuals/editor-preferences/#tools). 
...
Posicione a imagem e faça quaisquer outros ajustes necessários, talvez adicionando um Label ou outro elemento de UI. Quando terminar, defina o scheme ativo como **Any iOS Device (arm64)** (ou **Generic iOS Device**) e selecione **Product ▸ Build**. O Defold oferece suporte ao iOS 15.0 e versões posteriores em dispositivos de 64 bits; portanto, mantenha o deployment target em 15.0 ou posterior. Aguarde o processo de build terminar.
...
{% include shared/pt/apple-privacy-manifest.md %}
...
Se você acredita que seu projeto está isento, defina a chave [`ITSAppUsesNonExemptEncryption`](https://developer.apple.com/documentation/bundleresources/information-property-list/itsappusesnonexemptencryption) como `False` no `Info.plist` do projeto; veja [Manifestos de Aplicação](/pt/manuals/extensions-manifest-merge-tool) para mais detalhes.

## FAQ
{% include shared/pt/ios-faq.md %}