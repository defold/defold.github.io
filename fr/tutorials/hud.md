---
brief: Dans cet exemple de projet, vous découvrez des effets pour le comptage des points.
layout: tutorial
locale: fr
title: Exemple de code de HUD
---

# HUD - exemple de projet {#hud-sample-project}

<iframe width="560" height="315" src="https://www.youtube.com/embed/NoPHHG2kbOk" frameborder="0" allowfullscreen></iframe>

Dans cet exemple de projet, que vous pouvez [ouvrir depuis l'éditeur](/fr/manuals/project-setup/) ou [télécharger depuis GitHub](https://github.com/defold/sample-hud), nous présentons des effets pour le comptage des points. Les scores apparaissent à des endroits aléatoires de l'écran, simulant un jeu où le joueur obtient des points à différentes positions.

Les scores flottent pendant un moment après leur apparition. Pour obtenir cet effet, nous rendons les scores transparents, puis faisons apparaître progressivement leur couleur. Nous animons également leur déplacement vers le haut. C'est ce que fait `on_message()` ci-dessous.

Ils remontent ensuite vers le score total en haut de l'écran, où ils sont additionnés.
Ils s'estompent également légèrement en remontant. C'est ce que fait `float_done()`.

Lorsqu'ils atteignent le score en haut de l'écran, leurs valeurs sont ajoutées à un score cible vers lequel le score total augmente progressivement. C'est ce que fait `swoosh_done()`.

À chaque mise à jour, le script vérifie si le score cible a augmenté et si le score total doit être augmenté progressivement. Si c'est le cas, le score total est incrémenté d'une valeur plus petite.
L'échelle du score total est ensuite animée pour donner un effet de rebond. C'est ce que fait `update()`.

Chaque fois que le total est incrémenté, nous créons un certain nombre de petites étoiles et animons leur éloignement du score total. Les étoiles sont créées, animées et supprimées dans `spawn_stars()`, `fade_out_star()` et `delete_star()`.
...
Dans main.script, nous traitons les entrées tactiles ou de souris, puis envoyons un message au script d'interface graphique pour créer de nouveaux scores à la position du contact.
