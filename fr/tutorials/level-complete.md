---
brief: Dans cet exemple de projet, vous apprenez à créer des effets pour afficher le comptage du score qui peut avoir lieu lorsqu'un niveau est terminé.
layout: tutorial
locale: fr
title: Exemple de code de fin de niveau
---

# Fin de niveau - exemple de projet {#level-complete-sample-project}

<iframe width="560" height="315" src="https://www.youtube.com/embed/tSdTSvku1o8" frameborder="0" allowfullscreen></iframe>

Dans cet exemple de projet, que vous pouvez [ouvrir depuis l'éditeur](/fr/manuals/project-setup/) ou [télécharger depuis GitHub](https://github.com/defold/sample-levelcomplete), nous présentons des effets pour afficher le comptage du score qui peut avoir lieu lorsqu'un niveau est terminé. Un score total augmente progressivement et trois étoiles apparaissent lorsque différents seuils sont atteints. L'exemple utilise également la fonctionnalité de rechargement pour ajuster rapidement les valeurs et observer le résultat.

La scène est déclenchée par un message envoyé par le jeu.
Ce message contient le score total obtenu ainsi que les seuils auxquels les trois étoiles doivent apparaître.
À sa réception, le texte du titre (« Level completed! ») apparaît en fondu tout en rétrécissant jusqu'à sa taille normale (100%). Cette animation est réalisée dans `on_message()` ci-dessous.

Une fois l'animation du titre terminée, le comptage du score total commence. À chaque étape, le score actuel est augmenté d'un petit incrément. Nous vérifions ensuite si l'un des seuils d'apparition des étoiles a été franchi, auquel cas l'animation d'une étoile démarre (voir ci-dessous). Tant que nous n'avons pas atteint le score cible, le score total est animé avec un effet de rebond.
Sa taille augmente également vers une valeur maximale à mesure qu'il se rapproche du score total. De la même manière, sa couleur passe progressivement du blanc au vert. Ces animations sont réalisées dans `inc_score()`.

Chaque fois qu'une étoile apparaît, elle s'affiche en fondu et rétrécit jusqu'à sa taille normale. Cette animation est réalisée dans `animate_star()`.

Lorsque l'animation de l'étoile est terminée, de petites étoiles sont créées en cercle autour de la grande étoile. Cette opération est réalisée dans `spawn_small_stars()`.

Elles sont ensuite animées pour être projetées aléatoirement depuis l'étoile. Leur vitesse et leur taille varient aléatoirement à mesure qu'elles s'éloignent. Elles disparaissent ensuite en fondu et sont finalement supprimées. Ces opérations sont réalisées dans `animate_small_star()` et `delete_small_star()`.

Lorsque le score atteint le score total, la mention du meilleur score apparaît en fondu et rétrécit pour prendre sa place. Cette animation est déclenchée à la fin de `inc_score()` et réalisée dans `animate_imprint()`.

La fonction `setup()` veille à ce que les nœuds aient les bonnes valeurs initiales. En appelant `setup()` depuis `on_reload()`, nous veillons à ce que tout soit correctement initialisé à chaque rechargement du script depuis l'éditeur Defold.
