...
R : En tant que développeur, vous n'avez à vous soucier que d'une seule API de rendu utilisant une [chaîne de rendu entièrement programmable par script](/fr/manuals/render/). L'API des scripts de rendu de Defold traduit les opérations de rendu vers les API graphiques suivantes :

{% include shared/fr/graphics-api.md %}
...
{% include shared/fr/linux-faq.md %}


## Questions sur Android {#android-questions}
{% include shared/fr/android-faq.md %}
...
{% include shared/fr/windows-faq.md %}


## Questions sur les consoles {#console-questions}
{% include shared/fr/consoles-faq.md %}
...
R : En général, toutes les ressources sont déclarées statiquement, ce qui présente l'avantage de bénéficier automatiquement de leur gestion. Vous pouvez utiliser les [propriétés de ressource](/fr/manuals/script-properties/#resource-properties) pour changer la ressource attribuée à un composant.
...
R : Oui, consultez l'API de physique, en particulier [`physics.get_shape()`](https://defold.com/ref/physics-lua/#physics.get_shape:url-shape) et [`physics.set_shape()`](https://defold.com/ref/physics-lua/#physics.set_shape:url-shape-table). 
...
R : Oui, activez le paramètre *physics.debug* dans *game.project*. (Consultez la [documentation officielle sur les paramètres du projet](/fr/manuals/project-settings/#debug).)
...
R : Defold utilise une version modifiée de Box2D en arrière-plan, et le coût en performances devrait être assez similaire. Vous pouvez à tout moment voir combien de temps le moteur consacre à la physique en ouvrant le [profileur](/fr/manuals/debugging). Vous devez également tenir compte du type d'objets de collision utilisé. Par exemple, les objets statiques coûtent moins cher en performances. Consultez la [documentation officielle sur la physique](/fr/manuals/physics) de Defold pour plus de détails.
