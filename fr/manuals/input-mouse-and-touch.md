...
- anchor: mouse-triggers
  title: Déclencheurs de la souris
- anchor: mouse-buttons
  title: Boutons de la souris
- anchor: mouse-wheel
  title: Molette de la souris
- anchor: mouse-movement
  title: Déplacement de la souris
- anchor: touch-triggers
  title: Déclencheurs tactiles
- anchor: single-touch
  title: Tactile à un seul point de contact
- anchor: multi-touch
  title: Multitactile
- anchor: detecting-click-or-tap-on-objects
  title: Détection des clics ou des appuis sur les objets
- anchor: detecting-interaction-with-gui-nodes
  title: Détection des interactions avec les nœuds GUI
- anchor: detecting-interaction-with-game-objects
  title: Détection des interactions avec les objets de jeu
---

<div class='sidenote' markdown='1'>
Il est recommandé de vous familiariser avec le fonctionnement général des entrées dans Defold, la façon de les recevoir et leur ordre de réception dans vos fichiers de script. Pour en savoir plus sur le système d'entrée, consultez le [manuel de présentation des entrées](/fr/manuals/input).
</div>
...
Dans les deux cas, il est nécessaire d'effectuer une conversion entre les coordonnées de l'événement de souris ou de contact dans l'espace écran et les coordonnées des objets de jeu dans l'espace monde. Vous pouvez procéder de différentes façons :
  ...
  * Suivez manuellement la vue et la projection utilisées par le script de rendu et servez-vous-en pour effectuer la conversion vers et depuis l'espace monde. Consultez le [manuel de la caméra pour un exemple](/fr/manuals/camera/#converting-mouse-to-world-coordinates).
  * Utilisez une [solution de caméra tierce](/fr/manuals/camera/#third-party-camera-solutions) et ses fonctions de conversion de l'espace écran vers l'espace monde.