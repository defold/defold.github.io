...
- anchor: scripting-live-update
  title: Utiliser Live Update dans des scripts
- anchor: get-mounts
  title: Obtenir les montages
- anchor: scripting-with-excluded-collection-proxies
  title: Utiliser des proxys de collection exclus dans des scripts
---
...
Une fois un paquet monté, un proxy exclu et non chargé peut également être redirigé vers une autre collection compilée avec `collectionproxy.set_collection()`. Consultez [Changer la collection d'un proxy exclu](/fr/manuals/collection-proxy/#changing-an-excluded-proxys-collection) pour connaître les restrictions et la séquence de chargement.
