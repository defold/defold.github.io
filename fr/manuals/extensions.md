...
toc:
- anchor: native-extensions
  title: Extensions natives
- anchor: the-build-server
  title: Le serveur de build
- anchor: project-layout
  title: Organisation du projet
- anchor: manifest-files
  title: Fichiers manifeste
- anchor: r8-keep-rules-for-android
  title: Règles de conservation R8 pour Android
- anchor: custom-resources
  title: Ressources personnalisées
- anchor: sharing-an-extension
  title: Partager une extension
- anchor: a-simple-example-extension
  title: Un exemple dextension simple
- anchor: extension-lifecycle
  title: Cycle de vie dune extension
- anchor: defined-platform-identifiers
  title: Identifiants de plateforme définis
- anchor: build-server-logs
  title: Journaux du serveur de build
- anchor: example-extensions
  title: Exemples dextensions
---
...
Defold vous permet de commencer à utiliser les extensions natives sans aucune configuration, grâce à une solution de build dans le cloud. Toute extension native développée et ajoutée à un projet de jeu, directement ou par l'intermédiaire d'un [projet bibliothèque](/fr/manuals/libraries/), fait partie du contenu ordinaire du projet. Il n'est pas nécessaire de compiler des versions spéciales du moteur et de les distribuer aux membres de l'équipe, car cela se fait automatiquement : tout membre de l'équipe qui compile et exécute le projet obtient un exécutable du moteur propre au projet, intégrant toutes les extensions natives.

![Build dans le cloud](/manuals/images/extensions/cloud_build.png)

Defold fournit gratuitement le serveur de build dans le cloud, sans aucune restriction d'utilisation. Le serveur est hébergé en Europe, et l'URL à laquelle le code natif est envoyé se configure dans la [fenêtre des préférences de l'éditeur](/fr/manuals/editor-preferences/#extensions) ou au moyen de l'option de ligne de commande `--build-server` de [bob](/fr/manuals/bob/#usage). Si vous souhaitez configurer votre propre serveur, [suivez ces instructions](/fr/manuals/extender-local-setup).
...
```
*ext.manifest*
: Le dossier de l'extension _doit_ contenir un fichier *ext.manifest*. Ce fichier de configuration contient les options et les définitions utilisées lors du build d'une extension individuelle. La définition du format de fichier se trouve dans le [manuel du manifeste des extensions](https://defold.com/fr/manuals/extensions-ext-manifests/).
...
: Ce dossier facultatif contient les bibliothèques compilées dont dépend l'extension. Les fichiers de bibliothèque devraient être placés dans des sous-dossiers nommés selon `platform` ou `architecture-platform`, en fonction des architectures prises en charge par vos bibliothèques.
  ...
  {% include shared/fr/platforms.md %}
...
* `android` - Ce dossier accepte un fichier de manifeste partiel à fusionner avec celui de l'application principale ([comme décrit ici](/fr/manuals/extensions-manifest-merge-tool)).
  * Le dossier peut également contenir un fichier `build.gradle` avec des dépendances qui seront [résolues par Gradle](/fr/manuals/extensions-gradle).
...
* `ios` - Ce dossier accepte un fichier de manifeste partiel à fusionner avec celui de l'application principale ([comme décrit ici](/fr/manuals/extensions-manifest-merge-tool)).
  * Le dossier peut également contenir un fichier `Podfile` avec des dépendances qui seront [résolues par Cocoapods](/fr/manuals/extensions-cocoapods).
* `osx` - Ce dossier accepte un fichier de manifeste partiel à fusionner avec celui de l'application principale ([comme décrit ici](/fr/manuals/extensions-manifest-merge-tool)).
* `web` - Ce dossier accepte un fichier de manifeste partiel à fusionner avec celui de l'application principale ([comme décrit ici](/fr/manuals/extensions-manifest-merge-tool)).
...
Ces règles sont combinées au fichier de conservation sélectionné pour le projet lorsque [R8 est activé](/fr/manuals/android/#enabling-r8).
...
Par exemple, placez un fichier JSON dans `/myextension/data/settings.json`. Le chemin est relatif à la racine du projet et inclut le dossier de l'extension. Lorsque vous partagez l'extension sous forme de bibliothèque, incluez `myextension` dans [Include Dirs](/fr/manuals/libraries/#setting-up-library-sharing) de la bibliothèque pour que les projets utilisateurs reçoivent l'extension et ses données.
...
Consultez [Accès aux fichiers](/fr/manuals/file-access/#custom-resources) pour connaître la différence entre les ressources personnalisées et les ressources de bundle.

## Partager une extension {#sharing-an-extension}

Les extensions sont traitées comme toute autre ressource de votre projet et peuvent être partagées de la même manière. Si le dossier d'une extension native est ajouté comme dossier de bibliothèque, il peut être partagé et utilisé par d'autres comme dépendance de projet. Consultez le [manuel des projets bibliothèques](/fr/manuals/libraries/) pour plus d'informations.
...
![Manifeste](/manuals/images/extensions/manifest.png)
...
![Fichier C++](/manuals/images/extensions/cppfile.png)
