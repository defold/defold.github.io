...
Par défaut, Bob recherche un projet dans le répertoire courant et compile les ressources accessibles à partir de `game.project` dans *build/default*. Les ressources non référencées ne sont pas compilées. Si le code charge des fichiers bruts à partir de leur chemin à l'exécution, incluez-les au moyen du [paramètre de projet Custom Resources](/fr/manuals/project-settings/#custom-resources) ; les autres ressources Defold nécessitent une référence accessible à partir d'une racine de build.
