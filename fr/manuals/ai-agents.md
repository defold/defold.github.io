---
brief: Ce manuel explique comment connecter des agents de programmation indépendants du modèle aux interfaces d'automatisation de Defold tout en rendant explicites la vérification, les autorisations et la sécurité.
...
- anchor: when-an-ai-agent-is-useful
  title: Quand un agent IA est utile
- anchor: model-neutral-defold-interfaces
  title: Interfaces Defold indépendantes du modèle
- anchor: integration-layers
  title: Couches dintégration
- anchor: model-context-protocol
  title: Model Context Protocol
- anchor: community-mcp-integrations
  title: Intégrations MCP de la communauté
- anchor: project-instructions
  title: Instructions du projet
- anchor: documentation-discovery
  title: Découverte de la documentation
- anchor: bounded-change-and-verification-loops
  title: Boucles limitées de modification et de vérification
- anchor: multimodal-evaluation
  title: Évaluation multimodale
- anchor: security-isolation-and-good-practices
  title: Sécurité, isolation et bonnes pratiques
---

# Utiliser des agents de programmation IA avec Defold {#using-ai-coding-agents-with-defold}

Les agents de programmation utilisant des grands modèles de langage (LLM) et des modèles multimodaux peuvent inspecter, modifier et vérifier des projets Defold en appelant les mêmes interfaces indépendantes du modèle que celles utilisées par les développeurs, les scripts locaux, les intégrations IDE et la CI. Vous pouvez utiliser un agent lorsque le travail exige de mener une investigation et de s'adapter.
...
* Les [scripts de l'éditeur](/fr/manuals/editor-scripts) peuvent fournir des opérations sur les ressources et des outils propres au projet.
* L'[API HTTP de l'éditeur](/fr/manuals/editor-http-api) fournit les commandes de l'éditeur, les résultats de build, la sortie de la console, la recherche de références, les aperçus, les préférences et les routes des scripts de l'éditeur.
* Le [service du moteur et les API d'automatisation à l'exécution](/fr/manuals/engine-service) donnent accès à l'état en direct du moteur de débogage, aux entrées, aux captures d'écran et aux opérations définies par les extensions.
* [Bob](/fr/manuals/bob) permet les builds en ligne de commande, les rapports, les archives et les bundles.

Un modèle disponible uniquement au moyen d'une interface de chat peut suggérer des modifications de code, mais il ne peut pas inspecter de lui-même le projet local ni vérifier un résultat en cours d'exécution. La couche d'intégration supplémentaire détermine ce que l'agent peut réellement observer et faire.
...
Conservez les règles et les identifiants d'accès dans cette couche locale. Chaque opération modificatrice devrait renvoyer des résultats structurés ou mener à une étape de vérification déterministe.

Pour les opérations de l'éditeur, découvrez l'interface actuelle par `/openapi.json` au lieu de fournir à l'agent une copie d'API codée en dur et permanente. Pour les extensions d'exécution, vérifiez leur état de fonctionnement, la version de leur API et leurs fonctionnalités.
...
Lorsqu'ils sont utilisés dans des flux de travail agentiques, les grands modèles de langage disponibles donnent généralement de meilleurs résultats avec de bonnes instructions. C'est pourquoi des fichiers Markdown décrivant le comportement attendu des agents, ou des ensembles d'instructions appelés skills, sont souvent ajoutés aux projets. Pour obtenir les meilleurs résultats, il est préférable de concevoir et de rédiger des instructions propres à chaque projet, même si certaines connaissances et règles communes peuvent être réutilisées.
...
Un exemple communautaire d'instructions et de skills orientés Defold est disponible [ici sur le forum Defold](https://forum.defold.com/t/agent-config-collection-of-agents-md-and-skills/82387).

Nous recommandons de veiller à ce que vos instructions, dans des fichiers tels qu'AGENTS.md, et vos définitions de skills restent courtes, concises, faciles à réviser et à maintenir, et à les tenir à jour. Les instructions propres au projet peuvent être stockées dans le système de gestion de versions, ce qui rend les modifications traçables et aide à améliorer les performances du flux de travail au fil du temps.
...
Un agent peut diagnostiquer et réparer un échec déterministe de la CI, mais l'étape de CI elle-même devrait rester reproductible sans l'agent.

Les bonnes pratiques en matière de tests et de vérification automatisés sont décrites dans [ce manuel](/fr/manuals/automated-testing).

## Évaluation multimodale {#multimodal-evaluation}

Un agent capable de traiter des images peut inspecter les [aperçus de l'éditeur](/fr/manuals/editor-http-api/#rendering-scene-previews), les captures d'écran à l'exécution, les différences visuelles et les captures du navigateur.
...
Pour en savoir plus sur les aperçus de l'éditeur, les captures d'écran à l'exécution et l'inspection visuelle, consultez [ce manuel](/fr/manuals/automated-testing).
...
* La couche d'intégration locale peut lire `.internal/editor.token` lorsqu'elle est autorisée à utiliser `/eval`, mais elle ne devrait pas placer le jeton dans les prompts du modèle, les journaux ou les rapports.
...
* Exécutez les travaux autonomes de grande ampleur dans une branche, un worktree, une copie temporaire, un conteneur, un environnement isolé ou un compte restreint distinct.
