...
La manière la plus simple de déboguer votre code natif est d'utiliser le [débogage par affichage](http://en.wikipedia.org/wiki/Debugging#Techniques). Utilisez les fonctions de l'[espace de noms (namespace) `dmLog`](/ref/dmLog/) pour surveiller des variables ou indiquer le déroulement de l'exécution. Toutes les fonctions de journalisation affichent leurs messages dans la vue *Console* de l'éditeur et dans le [journal du jeu](/fr/manuals/debugging-game-and-system-logs).
