...
- anchor: debugging-native-code
  title: Débogage du code natif
- anchor: use-a-debugger
  title: Utiliser un débogueur
- anchor: use-print-debugging
  title: Utiliser le débogage par affichage
- anchor: analyze-a-crash-log
  title: Analyser un journal de plantage
- anchor: getting-the-crash-log-from-a-device
  title: Récupérer le journal de plantage dun appareil
- anchor: symbolicate-a-callstack
  title: Symboliquer une pile dappels
- anchor: uploading-symbols-to-google-play
  title: Importer des symboles dans Google Play
- anchor: symbolicate-an-android-callstack
  title: Symboliquer une pile dappels Android
- anchor: symbolicate-an-ios-callstack
  title: Symboliquer une pile dappels iOS
---
...
* Xcode -  macOS, iOS ([en savoir plus](/fr/manuals/debugging-native-code-ios))
* Android Studio - Android ([en savoir plus](/fr/manuals/debugging-native-code-android))
* WinDBG - Windows
...
La manière la plus simple de déboguer votre code natif est d'utiliser le [débogage par affichage](http://en.wikipedia.org/wiki/Debugging#Techniques). Utilisez les fonctions de l'[espace de noms (namespace) `dmLog`](/ref/stable/dmLog/) pour surveiller des variables ou indiquer le déroulement de l'exécution. Toutes les fonctions de journalisation affichent leurs messages dans la vue *Console* de l'éditeur et dans le [journal du jeu](/fr/manuals/debugging-game-and-system-logs).


## Analyser un journal de plantage {#analyze-a-crash-log}

Le moteur Defold enregistre un fichier `_crash` en cas de plantage brutal. Ce fichier contient des informations sur le système et sur le plantage. La [sortie du journal du jeu](/fr/manuals/debugging-game-and-system-logs) indique l'emplacement du fichier de plantage (qui varie selon le système d'exploitation, l'appareil et l'application).
...
Si l'application [autorise le débogage](/fr/manuals/project-settings/#android), vous pouvez récupérer le journal de plantage à l'aide de l'[outil Android Debug Bridge (ADB)](https://developer.android.com/studio/command-line/adb.html) et de la commande `adb shell` :
...
Si vous récupérez une pile d'appels à partir d'un fichier `_crash` ou d'un [fichier journal](/fr/manuals/debugging-game-and-system-logs), vous pouvez la symboliquer. Cela consiste à convertir chaque adresse de la pile d'appels en un nom de fichier et un numéro de ligne, ce qui aide à trouver la cause du problème.

Il est important d'utiliser le moteur correspondant à la pile d'appels, sinon vous risquez fort de déboguer les mauvais éléments ! Utilisez l'option [`--with-symbols`](https://www.defold.com/fr/manuals/bob/) lors de la création d'un bundle avec [bob](https://www.defold.com/fr/manuals/bob/), ou cochez la case « Generate debug symbols » dans la boîte de dialogue de création de bundles de l'éditeur :
...
Remarque : si vous disposez d'une trace de pile provenant des [journaux Android](/fr/manuals/debugging-game-and-system-logs), vous pouvez peut-être la symboliquer avec [ndk-stack](https://developer.android.com/ndk/guides/ndk-stack.html)
