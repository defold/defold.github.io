...
toc:
- anchor: live-update
  title: Mise à jour en direct (Live Update)
- anchor: preparing-content-for-live-update
  title: Préparer le contenu pour Live Update
- anchor: live-update-settings
  title: Paramètres de Live Update
- anchor: bundling-with-live-update
  title: Créer un bundle avec Live Update
- anchor: the-zip-archives
  title: Les archives .zip
- anchor: splitting-zip-archives
  title: Diviser les archives .zip
- anchor: live-update-on-android
  title: Live Update sur Android
- anchor: content-verification
  title: Vérification du contenu
- anchor: mounts
  title: Points de montage
- anchor: scripting-with-live-update
  title: Utiliser Live Update dans les scripts
- anchor: development-caveats
  title: Points à prendre en compte lors du développement
---
...
![Collection Mona Lisa](/manuals/images/live-update/mona-lisa.png)
...
![Proxy de collection exclu](/manuals/images/live-update/proxy-excluded.png)
...
![Paramètres de Live Update](/manuals/images/live-update/05-liveupdate-settings-zip.png)
...
![Créer un bundle d'application avec Live Update](/manuals/images/live-update/bundle-app.png)
...
Pour en savoir plus, consultez [l'utilisation de Live Update dans les scripts](/fr/manuals/live-update-scripting).
...
: Lorsque vous exécutez une version de votre jeu sous forme de bundle, vous n'avez pas d'accès direct à une console. Cela pose des problèmes pour le débogage. Cependant, vous pouvez lancer l'application depuis la ligne de commande ou en double-cliquant directement sur le fichier exécutable du bundle :
  ...
  ![Exécution d'une application sous forme de bundle](/manuals/images/live-update/run-bundle.png)

  Le jeu démarre alors avec une fenêtre de shell qui affiche la sortie de toutes les instructions `print()` :

  ![Sortie de la console](/manuals/images/live-update/run-bundle-console.png)
...
: Le développeur peut télécharger le contenu dans le fichier ou le dossier de son choix, mais celui-ci se trouve souvent sous le chemin de l'application. L'emplacement du dossier de données de l'application dépend du système d'exploitation. Vous pouvez le trouver avec `print(sys.get_save_file("", ""))`. Pour forcer un téléchargement, supprimez le paquet téléchargé et l'entrée correspondante dans tout état géré par l'application. Il n'existe aucune liste de points de montage gérée par le moteur à supprimer ; les points de montage ne sont pas conservés après un redémarrage.
  ...
  ![Stockage local](/manuals/images/live-update/local-storage.png)