...
| Collection en cours d'exécution | Messages, composants (components), entrées, physique, cycle de vie et comportement du moteur |
...
Une compilation réussie prouve que le projet peut être compilé, mais pas que le comportement du jeu est correct. Une capture d'écran ne prouve pas le bon fonctionnement des transitions complexes, des animations, des interactions ou du déroulement du jeu, mais les solutions multimodales modernes peuvent l'utiliser pour examiner l'apparence d'une image et vérifier que les shaders et la disposition visuelle sont corrects. Pour les tests automatisés, préférez toutefois les assertions déterministes dès que la condition peut être exprimée directement.

## Code Lua réutilisable et testable {#reusable-and-testable-lua-code}

Conservez la logique réutilisable dans des modules Lua qui dépendent le moins possible du moteur. Les transformations de données pures, les règles, les machines à états et les calculs peuvent ainsi être testés sans construire un monde de jeu (game world) complet.
...
Pour plus de détails, consultez le [manuel sur l'écriture du code](/fr/manuals/writing-code).

## Tests dans une collection en cours d'exécution {#tests-in-a-running-collection}

Utilisez une collection de test dédiée lorsque le comportement dépend d'objets de jeu (game objects), de composants, de messages, d'entrées, de la physique ou d'autres systèmes du moteur.

Chaque test devrait :

1. établir un état connu ;
2. exécuter un comportement ;
3. vérifier par une assertion et évaluer le résultat attendu ;
...
Ne laissez pas une collection bootstrap de test temporaire dans la configuration normale du projet. Dans la CI, préférez un fichier de paramètres dédié transmis à Bob. La CI ne doit pas modifier l'état du dépôt ; elle doit uniquement effectuer des modifications temporaires lorsque cela est nécessaire.

Pour les jeux complexes, vous pouvez créer de petites collections de « salles de développement » avec des scénarios prédéfinis et des ébauches simples. Elles rendent les mécaniques reproductibles et facilitent les tests pendant le développement sans avoir à parcourir des états et des sections du jeu sans rapport avec les tests.
...
Un collecteur devrait traiter chaque ligne indépendamment, rechercher le préfixe `TEST`, analyser le JSON qui suit et ignorer les sorties du moteur sans rapport avec les tests.

Incluez un identifiant d'exécution unique afin que la sortie d'un processus ancien ou simultané ne puisse pas terminer l'exécution actuelle. Chaque suite devrait émettre un événement final sans ambiguïté (comme `Pass`, `Failure`, `Crash`, `Timeout`, etc.).
...
Pour en savoir plus, consultez le [manuel de l'API HTTP de l'éditeur](/fr/manuals/editor-http-api/#reading-console-output).

### Journaux persistants {#persisted-logs}

Defold peut également conserver le journal du jeu en activant `Write Log File` dans `game.project`. Consultez [Journaux du jeu et du système](/fr/manuals/debugging-game-and-system-logs/). La journalisation dans un fichier est utile pour les applications empaquetées et pour tester des appareils cibles sur lesquels la console de l'éditeur n'est pas disponible.
...
Pour plus de détails, consultez le [manuel du service du moteur](/fr/manuals/engine-service/#automation-bridge-extension).
...
Les états définis par l'application et les identifiants d'automatisation utilisent l'API Lua facultative d'Automation Bridge, réservée au débogage, que le projet doit activer et publier. Une pause de durée fixe est sensible à la vitesse de la machine et au rythme des images ; la vérification répétée d'un état défini avec un délai maximal est plus fiable.
...
L'éditeur peut créer et servir un build HTML5 au moyen de sa commande actuelle `build-html5`, comme indiqué dans le [manuel de l'API HTTP de l'éditeur](/fr/manuals/editor-http-api/#building-html5). Bob peut également créer un bundle HTML5 sans l'éditeur.
...
Limitez la durée des tests dans le navigateur. Distinguez dans le rapport final l'échec du chargement de la page, l'absence du canvas, une erreur JavaScript, l'expiration du test et l'échec d'une assertion du jeu.
...
Vous pouvez l'utiliser pour résoudre les dépendances, compiler un jeu, créer une archive ou un bundle autonome, et générer un rapport JSON :
...
Le [manuel de Bob](/fr/manuals/bob) décrit les plateformes, les fichiers de paramètres, les bundles, les caches, les extensions natives et les rapports de build.

## Rapports d'échec et artefacts {#failure-reports-and-artifacts}

De bons résultats de test devraient conserver suffisamment d'éléments de preuve pour reproduire et diagnostiquer un échec :
...
Le même format devrait pouvoir être utilisé par un développeur, un script local, un service CI ou un [agent de programmation IA](/fr/manuals/ai-agents). La vérification reste ainsi déterministe, même lorsque le diagnostic ou la réparation sont délégués.