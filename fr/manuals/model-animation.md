...
toc:
- anchor: 3d-model-animation
  title: Animation de modèles 3D
- anchor: playing-animations
  title: Lecture des animations
- anchor: morph-targets
  title: Cibles de morphing
- anchor: shader-support
  title: Prise en charge par les shaders
- anchor: the-bone-hierarchy
  title: Hiérarchie des os
- anchor: cursor-animation
  title: Animation du curseur
- anchor: completion-callbacks
  title: Callbacks de fin d’animation
- anchor: playback-modes
...
Pour savoir comment importer des données 3D dans un composant Model afin de l’animer, consultez la [documentation des modèles](/fr/manuals/model).

  ![Animation dans Blender](/manuals/images/animation/blender_animation.png)
  ![Ondulation en boucle](/manuals/images/animation/suzanne.gif)
...
Outre l’utilisation de `model.play_anim()` pour faire avancer une animation de modèle, les composants *Model* exposent une propriété `cursor` que vous pouvez manipuler avec `go.animate()` (pour en savoir plus, consultez les [animations de propriétés](/fr/manuals/property-animation)) :
