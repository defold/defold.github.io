...
- anchor: writing-modern-glsl-shaders
  title: Écrire des shaders GLSL modernes
- anchor: version-declaration
  title: Déclaration de version
- anchor: attributes
  title: Attributs
- anchor: varyings
  title: Variables interpolées
- anchor: uniforms-called-constants-in-defold
  title: Variables uniformes (appelées constantes dans Defold)
- anchor: built-in-variables
  title: Variables intégrées
- anchor: texture-functions
  title: Fonctions de texture
- anchor: precision
  title: Précision
- anchor: putting-it-together
  title: Exemple complet
- anchor: including-snippets-into-shaders
  title: Inclure des extraits de code dans les shaders
- anchor: header-guards
  title: Gardes dinclusion
- anchor: editor-specific-shader-code
  title: Code de shader propre à léditeur
- anchor: the-rendering-process
  title: Le processus de rendu
...
* Consultez le [manuel du rendu](/fr/manuals/render) pour en savoir plus sur le pipeline de rendu.
* Consultez le [manuel des matériaux](/fr/manuals/material) pour en savoir plus sur les matériaux.
* Consultez le [manuel des programmes de calcul](/fr/manuals/compute) pour en savoir plus sur les programmes de calcul.
...
: Les positions des sommets qui constituent la forme d'un modèle sont stockées relativement à l'origine du modèle. C'est ce que l'on appelle l'« espace modèle ». Le monde de jeu (game world), en revanche, est un « espace monde » où la position, l'orientation et l'échelle de chaque sommet sont exprimées relativement à l'origine du monde. En séparant ces deux espaces, le moteur de jeu peut déplacer, faire pivoter et redimensionner chaque modèle sans détruire les valeurs d'origine des sommets stockées dans le composant (component) de modèle.
  ...
  ![Transformation monde](/manuals/images/shader/world_transform.png)
...
: Les constantes d'un shader restent constantes pendant toute la durée de l'appel de dessin du rendu. Les constantes sont ajoutées aux sections *Constants* du fichier de matériau, puis déclarées comme `uniform` dans le programme shader. Les variables uniformes d'échantillonneur sont ajoutées à la section *Samplers* du matériau, puis déclarées comme `uniform` dans le programme shader. Les matrices nécessaires pour effectuer les transformations de sommets dans un shader de sommets sont disponibles sous forme de constantes :
  ...
  Le [manuel des matériaux](/fr/manuals/material) explique comment spécifier les constantes.
...
: Les shaders peuvent déclarer des variables uniformes de type *sampler*. Les échantillonneurs servent à lire des valeurs dans une image source :
  ...
  Vous ne pouvez utiliser un échantillonneur que dans les fonctions de lecture de texture de la bibliothèque standard GLSL. Le [manuel des matériaux](/fr/manuals/material) explique comment spécifier les paramètres des échantillonneurs.
...
: Une coordonnée 2D est associée à un sommet et correspond à un point sur une texture 2D. Une partie ou la totalité de la texture peut donc être appliquée à la forme décrite par un ensemble de sommets.
  ...
  ![Coordonnées UV](/manuals/images/shader/uv_map.png)
...
: Les variables de type `Varying` servent à transmettre des informations entre l'étape de traitement des sommets et l'étape de traitement des fragments.
  ...
  ![Interpolation des variables](/manuals/images/shader/varying_vertex.png)
  ...
  ![Interpolation des variables](/manuals/images/shader/varying.png)
...
![Pipeline de rendu](/manuals/images/shader/pipeline.png)
...
![Shader de sommets](/manuals/images/shader/vertex_shader.png)
...
![Shader de fragments](/manuals/images/shader/fragment_shader.png)
