...
Il est distinct du [serveur de l'éditeur](/fr/manuals/editor-http-api), qui appartient à l'éditeur Defold et contrôle le projet ouvert.
...
![Informations sur le port du service du moteur dans un build de débogage Defold](/manuals/images/automation/engine-service.png)

```text
INFO:ENGINE: Engine service started on port <port>
```

Cette ligne apparaît dans la console de l'éditeur lorsque le jeu a été lancé depuis celui-ci. Un contrôleur local simple peut l'analyser, mais une intégration réutilisable devrait laisser l'éditeur, ou l'outil qui l'encapsule, suivre l'instance du moteur et le port enregistré. Cela évite de confondre un ancien port avec celui d'un processus nouvellement démarré ou réutilisé.

Le moteur annonce également les cibles de développement au moyen de la découverte de services sur les plateformes prises en charge. Ce mécanisme est principalement utilisé par les outils Defold et ne devrait pas être remplacé par un port codé en dur de manière permanente.

Le serveur est accessible sur localhost (`127.0.0.1`) au port indiqué :

![Accès au serveur du moteur](/manuals/images/automation/engine-server.png)
...
Une API d'automatisation à l'exécution définie par une extension devrait :
...
| Opération | Action |
| --- | --- |
| `GET  /automation-bridge/v1/health` | rapport d'état, fonctionnalités et compatibilité de l'API |
...
La séparation entre `project` et `game` rend explicite la limite entre les processus. Les opérations de l'éditeur restent sur son serveur, tandis que les observations et les actions sur le jeu en cours d'exécution restent sur le service du moteur.
...
Le service du moteur et les routes définies par les extensions sont des outils de développement et devraient être traités comme tels.

<div class='important' markdown='1'>
Le service du moteur ne publie actuellement pas de document OpenAPI. Les intégrations devraient se limiter aux comportements documentés ou à l'API versionnée d'une extension.
</div>

Les scripts d'exécution, la physique, les entrées, les objets créés dynamiquement et le rendu propre à une plateforme nécessitent un moteur en cours d'exécution et devraient être vérifiés par des [tests d'exécution automatisés](/fr/manuals/automated-testing).
