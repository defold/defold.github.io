...
- anchor: scripts
  title: Scripts
- anchor: script-types
  title: Types de scripts
- anchor: script-execution-callbacks-and-self
  title: Exécution des scripts, fonctions de rappel et self
- anchor: reactive-logic
  title: Logique réactive
- anchor: preprocessing
  title: Prétraitement
- anchor: editor-support
  title: Prise en charge dans léditeur
---

# Scripts {#scripts}

Les composants script (script components) vous permettent de créer la logique du jeu avec le [langage de programmation Lua](/fr/manuals/lua).
...
Lorsque des messages sont envoyés au composant script avec [`msg.post()`](/ref/msg#msg.post), le moteur appelle cette fonction du composant destinataire. Pour en savoir [plus sur l'échange de messages](/fr/manuals/message-passing).
...
Si ce composant a acquis le focus d'entrée (voir [`acquire_input_focus`](/ref/go/#acquire_input_focus)), le moteur appelle cette fonction lorsqu'une entrée est détectée. Pour en savoir [plus sur la gestion des entrées](/fr/manuals/input).
...
Cette fonction est appelée lorsque le script est rechargé au moyen de la fonction de rechargement à chaud de l'éditeur (<kbd>Edit ▸ Reload Resource</kbd>). Elle est très utile pour le débogage, les tests et les ajustements. Pour en savoir [plus sur le rechargement à chaud](/fr/manuals/hot-reload).
...
![Saisie semi-automatique](/manuals/images/script/completion.png)