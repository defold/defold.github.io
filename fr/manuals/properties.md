...
- anchor: game-object-and-component-properties
  title: Propriétés des objets de jeu et des composants
- anchor: gui-node-properties
  title: Propriétés des nœuds dinterface graphique
---
...
* Propriétés des composants script définies par l'utilisateur dans des scripts Lua (consultez la [documentation des propriétés de script](/fr/manuals/script-properties) pour plus de détails)
* Propriétés des nœuds d'interface graphique
* Constantes de shader définies dans les shaders et les fichiers de matériau (consultez la [documentation des matériaux](/fr/manuals/material) pour plus de détails)
...
| ---------- | -------------------------------------- | --------------- | ---------------- |
| *position* | La position locale de l'objet de jeu. | `vector3`      | `get+set` |
| *rotation* | La rotation locale de l'objet de jeu, exprimée sous forme de `quaternion`.  | `quaternion` | `get+set` |
| *euler*    | La rotation locale de l'objet de jeu, en angles d'Euler. | `vector3` | `get+set` |
| *scale*    | L'échelle locale non uniforme de l'objet de jeu, exprimée sous forme de vecteur dont chaque composante contient un multiplicateur sur chaque axe. Pour doubler la taille en X et Y sans modifier Z, utilisez `vmath.vector3(2.0, 2.0, 1.0)`. | `vector3` | `get+set` |
| *scale.xy*    | L'échelle locale non uniforme de l'objet de jeu sur les axes X et Y. Utilisez cette propriété ou `go.set_scale_xy()` lorsque vous ne souhaitez pas modifier l'échelle en Z. | `vector3` | `get+set` |
...
| propriété   | description                            | type            |                  |
| ---------- | -------------------------------------- | --------------- | ---------------- |
| *size*     | La taille du sprite avant mise à l'échelle, telle qu'elle est définie dans l'atlas source. | `vector3` | `get` |
| *image* | Le hachage du chemin de la texture du sprite. | `hash` | `get`|
| *scale* | L'échelle non uniforme du sprite. | `vector3` | `get+set`|
| *scale.xy* | L'échelle non uniforme du sprite sur les axes X et Y. | `vector3` | `get+set`|
| *material* | Le matériau utilisé par le sprite. | `hash` | `get+set`|
| *cursor* | La position (entre 0 et 1) du curseur de lecture. | `number` | `get+set`|
| *playback_rate* | La fréquence d'images de l'animation par images successives (flipbook). | `number` | `get+set`|

*Propriétés des composants objet de collision*

| propriété   | description                            | type            |                  |
| ---------- | -------------------------------------- | --------------- | ---------------- |
| *mass*     | La masse de l'objet de collision. | `number` | `get` |
| *linear_velocity* | La vitesse linéaire actuelle de l'objet de collision. | `vector3` | `get` |
| *angular_velocity* | La vitesse angulaire actuelle de l'objet de collision. | `vector3` | `get` |
| *linear_damping* | L'amortissement linéaire de l'objet de collision. | `vector3` | `get+set` |
| *angular_damping* | L'amortissement angulaire de l'objet de collision. | `vector3` | `get+set` |

*Propriétés des composants modèle (3D)*

| propriété   | description                            | type            |                  |
| ---------- | -------------------------------------- | --------------- | ---------------- |
| *animation* | L'animation actuelle.                | `hash`          | `get`     |
| *texture0*--*texture15* | Les hachages des chemins des textures du modèle. | `hash` | `get+set`|
| *cursor*  | La position (entre 0 et 1) du curseur de lecture. | `number`   | `get+set` |
| *playback_rate* | La vitesse de lecture de l'animation. Un multiplicateur de la vitesse de lecture de l'animation. | `number` | `get+set` |
| *material* | Le matériau utilisé par le modèle. | `hash` | `get+set`|

*Propriétés des composants label*

| propriété   | description                            | type            |                  |
| ---------- | -------------------------------------- | --------------- | ---------------- |
| *text* | Le contenu textuel du label. Disponible depuis Defold 1.13.2. | `string` | `get+set` |
| *scale* | L'échelle du label. | `vector3` | `get+set` |
| *scale.xy* | L'échelle du label sur les axes X et Y. | `vector3` | `get+set`|
| *color*     | La couleur du label. | `vector4` | `get+set` |
| *outline* | La couleur du contour du label. | `vector4` | `get+set` |
| *shadow* | La couleur de l'ombre du label. | `vector4` | `get+set` |
| *size* | La taille du label. Cette taille limite le texte si le retour à la ligne est activé. | `vector3` | `get+set` |
| *material* | Le matériau utilisé par le label. | `hash` | `get+set`|
| *font* | La police utilisée par le label. | `hash` | `get+set`|
