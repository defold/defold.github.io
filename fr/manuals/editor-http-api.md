...
![Page d'accueil du serveur local de l'éditeur](/manuals/images/automation/editor_server.png)
...
Attendez que l'URL soit disponible avant de lancer les tests dans le navigateur. Consultez [Tests HTML5 dans un navigateur](/fr/manuals/automated-testing/#browser-tests-for-html5) pour plus de détails.
...
Pour la délimitation des résultats de test et la classification des échecs, consultez [Tests et vérification automatisés](/fr/manuals/automated-testing/#structured-test-results).
...
![Aperçu de la collection principale généré par l'éditeur](/manuals/images/automation/main-preview.png)
...
![Aperçu du modèle de cube généré par l'éditeur](/manuals/images/automation/cube-preview.png)
...
Les scripts de l'éditeur peuvent définir des routes supplémentaires avec [`get_http_server_routes()`](/fr/manuals/editor-scripts/#http-server). Une table d'opération OpenAPI facultative expose une route dans le même document `/openapi.json` que les opérations intégrées.
...
Le serveur de l'éditeur appartient au processus de l'éditeur. Un jeu en cours d'exécution utilise un autre port et assume d'autres responsabilités, décrites dans le [manuel du service du moteur et de l'API HTTP d'exécution](/fr/manuals/engine-service).