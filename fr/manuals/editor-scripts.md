...
- anchor: editor-scripts
  title: Scripts de léditeur
- anchor: editor-script-runtime
  title: Environnement dexécution des scripts de léditeur
- anchor: anatomy-of-editor_script
  title: Anatomie dun fichier .editor_script
- anchor: editor-api
  title: API de léditeur
- anchor: commands
  title: Commandes
- anchor: use-commands-to-change-the-in-memory-editor-state
  title: Utiliser les commandes pour modifier létat de léditeur en mémoire
- anchor: use-commands-with-the-active-editor-view
  title: Utiliser les commandes avec la vue active de léditeur
- anchor: use-shell-commands
  title: Utiliser les commandes shell
...
- anchor: editor-scripts-in-libraries
  title: Scripts de léditeur dans les bibliothèques
- anchor: preferences
  title: Préférences
- anchor: execution-modes
  title: Modes dexécution
- anchor: actions
  title: Actions
- anchor: undoable-actions
  title: Actions annulables
- anchor: non-undoable-actions
  title: Actions non annulables
- anchor: mixing-actions-and-side-effects
  title: Combiner actions et effets de bord
---
...
- `editor.ui.*` — diverses fonctions liées à l'interface utilisateur, voir le [manuel de l'interface utilisateur](/fr/manuals/editor-scripts-ui).
- `editor.prefs.*` — fonctions permettant d'interagir avec les préférences de l'éditeur, voir les [préférences](#preferences).

Vous trouverez la référence complète de l'API de l'éditeur [ici](/ref/editor/).
...
- `on_bundle_started(opts)` — exécuté lorsque vous créez un bundle ou générez une version HTML5 du jeu. Comme avec `on_build_started`, les modifications déclenchées par ce hook apparaîtront dans le bundle et les erreurs interrompront sa création. `opts` contient les clés suivantes :
  ...
  - `platform` — la plateforme pour laquelle le bundle du jeu est créé. Consultez la liste des valeurs de plateforme possibles dans le [manuel de Bob](/fr/manuals/bob).
...
L'éditeur prend en charge une partie du [Language Server Protocol](https://microsoft.github.io/language-server-protocol/) : diagnostics (analyse statique), complétion, informations au survol, symboles du document dans le panneau Structure, accès à la définition, recherche de références, renommage de symboles et formatage du document ou d'une plage. Survolez un symbole pour voir les informations du serveur de langage. Lorsque le curseur est sur un symbole, utilisez <kbd>F2</kbd> pour le renommer, <kbd>F12</kbd> pour accéder à sa définition ou <kbd>Shift+F12</kbd> pour rechercher ses références. Ces actions sont également disponibles dans le menu <kbd>Edit</kbd>. Consultez [Formatage du code](/fr/manuals/writing-code/#formatting-code) pour la commande de formatage et la préférence de formatage à l'enregistrement.

Le serveur de langage Lua fourni inclut les annotations de types Defold pour les API d'exécution et de scripts d'éditeur. Dans les fichiers `.editor_script`, la complétion et les diagnostics reconnaissent les fonctions `editor.*` ainsi que les types de leurs arguments et valeurs de retour. Consultez [Complétion du code](/fr/manuals/writing-code/#code-completion).

Pour enregistrer un serveur de langage supplémentaire, définissez la fonction `get_language_servers` de votre script d'éditeur comme suit :
...
- `languages` (obligatoire) — une liste des langages qui intéressent le serveur, tels que définis [ici](https://code.visualstudio.com/docs/languages/identifiers#known-language-identifiers) (les extensions de fichier fonctionnent également) ;
