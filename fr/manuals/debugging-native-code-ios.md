...
- anchor: launch-the-debugger
  title: Lancement du débogueur
- anchor: debug-symbols
  title: Symboles de débogage
- anchor: path-mappings
  title: Correspondances des chemins
- anchor: breakpoints
  title: Points darrêt
- anchor: notes
  title: Remarques
- anchor: check-uuid-of-binary
  title: Vérification de lUUID du binaire
---
...
* Créez un bundle de l'application avec Bob, en utilisant l'option `--with-symbols` ([plus d'informations](/fr/manuals/debugging-native-code/#symbolicate-a-callstack)) :
...
* Récupérez le dossier `.dSYM` (c'est-à-dire les symboles de débogage)
	...
	* Si vous utilisez une extension native, le dossier `.dSYM` est généré lorsque vous créez un build avec [bob.jar](https://www.defold.com/fr/manuals/bob/). Seul le build est nécessaire (sans archivage ni création de bundle) :
...
* Créez un projet Xcode et choisissez le modèle `Game`
	...
	![Modèle de projet](/manuals/images/extensions/debugging/ios/project_template.png)
...
* Ajoutez votre code à l'application
	...
	![Ajout de fichiers](/manuals/images/extensions/debugging/ios/add_files.png)

* Vérifiez que la case « Copy items if needed » est décochée.

	![Ajout du code source](/manuals/images/extensions/debugging/ios/add_source.png)

* Voici le résultat final

	![Code source ajouté](/manuals/images/extensions/debugging/ios/added_source.png)


* Désactivez l'étape `Build`

	![Modification du schéma](/manuals/images/extensions/debugging/ios/edit_scheme.png)

	![Désactivation du build](/manuals/images/extensions/debugging/ios/disable_build.png)

* Définissez la version `Deployment target` de sorte qu'elle soit désormais supérieure à la version d'iOS de votre appareil

	![Version cible du déploiement](/manuals/images/extensions/debugging/ios/deployment_version.png)

* Sélectionnez l'appareil cible

	![Sélection de l'appareil](/manuals/images/extensions/debugging/ios/select_device.png)
...
2. Ou choisissez `Attach to process by PID or Process name`
	...
	![Sélection de l'appareil](/manuals/images/extensions/debugging/ios/attach_to_process_name.png)
...
* Ouvrez un fichier dans la vue du projet et définissez un point d'arrêt
	...
	![Point d'arrêt](/manuals/images/extensions/debugging/ios/breakpoint.png)
