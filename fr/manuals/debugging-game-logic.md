...
toc:
- anchor: debugging-game-logic
  title: Débogage de la logique du jeu
- anchor: print-and-visual-debugging
  title: Débogage par affichage et débogage visuel
- anchor: running-the-debugger
  title: Exécution du débogueur
- anchor: breakpoints-tab
  title: Onglet Breakpoints
- anchor: lua-debug-library
  title: Bibliothèque de débogage Lua
- anchor: debugging-checklist
  title: Liste de vérification du débogage
- anchor: debugging-problems-with-physics
  title: Débogage des problèmes de physique
---

# Débogage de la logique du jeu {#debugging-game-logic}

Defold intègre un débogueur Lua doté de fonctions d'inspection. Associé aux [outils de profilage](/fr/manuals/profiling) intégrés, il constitue un outil puissant qui peut vous aider à trouver la cause des bogues dans la logique de votre jeu ou à analyser les problèmes de performances.

## Débogage par affichage et débogage visuel {#print-and-visual-debugging}

La méthode la plus simple pour déboguer votre jeu dans Defold consiste à utiliser le [débogage par affichage](http://en.wikipedia.org/wiki/Debugging#Techniques). Utilisez des instructions `print()` ou [`pprint()`](/ref/builtins#pprint) pour surveiller les variables ou indiquer le déroulement de l'exécution. Si un objet de jeu (game object) sans script se comporte de façon étrange, vous pouvez simplement lui associer un script dans le seul but de le déboguer. Chacune de ces fonctions d'affichage écrit dans la vue *Console* de l'éditeur et dans le [journal du jeu](/fr/manuals/debugging-game-and-system-logs).
...
![vue d'ensemble](/manuals/images/debugging/overview.png)
...
: <img src='/manuals/images/debugging/play.svg' width='60px'/>
  ...
  ![point d'arrêt](/manuals/images/debugging/break.png)
...
: <img src='/manuals/images/debugging/step_over.svg' width='60px'/>
  ...
  ![pas](/manuals/images/debugging/step.png)
...
: <img src='/manuals/images/debugging/step_in.svg' width='60px'/>
  ...
  ![entrée dans la fonction](/manuals/images/debugging/step_into.png)
...
: Vous pouvez définir autant de points d'arrêt que vous le souhaitez dans votre code Lua. Lorsque le jeu s'exécute avec le débogueur attaché, l'exécution s'interrompt au prochain point d'arrêt rencontré et attend une nouvelle intervention de votre part.
  ...
  ![ajout d'un point d'arrêt](/manuals/images/debugging/add_breakpoint.png)
...
: Vous pouvez désactiver temporairement les points d'arrêt sans les supprimer. Lorsqu'ils sont désactivés, ils sont ignorés pendant l'exécution, mais vous pouvez les réactiver à tout moment. Faites un clic droit sur le point d'arrêt dans la marge de l'éditeur de code, puis cochez ou décochez la case `Enabled`. Les points d'arrêt désactivés apparaissent creux pour indiquer qu'ils sont inactifs.
  ...
  ![désactivation d'un point d'arrêt](/manuals/images/debugging/disable_breakpoint.png)
...
: Vous pouvez configurer votre point d'arrêt pour qu'il contienne une condition qui doit être vraie pour le déclencher. Cette condition peut accéder aux variables locales disponibles à cette ligne pendant l'exécution du code.
  ...
  ![modification d'un point d'arrêt](/manuals/images/debugging/edit_breakpoint.png)
...
## Onglet Breakpoints {#breakpoints-tab}
  ...
  ![onglet Breakpoints](/manuals/images/debugging/breakpoints_tab.png)
...
![paramètre de débogage de la physique](/manuals/images/debugging/physics_debug_setting.png)

Lorsque cette case est cochée, Defold dessine toutes les formes de collision et tous les points de contact des collisions :

![visualisation du débogage de la physique](/manuals/images/debugging/physics_debug_visualisation.png)