...
Une ressource Buffer peut servir d'entrée à un [composant (component) Mesh](/fr/manuals/mesh). Les ressources Buffer peuvent également être créées à l'exécution avec `buffer.create()` et les [fonctions associées de l'API](/ref/stable/buffer/#buffer.create:element_count-declaration).