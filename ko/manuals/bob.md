...
기본적으로 Bob은 현재 디렉토리에서 프로젝트를 찾고, `game.project`에서 도달할 수 있는 리소스를 *build/default*에 빌드합니다. 참조되지 않은 리소스는 컴파일되지 않습니다. 코드가 런타임에 원시 파일을 경로로 로드한다면 [Custom Resources 프로젝트 설정](/ko/manuals/project-settings/#custom-resources)을 통해 포함하세요. 그 밖의 Defold 리소스는 빌드 루트에서 도달할 수 있는 참조가 필요합니다.
