...
[런타임 폰트 활성화](/ko/manuals/font#enabling-runtime-fonts)를 참고하세요.
...
쉐이더 uniform에 대한 자세한 내용은 [쉐이더 매뉴얼](/ko/manuals/shader)을 참고하세요.
...
* [App Manifest](/ko/manuals/app-manifest)를 추가하고 `Use full text layout system` 옵션을 활성화합니다.
...
self.language_ttf_hash = font_info.fonts[1].path_hash -- 첫 번째 폰트 가져오기(에디터에서 지정한 것)
font.add_font(self.font_collection, self.language_ttf_hash) -- 폰트의 참조 카운트를 증가시킵니다
```
