...
중앙 편집 영역에는 GUI가 표시됩니다. 편집 영역 오른쪽 위의 툴바에는 *Move*, *Rotate*, *Scale* 도구와 [레이아웃](/ko/manuals/gui-layouts) 선택기가 있습니다.
...
: 이 GUI 컴포넌트가 추적하는 최대 동적 텍스쳐 수이며 기본값은 `128`입니다. [`gui.new_texture()`](/ref/gui/#gui.new_texture:texture_id-width-height-type-buffer-flip)로 생성한 텍스쳐와 `go.set(..., "textures", ...)` 또는 `gui.set(msg.url(), "textures", ...)`로 GUI에 할당한 외부 텍스쳐가 포함됩니다. 외부 텍스쳐를 많이 교체하는 프로젝트에서는 이 제한을 늘려야 할 수 있습니다.
...
: ![box node](/manuals/images/icons/gui-box-node.png)
  단일 색상, 텍스쳐 또는 플립북 애니메이션을 가진 사각형 노드입니다. 자세한 내용은 [Box node 문서](/ko/manuals/gui-box)를 참고하세요.

<div style="clear: both;"></div>

Text node
: ![text node](/manuals/images/icons/gui-text-node.png)
  텍스트를 표시합니다. 자세한 내용은 [Text node 문서](/ko/manuals/gui-text)를 참고하세요.

<div style="clear: both;"></div>

Pie node
: ![pie node](/manuals/images/icons/gui-pie-node.png)
  일부만 채우거나 반전할 수 있는 원형 또는 타원형 노드입니다. 자세한 내용은 [Pie node 문서](/ko/manuals/gui-pie)를 참고하세요.

<div style="clear: both;"></div>

Template node
: ![template node](/manuals/images/icons/gui.png)
  템플릿은 다른 GUI 씬 파일을 기반으로 인스턴스를 만드는 데 사용됩니다. 자세한 내용은 [Template node 문서](/ko/manuals/gui-template)를 참고하세요.

<div style="clear: both;"></div>

ParticleFX node
: ![particlefx node](/manuals/images/icons/particlefx.png)
  파티클 효과를 재생합니다. 자세한 내용은 [ParticleFX node 문서](/ko/manuals/gui-particlefx)를 참고하세요.
...
: 노드의 위치, 방향, 스케일을 제어합니다. *Move*, *Rotate*, *Scale* 도구를 사용해 이 값을 변경할 수 있습니다. 값은 스크립트에서 애니메이션할 수 있습니다([자세히 알아보기](/ko/manuals/property-animation)).

Size (box, text and pie nodes)
: 노드의 크기는 기본적으로 자동이지만 *Size Mode*를 `Manual`로 설정하면 값을 변경할 수 있습니다. 크기는 노드의 경계를 정의하며 입력 선택을 할 때 사용됩니다. 이 값은 스크립트에서 애니메이션할 수 있습니다([자세히 알아보기](/ko/manuals/property-animation)).
...
Slice 9 (box nodes)
: 노드 크기가 변경될 때 가장자리 주변 노드 텍스쳐의 픽셀 크기를 보존하도록 설정합니다. 자세한 내용은 [Box node 문서](/ko/manuals/gui-box)를 참고하세요.

Inner Radius (pie nodes)
: X축을 따라 표현되는 노드의 내부 반지름입니다. 자세한 내용은 [Pie node 문서](/ko/manuals/gui-pie)를 참고하세요.

Outer Bounds (pie nodes)
: 외부 경계의 동작을 제어합니다. 자세한 내용은 [Pie node 문서](/ko/manuals/gui-pie)를 참고하세요.

Perimeter Vertices (pie nodes)
: 모양을 만드는 데 사용할 세그먼트 수입니다. 자세한 내용은 [Pie node 문서](/ko/manuals/gui-pie)를 참고하세요.

Pie Fill Angle (pie nodes)
: 파이를 얼마나 채울지 정합니다. 자세한 내용은 [Pie node 문서](/ko/manuals/gui-pie)를 참고하세요.

Template (template nodes)
: 노드의 템플릿으로 사용할 GUI 씬 파일입니다. 자세한 내용은 [Template node 문서](/ko/manuals/gui-template)를 참고하세요.

ParticleFX (particlefx nodes)
: 이 노드에서 사용할 파티클 효과입니다. 자세한 내용은 [ParticleFX node 문서](/ko/manuals/gui-particlefx)를 참고하세요.

Color
: 노드의 색상입니다. 노드에 텍스쳐가 있으면 이 색상이 텍스쳐에 색조를 입힙니다. 색상은 스크립트에서 애니메이션할 수 있습니다([자세히 알아보기](/ko/manuals/property-animation)).

Alpha
: 노드의 투명도입니다. 알파 값은 스크립트에서 애니메이션할 수 있습니다([자세히 알아보기](/ko/manuals/property-animation)).
...
: 노드의 클리핑 모드를 설정합니다.
  ...
  자세한 내용은 [GUI 클리핑 매뉴얼](/ko/manuals/gui-clipping)을 참고하세요.

Clipping Visible (box and pie nodes)
: 스텐실 영역 안에 노드의 컨텐츠를 렌더링하도록 설정합니다. 자세한 내용은 [GUI 클리핑 매뉴얼](/ko/manuals/gui-clipping)을 참고하세요.

Clipping Inverted (box and pie nodes)
: 스텐실 마스크를 반전합니다. 자세한 내용은 [GUI 클리핑 매뉴얼](/ko/manuals/gui-clipping)을 참고하세요.
