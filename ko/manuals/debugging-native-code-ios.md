...
* `--with-symbols` 옵션을 사용해 bob으로 앱을 번들링합니다([자세한 정보](/ko/manuals/debugging-native-code/#symbolicate-a-callstack)).
...
* `.dSYM` 폴더(debug symbols)를 가져옵니다.
	...
	* 네이티브 익스텐션을 사용한다면 [bob.jar](https://www.defold.com/ko/manuals/bob/)로 빌드할 때 `.dSYM` 폴더가 생성됩니다. 빌드만 필요합니다(아카이브나 번들링은 필요하지 않음).
