...
오브젝트를 정확히 분리해야 하는 게임이나 어플리케이션에서는 `"contact_point_response"` 메세지가 필요한 모든 정보를 제공합니다. 그러나 주어진 충돌 쌍마다 충돌의 특성에 따라 매 프레임 여러 개의 `"contact_point_response"` 메세지를 받을 수 있다는 점에 유의하세요. 자세한 내용은 [충돌 해결](/ko/manuals/physics-resolving-collisions)을 참고하세요.
