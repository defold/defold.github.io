...
: 최근에 열었던 프로젝트가 표시되어 빠르게 액세스할 수 있습니다. 시작 화면의 기본 뷰입니다.
  ...
  ![내 프로젝트](/manuals/images/editor/start_no_projects.png)


  이전에 연 프로젝트가 있으면 아래 그림처럼 프로젝트 목록이 표시됩니다.

  ![내 프로젝트](/manuals/images/editor/start_my_projects.png)
...
: 특정 사용 사례를 보여 주기 위해 준비된 프로젝트가 들어 있습니다.
  ...
  ![새 프로젝트](/manuals/images/editor/start_templates.png)

새 프로젝트를 만들면 로컬 드라이브에 저장되며, 모든 편집 내용도 로컬에 저장됩니다.

여러 옵션에 대한 자세한 내용은 [Project Setup 매뉴얼](https://www.defold.com/ko/manuals/project-setup/)에서 확인할 수 있습니다.
...
![언어](/manuals/images/editor/languages.png)

## 에디터 창 {#the-editor-views}

Defold Editor는 특정 정보를 표시하는 여러 창 또는 뷰로 나뉩니다.

![에디터 2](/manuals/images/editor/editor_overview.png)
...
*Assets* pane에서 삭제한 파일과 폴더는 플랫폼이 지원하면 시스템 Trash 또는 Recycle Bin으로 이동됩니다. 항목을 휴지통으로 옮기는 기능이 지원되지 않거나 실패하면 에디터가 영구 삭제합니다.
...
- 직교 및 원근 카메라 모드가 있는 [2D 및 3D 씬 탐색](/ko/manuals/scene-editing/#2d-and-3d-scene-orientation)
- 오브젝트를 이동, 회전, 확대/축소하기 위한 [변형 도구](/ko/manuals/scene-editing/#manipulating-objects)
- 1인칭 3D 탐색을 위한 [Free Camera Mode](/ko/manuals/scene-editing/#free-camera-mode)
- 크기, 평면, 모양을 설정할 수 있는 [Grid settings](/ko/manuals/scene-editing/#grid-settings)
- 컴포넌트 타입과 가이드를 토글하는 [Visibility filters](/ko/manuals/scene-editing/#visibility-filters)

자세한 내용은 [Scene Editor 매뉴얼](/ko/manuals/scene-editing/)을 읽어 보세요.
...
*Curve Editor* : [Particle Editor](/ko/manuals/particlefx/)에서 커브를 편집할 때 사용됩니다.

Tools pane은 통합 디버거와 상호작용하는 데에도 사용됩니다. 자세한 내용은 [Debugging 매뉴얼](/ko/manuals/debugging/)에서 확인하세요.

### 6. Changed Files pane

프로젝트가 분산 버전 관리 시스템인 Git을 사용한다면 이 뷰는 프로젝트에서 변경, 추가 또는 삭제된 모든 파일을 나열합니다. 프로젝트를 정기적으로 동기화하면 로컬 복사본을 프로젝트 Git 저장소에 저장된 내용과 맞출 수 있으며, 이를 통해 팀 안에서 공동작업을 할 수 있고 문제가 생겨도 작업을 잃지 않습니다. Git에 대한 자세한 내용은 [Version Control 매뉴얼](/ko/manuals/version-control/)에서 확인할 수 있습니다. 이 뷰에서 일부 파일 관련 작업을 수행할 수 있습니다.
...
![창 표시 여부](/manuals/images/editor/editor_panes.png)
...
- `➝| Open As`를 선택해서 기본 에디터가 아닌 다른 에디터나 `File ▸ Preferences ▸ Code ▸ Custom Editor`에 설정된 연결된 외부 도구를 사용합니다. 자세한 내용은 [Preferences 매뉴얼](/ko/manuals/editor-preferences)에서 확인하세요.

![탭](/manuals/images/editor/tabs_custom.png)
...
![2개 창](/manuals/images/editor/2-panes.png)
...
![파일 만들기](/manuals/images/editor/create_file.png)

새 파일에 적합한 *Name*을 입력하고, 필요하면 *Location*을 변경합니다. 파일 타입 접미사를 포함한 전체 파일 이름이 다이얼로그의 *Preview* 아래에 표시됩니다.

![파일 이름 만들기](/manuals/images/editor/create_file_name.png)

## 템플릿

각 프로젝트에 커스텀 템플릿을 지정할 수 있습니다. 이렇게 하려면 프로젝트의 루트 디렉토리에 `templates`라는 새 폴더를 만들고, `/templates/default.gui` 또는 `/templates/default.script`처럼 원하는 확장자를 가진 `default.*` 이름의 새 파일을 추가합니다. 또한 이 파일에서 `{% raw %}{{NAME}}{% endraw %}` 토큰을 사용하면 파일 생성 창에 지정된 파일 이름으로 대체됩니다.

지정된 파일 타입에 사용할 수 있는 템플릿이 있으면 이 타입의 새 파일을 만들 때마다 `templates`에 있는 파일의 컨텐츠로 초기화됩니다.


![템플릿](/manuals/images/editor/templates.png)

## 프로젝트로 파일 임포트하기

프로젝트에 에셋 파일(이미지, 사운드, 모델 등)을 추가하려면 *Assets* 브라우저의 올바른 위치로 간단히 드래그-앤-드롭하면 됩니다. 이렇게 하면 프로젝트 파일 구조의 선택한 위치에 파일의 _복사본_이 만들어집니다. 자세한 내용은 [매뉴얼의 에셋 임포트 방법](/ko/manuals/importing-assets/)을 읽어 보세요.

![파일 임포트](/manuals/images/editor/import.png)
...
자세한 내용은 [Getting Help 매뉴얼](/ko/manuals/getting-help/#getting-help)을 읽어 보세요.

![Show Logs](/manuals/images/editor/show_logs.png)
...
에디터의 외형은 커스텀 스타일링으로 변경할 수 있습니다. 자세한 내용은 [Editor Styling 매뉴얼](/ko/manuals/editor-styling)을 읽어 보세요.

## FAQ
{% include shared/ko/editor-faq.md %}