...
패키지를 마운트한 뒤에는 제외되어 있고 로드되지 않은 프록시를 `collectionproxy.set_collection()`으로 다른 컴파일된 컬렉션으로 전환할 수도 있습니다. 제한사항과 로드 순서는 [제외된 프록시의 컬렉션 변경하기](/ko/manuals/collection-proxy/#changing-an-excluded-proxys-collection)를 참고하세요.
