...
배열에는 지원되는 기능만 포함되며 기능 상수를 키로 사용하는 테이블이 아닙니다. 게임을 여러 그래픽 어댑터에서 실행할 수 있거나 드라이버 지원이 서로 다른 기기에서 실행할 수 있다면 컴퓨트 쉐이더를 사용하기 전에 항상 이 검사를 수행하십시오. OpenGL 및 OpenGL ES 지원 여부는 API 버전과 드라이버에 따라 달라집니다. Vulkan과 MoltenVK를 통한 Metal은 버전 1.0부터 컴퓨트 쉐이더를 지원합니다. Vulkan이 아직 기본 그래픽 백엔드가 아닌 플랫폼에서 Vulkan을 선택하려면 [애플리케이션 매니페스트](/ko/manuals/app-manifest)를 사용하십시오.