...
![팩토리 컴포넌트](/manuals/images/factory/factory_collection.png)

![팩토리 컴포넌트](/manuals/images/factory/factory_component.png)
...
![스폰된 게임 오브젝트](/manuals/images/factory/factory_spawned.png)
...
`[properties]`
: (선택 사항) 게임 오브젝트를 초기화할 때 사용할 스크립트 프로퍼티 값이 담긴 Lua 테이블입니다. 스크립트 프로퍼티에 대한 자세한 내용은 [스크립트 프로퍼티 매뉴얼](/ko/manuals/script-properties)을 참고하세요.
...
![프로퍼티와 스케일이 적용된 스폰된 게임 오브젝트](/manuals/images/factory/factory_spawned2.png)
...
Defold의 주소 지정(addressing) 메커니즘을 사용하면 실행 중인 게임의 모든 오브젝트와 컴포넌트에 액세스할 수 있습니다. [주소 지정 매뉴얼](/ko/manuals/addressing/)에서는 이 시스템이 동작하는 방식을 자세히 설명합니다. 스폰된 게임 오브젝트와 그 컴포넌트에도 같은 주소 지정 메커니즘을 사용할 수 있습니다. 예를 들어 메세지를 보낼 때는 스폰된 오브젝트의 id만 사용해도 충분한 경우가 많습니다.
...
![동적 로딩](/manuals/images/factory/load_dynamically.png)
...
![동적 프로토타입](/manuals/images/factory/dynamic_prototype.png)
...
*Collection* 아래의 *Max Instances* 프로젝트 설정은 각 컬렉션(월드)에 존재할 수 있는 게임 오브젝트 수의 상한입니다. Defold는 안전하다고 판단할 수 있는 경우 빌드 시 더 작은 용량을 할당할 수 있습니다. 에디터에서 배치했든 런타임에 스폰했든, 월드에서 동시에 살아 있는 모든 게임 오브젝트가 용량에 포함됩니다.

![최대 인스턴스](/manuals/images/factory/factory_max_instances.png)

실제 할당량은 빌드 타임 분석 결과에 따라 달라집니다.

* 빌드가 고정된 게임 오브젝트 수를 파악할 수 있으면(보통 컬렉션에 팩토리나 컬렉션 팩토리가 없는 경우) 용량은 *Max Instances*를 상한으로 하는 컴파일된 오브젝트 수입니다.
* 팩토리나 컬렉션 팩토리를 포함하는 컬렉션은 게임 오브젝트 용량으로 *Max Instances*를 사용합니다. 정적으로 참조된 프로토타입의 경우 빌드는 팩토리가 스폰할 수 있는 컴포넌트 타입을 식별합니다. 해당 타입은 각각의 프로젝트 최대 개수를 사용하고, 영향을 받지 않는 컴포넌트 타입은 계속 정확한 개수를 사용할 수 있습니다.
* *Dynamic Prototype*을 활성화하면 이를 소유한 컬렉션의 컴포넌트 개수 분석이 비활성화되므로 *Max Instances*와 설정된 컴포넌트별 최대 개수를 사용합니다.

동적 월드에서 동시에 살아 있을 수 있는 가장 많은 게임 오브젝트 수를 기준으로 *Max Instances*를 계획하세요. 다른 컴포넌트 제한이 계산되는 방식은 [컴포넌트 최대 개수 최적화](/ko/manuals/project-settings/#component-max-count-optimizations)를 참고하세요.
