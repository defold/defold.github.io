...
- anchor: creating-an-ios-application-bundle
  title: iOS 어플리케이션 번들 생성하기
- 커스텀 Info.plist 및 로컬 타겟 검색
- 연결된 iPhone에 번들 설치하고 실행하기
- anchor: creating-a-storyboard
  title: storyboard 생성하기
...
iOS에서는 빌드해서 휴대폰이나 태블릿에서 실행하려는 _모든_ 앱이 Apple에서 발급한 인증서와 provisioning profile로 서명되어야 합니다. 이 매뉴얼은 iOS용 게임을 번들링하는 데 필요한 단계를 설명합니다. 개발 중에는 컨텐츠와 코드를 기기로 직접 핫 리로드할 수 있으므로 [development app](/ko/manuals/dev-app)을 통해 게임을 실행하는 방식이 선호되는 경우가 많습니다.
...
앱에서 사용할 아이콘, launch screen storyboard 등은 [iOS section](/ko/manuals/project-settings/#ios)의 *game.project* 프로젝트 설정 파일에서 지정합니다.

### 커스텀 Info.plist 및 로컬 타겟 검색

내장 iOS `Info.plist`에는 릴리스가 아닌 빌드에서 에디터 타겟을 자동으로 검색하는 데 필요한 Bonjour 서비스와 로컬 네트워크 사용 설명이 포함되어 있습니다. 커스텀 `Info.plist`는 이 내장 기본 메니페스트를 대체합니다. 디버그 빌드에 커스텀 메니페스트를 사용하면서 로컬 네트워크를 통한 타겟 검색, 프로파일링, 핫 리로드 또는 로그 스트리밍이 필요하다면 다음 항목을 포함하세요.
...
Mustache 조건은 릴리스 번들에서 검색 항목을 제외합니다. 사용 설명 문자열은 iOS가 사용자에게 표시하며 조정하거나 현지화할 수 있습니다. 릴리스 어플리케이션 자체가 동일한 Bonjour 서비스와 로컬 네트워크 기능을 사용하는 경우에만 조건을 제거하세요.
...
에디터가 ios-deploy 도구의 설치 위치를 감지하지 못하면 [Preferences](/ko/manuals/editor-preferences/#tools)에서 위치를 지정해야 합니다.
...
이미지를 배치하고 필요한 다른 조정을 수행합니다. Label이나 다른 UI 요소를 추가할 수도 있습니다. 완료되면 active scheme을 **Any iOS Device (arm64)**(또는 **Generic iOS Device**)로 설정하고 **Product ▸ Build**를 선택합니다. Defold는 64-bit 디바이스의 iOS 15.0 이상을 지원하므로 deployment target을 15.0 이상으로 유지하세요. 빌드 프로세스가 끝날 때까지 기다립니다.
...
{% include shared/ko/apple-privacy-manifest.md %}
...
프로젝트가 면제 대상이라고 판단되면 프로젝트의 `Info.plist`에서 [`ITSAppUsesNonExemptEncryption`](https://developer.apple.com/documentation/bundleresources/information-property-list/itsappusesnonexemptencryption) 키를 `False`로 설정하세요. 자세한 내용은 [Application Manifests](/ko/manuals/extensions-manifest-merge-tool)를 참고하세요.
