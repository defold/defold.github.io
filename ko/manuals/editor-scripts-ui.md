...
이 매뉴얼은 Lua로 작성한 에디터 스크립트를 사용해 에디터에서 상호작용형 UI 요소를 만드는 방법을 설명합니다. 에디터 스크립트를 시작하려면 [Editor Scripts 매뉴얼](/ko/manuals/editor-scripts)을 참고하세요. 전체 editor API reference는 [여기](/ref/stable/editor-lua/)에서 볼 수 있습니다. 현재는 상호작용형 dialog만 만들 수 있지만, 앞으로 UI scripting 지원을 에디터의 나머지 영역으로 확장하려고 합니다.
...
![정렬](/manuals/images/editor_scripts/alignment.png)
...
![Padding과 Spacing](/manuals/images/editor_scripts/padding_and_spacing.png)
...
editor.ui.grid({
    ...
    columns = {% raw %}{{}, {grow = true}}{% endraw %}, -- 두 번째 column을 grow하도록 설정합니다
    children = {
        {
            editor.ui.label({
                text = "Level Name",
...
![입력](/manuals/images/editor_scripts/inputs_demo.png)
