...
- `editor.get(node_id, property)` — 에디터 안의 어떤 노드 값을 가져옵니다. 에디터의 노드는 스크립트 또는 컬렉션 파일, 컬렉션 안의 게임 오브젝트, 리소스로 로드된 json 파일 등 다양한 엔티티입니다. `node_id`는 에디터가 에디터 스크립트에 전달하는 userdata입니다. 또는 노드 id 대신 리소스 경로를 전달할 수 있습니다. 예: `"/main/game.script"`. `property`는 문자열입니다. 현재 지원되는 프로퍼티는 다음과 같습니다.
  ...
  - `"parent"` — 부모가 있는 Outline 노드의 부모 에디터 노드입니다.
  ...
  - GUI 파일: `layers`, `fonts`, `materials`, `textures`, `particlefxs`, `nodes`, `layouts` 같은 노드 목록
...
- `editor.properties(node_id)` — 노드에서 읽을 수 있는 프로퍼티 이름을 context에 맞게 정렬한 목록으로 반환합니다. 예: `pprint(editor.properties("/game.project"))`. 나열된 프로퍼티를 변경, reset, 추가 또는 순서 변경할 수 있는지는 `editor.can_*` 함수로 확인하세요.
...
- `editor.ui.*` — 다양한 UI 관련 함수입니다. [UI 매뉴얼](/ko/manuals/editor-scripts-ui)을 참고하세요.
...
에디터 스크립트 모듈이 `get_commands()`를 정의하면 익스텐션을 다시 로드할 때 호출됩니다. 반환된 커맨드는 `locations`에 따라 메뉴 바의 메뉴와 Assets, Outline, Scene, Code 컨텍스트 메뉴에 나타날 수 있습니다. 예:
...
- `locations`(필수) — 이 커맨드를 사용할 위치를 설명하는 배열입니다. 지원되는 값은 해당 메뉴 바 메뉴를 의미하는 `"Edit"`, `"View"`, `"Project"`, `"Debug"`, `"Help"`, **Project → Bundle** 하위 메뉴를 의미하는 `"Bundle"`, 해당 컨텍스트 메뉴를 의미하는 `"Assets"`, `"Outline"`, `"Scene"`, `"Code"`입니다.
...
outline 프로퍼티 외에도 GUI 파일은 여러 노드 목록 프로퍼티를 정의합니다.

- `layers` — 레이어 에디터 노드 목록입니다(순서 변경 가능).
- `fonts` — 폰트 에디터 노드 목록입니다.
- `materials` — 메터리얼 에디터 노드 목록입니다.
- `textures` — 텍스쳐 에디터 노드 목록입니다.
- `particlefxs` — Particle FX 에디터 노드 목록입니다.
- `nodes` — GUI 노드 에디터 노드 목록입니다.
- `layouts` — GUI layout 에디터 노드 목록입니다.
...
- `on_bundle_started(opts)` — 번들을 만들거나 게임의 HTML5 버전을 Build할 때 실행됩니다. `on_build_started`와 마찬가지로, 이 hook이 트리거한 변경사항은 번들에 나타나고, 오류는 번들을 중단시킵니다. `opts`는 다음 키를 가집니다.
  ...
  - `platform` — 게임이 번들되는 플랫폼입니다. 가능한 플랫폼 값 목록은 [Bob 매뉴얼](/ko/manuals/bob)을 참고하세요.
...
에디터는 [Language Server Protocol](https://microsoft.github.io/language-server-protocol/)의 일부를 지원합니다. 진단(lint), 자동 완성, 마우스를 올렸을 때 표시되는 정보, Structure 창의 문서 심볼, 정의로 이동, 참조 찾기, 심볼 이름 바꾸기, 문서 및 범위 포멧팅을 지원합니다. 심볼 위에 마우스를 올리면 언어 서버의 정보를 볼 수 있습니다. 심볼에 커서를 둔 상태에서 <kbd>F2</kbd>로 이름을 바꾸고, <kbd>F12</kbd>로 정의로 이동하거나, <kbd>Shift+F12</kbd>로 참조를 찾을 수 있습니다. 이 동작은 <kbd>Edit</kbd> 메뉴에서도 사용할 수 있습니다. 포멧팅 명령과 저장 시 포멧팅 환경설정은 [코드 포멧팅](/ko/manuals/writing-code/#formatting-code)을 참고하세요.

기본으로 제공되는 Lua 언어 서버에는 런타임 및 에디터 스크립트 API의 Defold 타입 어노테이션이 포함되어 있습니다. `.editor_script` 파일에서는 자동 완성과 진단 기능이 `editor.*` 함수와 인자 및 반환 타입을 인식합니다. [코드 자동 완성](/ko/manuals/writing-code/#code-completion)을 참고하세요.
...
- `languages`(필수) — 서버가 관심을 가지는 언어 목록입니다. [여기](https://code.visualstudio.com/docs/languages/identifiers#known-language-identifiers)에 정의되어 있습니다(파일 확장자도 동작합니다).
