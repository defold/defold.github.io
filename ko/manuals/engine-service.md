...
이 서비스는 Defold 에디터에 속하며 열린 프로젝트를 제어하는 [에디터 서버](/ko/manuals/editor-http-api)와 별개입니다.
...
![Defold 디버그 빌드의 엔진 서비스 포트 정보](/manuals/images/automation/engine-service.png)
...
![엔진 서버 액세스](/manuals/images/automation/engine-server.png)
...
`/post` 경로는 핫 리로드, 재부팅, 크기 변경 및 프로세스 제어와 같은 개발 동작에서 사용됩니다. 본문은 경로에 이름이 지정된 유형의 바이너리 Protobuf 메세지이며 JSON 메세지 API가 아닙니다.
...
런타임 스크립트, 물리, 입력, 동적으로 생성된 오브젝트 및 플랫폼 렌더링에는 실행 중인 엔진이 필요하며 [자동화 런타임 테스트](/ko/manuals/automated-testing)를 통해 검증해야 합니다.
