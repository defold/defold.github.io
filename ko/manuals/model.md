...
![Model in game object](/manuals/images/model/model_gltf.png)
...
*Scene*
: 모델의 지오메트리가 들어 있는 glTF *.gltf* 또는 *.glb* 파일입니다. 파일에 morph target이 포함되어 있으면 씬과 함께 임포트합니다. Defold 1.13.2 이전에는 이 프로퍼티의 이름이 *Mesh*였습니다.

*Mesh*
: 선택한 *Scene*의 이름 있는 메쉬를 선택하는 옵션이며, Defold 1.13.2부터 사용할 수 있습니다. 이 필드를 비워 두면 임포트된 트랜스폼을 적용해 전체 씬을 렌더링합니다. 메쉬를 선택하면 glTF 노드 트랜스폼 없이 해당 메쉬의 로컬 좌표로 한 번 렌더링합니다. 선택한 메쉬를 배치하려면 모델 컴포넌트나 게임 오브젝트의 위치, 회전, 스케일을 조정하세요.
...
: 이 프로퍼티를 텍스쳐가 있는 3D 오브젝트에 적합하도록 직접 만든 메터리얼로 설정합니다. 시작점으로 사용할 수 있는 내장 메터리얼이 몇 가지 있습니다:
  ...
  * 정적 인스턴싱된 모델에는 *model_instanced.material*을 사용합니다
  * 스킨드(애니메이션되는) 인스턴싱되지 않은 모델에는 *model_skinned.material*을 사용합니다
  * 스킨드(애니메이션되는) 인스턴싱된 모델에는 *model_skinned_instanced.material*을 사용합니다
...
![Wiggler ingame](/manuals/images/model/runtime.png)

### 런타임 애니메이션

Defold는 런타임에 애니메이션을 제어하는 강력한 지원을 제공합니다. 자세한 내용은 [모델 애니메이션 매뉴얼](/ko/manuals/model-animation)을 참고하세요:
...
모델은 glTF morph target 애니메이션도 사용할 수 있습니다. morph target weight는 다른 모델 애니메이션처럼 `model.play_anim()`으로 애니메이션되며, 런타임에 [`model.get_blend_weights()`](/ref/model#model.get_blend_weights)와 [`model.set_blend_weights()`](/ref/model#model.set_blend_weights)를 사용해 읽거나 오버라이드할 수 있습니다. 자세한 내용은 모델 애니메이션 매뉴얼의 [morph target 섹션](/ko/manuals/model-animation#morph-targets)을 참고하세요.
...
`textureN`
: N이 0-15인 모델 텍스쳐입니다(`hash`). `go.get()`으로 이 프로퍼티를 읽고, 텍스쳐 리소스 프로퍼티와 `go.set()`을 사용해 변경할 수 있습니다. Defold는 드로우마다 최대 16개의 텍스쳐를 지원하지만, 텍스쳐 sampler 제한이 더 작은 그래픽 어댑터에서는 쉐이더가 사용할 수 있는 수가 더 적을 수 있습니다.
...
내장 모델 메터리얼은 로컬 버텍스 공간을 사용합니다. 스킨드 모델에서 로컬 버텍스 공간을 사용하면 버텍스 쉐이더가 본 행렬 텍스쳐를 사용해 GPU에서 스키닝을 수행할 수 있습니다. 모델 인스턴싱에도 로컬 버텍스 공간이 필요합니다. 따라서 GPU 스키닝 또는 인스턴싱 모델용 커스텀 메터리얼은 *Local* 버텍스 공간 설정을 사용해야 합니다.

본 행렬 캐시는 `RGBA32F` 텍스쳐를 사용합니다. 활성 그래픽 어댑터가 이 텍스쳐 형식을 지원하지 않으면 Defold는 로컬 공간 메터리얼을 사용하는 애니메이션 Model 컴포넌트를 생성할 수 없습니다. 따라서 OpenGL ES 2.0과 WebGL 1.0에서는 어댑터의 부동 소수점 텍스쳐 확장 지원 여부에 따라 달라집니다. 이를 지원하지 않는 어댑터와 호환하려면 CPU 스키닝을 사용하고 모델 인스턴싱을 사용할 수 없는 커스텀 월드 공간 메터리얼을 사용하세요. 캐시 크기는 [Model 프로젝트 설정](/ko/manuals/project-settings/#model)에서 조정할 수 있습니다.

모델용 커스텀 메터리얼을 만들어야 한다면 [메터리얼 문서](/ko/manuals/material)를 참고하세요. [쉐이더 매뉴얼](/ko/manuals/shader)에는 쉐이더 프로그램이 어떻게 동작하는지에 대한 정보가 있습니다.


### 메터리얼 상수

{% include shared/ko/material-constants.md component='model' variable='tint' %}
...
렌더 스크립트가 어떻게 동작하는지에 대한 자세한 내용은 [렌더 문서](/ko/manuals/render)를 참고하세요.