...
![프로젝트 설정](/manuals/images/project-settings/settings.jpg)
...
[`sys.get_config_string(key)`](/ref/sys/#sys.get_config_string), [`sys.get_config_number(key)`](/ref/sys/#sys.get_config_number), [`sys.get_config_int(key)`](/ref/sys/#sys.get_config_int), [`sys.get_config_boolean(key)`](/ref/sys/#sys.get_config_boolean)을 사용하면 런타임에 *game.project* 값을 읽을 수 있습니다. 예:
...
#### Dependencies
프로젝트 *Library URL*들의 URL 목록입니다. 자세한 내용은 [Libraries 매뉴얼](/ko/manuals/libraries/)을 참고하세요.
...
#### Custom Resources
`custom_resources`
{% include shared/ko/custom-resources.md %}

커스텀 리소스를 로드하는 방법은 [File Access 매뉴얼](/ko/manuals/file-access/#how-to-access-files-bundled-with-the-application)에서 더 자세히 다룹니다.

`ext.properties`의 `custom_resources.default`를 통해 익스텐션이 제공하는 경로는 이 설정과 결합됩니다. 예제는 [익스텐션의 커스텀 리소스](/ko/manuals/extensions/#custom-resources)를 참고하세요.

#### Bundle Resources
`bundle_resources`
{% include shared/ko/bundle-resources.md %}

번들 리소스를 로드하는 방법은 [File Access 매뉴얼](/ko/manuals/file-access/#how-to-access-files-bundled-with-the-application)에서 더 자세히 다룹니다.
...
#### Include Dirs
라이브러리 공유를 통해 프로젝트에서 공유할 디렉토리를 공백으로 구분한 목록입니다. 자세한 내용은 [Libraries 매뉴얼](/ko/manuals/libraries/)을 참고하세요.
...
이 설정은 창을 제어합니다. 오프스크린 [멀티샘플 렌더 타겟](/ko/manuals/render/#multisampled-render-targets)은 자체 샘플 수를 사용합니다.
...
#### Display Profiles
사용할 display profiles 파일을 지정합니다. 기본값은 `/builtins/render/default.display_profilesc`입니다. 자세한 내용은 [GUI Layouts 매뉴얼](/ko/manuals/gui-layouts/#creating-display-profiles)을 참고하세요.
...
`graphics.vulkan_version_major`는 Vulkan context/API major 버전 hint입니다. Vulkan 그래픽 백엔드를 선택한 경우에만 적용됩니다. 기본값은 `1`입니다.
...
`shader.glsl_es_default_precision_float`는 cross-compile된 GLSL ES 쉐이더에서 부동 소수점 값의 기본 전역 precision qualifier를 설정합니다. 유효한 값은 `mediump`와 `highp`이며 기본값은 `mediump`입니다.

#### GLSL ES Default Precision Int
`shader.glsl_es_default_precision_int`는 cross-compile된 GLSL ES 쉐이더에서 정수 값의 기본 전역 precision qualifier를 설정합니다. 유효한 값은 `mediump`와 `highp`이며 기본값은 `highp`입니다.
...
GUI 스크립트는 [`gui.set_safe_area_mode()`](/ref/gui/#gui.set_safe_area_mode)를 사용하여 해당 씬의 모드를 재정의할 수 있습니다. 플랫폼 지원과 커스텀 레이아웃에 대한 내용은 [안전 영역 안내](/ko/manuals/porting-guidelines/#mobile-phones-and-notch-and-hole-punch-cameras)를 참고하세요.
...
#### Launch Screen
Storyboard 파일(.storyboard)입니다. 만드는 방법에 대한 자세한 내용은 [iOS 매뉴얼](/ko/manuals/ios/#creating-a-storyboard)을 참고하세요.
...
#### Info.plist
지정하면 앱을 번들링할 때 내장 iOS 기본 메니페스트 대신 이 *`Info.plist`* 파일을 사용합니다. 내장 메니페스트에는 릴리스가 아닌 빌드에서 에디터 타겟을 검색하는 데 필요한 로컬 네트워크 및 Bonjour 항목이 들어 있습니다. 커스텀 메니페스트를 제공하면서 디바이스의 타겟 검색, 프로파일링, 핫 리로드 또는 로그 스트리밍이 필요하다면 [iOS 매뉴얼](/ko/manuals/ios/#creating-an-ios-application-bundle)에 설명된 항목을 유지하세요.
...
Defold의 기본 규칙을 바로 사용하려면 `/builtins/manifests/android/dmengine.keep`를 선택합니다. 익스텐션은 자체 [keep 규칙](/ko/manuals/extensions/#r8-keep-rules-for-android)을 제공하며 이 파일과 결합됩니다.

프로젝트별 규칙을 추가해야 할 때만 내장 파일을 프로젝트로 복사하세요. 커스텀 파일을 선택하면 프로젝트 규칙 전체를 대체하므로, 복사본에 내장 규칙을 보존해야 합니다.

R8을 활성화하고 릴리스 번들과 함께 난독화 매핑을 보관하는 방법은 [Android 매뉴얼](/ko/manuals/android/#shrinking-java-code-with-r8)을 참고하세요.
...
#### App Icon
Windows에서 어플리케이션 아이콘으로 사용할 이미지 파일(.ico)입니다. .ico 파일을 만드는 방법은 [Windows 매뉴얼](/ko/manuals/windows)에서 더 자세히 확인할 수 있습니다.

---

### HTML5

이 옵션들 중 다수에 대한 자세한 내용은 [HTML5 platform 매뉴얼](/ko/manuals/html5/)을 참고하세요.
...
#### Retry Count
시작 중 다운로드가 실패한 후 재시도하는 횟수입니다. 엔진의 JavaScript 또는 WebAssembly 파일에서 발생한 네트워크 오류, 실패한 HTTP 상태 및 크기 불일치가 포함됩니다. 최초 요청은 별도로 계산합니다. 아카이브 파일 검증에는 자체 재시도 제한이 있습니다. [다운로드 검증](/ko/manuals/html5/#download-verification)과 `Retry Time`을 참고하세요.

#### Retry Time
다운로드가 실패했을 때 파일 다운로드를 다시 시도하기 전까지 기다릴 시간입니다. 단위는 초입니다(`Retry Count` 참고).

#### Verify Downloaded File Size
`html5.verify_downloaded_file_size`는 다운로드한 엔진 및 아카이브 파일을 예상 크기와 비교합니다. 기본적으로 활성화되어 있습니다(`true`). 서버, 프록시 또는 CDN이 파일을 의도적으로 변경하여 크기가 달라지는 경우에만 `false`로 설정하세요. 검증에 실패하면 시작이 실패하기 전에 다운로드를 재시도합니다. 엔진 다운로드와 아카이브 파일 검증의 재시도 제한은 다릅니다. [다운로드 검증](/ko/manuals/html5/#download-verification)을 참고하세요.
...
`liveupdate.enabled`는 런타임에 Live update 시스템을 활성화합니다. 기본적으로 활성화되어 있습니다. 리소스를 제외하고, 다운로드하고, 마운트하는 방법은 [Live update 매뉴얼](/ko/manuals/live-update/)을 참고하세요.
...
#### _App Manifest_
설정하면 app manifest를 사용하여 엔진 빌드를 커스터마이즈합니다. 이를 통해 엔진에서 사용하지 않는 부분을 제거하여 최종 바이너리 크기를 줄일 수 있습니다. 사용하지 않는 기능을 제외하는 방법은 [application manifest 매뉴얼](/ko/manuals/app-manifest)에서 확인하세요.

---

### Profiler

App Manifest의 **Profiler** 설정은 프로파일러 코드를 디버그 빌드와 릴리스 빌드에 링크할지 제어합니다. 아래 설정은 선택한 빌드에 포함된 프로파일러 코드의 런타임 동작을 제어합니다. 자세한 내용은 [프로파일링 매뉴얼](/ko/manuals/profiling/)을 참고하세요.
...
#### Max Sample Count
`profiler.max_sample_count`는 thread마다 프레임마다 기록되는 프로파일러 sample의 최대 수입니다. 기본값은 `4096`, 최소값은 `128`입니다. 실제 프로파일이 제한을 초과할 때만 늘리세요. 먼저 네이티브 익스텐션 프로파일링 코드에서 짝이 맞지 않는 scope begin/end 호출을 확인해야 합니다.
...
커스텀 값은 [런타임 액세스](#runtime-access)에 설명된 일치하는 함수로 읽을 수 있습니다.
...
{% include shared/ko/component-max-count-optimizations.md %}


## 커스텀 프로젝트 설정

메인 프로젝트 또는 [네이티브 익스텐션](/ko/manuals/extensions/)에 커스텀 설정을 정의할 수 있습니다. 메인 프로젝트의 커스텀 설정은 프로젝트 루트의 `game.properties` 파일에 정의해야 합니다. `ext.properties`라는 파일은 프로젝트와 가져온 라이브러리 종속성 어디에서나 발견되며, 이웃한 `ext.manifest`가 필요하지 않습니다. 발견된 모든 익스텐션 메타데이터를 병합한 뒤 루트의 `game.properties` 파일을 적용하며, 이 파일이 앞선 값을 override할 수 있습니다.
...
Bob과 에디터 모두 이 메타데이터 파일을 파싱합니다. 에디터는 이를 사용해 *game.project* viewer에 대응하는 필드, 선택 항목, 검증, help tooltip을 만듭니다.