...
초기 카메라 뷰는 리소스에 따라 다릅니다. 모델과 glTF 씬 같은 3D 리소스는 기본적으로 **원근 투영(perspective)**을, 스프라이트, 타일맵, GUI 씬 같은 2D 리소스는 **직교 투영(orthographic)**을 사용합니다. 씬 툴바에서 카메라 방향, 투영, 그리드를 변경할 수 있습니다.
...
## 저장되는 씬 뷰 {#remembered-scene-views}

에디터는 씬 리소스의 탭을 닫거나 에디터를 종료할 때 각 리소스의 카메라 상태를 저장합니다. 같은 리소스를 다시 열면 뷰를 복원하므로, 컬렉션이나 모델마다 서로 다른 카메라 위치, 방향, 투영을 유지할 수 있습니다.

가시성 필터도 씬별로 저장됩니다. 한 씬에서 모델이나 컴포넌트 가이드를 숨겨도 다른 씬에 같은 필터를 사용할 필요는 없습니다. 에디터 뷰 설정이므로 게임의 카메라나 런타임 가시성에는 영향을 주지 않습니다.

저장된 카메라 상태가 없는 리소스 중 모델, 메쉬, glTF 리소스는 원근 투영으로 시작합니다. 충돌 오브젝트는 프로젝트의 2D/3D 물리 설정에 따라 뷰를 선택하고, 컬렉션과 게임 오브젝트는 씬 지오메트리를 기준으로 초기 뷰를 선택합니다.
...
![오브젝트 이동](/manuals/images/editor/move.png)![3D 오브젝트 이동](/manuals/images/editor/move_3d.png)
...
![오브젝트 회전](/manuals/images/editor/rotate.png)![3D 오브젝트 회전](/manuals/images/editor/rotate_3d.png)
...
![오브젝트 스케일 조정](/manuals/images/editor/scale.png)![3D 오브젝트 스케일 조정](/manuals/images/editor/scale_3d.png)
...
에디터는 2D와 3D 뷰의 그리드 설정을 따로 유지합니다. 원하는 모드가 활성화된 상태에서 크기, 평면, 표시 모양을 설정하세요. 모드를 전환하면 해당 모드의 그리드 설정을 복원합니다. **Reset to Defaults**는 활성 모드의 설정을 초기화합니다.

![Grid Settings](/manuals/images/editor/grid_popup.png)
...
![Perspective Camera Settings](/manuals/images/editor/camera_popup.png)
