...
![Blender의 모델](/manuals/images/model/blender_gltf.png)
...
![임포트된 모델 에셋](/manuals/images/model/assets_gltf.png)
...
Defold 1.13.2부터 [Model 컴포넌트](/ko/manuals/model/#model-properties)에서 임포트된 씬의 이름 있는 메쉬 하나를 선택할 수 있습니다. *Mesh* 필드를 비워 두면 전체 씬을 사용하고 위에서 설명한 트랜스폼을 보존합니다. 메쉬를 선택하면 glTF 노드 트랜스폼 없이 로컬 지오메트리를 사용하므로, 모델 컴포넌트나 게임 오브젝트의 트랜스폼으로 배치하세요.

이전 버전의 Defold로 만든 모델을 다시 임포트한 후 위치나 방향이 달라졌다면 Blender 또는 다른 저작 도구에서 트랜스폼을 수정하고 *.gltf* 또는 *.glb* 파일을 다시 익스포트하세요.
</div>

## 모델 사용하기
모델을 임포트한 뒤에는 [모델 컴포넌트](/ko/manuals/model)에서 사용합니다:

1. *Assets* pane에서 <kbd>New... ▸ Model</kbd>로 Model 파일을 만들거나, <kbd>Add Component ▸ Model</kbd>로 게임 오브젝트에 Model 컴포넌트를 직접 추가합니다.
2. *Scene* 프로퍼티를 임포트된 *.gltf* 또는 *.glb* 파일로 설정합니다. 전체 씬을 사용하려면 *Mesh*를 비워 두고, 로컬 지오메트리만 사용하려면 이름 있는 메쉬를 선택합니다.
3. 애니메이션되는 모델의 경우 *Skeleton* 프로퍼티를 스켈레톤이 들어 있는 *.gltf* 또는 *.glb* 파일로 설정합니다. 메쉬, 스켈레톤, 애니메이션을 함께 익스포트했다면 *Scene*에 사용한 파일과 같은 경우가 많습니다.
4. 애니메이션용 *Animation Set* 파일을 만들고 *Animations* 프로퍼티에 할당합니다. 애니메이션을 자동으로 시작하려면 *Default Animation*을 설정합니다.
5. *Material* 프로퍼티를 모델에 적합한 메터리얼로 설정합니다. 내장 *model.material*, *model_instanced.material*, *model_skinned.material*, *model_skinned_instanced.material* 파일은 유용한 시작점입니다. 스킨드 메터리얼은 GPU에서 스키닝을 실행할 수 있도록 로컬 버텍스 공간을 사용합니다. GPU 스키닝 또는 인스턴싱 모델용 커스텀 메터리얼도 로컬 버텍스 공간을 사용해야 합니다. 그래픽 어댑터 요구 사항은 [모델 매뉴얼](/ko/manuals/model/#material)을 참고하세요.
...
![모두 선택](/manuals/images/model/blender_select_all_vertices.png)

모든 버텍스를 선택한 뒤에는 메쉬를 언랩하여 UV 레이아웃을 얻습니다:

![메쉬 언랩](/manuals/images/model/blender_unwrap_mesh.png)

그런 다음 UV 레이아웃을 텍스쳐로 사용할 수 있는 이미지로 익스포트합니다:

![UV 레이아웃 익스포트](/manuals/images/model/blender_export_uv_layout.png)

![UV 레이아웃 익스포트 결과](/manuals/images/model/blender_export_uv_layout_result.png)

### Blender로 익스포트하기
<kbd>File ▸ Export ▸ glTF 2.0 (.glb/.gltf)</kbd>를 사용해 Blender에서 모델을 익스포트합니다.

![Blender로 익스포트하기](/manuals/images/model/export_gltf.png)
...
![Blender로 익스포트하기](/manuals/images/model/export_settings.png)