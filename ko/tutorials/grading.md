...
Defold 에디터 사용법을 알고 있으며, GL 쉐이더와 Defold 렌더링 파이프라인을 기본적으로 이해하고 있다고 가정합니다. 이 주제들을 더 읽어야 한다면 [Shader 매뉴얼](/ko/manuals/shader/)과 [Render 매뉴얼](/ko/manuals/render/)을 확인하세요.
...
컴포넌트의 쉐이더 프로그램을 픽셀 데이터를 수정하거나 완전히 새로운 픽셀 색상을 프로그래밍 방식으로 만드는 쉐이더 프로그램으로 교체할 수 있습니다. [Shadertoy 튜토리얼](/ko/tutorials/shadertoy)에서 그 방법을 배울 수 있습니다.
...
![렌더 타겟](/tutorials/images/grading/render_target.png)
...
![채도가 낮아진 게임](/tutorials/images/grading/desaturated_game.png)
...
![색상 큐브](/tutorials/images/grading/color_cube.png)
...
![렌더 타겟 그레이딩](/tutorials/images/grading/render_target_grading.png)
...
![룩업 텍스쳐](/tutorials/images/grading/lut.png)
...
![룩업 테이블](/tutorials/images/grading/lut_lookup.png)
...
![룩업 테이블 필터링](/tutorials/images/grading/lut_filtering.png)
...
3. *`Filter min`* 프로퍼티를 `FILTER_MODE_MIN_LINEAR`로 설정하고 *`Filter mag`* 프로퍼티를 `FILTER_MODE_MAG_LINEAR`로 설정합니다.
    ...
    ![룩업 테이블 샘플러](/tutorials/images/grading/material_lut_sampler.png)

4. 다음 룩업 테이블 텍스쳐(*`lut16.png`*)를 다운로드하여 프로젝트에 추가합니다.

    ![16색 룩업 테이블](/tutorials/images/grading/lut16.png)

5. *`main.collection`*을 열고 *`lut`* 텍스쳐 프로퍼티를 다운로드한 룩업 텍스쳐로 설정합니다.

    ![quad 모델 룩업 테이블](/tutorials/images/grading/quad_lut.png)
...
![원래 모습의 월드](/tutorials/images/grading/world_original.png)

지금까지는 모든 것을 올바르게 처리한 것처럼 보이지만, 표면 아래에 문제가 숨어 있습니다. 그라디언트 테스트 텍스쳐가 있는 스프라이트를 추가하면 어떤 일이 일어나는지 보세요.

![파란색 밴딩](/tutorials/images/grading/blue_banding.png)
...
![밴딩이 없는 파란색](/tutorials/images/grading/blue_no_banding.png)
...
![Affinity의 월드](/tutorials/images/grading/world_graded_affinity.png)
...
![그레이딩된 월드](/tutorials/images/grading/world_graded.png)
