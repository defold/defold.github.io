...
에디터에서 [열거나](/ko/manuals/project-setup/) [GitHub에서 다운로드할 수 있는](https://github.com/defold/sample-levelcomplete) 이 샘플 프로젝트에서는 레벨이 완료되었을 때 점수가 올라가는 모습을 보여주는 효과를 시연합니다. 총점이 올라가며, 서로 다른 점수 단계에 도달하면 별 세 개가 나타납니다. 이 샘플은 값을 조정할 때 빠르게 반복 작업할 수 있도록 리로드 기능도 사용합니다.
