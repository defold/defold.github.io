...
![근사 속도 적분](/tutorials/images/platformer/integration.png)
...
![속도 방정식](/tutorials/images/platformer/equationofvelocity.png)

주황색 영역은 현재 프레임 동안 플레이어 캐릭터에 적용해야 하는 이동량입니다. 기하학적으로 이 영역은 다음과 같이 근사할 수 있습니다.

![이동량 방정식](/tutorials/images/platformer/equationoftranslation.png)
...
Defold에서 입력을 처리하는 방법이 확실하지 않다면 [여기](/ko/manuals/input)의 가이드를 참고하세요.
...
![플레이어 캐릭터에 작용하는 접촉 normal](/tutorials/images/platformer/collision.png)
...
![레벨 지오메트리와 플레이어가 월드에 배치된 Defold 에디터](/tutorials/images/platformer/editor.png)
...
![하나로 이어 붙인 여러 타일 기반 폴리곤](/tutorials/images/platformer/stitching.png)

위는 플랫포머 그래픽 조각으로 이웃한 타일 다섯 개를 만든 예입니다. 이미지에서 배치된 타일(위쪽)이 하나로 이어 붙여진 단일 모양(아래쪽 회색 윤곽)에 어떻게 대응하는지 볼 수 있습니다.

자세한 내용은 [물리](/ko/manuals/physics)와 [타일](/ko/manuals/2dgraphics) 가이드를 확인하세요.
