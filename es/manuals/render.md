...
- Recursos de render
- anchor: multisampled-render-targets
  title: Render targets con multimuestreo
- Handles de textura
...
![Render integrado](/manuals/images/render/builtin.png)
...
Puedes encontrar una descripción detallada de cómo funcionan los materiales en la [documentación de Material](/es/manuals/material).
...
![Proyección Stretch](/manuals/images/render/stretch_projection.png)

*Proyección Stretch con el tamaño original de la ventana*

![Proyección Stretch al cambiar el tamaño](/manuals/images/render/stretch_projection_resized.png)
...
![Proyección Fixed Fit](/manuals/images/render/fixed_fit_projection.png)

*Proyección Fixed Fit con el tamaño original de la ventana*

![Proyección Fixed Fit al cambiar el tamaño](/manuals/images/render/fixed_fit_projection_resized.png)

*Proyección Fixed Fit con la ventana estirada horizontalmente*

![Proyección Fixed Fit al reducir el tamaño](/manuals/images/render/fixed_fit_projection_resized_smaller.png)
...
![Proyección Fixed](/manuals/images/render/fixed_projection_zoom_2_0.png)

*Proyección Fixed con el zoom definido en 2*

![Proyección Fixed](/manuals/images/render/fixed_projection_zoom_0_5.png)

*Proyección Fixed con el zoom definido en 0.5*

![Proyección Fixed](/manuals/images/render/fixed_projection_zoom_2_0_resized.png)
...
1 = La caja delimitadora de Mesh debe definirla el desarrollador. [Más información](/es/manuals/mesh/#frustum-culling).
...
![Recursos de render](/manuals/images/render/render_resources.png)
...
### Render targets con multimuestreo {#multisampled-render-targets}

Los render targets admiten anti-aliasing con multimuestreo (MSAA). Esto suaviza los bordes de la geometría en una pasada de renderizado fuera de pantalla. El número de muestras del target es independiente de [Display ▸ Samples](/es/manuals/project-settings/#samples), que controla el anti-aliasing de la ventana.

Para un recurso `.render_target`, define **Sample Count** en el editor como `1`, `2`, `4`, `8` o `16`. El valor `1` desactiva el multimuestreo. Agrega el recurso a la tabla **Render Resources** de tu archivo `.render` y usa su nombre asignado con `render.set_render_target()`, como en el ejemplo anterior.

También puedes crear un target en `init()` de tu script de render. Coloca `sample_count` en la tabla de parámetros externa, junto a los adjuntos:
...
Este ejemplo usa un target solo de color. Todos los adjuntos de color, profundidad y stencil de un target comparten su número de muestras. Agrega un adjunto de profundidad y el estado habitual de prueba de profundidad si la pasada requiere comprobar la profundidad.

Para el siguiente fragmento de `update()`, asigna la etiqueta `scene` a los materiales de la escena y la etiqueta `present` al material de un cuadrilátero de pantalla completa. El material del cuadrilátero debe muestrear la unidad de textura `0`. Define la vista y la proyección adecuadas para cada pasada:
...
Cambiar a otro target finaliza la pasada y resuelve automáticamente sus adjuntos de color con multimuestreo. `render.enable_texture()` enlaza la textura de color resuelta, por lo que el cuadrilátero usa un sampler de textura normal. No se requiere un comando de resolución adicional.

El número de muestras solicitado es `1` de forma predeterminada y debe ser un entero positivo. Los backends gráficos reducen las solicitudes no compatibles a una potencia de dos admitida, recurriendo a `1` si es necesario, y registran una advertencia cuando el número cambia. Un número mayor de muestras aumenta la memoria necesaria para los adjuntos.

Al usar un recurso de render target, consulta su número efectivo de muestras desde un `.script` de objeto de juego con `resource.get_render_target_info()`. Por ejemplo, después de agregar `/render/offscreen.render_target` a **Render Resources**:
...
Usa este número efectivo para comprobar la compatibilidad del dispositivo en lugar de suponer que el número solicitado estaba disponible. Consulta [`render.render_target()`](/ref/beta/render/#render.render_target:parameters) y [`resource.get_render_target_info()`](/ref/beta/resource/#resource.get_render_target_info:path) para ver las tablas completas de parámetros y resultados.
...
Una draw call es el término usado para describir el proceso de configurar la GPU para dibujar un objeto en la pantalla usando una textura y un material con ajustes adicionales opcionales. Este proceso suele consumir muchos recursos y se recomienda que el número de draw calls sea lo más bajo posible. Puedes medir el número de draw calls y el tiempo que lleva renderizarlas usando el [profiler integrado](/es/manuals/profiling/).
...
Cada llamada a `render.draw()` controla cómo se ordenan las entradas coincidentes ordenadas en el mundo. El valor predeterminado es `render.SORT_BACK_TO_FRONT`; usa `render.SORT_FRONT_TO_BACK` para renderizar de cerca a lejos, o `render.SORT_NONE` para conservar el orden de inserción:
...
El orden seleccionado determina qué entradas quedan adyacentes y, por tanto, puede afectar al batching. En esa lista ordenada, cada objeto se agrupa en la misma draw call que el objeto anterior si se cumplen las siguientes condiciones:
...
Esto significa que si dos componentes sprite del mismo proxy de colección quedan adyacentes después del ordenamiento seleccionado y usan la misma textura, material y constantes, se agruparán en la misma draw call.
