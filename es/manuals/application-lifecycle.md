...
![Visión general del ciclo de vida](/manuals/images/application_lifecycle/application_lifecycle.png)
...
![Inicialización](/manuals/images/application_lifecycle/initialization.png)
...
![Preinicialización](/manuals/images/application_lifecycle/pre_init.png)
...
![Inicialización de colección](/manuals/images/application_lifecycle/collection_init.png)
...
![Post Update](/manuals/images/application_lifecycle/post_init.png)
...
Es completamente posible cargar un [proxy de colección](/es/manuals/collection-proxy) durante `init()`, asegurarse de que todos los objetos que contiene se inicialicen y luego descargar la colección a través del proxy, todo esto antes de que se llame al primer `update()` de un componente; es decir, antes de que el motor haya salido de la etapa de inicialización y haya entrado en el bucle de actualización:
...
![Bucle de actualización](/manuals/images/application_lifecycle/update_loop.png)
...
El input se lee desde los dispositivos disponibles, se mapea contra [input bindings](/es/manuals/input) y luego se despacha. Cualquier objeto de juego que haya adquirido el foco de input recibe input enviado a las funciones `on_input()` de todos sus componentes. Un objeto de juego con un componente script y un componente GUI con un script GUI recibirá input en las funciones `on_input()` de ambos componentes, siempre que estén definidas y hayan adquirido el foco de input.

![Fase de Input](/manuals/images/application_lifecycle/input_phase.png)
...
![Fase de Update](/manuals/images/application_lifecycle/update_phase.png)
...
Si se usa un [fixed timestep](/es/manuals/physics/#physics-updates) para la simulación de físicas, también puede haber una llamada a la función `fixed_update()` en todos los componentes script. Esta función es útil en juegos basados en físicas cuando quieres manipular objetos físicos a intervalos regulares para lograr una simulación de físicas estable.
...
![Fase de Render Update](/manuals/images/application_lifecycle/render_update_phase.png)
...
![Fase de Post Update](/manuals/images/application_lifecycle/post_update_phase.png)
...
![Fase de renderizado](/manuals/images/application_lifecycle/render_phase.png)

Luego se renderizan los gráficos, junto con cualquier renderizado del profiler visual (consulta la [documentación de depuración](/es/manuals/debugging)). Después del renderizado de gráficos, se realiza una captura de video.
...
(Consulta el [manual de proxy de colección](/es/manuals/collection-proxy) y [`set_time_step`](/ref/collectionproxy#set-time-step) para más detalles)
...
![Finalización](/manuals/images/application_lifecycle/finalization.png)
...
![Despacho de mensajes](/manuals/images/application_lifecycle/dispatch_messages.png)
