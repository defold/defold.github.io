...
Después de montar un paquete, un proxy excluido y no cargado también puede redirigirse a otra colección compilada con `collectionproxy.set_collection()`. Consulta [Cambiar la colección de un proxy excluido](/es/manuals/collection-proxy/#changing-an-excluded-proxys-collection) para conocer las restricciones y la secuencia de carga.
