...
Las versiones instaladas de Defold se pueden localizar mediante `installations.json`, como se describe en el [manual del editor](/es/manuals/editor/#editor-installation-metadata). Su campo `launcherPath` contiene el ejecutable que se debe iniciar. Pasa la ruta de `game.project` como primer argumento posicional para abrir ese proyecto directamente.
...
El editor es una aplicación gráfica de escritorio. Inícialo en una sesión de usuario interactiva con acceso a la pantalla. Utiliza [Bob](/es/manuals/bob) cuando no haya una sesión gráfica disponible, como en un entorno de CI headless, o para crear bundles independientes. Un editor abierto también admite automatizar únicamente la compilación mediante `/command/compile`.
...
![Página principal del servidor local del editor](/manuals/images/automation/editor_server.png)
...
Muestra las rutas documentadas de los comandos del editor:
...
En Defold 1.13.2 y posteriores, cada comando tiene su propia ruta en el documento OpenAPI. Las versiones anteriores describen los comandos mediante una ruta `/command/{command}` y una enumeración de nombres de comandos.
...
Invoca los comandos del editor enviando una solicitud `POST` a la ruta documentada del comando, por ejemplo:
...
Estas tuberías muestran el cuerpo de la respuesta. En los scripts de automatización, comprueba también el estado HTTP y `success`, siguiendo el patrón de [crear una build HTML5](#building-html5).

<div class='sidenote' markdown='1'>
Desde Defold 1.13.2, `/command/build` es un alias de compatibilidad obsoleto de `/command/run` y no aparece en OpenAPI. Usa `/command/run` en nuevas integraciones.
</div>

Una compilación correcta devuelve el estado HTTP `200` con un resultado estructurado:
...
Las respuestas dependen del comando. En Defold 1.13.2 y posteriores, `compile`, `run`, `clean-build`, `build-html5`, `debugger-start` y `hot-reload` esperan a que finalice el comando y devuelven un resultado estructurado con `success` e `issues`, como se muestra arriba. Un resultado correcto devuelve HTTP `200`; un fallo de compilación o validación devuelve `422`.

Otros comandos aún pueden devolver `202`, por ejemplo `debugger-break`. Consulta la operación en el esquema OpenAPI actual y gestiona el estado HTTP real de la respuesta:
...
Si el documento OpenAPI actual incluye `/command/build-html5`, invócalo mediante esa ruta. En un script de shell, captura el estado HTTP por separado del cuerpo de la respuesta y detente si falla la solicitud o la build:
...
En Defold 1.13.2 y posteriores, esta solicitud espera a que termine la build y devuelve un resultado estructurado. El ejemplo imprime el cuerpo de la respuesta, incluidas las incidencias de compilación, y continúa solo con HTTP `200` y `success: true`. Después de una build correcta, el editor abre el juego en un navegador y lo sirve en:

```text
http://127.0.0.1:<editor-port>/html5/
```

Que la build haya terminado no significa que el juego haya terminado de cargar en el navegador. Espera a que el canvas y la aplicación estén listos antes de enviar input o comprobar el funcionamiento del juego. Consulta [pruebas de navegador para HTML5](/es/manuals/automated-testing/#browser-tests-for-html5) para obtener más detalles.
...
Para obtener información sobre el enmarcado de los resultados de pruebas y la clasificación de fallos, consulta [Pruebas automatizadas y verificación](/es/manuals/automated-testing/#structured-test-results).
...
![Vista previa de la colección principal renderizada por el editor](/manuals/images/automation/main-preview.png)
...
![Vista previa del modelo de cubo renderizada por el editor](/manuals/images/automation/cube-preview.png)
...
<div class='important' markdown='1'>
Una vista previa del editor no es una captura de pantalla del juego en ejecución. No verifica objetos creados dinámicamente, posprocesamiento de runtime ni renderizado específico de la plataforma. Utiliza una [captura de pantalla del runtime](/es/manuals/automated-testing/#editor-previews-and-runtime-screenshots) cuando necesites esos elementos.
</div>
...
El código evaluado puede utilizar la [API del editor](https://defold.com/ref/editor-lua/) y el entorno de scripting del editor. No puede utilizar API de runtime del juego como `go.*` para manipular un juego en ejecución. Utiliza una prueba de runtime, el debugger, una prueba de navegador o una [API de automatización de runtime](/es/manuals/engine-service/#automation-bridge-extension) para el gameplay.
...
Los scripts del editor pueden definir rutas adicionales mediante [`get_http_server_routes()`](/es/manuals/editor-scripts/#http-server). Una tabla opcional de operaciones OpenAPI expone una ruta mediante el mismo documento `/openapi.json` que las operaciones integradas.
...
El servidor del editor pertenece al proceso del editor. Un juego en ejecución tiene un puerto diferente y responsabilidades distintas, descritas en el [manual del servicio del motor y la API HTTP de runtime](/es/manuals/engine-service).