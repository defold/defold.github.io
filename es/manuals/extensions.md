...
- anchor: r8-keep-rules-for-android
  title: Reglas de conservación de R8 para Android
...
- anchor: build-server-logs
  title: Logs del servidor de build
...
: Esta carpeta opcional contiene cualquier librería compilada de la que dependa la extensión. Los archivos de librería deben colocarse en subcarpetas nombradas por `platform` o `architecture-platform`, según las arquitecturas que soporten tus librerías.
  ...
  {% include shared/es/platforms.md %}
...
* `android` - Esta carpeta acepta un archivo stub de manifest para fusionarlo con la aplicación principal ([como se describe aquí](/es/manuals/extensions-manifest-merge-tool)).
  * La carpeta también puede contener un archivo `build.gradle` con dependencias para ser [resueltas por Gradle](/es/manuals/extensions-gradle).
...
* `ios` - Esta carpeta acepta un archivo stub de manifest para fusionarlo con la aplicación principal ([como se describe aquí](/es/manuals/extensions-manifest-merge-tool)).
  * La carpeta también puede contener un archivo `Podfile` con dependencias para ser [resueltas por Cocoapods](/es/manuals/extensions-cocoapods).
* `osx` - Esta carpeta acepta un archivo stub de manifest para fusionarlo con la aplicación principal ([como se describe aquí](/es/manuals/extensions-manifest-merge-tool)).
* `web` - Esta carpeta acepta un archivo stub de manifest para fusionarlo con la aplicación principal ([como se describe aquí](/es/manuals/extensions-manifest-merge-tool)).
...
Por ejemplo, coloca un archivo JSON en `/myextension/data/settings.json`. La ruta es relativa a la raíz del proyecto e incluye la carpeta de la extensión. Al compartir la extensión como biblioteca, incluye `myextension` en [Include Dirs](/es/manuals/libraries/#setting-up-library-sharing) de la biblioteca para que los proyectos que la usen reciban la extensión y sus datos.
...
Consulta [acceso a archivos](/es/manuals/file-access/#custom-resources) para conocer la diferencia entre recursos personalizados y recursos del bundle.

## Compartir una extensión

Las extensiones se tratan igual que cualquier otro asset en tu proyecto y se pueden compartir de la misma manera. Si una carpeta de extensión nativa se agrega como carpeta de Library, puede compartirse y otros pueden usarla como dependencia del proyecto. Consulta el [manual de proyectos de biblioteca](/es/manuals/libraries/) para obtener más información.
...
![Archivo manifest](/manuals/images/extensions/manifest.png)
...
![Archivo C++](/manuals/images/extensions/cppfile.png)
