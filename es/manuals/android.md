...
- anchor: shrinking-java-code-with-r8
  title: Reducir el código Java con R8
- anchor: enabling-r8
  title: Activar R8
- anchor: adding-rules-to-an-extension
  title: Agregar reglas a una extensión
- anchor: keeping-the-obfuscation-mapping
  title: Conservar el mapa de ofuscación
- Permisos
...
Los dispositivos Android te permiten ejecutar libremente tus propias apps en ellos. Es muy fácil crear una versión de tu juego y copiarla a un dispositivo Android. Este manual explica los pasos necesarios para crear un bundle de tu juego para Android. Durante el desarrollo, suele ser preferible ejecutar tu juego mediante la [app de desarrollo](/es/manuals/dev-app), ya que te permite recargar contenido y código en caliente directamente en tu dispositivo.
...
Cuando creas un bundle de aplicación Android desde el editor Defold o desde la [herramienta de línea de comando](/es/manuals/bob), puedes proporcionar un keystore (que contiene tu certificado y clave) y la contraseña del keystore, que se usarán al firmar tu aplicación. Si no lo haces, Defold genera un keystore de depuración y lo usa al firmar el bundle de la aplicación.
...
El editor te permite crear fácilmente un bundle de aplicación independiente para tu juego. Antes de crear el bundle, puedes especificar qué iconos usar para la app, definir el código de versión, etc. en el [archivo de configuración del proyecto](/es/manuals/project-settings/#android) *game.project*.
...
![Firmado de bundle Android](/manuals/images/android/sign_bundle.png)
...
![Firmado de bundle Android](/manuals/images/android/sign_bundle2.png)
...
![Archivo de paquete de aplicación Android](/manuals/images/android/apk_file.png)
...
{% include shared/es/android-adb.md %}
...
![Instalar y ejecutar APK](/manuals/images/android/install_and_launch.png)

Para que esta funcionalidad funcione, necesitarás tener *ADB* instalado y *USB debugging* activado en el dispositivo conectado. Si el editor no puede detectar la ubicación de instalación de la herramienta de línea de comando ADB, tendrás que especificarla en [Preferences](/es/manuals/editor-preferences/#tools).
...
Selecciona `/builtins/manifests/android/dmengine.keep` en **Android ▸ R8 Keep Rules** en *game.project*. Esto usa directamente las reglas predeterminadas de Defold:
...
Asegúrate de que cada extensión con código Java proporcione un archivo `.keep` para las clases que necesita en runtime. Las reglas de las extensiones se combinan con las reglas del proyecto seleccionadas al compilar. Prueba una build de release en un dispositivo después de activar R8.
...
Las reglas de conservación de una extensión van en su directorio `manifests/android`, junto a `build.gradle`. Consulta [reglas de conservación de R8 para extensiones de Android](/es/manuals/extensions/#r8-keep-rules-for-android) para saber cómo agregar un archivo y conservar las clases Java de la extensión.

### Conservar el mapa de ofuscación {#keeping-the-obfuscation-mapping}

Activa **Generate debug symbols** en el diálogo de bundle de Android, o pasa `--with-symbols` a Bob, para conservar el archivo `mapping.txt` de R8 cuando la build lo genere. Por ejemplo, desde el directorio del proyecto:
...
El mapa se guarda como `<binary-name>.apk.symbols/mapping.txt` junto al APK o AAB generado. Por ejemplo, con el título de proyecto `My Game`, el comando anterior genera `build/android/MyGame/MyGame.apk.symbols/mapping.txt`.

Guarda el archivo de mapa junto a la release exacta de la que procede. Permite relacionar los nombres Java ofuscados con los originales para interpretar trazas de pila; un mapa de otra build puede dar resultados incorrectos.

## Permisos

El motor Defold requiere varios permisos para que funcionen todas sus funcionalidades. Los permisos se definen en `AndroidManifest.xml`, especificado en el [archivo de configuración del proyecto](/es/manuals/project-settings/#android) *game.project*. Puedes leer más sobre los permisos de Android en [la documentación oficial](https://developer.android.com/guide/topics/permissions/overview). En el manifiesto predeterminado se solicitan los siguientes permisos:
