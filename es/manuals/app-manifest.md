...
Los ajustes del solver de Box2D son específicos de cada versión. Consulta las [opciones de proyecto de Box2D](/es/manuals/project-settings/#box2d) para más detalles.
...
Controla la funcionalidad de rig y modelo, o selecciona None para excluir por completo modelo y rig. (Consulta la documentación de [`Model`](https://defold.com/es/manuals/model/#model-component)).
...
El ajuste del manifiesto de la aplicación controla si el código del profiler se enlaza con una build. Los ajustes de `profiler` en *game.project* controlan su comportamiento en runtime. Aprende a usar las funciones disponibles en el [manual de profiling](/es/manuals/profiling/).
...
Incluye soporte para recursos de sonido Ogg Opus. El decodificador Opus se excluye de forma predeterminada, por lo que esta opción debe activarse antes de poder reproducir recursos `.opus`. Consulta el [manual de sonido](/es/manuals/sound/) para conocer los formatos compatibles.
...
Excluye del motor la [biblioteca de compresión de texturas](/es/manuals/texture-profiles) Basis Universal.


## Usar Android Support Lib (`Use Android Support Lib`)

Usa la Android Support Library obsoleta en lugar de Android X. [Más información](https://defold.com/es/manuals/android/#using-androidx).
...
Esta opción proporciona el valor predeterminado en tiempo de compilación. El valor [`html5.heap_size`](/es/manuals/html5/#heap-size) de *game.project* lo sobrescribe en runtime.
