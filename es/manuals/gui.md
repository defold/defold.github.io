...
![Nuevo archivo GUI](/manuals/images/gui/new_gui_file.png)

Defold ahora abre automáticamente el archivo en el editor de escenas GUI.

![Nueva GUI](/manuals/images/gui/new_gui.png)

*Outline* lista todo el contenido de la GUI: su lista de nodos y cualquier dependencia (ver abajo).

El área central de edición muestra la GUI. La barra de herramientas en la esquina superior derecha del área de edición contiene las herramientas *Move*, *Rotate* y *Scale*, además de un selector de [layout](/es/manuals/gui-layouts).

![barra de herramientas](/manuals/images/gui/toolbar.png)
...
: La cantidad máxima de texturas dinámicas que controla este componente GUI; el valor predeterminado es `128`. Esto incluye las texturas creadas con [`gui.new_texture()`](/ref/gui/#gui.new_texture:texture_id-width-height-type-buffer-flip) y las texturas externas asignadas a la GUI mediante `go.set(..., "textures", ...)` o `gui.set(msg.url(), "textures", ...)`. Los proyectos que sustituyan muchas texturas externas pueden necesitar aumentar este límite.
...
![dependencias](/manuals/images/gui/dependencies.png)
...
Nodo caja
: ![nodo caja](/manuals/images/icons/gui-box-node.png)
  Nodo rectangular con un solo color, una textura o una animación flip-book. Consulta la [documentación de nodo caja](/es/manuals/gui-box) para obtener detalles.

<div style="clear: both;"></div>

Nodo de texto
: ![nodo de texto](/manuals/images/icons/gui-text-node.png)
  Muestra texto. Consulta la [documentación de nodo de texto](/es/manuals/gui-text) para obtener detalles.

<div style="clear: both;"></div>

Nodo circular
: ![nodo circular](/manuals/images/icons/gui-pie-node.png)
  Un nodo circular o elipsoide que se puede rellenar parcialmente o invertir. Consulta la [documentación de nodo circular](/es/manuals/gui-pie) para obtener detalles.

<div style="clear: both;"></div>

Nodo template
: ![nodo template](/manuals/images/icons/gui.png)
  Los templates se usan para crear instancias basadas en otros archivos de escena GUI. Consulta la [documentación de nodo template](/es/manuals/gui-template) para obtener detalles.

<div style="clear: both;"></div>

Nodo ParticleFX
: ![nodo particlefx](/manuals/images/icons/particlefx.png)
  Reproduce un efecto de partículas. Consulta la [documentación de nodo ParticleFX](/es/manuals/gui-particlefx) para obtener detalles.
...
![Añadir nodos](/manuals/images/gui/add_node.png)
...
: Controla la ubicación, orientación y estiramiento del nodo. Puedes usar las herramientas *Move*, *Rotate* y *Scale* para cambiar estos valores. Los valores se pueden animar desde script ([más información](/es/manuals/property-animation)).

Size (nodos caja, de texto y circulares)
: El tamaño del nodo es automático de forma predeterminada, pero al definir *Size Mode* en `Manual` puedes alterar el valor. El tamaño define los límites del nodo y se usa al hacer selección por input. Este valor se puede animar desde script ([más información](/es/manuals/property-animation)).
...
Slice 9 (nodos caja)
: Se define para conservar el tamaño en píxeles de la textura del nodo alrededor de los bordes cuando se redimensiona el nodo. Consulta la [documentación de nodo caja](/es/manuals/gui-box) para obtener detalles.

Inner Radius (nodos circulares)
: El radio interior del nodo, expresado a lo largo del eje X. Consulta la [documentación de nodo circular](/es/manuals/gui-pie) para obtener detalles.

Outer Bounds (nodos circulares)
: Controla el comportamiento de los límites exteriores. Consulta la [documentación de nodo circular](/es/manuals/gui-pie) para obtener detalles.

Perimeter Vertices (nodos circulares)
: La cantidad de segmentos que se usarán para construir la forma. Consulta la [documentación de nodo circular](/es/manuals/gui-pie) para obtener detalles.

Pie Fill Angle (nodos circulares)
: Qué porción del nodo circular debe rellenarse. Consulta la [documentación de nodo circular](/es/manuals/gui-pie) para obtener detalles.

Template (nodos template)
: El archivo de escena GUI que se usa como template para el nodo. Consulta la [documentación de nodo template](/es/manuals/gui-template) para obtener detalles.

ParticleFX (nodos ParticleFX)
: El efecto de partículas que se usa en este nodo. Consulta la [documentación de nodo ParticleFX](/es/manuals/gui-particlefx) para obtener detalles.

Color
: El color del nodo. Si el nodo tiene textura, el color tiñe la textura. El color se puede animar desde script ([más información](/es/manuals/property-animation)).

Alpha
: La translucidez del nodo. El valor alpha se puede animar desde script ([más información](/es/manuals/property-animation)).
...
: Define el punto pivot del nodo. Esto se puede ver como el "punto central" del nodo. Cualquier rotación, escalado o cambio de tamaño ocurrirá alrededor de este punto.
  ...
  ![punto pivot](/manuals/images/gui/pivot.png)
...
: El anclaje controla cómo se altera la posición vertical y horizontal del nodo cuando los límites de la escena, o los límites del nodo padre, se estiran para ajustarse al tamaño físico de la pantalla.
  ...
  ![Anclaje sin ajustar](/manuals/images/gui/anchoring_unadjusted.png)
  ...
  ![Anclaje](/manuals/images/gui/anchoring.png)
...
: Define el modo de ajuste del nodo. La configuración del modo de ajuste controla qué le ocurre a un nodo cuando los límites de la escena, o los límites del nodo padre, se ajustan para encajar con el tamaño físico de la pantalla.
  ...
  ![Sin ajustar](/manuals/images/gui/unadjusted.png)
  ...
  ![Modos de ajuste](/manuals/images/gui/adjusted.png)
...
: Define el modo de clipping en el nodo:
  ...
  Consulta el [manual de clipping de GUI](/es/manuals/gui-clipping) para obtener detalles.

Clipping Visible (nodos caja y circulares)
: Se define para renderizar el contenido del nodo en el área stencil. Consulta el [manual de clipping de GUI](/es/manuals/gui-clipping) para obtener detalles.

Clipping Inverted (nodos caja y circulares)
: Invierte la máscara stencil. Consulta el [manual de clipping de GUI](/es/manuals/gui-clipping) para obtener detalles.
...
![Orden de dibujo](/manuals/images/gui/draw_order.png)
...
![Padre hijo](/manuals/images/gui/parent_child.png)
...
![Jerarquía que rompe el lote](/manuals/images/gui/break_batch.png)
...
![Capas](/manuals/images/gui/layers.png)
