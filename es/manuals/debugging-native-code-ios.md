...
* Crea el bundle de la app usando bob, con la opción `--with-symbols` ([más información](/es/manuals/debugging-native-code/#symbolicate-a-callstack)):
...
* Obtén la carpeta `.dSYM` (es decir, los símbolos de depuración)
	...
	* Si estás usando una extensión nativa, la carpeta `.dSYM` se genera cuando haces la build con [bob.jar](https://www.defold.com/es/manuals/bob/). Solo es necesario hacer la build (sin archive ni bundling):
