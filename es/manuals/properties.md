...
| *scale*    | Escala local no uniforme del objeto de juego, expresada como un vector donde cada componente contiene un multiplicador a lo largo de cada eje. Para duplicar el tamaño en x e y, proporciona `vmath.vector3(2.0, 2.0, 0)` | `vector3` | `get+set` |
| *scale.xy* | Escala local no uniforme del objeto de juego, expresada como un vector donde cada componente contiene un multiplicador a lo largo de los ejes X e Y. | `vector3` | `get+set` |
...
| *texture0* | El hash de la ruta de textura del modelo. | `hash` | `get`|
| *cursor*  | Posición (entre 0--1) del cursor de reproducción. | `number`   | `get+set` |
