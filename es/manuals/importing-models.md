...
![Modelo en Blender](/manuals/images/model/blender_gltf.png)
...
![Assets de modelo importados](/manuals/images/model/assets_gltf.png)

<div class='sidenote' markdown='1'>
A partir de Defold 1.13.0, Defold conserva las posiciones y transformaciones del archivo glTF importado y no vuelve a centrar automáticamente el modelo durante la importación. La vista previa del editor y la ejecución usan las transformaciones importadas de forma coherente: las mallas con skinning o vinculadas a huesos conservan sus transformaciones locales relativas al esqueleto, mientras que las mallas rígidas mantienen su ubicación global aplanada.

Desde Defold 1.13.2, un [componente Model](/es/manuals/model/#model-properties) puede seleccionar una sola malla con nombre de la escena importada. Dejar su campo *Mesh* vacío usa toda la escena y conserva las transformaciones descritas anteriormente. Seleccionar una malla usa su geometría local sin las transformaciones de los nodos glTF, por lo que debes colocarla mediante la transformación del componente Model o del objeto de juego.

Si un modelo creado con una versión anterior de Defold cambia de posición u orientación después de volver a importarlo, corrige la transformación en Blender u otra herramienta de creación y vuelve a exportar el archivo *.gltf* o *.glb*.
</div>

## Uso de un modelo
Una vez que hayas importado el modelo, úsalo en un [componente Model](/es/manuals/model):
...
2. Define la propiedad *Scene* con el archivo *.gltf* o *.glb* importado. Deja *Mesh* vacío para usar toda la escena, o selecciona una malla con nombre para usar solo su geometría local.
3. Para un modelo animado, define la propiedad *Skeleton* con el archivo *.gltf* o *.glb* que contiene el esqueleto. A menudo es el mismo archivo usado para *Scene* cuando la malla, el esqueleto y las animaciones se exportan juntos.
...
5. Define la propiedad *Material* con un material adecuado para el modelo. Los archivos integrados *model.material*, *model_instanced.material*, *model_skinned.material* y *model_skinned_instanced.material* son puntos de partida útiles. Los materiales skinned usan el espacio local de vértices para que el skinning pueda ejecutarse en la GPU; los materiales personalizados para modelos con skinning por GPU o instanciados también deben usar el espacio local de vértices. Consulta el [manual de modelos](/es/manuals/model/#material) para conocer los requisitos del adaptador gráfico.
...
![Seleccionar todo](/manuals/images/model/blender_select_all_vertices.png)

Cuando todos los vértices estén seleccionados, desenvuelve la malla para obtener el layout UV:

![Desenvolver malla](/manuals/images/model/blender_unwrap_mesh.png)

Luego puedes exportar el layout UV a una imagen que pueda usarse como textura:

![Exportar layout UV](/manuals/images/model/blender_export_uv_layout.png)

![Resultado de exportar layout UV](/manuals/images/model/blender_export_uv_layout_result.png)

### Exportación con Blender
Exporta tu modelo desde Blender con <kbd>File ▸ Export ▸ glTF 2.0 (.glb/.gltf)</kbd>.

![Exportación con Blender](/manuals/images/model/export_gltf.png)
...
![Exportación con Blender](/manuals/images/model/export_settings.png)