...
- `callback` (function) - Register callback function.
  ...
  - `token` (string) - The APNs device token on iOS or Firebase Installation ID (FID) on Android if registration is successful.
...
function init(self)
     push.register({}, function (self, installation_id, error)
