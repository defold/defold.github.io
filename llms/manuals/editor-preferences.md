...
You can configure editor's keyboard shortcuts and mouse controls in the Keymap tab. To change a command, double-click it, press `Enter` or `Space`, or use the row context menu.
...
- `MB` means a mouse button binding, optionally combined with `Shift`, `Ctrl`/`Control`, or `Alt`.
- `MM` means a modifier key used by a mouse action.
