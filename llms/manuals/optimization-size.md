...
To get a better understanding of what makes up the size of your application you can [generate a build report](https://defold.com/llms/manuals/bundling.md) when bundling. It is quite common that sounds and graphics is what takes up the bulk of the size of any game.
...
A quick way to reduce the engine size is to remove functionality in the engine that you do not use. This is done [application manifest file](https://defold.com/llms/manuals/app-manifest.md) where it is possible to remove engine components that you do not need. Examples:
...
* GUI, particle effects and tilemaps - These components can be excluded separately with the [App Manifest component switches](https://defold.com/llms/manuals/app-manifest.md). Remove component references and API calls for any feature you exclude. Excluding particle effects also removes support for particle nodes in GUI scenes.
* Rich text - Disable [Use Rich Text](https://defold.com/llms/manuals/app-manifest.md) if labels and GUI text only need plain text. This removes rich text parsing and style effects while retaining ordinary text rendering.
...
Defold supports 8-bit and 16-bit PCM Wave files. Ogg Vorbis and Ogg Opus use their respective compressed formats rather than a PCM bit-depth requirement. The Opus decoder is not included by default; enable **Include Sound Decoder: Opus** in the [App Manifest](https://defold.com/llms/manuals/app-manifest.md) before using `.opus` resources.
...
### Optimize fonts
The size of your fonts will be smaller if you specify what symbols you are going to use and set this in [Characters](https://defold.com/llms/manuals/font.md) instead of using the All Chars checkbox.
...
You can also make use of a combination of [APK Expansion Files](https://developer.android.com/google/play/expansion-files) and [Live Update content](https://defold.com/llms/manuals/live-update.md) thanks to the [APKX extension in the Asset Portal](https://defold.com/assets/apkx/).