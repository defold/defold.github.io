...
To get a better understanding of what makes up the size of your application you can [generate a build report](bundling.md) when bundling. It is quite common that sounds and graphics is what takes up the bulk of the size of any game.
...
A quick way to reduce the engine size is to remove functionality in the engine that you do not use. This is done [application manifest file](app-manifest.md) where it is possible to remove engine components that you do not need. Examples:
...
### Optimize fonts
The size of your fonts will be smaller if you specify what symbols you are going to use and set this in [Characters](font.md) instead of using the All Chars checkbox.
...
You can also make use of a combination of [APK Expansion Files](https://developer.android.com/google/play/expansion-files) and [Live Update content](live-update.md) thanks to the [APKX extension in the Asset Portal](https://defold.com/assets/apkx/).