...
* Xcode -  macOS, iOS ([learn more](debugging-native-code-ios.md))
* Android Studio - Android ([learn more](debugging-native-code-android.md))
* WinDBG - Windows
...
The simplest way to debug your native code is to use [print debugging](http://en.wikipedia.org/wiki/Debugging#Techniques). Use the functions in the [`dmLog` namespace](https://defold.com/ref/stable/dmLog/) to watch variables or indicate the flow of execution. Using any of the log functions will print to the *Console* view in the editor and to the [game log](debugging-game-and-system-logs.md).

## Analyze a crash log

The Defold engine saves a `_crash` file if it does a hard crash. The crash file will contain information about the system as well as the crash. The [game log output](debugging-game-and-system-logs.md) will write where the crash file is located (it varies depending on operating system, device and application).
...
If the app is [debuggable](project-settings.md), you can get the crash log using the [Android Debug Bridge (ADB) tool](https://developer.android.com/studio/command-line/adb.html) and the `adb shell` command:
```
...
If you get a callstack from either a `_crash` file or a [log file](debugging-game-and-system-logs.md), you can symbolicate it. This means translating each address in the callstack into a filename and line number, which in turn helps when finding out the root cause.

It is important that you match the correct engine with the callstack, otherwise it's very likely to send you debugging the incorrect things! Use the flag [`--with-symbols`](bob.md) when bundling with [bob](bob.md) or check the "Generate debug symbols" checkbox from the bundle dialog in the editor:
...
Note: If you get hold of a stack trace from the [Android logs](debugging-game-and-system-logs.md), you might be able to symbolicate it using [ndk-stack](https://developer.android.com/ndk/guides/ndk-stack.html)
