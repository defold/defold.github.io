...
* Xcode -  macOS, iOS ([learn more](https://defold.com/llms/manuals/debugging-native-code-ios.md))
* Android Studio - Android ([learn more](https://defold.com/llms/manuals/debugging-native-code-android.md))
* WinDBG - Windows
...
The Defold engine saves a `_crash` file if it does a hard crash. The crash file will contain information about the system as well as the crash. The [game log output](https://defold.com/llms/manuals/debugging-game-and-system-logs.md) will write where the crash file is located (it varies depending on operating system, device and application).
...
If the app is [debuggable](https://defold.com/llms/manuals/project-settings.md), you can get the crash log using the [Android Debug Bridge (ADB) tool](https://developer.android.com/studio/command-line/adb.html) and the `adb shell` command:
```
...
If you get a callstack from either a `_crash` file or a [log file](https://defold.com/llms/manuals/debugging-game-and-system-logs.md), you can symbolicate it. This means translating each address in the callstack into a filename and line number, which in turn helps when finding out the root cause.

It is important that you match the correct engine with the callstack, otherwise it's very likely to send you debugging the incorrect things! Use the flag [`--with-symbols`](https://defold.com/llms/manuals/bob.md) when bundling with [bob](https://defold.com/llms/manuals/bob.md) or check the "Generate debug symbols" checkbox from the bundle dialog in the editor:
...
* HTML5 - the `_symbols` directory alongside the HTML5 bundle contains `_wasm.js.symbols` and, when the `wasm_pthread-web` architecture is selected, `_pthread_wasm.js.symbols`.
...
Note: If you get hold of a stack trace from the [Android logs](https://defold.com/llms/manuals/debugging-game-and-system-logs.md), you might be able to symbolicate it using [ndk-stack](https://developer.android.com/ndk/guides/ndk-stack.html)
