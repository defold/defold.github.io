...
`--texture-compression` is a switch with no value. Include it to enable the compression selected by the texture profiles; omit it to disable texture compression. The legacy `--texture-compression=true` form is still accepted. The legacy `--texture-compression=false` form is ignored and produces a warning; omit the switch instead.
...
`build`
: Builds the dependency graph reachable from the selected build roots. By default the root is `game.project`; `--build-input` and `--build-input-file` can specify alternative roots. Files are not built merely because they exist under the project root. When `game.project` is a build root, add `--archive` to build the game data archive in the build directory.
...
By default, Bob looks in the current directory for a project and builds resources reachable from `game.project` into *build/default*. Unreferenced resources are not compiled. If code loads raw files by path at runtime, include them through the [Custom Resources project setting](https://defold.com/llms/manuals/project-settings.md); other Defold resources need a reference reachable from a build root.
```sh
