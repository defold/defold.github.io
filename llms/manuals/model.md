...
Models can also use glTF morph target animations. Morph target weights are animated with `model.play_anim()` like other model animations, and can be read or overridden at runtime using [`model.get_blend_weights()`](https://defold.com/ref/model#model.get_blend_weights) and [`model.set_blend_weights()`](https://defold.com/ref/model#model.set_blend_weights). See the [morph targets section](https://defold.com/llms/manuals/model-animation#morph-targets.md) in the model animation manual for details.
...
The bone-matrix cache uses an `RGBA32F` texture. If the active graphics adapter does not support that texture format, Defold cannot create an animated Model component that uses a local-space material. On OpenGL ES 2.0 and WebGL 1.0, support therefore depends on the adapter's floating-point texture extension. For compatibility with an adapter that lacks it, use a custom world-space material, which uses CPU skinning and cannot use model instancing. The cache dimensions can be adjusted with the [Model project settings](https://defold.com/llms/manuals/project-settings.md).
...
The default model material has the following constants that can be changed using [go.set()](https://defold.com/ref/go/#go.set) or [go.animate()](https://defold.com/ref/go/#go.animate) (refer to the [Material manual for more details](https://defold.com/llms/manuals/material.md)). Examples:
```lua
