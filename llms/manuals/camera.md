...
For an orthographic camera, [`camera.screen_xy_to_world()`](https://defold.com/ref/camera/#camera.screen_xy_to_world:x-y-[camera]) returns the world-space point on the camera's near plane for a screen pixel:
```lua
...
For a perspective camera, [`camera.screen_to_world()`](https://defold.com/ref/camera/#camera.screen_to_world:pos-[camera]) takes a `vector3` whose Z component is view depth in world units measured from the camera plane:
```lua
local depth = 10
...
[`camera.world_to_screen()`](https://defold.com/ref/camera/#camera.world_to_screen:world_pos-[camera]) performs the reverse conversion. It returns screen-pixel X and Y plus the same view-depth convention in Z, so its result can be passed back to `camera.screen_to_world()`:
```lua
