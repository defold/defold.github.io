...
Use `graphics.get_adapter_info()` to check if the active graphics adapter supports compute shaders. The `features` field contains an array of the context feature constants supported by the adapter:
```lua
...
Only supported features are included in the array; it is not a table keyed by the feature constants. Always perform this check before using compute shaders when the game can run with different graphics adapters or on devices with varying driver support. OpenGL and OpenGL ES support depends on the API version and driver. Vulkan and Metal via MoltenVK support compute shaders from version 1.0. Use an [application manifest](https://defold.com/llms/manuals/app-manifest.md) to select Vulkan on platforms where it is not already the default graphics backend.