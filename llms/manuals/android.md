...
Defold uses a keystore for the Android signing process. [More information is available in this forum post](https://forum.defold.com/t/upcoming-change-to-the-android-build-pipeline/66084).
...
Keep rules for an extension belong in its `manifests/android` directory, next to `build.gradle`. See [R8 keep rules for Android extensions](https://defold.com/llms/manuals/extensions.md) for how to add a file and preserve the extension's Java classes.
...
The mapping is saved as `.apk.symbols/mapping.txt` beside the generated APK or AAB. For example, with the project title `My Game`, the command above produces `build/android/MyGame/MyGame.apk.symbols/mapping.txt`.
