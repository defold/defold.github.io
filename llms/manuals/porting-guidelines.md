...
Defold has built-in safe-area support on Android and iOS. Set `gui.safe_area_mode` in *game.project* to control which opposing safe-area insets affect GUI adjustment. `none` is the default and ignores the insets; `long` applies left/right in landscape and top/bottom in portrait; `short` applies the opposite pair; and `both` applies all four edges. A GUI script can override the project-wide mode for its scene with [`gui.set_safe_area_mode()`](https://defold.com/ref/gui/#gui.set_safe_area_mode). For custom GUI or rendering logic, [`window.get_safe_area()`](https://defold.com/ref/window/#window.get_safe_area) returns the safe rectangle and the individual edge insets. Platforms without built-in safe-area insets return the full window and zero insets.
