...
- anchor: verification-levels
  title: Überprüfungsebenen
- anchor: reusable-and-testable-lua-code
  title: Wiederverwendbarer und testbarer Lua-Code
- anchor: tests-in-a-running-collection
  title: Tests in einer laufenden Sammlung
- anchor: test-frameworks
  title: Testframeworks
- anchor: structured-test-results
  title: Strukturierte Testergebnisse
- anchor: collecting-console-output
  title: Konsolenausgabe erfassen
- anchor: persisted-logs
  title: Gespeicherte Protokolle
- anchor: testing-a-running-game-through-a-runtime-api
  title: Ein laufendes Spiel über eine Laufzeit-API testen
- anchor: browser-tests-for-html5
  title: Browsertests für HTML5
- anchor: editor-previews-and-runtime-screenshots
  title: Editorvorschauen und Bildschirmaufnahmen zur Laufzeit für die visuelle Prüfung
- anchor: headless-tests-and-ci
  title: Tests ohne grafische Oberfläche und CI
- anchor: failure-reports-and-artifacts
  title: Fehlerberichte und Artefakte
---
...
Weitere Einzelheiten findest du im [Handbuch zum Schreiben von Code](/de/manuals/writing-code).
...
Weitere Informationen findest du im [Handbuch zur HTTP-API des Editors](/de/manuals/editor-http-api/#reading-console-output).

### Gespeicherte Protokolle {#persisted-logs}

Defold kann das Spielprotokoll auch speichern, wenn du `Write Log File` in `game.project` aktivierst. Siehe [Spiel- und Systemprotokolle](/de/manuals/debugging-game-and-system-logs/). Die Protokollierung in einer Datei ist nützlich für paketierte Anwendungen und zum Testen von Zielgeräten, auf denen die Editorkonsole nicht verfügbar ist.
...
Weitere Einzelheiten findest du im [Handbuch zum Engine-Dienst](/de/manuals/engine-service/#automation-bridge-extension).
...
Der Editor kann über seinen derzeitigen Befehl `build-html5` einen HTML5-Build erstellen und bereitstellen, wie im [Handbuch zur HTTP-API des Editors](/de/manuals/editor-http-api/#building-html5) beschrieben. Bob kann auch ohne den Editor ein HTML5-Bundle erstellen.
...
Das [Bob-Handbuch](/de/manuals/bob) beschreibt Plattformen, Einstellungsdateien, Bundles, Caches, native Erweiterungen und Build-Berichte.
...
Dasselbe Format sollte von einem Entwickler, einem lokalen Skript, einem CI-Dienst oder einem [KI-Programmieragenten](/de/manuals/ai-agents) genutzt werden können. So bleibt die Überprüfung deterministisch, auch wenn Diagnose oder Fehlerbehebung delegiert werden.