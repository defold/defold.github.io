...
- anchor: game-object-and-component-properties
  title: Eigenschaften von Spielobjekten und Komponenten
- anchor: gui-node-properties
  title: Eigenschaften von GUI-Knoten
---
...
* Benutzerdefinierte Eigenschaften von Skriptkomponenten, die in Lua-Skripten definiert werden (Einzelheiten findest du in der [Dokumentation zu Skripteigenschaften](/de/manuals/script-properties))
* Eigenschaften von GUI-Knoten
* Shader-Konstanten, die in Shadern und Materialdateien definiert werden (Einzelheiten findest du in der [Dokumentation zu Materialien](/de/manuals/material))
...
Die Funktionen `go.get()`, `go.set()` und `go.animate()` nehmen als ersten Parameter eine Referenz und als zweiten einen Eigenschaftsbezeichner entgegen. Die Referenz bezeichnet das Spielobjekt oder die Komponente und kann eine Zeichenfolge, ein Hashwert oder eine URL sein. URLs werden im [Handbuch zur Adressierung](/de/manuals/addressing) ausführlich erklärt. Der Eigenschaftsbezeichner ist eine Zeichenfolge oder ein Hashwert, der die Eigenschaft benennt:
...
| ---------- | -------------------------------------- | --------------- | ---------------- |
| *position* | Die lokale Position des Spielobjekts. | `vector3`      | `get+set` |
| *rotation* | Die lokale Drehung des Spielobjekts, ausgedrückt als `quaternion`.  | `quaternion` | `get+set` |
| *euler*    | Die lokale Drehung des Spielobjekts als Euler-Winkel. | `vector3` | `get+set` |
| *scale*    | Die lokale nicht gleichmäßige Skalierung des Spielobjekts, ausgedrückt als Vektor, dessen Komponenten jeweils einen Multiplikator entlang einer Achse enthalten. Um die Größe in X und Y zu verdoppeln, ohne Z zu verändern, verwende `vmath.vector3(2.0, 2.0, 1.0)`. | `vector3` | `get+set` |
| *scale.xy*    | Die lokale nicht gleichmäßige Skalierung des Spielobjekts entlang der X- und Y-Achse. Verwende diese Eigenschaft oder `go.set_scale_xy()`, wenn keine Skalierung in Z beabsichtigt ist. | `vector3` | `get+set` |
...
| Eigenschaft   | Beschreibung                            | Typ            |                  |
| ---------- | -------------------------------------- | --------------- | ---------------- |
| *size*     | Die unskalierte Größe des Sprites---seine Größe aus dem Quellatlas. | `vector3` | `get` |
| *image* | Der Hashwert des Texturpfads des Sprites. | `hash` | `get`|
| *scale* | Die nicht gleichmäßige Skalierung des Sprites. | `vector3` | `get+set`|
| *scale.xy* | Die nicht gleichmäßige Skalierung des Sprites entlang der X- und Y-Achse. | `vector3` | `get+set`|
| *material* | Das vom Sprite verwendete Material. | `hash` | `get+set`|
| *cursor* | Die Position (im Bereich 0--1) des Wiedergabezeigers. | `number` | `get+set`|
| *playback_rate* | Die Bildrate (Bilder pro Sekunde, FPS) der Flipbook-Animation. | `number` | `get+set`|

*Eigenschaften von Kollisionsobjekt-Komponenten*

| Eigenschaft   | Beschreibung                            | Typ            |                  |
| ---------- | -------------------------------------- | --------------- | ---------------- |
| *mass*     | Die Masse des Kollisionsobjekts. | `number` | `get` |
| *linear_velocity* | Die aktuelle lineare Geschwindigkeit des Kollisionsobjekts. | `vector3` | `get` |
| *angular_velocity* | Die aktuelle Winkelgeschwindigkeit des Kollisionsobjekts. | `vector3` | `get` |
| *linear_damping* | Die lineare Dämpfung des Kollisionsobjekts. | `vector3` | `get+set` |
| *angular_damping* | Die Winkeldämpfung des Kollisionsobjekts. | `vector3` | `get+set` |

*Eigenschaften von Modellkomponenten (3D)*

| Eigenschaft   | Beschreibung                            | Typ            |                  |
| ---------- | -------------------------------------- | --------------- | ---------------- |
| *animation* | Die aktuelle Animation.                | `hash`          | `get`     |
| *texture0*--*texture15* | Die Hashwerte der Texturpfade des Modells. | `hash` | `get+set`|
| *cursor*  | Die Position (im Bereich 0--1) des Wiedergabezeigers. | `number`   | `get+set` |
| *playback_rate* | Die Wiedergabegeschwindigkeit der Animation. Ein Multiplikator für die Wiedergabegeschwindigkeit der Animation. | `number` | `get+set` |
| *material* | Das vom Modell verwendete Material. | `hash` | `get+set`|

*Eigenschaften von Beschriftungskomponenten*

| Eigenschaft   | Beschreibung                            | Typ            |                  |
| ---------- | -------------------------------------- | --------------- | ---------------- |
| *text* | Der Textinhalt der Beschriftung (label). Verfügbar seit Defold 1.13.2. | `string` | `get+set` |
| *scale* | Die Skalierung der Beschriftung. | `vector3` | `get+set` |
| *scale.xy* | Die Skalierung der Beschriftung entlang der X- und Y-Achse. | `vector3` | `get+set`|
| *color*     | Die Farbe der Beschriftung. | `vector4` | `get+set` |
| *outline* | Die Konturfarbe der Beschriftung. | `vector4` | `get+set` |
| *shadow* | Die Schattenfarbe der Beschriftung. | `vector4` | `get+set` |
| *size* | Die Größe der Beschriftung. Die Größe begrenzt den Text, wenn der Zeilenumbruch aktiviert ist. | `vector3` | `get+set` |
| *material* | Das von der Beschriftung verwendete Material. | `hash` | `get+set`|
| *font* | Die von der Beschriftung verwendete Schriftart. | `hash` | `get+set`|
