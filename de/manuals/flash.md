...
- anchor: movie-clips-and-game-objects
  title: Movieclips und Spielobjekte
- anchor: flashmanually-creating-movie-clips
  title: Flash—Movieclips manuell erstellen
- anchor: defoldmanually-creating-game-objects
  title: Defold—Spielobjekte manuell erstellen
- anchor: flashreferencing-manually-created-movie-clips
  title: Flash—manuell erstellte Movieclips referenzieren
- anchor: defoldgame-object-id
  title: Defold—Spielobjekt-ID
- anchor: flashdynamically-creating-movie-clips
  title: Flash—Movieclips dynamisch erstellen
- anchor: defoldcreating-game-objects-using-factories
  title: Defold—Spielobjekte mit Fabriken erstellen
- anchor: flashstage
  title: Flash—Bühne
- anchor: defoldcollections
  title: Defold—Sammlungen
- anchor: flashtimeline
  title: Flash—Zeitleiste
- anchor: defoldproperty-animation
  title: Defold—Eigenschaftsanimation
- anchor: flashdepth-index
  title: Flash—Tiefenindex
- Defold—z-Position
- anchor: flash-hittestobject-and-hittestpoint-collision-detection
  title: Kollisionserkennung in Flash mit hitTestObject und hitTestPoint
- anchor: defoldcollision-objects
  title: Defold—Kollisionsobjekte
- anchor: flashevent-handling
  title: Flash—Ereignisverarbeitung
- anchor: defoldcall-back-functions-and-messaging
  title: Defold—Callback-Funktionen und Nachrichtenübermittlung
- anchor: flashbutton-symbols
  title: Flash—Schaltflächensymbole
- anchor: defoldgui-scenes-and-scripts
  title: Defold—GUI-Szenen und Skripte
- Debugging
...
Flash-Spiele werden in ActionScript programmiert (wobei 3.0 die neueste Version ist), während Skripte in Defold in Lua geschrieben werden. Dieses Handbuch geht nicht auf einen detaillierten Vergleich von Lua und ActionScript 3.0 ein. Das [Defold-Handbuch](/de/manuals/lua) bietet eine gute Einführung in die Lua-Programmierung in Defold und verweist auf das äußerst hilfreiche Buch [Programming in Lua](https://www.lua.org/pil/) (erste Auflage), das kostenlos online verfügbar ist.
...
![Spielobjekt und Movieclip](/manuals/images/flash/go_movieclip.png)

Anders als Flash-Movieclips haben Defold-Spielobjekte keine Zeitleisten. Stattdessen besteht ein Spielobjekt aus mehreren Komponenten (components). Zu den Komponenten gehören unter anderem Sprites, Audiokomponenten und Skripte (weitere Informationen zu den verfügbaren Komponenten findest du in der [Dokumentation zu den Bausteinen](/de/manuals/building-blocks) und den zugehörigen Artikeln). Das Spielobjekt in der folgenden Bildschirmaufnahme besteht aus einem Sprite und einem Skript. Mit der Skriptkomponente steuerst du das Verhalten und das Aussehen von Spielobjekten während ihres gesamten Lebenszyklus:

![Skriptkomponente](/manuals/images/flash/script_component.png)
...
![Manuell erstellte Spielobjekte](/manuals/images/flash/manual_game_objects.png)
...
![Instanzname in Flash](/manuals/images/flash/flash_instance_name.png)
...
![Spielobjekt-ID](/manuals/images/flash/game_object_id.png)
...
Das Adressierungsmodell und die Nachrichtenübermittlung sind zentrale Konzepte der Spieleentwicklung mit Defold. Das [Handbuch zur Adressierung](/de/manuals/addressing) und das [Handbuch zur Nachrichtenübermittlung](/de/manuals/message-passing) erläutern sie ausführlich.
...
![Fabrik für das Logo](/manuals/images/flash/logo_factory.png)
...
![Fabrikkomponente](/manuals/images/flash/factory_component.png)
...
![Zeitleiste und Bühne](/manuals/images/flash/stage.png)
...
Sammlungen sind Container, mit denen du im Editor Spielobjekte und andere Sammlungen organisierst. Der Inhalt einer Sammlung kann auch per Skript zur Laufzeit mithilfe einer [Sammlungsfabrik](/de/manuals/collection-factory/#spawning-a-collection) (collection factory) dynamisch erzeugt werden. Sie funktioniert genauso wie eine normale Spielobjektfabrik. Das ist beispielsweise nützlich, um Gruppen von Gegnern oder eine Anordnung einsammelbarer Münzen dynamisch zu erzeugen. In der folgenden Bildschirmaufnahme haben wir zwei Instanzen der Sammlung `logos` manuell in der Sammlung `main` platziert.

![Sammlung](/manuals/images/flash/collection.png)

In manchen Fällen möchtest du eine völlig neue Spielwelt laden. Mit der Komponente [Sammlungs-Proxy](/de/manuals/collection-proxy/) kannst du anhand des Inhalts einer Sammlungsdatei eine neue Spielwelt erstellen. Das ist beispielsweise beim Laden neuer Spiellevel, Minispiele oder Zwischensequenzen nützlich.
...
![Zeitleiste](/manuals/images/flash/timeline.png)
...
![Flipbook-Animation](/manuals/images/flash/flipbook.png)
...
![Tiefenindex](/manuals/images/flash/depth_index.png)
...
![Tiefenindex](/manuals/images/flash/depth_index_2.png)

## Defold—z-Position

Die Positionen von Spielobjekten in Defold werden durch Vektoren mit drei Variablen dargestellt: x, y und z. Die z-Position bestimmt die Tiefe eines Spielobjekts. Im standardmäßigen [Render-Skript](/de/manuals/render) liegen die verfügbaren z-Positionen im Bereich von -1 bis 1.
...
![z-Reihenfolge](/manuals/images/flash/z_order.png)
...
![z-Reihenfolge](/manuals/images/flash/z_order_outline.png)
...
![Trefferprüfung](/manuals/images/flash/hittest.png)
...
![Trefferprüfung mit Begrenzungsrechteck](/manuals/images/flash/hitboundingbox.png)
...
Die oben aufgeführten Callback-Funktionen sind alle optional und können entfernt werden, wenn du sie nicht verwendest. Einzelheiten zum Einrichten der Eingabe findest du im [Eingabehandbuch](/de/manuals/input). Bei der Arbeit mit Sammlungs-Proxys gibt es eine häufige Stolperfalle. Weitere Informationen dazu findest du in [diesem Abschnitt](/de/manuals/input/#input-dispatch-and-on_input) des Eingabehandbuchs.
...
![Schaltfläche](/manuals/images/flash/button.png)

## Defold—GUI-Szenen und Skripte {#defoldgui-scenes-and-scripts}

Defold enthält keine native Schaltflächenkomponente. Auch lassen sich Klicks auf die Form eines bestimmten Spielobjekts nicht so einfach erkennen, wie es bei Schaltflächen in Flash der Fall ist. Die Verwendung einer [GUI](/de/manuals/gui)-Komponente ist die häufigste Lösung, unter anderem, weil die Positionen der GUI-Komponenten in Defold nicht von der Kamera im Spiel beeinflusst werden (falls eine verwendet wird). Die GUI-API enthält außerdem Funktionen, mit denen du erkennen kannst, ob Nutzereingaben wie Klicks und Berührungsereignisse innerhalb der Grenzen eines GUI-Elements liegen.
...
![Ohne Debugging](/manuals/images/flash/no_debug.png)
...
![Mit Debugging](/manuals/images/flash/with_debug.png)

Bei Kollisionen leuchten die betreffenden Kollisionsobjekte auf. Zusätzlich wird der Kollisionsvektor angezeigt:

![Kollision](/manuals/images/flash/collision.png)

Informationen zur Überwachung der CPU-Auslastung und des Speicherverbrauchs findest du schließlich in der [Profiler-Dokumentation](/ref/profiler/). Weitere Informationen zu fortgeschrittenen Debugging-Techniken findest du im [Abschnitt zum Debugging](/de/manuals/debugging) des Defold-Handbuchs.
