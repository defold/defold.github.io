...
- anchor: editor-script-runtime
  title: Laufzeitumgebung für Editor-Skripte
- anchor: anatomy-of-editor_script
  title: Aufbau einer .editor_script-Datei
- Editor-API
- anchor: commands
  title: Befehle
- anchor: use-commands-to-change-the-in-memory-editor-state
  title: Den Zustand des Editors im Arbeitsspeicher mit Befehlen ändern
- anchor: use-commands-with-the-active-editor-view
  title: Befehle mit der aktiven Editoransicht verwenden
- anchor: use-shell-commands
  title: Shell-Befehle verwenden
...
- anchor: editor-scripts-in-libraries
  title: Editor-Skripte in Bibliotheken
- anchor: preferences
...
- anchor: undoable-actions
  title: Aktionen, die rückgängig gemacht werden können
- anchor: non-undoable-actions
  title: Aktionen, die nicht rückgängig gemacht werden können
- anchor: mixing-actions-and-side-effects
  title: Aktionen und Seiteneffekte kombinieren
---
...
- `editor.ui.*` — verschiedene Funktionen für die Benutzeroberfläche, siehe [UI-Handbuch](/de/manuals/editor-scripts-ui).
...
- `on_bundle_started(opts)` — wird ausgeführt, wenn du ein Bundle erstellst oder mit Build HTML5 eine HTML5-Version eines Spiels erstellst. Wie bei `on_build_started` erscheinen die durch diesen Hook ausgelösten Änderungen im Bundle, und Fehler brechen die Bundle-Erstellung ab. `opts` enthält folgende Schlüssel:
  ...
  - `platform` — die Plattform, für die das Bundle des Spiels erstellt wird. Eine Liste möglicher Plattformwerte findest du im [Bob-Handbuch](/de/manuals/bob).
...
- `languages` (erforderlich) — eine Liste der Sprachen, für die der Server zuständig ist, wie [hier](https://code.visualstudio.com/docs/languages/identifiers#known-language-identifiers) definiert (Dateierweiterungen funktionieren ebenfalls);
