...
- anchor: runtime-access
  title: Zugriff zur Laufzeit
- anchor: sections-and-settings
  title: Abschnitte und Einstellungen
- Project
- Bootstrap
- Library
- Engine
...
- anchor: setting-config-values-on-engine-startup
  title: Konfigurationswerte beim Start der Engine setzen
- anchor: custom-project-settings
  title: Benutzerdefinierte Projekteinstellungen
---
...
![Projekteinstellungen](/manuals/images/project-settings/settings.jpg)
...
#### Dependencies
Eine Liste der *Library URL*s des Projekts. Weitere Informationen findest du im [Handbuch zu Bibliotheken](/de/manuals/libraries/).
...
#### Custom Resources
`custom_resources`
{% include shared/de/custom-resources.md %}

Das Laden benutzerdefinierter Ressourcen wird im [Handbuch zum Dateizugriff](/de/manuals/file-access/#how-to-access-files-bundled-with-the-application) genauer beschrieben.

Pfade, die Erweiterungen über `custom_resources.default` in `ext.properties` bereitstellen, werden mit dieser Einstellung kombiniert. Ein Beispiel findest du unter [benutzerdefinierte Ressourcen von Erweiterungen](/de/manuals/extensions/#custom-resources).

#### Bundle Resources
`bundle_resources`
{% include shared/de/bundle-resources.md %}

Das Laden von Bundle-Ressourcen wird im [Handbuch zum Dateizugriff](/de/manuals/file-access/#how-to-access-files-bundled-with-the-application) genauer beschrieben.
...
#### Include Dirs
Eine durch Leerzeichen getrennte Liste von Verzeichnissen, die aus deinem Projekt als Bibliothek freigegeben werden sollen. Weitere Informationen findest du im [Handbuch zu Bibliotheken](/de/manuals/libraries/).
...
Diese Einstellung steuert das Fenster. Außerhalb des Bildschirms gerenderte [Renderziele mit Multisampling](/de/manuals/render/#multisampled-render-targets) haben eine eigene Sample-Anzahl.
...
#### Update Frequency
Die gewünschte Bildrate in Hertz. Setze den Wert für eine variable Bildrate auf 0. Ein Wert größer als 0 führt zu einer festen Bildrate, die zur Laufzeit auf die tatsächliche Bildrate begrenzt wird (du kannst die Spielschleife also nicht zweimal in einem Engine-Frame aktualisieren). Verwende [`sys.set_update_frequency(hz)`](https://defold.com/ref/sys/?q=set_update_frequency#sys.set_update_frequency:frequency), um diesen Wert zur Laufzeit zu ändern. Diese Einstellung funktioniert auch in Builds ohne grafische Oberfläche (headless).
...
#### Display Profiles
Gibt die zu verwendende Datei mit Anzeigeprofilen an; standardmäßig `/builtins/render/default.display_profilesc`. Weitere Informationen findest du im [Handbuch zu GUI-Layouts](/de/manuals/gui-layouts/#creating-display-profiles).
...
#### Launch Screen
Storyboard-Datei (.storyboard). Wie du eine solche Datei erstellst, erfährst du im [iOS-Handbuch](/de/manuals/ios/#creating-a-storyboard).
...
Wähle `/builtins/manifests/android/dmengine.keep`, um die Standardregeln von Defold direkt zu verwenden. Erweiterungen liefern eigene [Regeln zum Beibehalten von Code](/de/manuals/extensions/#r8-keep-rules-for-android), die mit dieser Datei kombiniert werden.
...
Im [Android-Handbuch](/de/manuals/android/#shrinking-java-code-with-r8) erfährst du, wie du R8 aktivierst und seine Zuordnung der verschleierten Namen zusammen mit einem Release-Bundle aufbewahrst.
...
#### App Icon {#app-icon-1}
Bilddatei (.ico), die unter Windows als Anwendungssymbol verwendet werden soll. Wie du eine .ico-Datei erstellst, erfährst du im [Windows-Handbuch](/de/manuals/windows).

---

### HTML5

Weitere Informationen zu vielen dieser Optionen findest du im [Handbuch zur HTML5-Plattform](/de/manuals/html5/).
...
#### Retry Count
Die Anzahl der Wiederholungsversuche nach einem fehlgeschlagenen Download beim Start, einschließlich Netzwerkfehlern, HTTP-Fehlerstatus und Größenabweichungen bei der JavaScript- oder WebAssembly-Datei der Engine. Die erste Anfrage zählt separat. Für die Überprüfung der Archivdateien gilt eine eigene Höchstzahl an Wiederholungsversuchen; siehe [Download-Überprüfung](/de/manuals/html5/#download-verification) und `Retry Time`.
...
`liveupdate.enabled` aktiviert das Live-Update-System zur Laufzeit. Standardmäßig aktiviert. Im [Handbuch zu Live Update](/de/manuals/live-update/) erfährst du, wie du Ressourcen ausschließt, herunterlädst und einbindest.
...
#### _App Manifest_
Wenn gesetzt, wird das Anwendungsmanifest verwendet, um den Engine-Build anzupassen. Damit kannst du ungenutzte Teile aus der Engine entfernen, um die Größe der fertigen Binärdatei zu verringern. Wie du ungenutzte Funktionen ausschließt, erfährst du [im Handbuch zum Anwendungsmanifest](/de/manuals/app-manifest).

---

### Profiler

Die Einstellung **Profiler** im Anwendungsmanifest steuert, ob Profiler-Code in Debug- und Release-Builds gelinkt wird. Die folgenden Einstellungen steuern das Laufzeitverhalten des Profiler-Codes, der im ausgewählten Build enthalten ist. Einzelheiten findest du im [Handbuch zum Profiling](/de/manuals/profiling/).
