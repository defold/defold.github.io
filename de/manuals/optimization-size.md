...
toc:
- anchor: optimizing-game-size
  title: Die Spielgröße optimieren
- anchor: ios-and-android
  title: iOS und Android
- HTML5
- anchor: size-optimization-strategies
  title: Strategien zur Größenoptimierung
- anchor: optimize-engine-size
  title: Die Größe der Engine optimieren
- anchor: optimize-asset-size
  title: Die Größe der Assets optimieren
- anchor: optimize-sounds
  title: Audiodateien optimieren
- anchor: optimize-textures
  title: Texturen optimieren
- anchor: optimize-fonts
  title: Schriftarten optimieren
- anchor: exclude-content-for-download-on-demand
  title: Inhalte für den Download bei Bedarf ausschließen
- anchor: android-specific-size-optimizations
  title: Android-spezifische Größenoptimierungen
---
...
Um besser zu verstehen, woraus sich die Größe deiner Anwendung zusammensetzt, kannst du bei der Bundle-Erstellung [einen Build-Bericht erzeugen](/de/manuals/bundling/#build-reports). Häufig machen Audiodateien und Grafiken den größten Teil der Größe eines Spiels aus.
...
Eine schnelle Möglichkeit, die Größe der Engine zu reduzieren, besteht darin, nicht verwendete Funktionen aus der Engine zu entfernen. Dazu dient die [Anwendungsmanifestdatei](https://defold.com/de/manuals/app-manifest/), über die du nicht benötigte Engine-Komponenten entfernen kannst. Beispiele:
...
* GUI, Partikeleffekte und Kachelkarten (tile maps) - Diese Komponenten lassen sich mit den [Komponentenschaltern im Anwendungsmanifest](/de/manuals/app-manifest/#exclude-gui) einzeln ausschließen. Entferne Komponentenreferenzen und API-Aufrufe für jede Funktion, die du ausschließt. Wenn du Partikeleffekte ausschließt, entfällt auch die Unterstützung für Partikelknoten in GUI-Szenen.
* Formatierter Text (rich text) - Deaktiviere [Use Rich Text](/de/manuals/app-manifest/#use-rich-text), wenn Beschriftungen (labels) und GUI-Texte nur unformatierten Text benötigen. Dadurch entfallen die Verarbeitung von formatiertem Text und Stileffekte, während das gewöhnliche Rendern von Text erhalten bleibt.
...
Deine Schriftarten werden kleiner, wenn du die verwendeten Zeichen festlegst und sie unter [Characters](/de/manuals/font/#properties) angibst, anstatt das Kontrollkästchen All Chars zu verwenden.
...
Ausgeschlossene Inhalte können von ganzen Levels bis zu freischaltbaren Charakteren, Skins, Waffen oder Fahrzeugen reichen. Wenn dein Spiel viele Inhalte hat, organisiere den Ladevorgang so, dass die Startsammlung und die Sammlung des ersten Levels nur die für dieses Level unbedingt erforderlichen Ressourcen enthalten. Das erreichst du mit Sammlungs-Proxys (collection proxies) oder Fabriken (factories), bei denen das Kontrollkästchen „Exclude“ aktiviert ist. Teile die Ressourcen entsprechend dem Spielfortschritt auf. Dieser Ansatz sorgt für ein effizientes Laden der Ressourcen und hält den anfänglichen Speicherverbrauch niedrig. Erfahre mehr im [Handbuch zu Live Update](/de/manuals/live-update/).

## Android-spezifische Größenoptimierungen {#android-specific-size-optimizations}
Android-Builds müssen sowohl 32-Bit- als auch 64-Bit-CPU-Architekturen unterstützen. Wenn du [ein Bundle für Android erstellst](/de/manuals/android), kannst du angeben, welche CPU-Architekturen enthalten sein sollen:

![Ein Android-Bundle signieren](/manuals/images/android/sign_bundle.png)
...
Du kannst auch eine Kombination aus [APK Expansion Files](https://developer.android.com/google/play/expansion-files) und [Live-Update-Inhalten](/de/manuals/live-update) verwenden, die dir die [APKX-Erweiterung im Asset Portal](https://defold.com/assets/apkx/) ermöglicht.