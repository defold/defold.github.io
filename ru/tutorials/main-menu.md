---
brief: В этом примере проекта вы познакомитесь с эффектами для оформления главного меню.
language: ru
layout: tutorial
title: Пример анимации главного меню
---

# Анимация главного меню — пример проекта

<iframe width="560" height="315" src="https://www.youtube.com/embed/dPQpSlt3ahw" frameborder="0" allowfullscreen></iframe>

В этом примере проекта, который можно [открыть из редактора](/ru/manuals/project-setup/) или [скачать с GitHub](https://github.com/defold/sample-main-menu-animation), мы показываем эффекты для оформления главного меню. Меню содержит фон и два пункта.
...
Первая анимация заставляет каждый узел плавно появляться, одновременно масштабируясь с 70% до 110%.
Это выполняется в `anim1()`.

Во время следующих анимаций масштаб меняется туда и обратно: с 110% до 98%, затем до 106% и после этого до 100%. Так получается эффект упругого отскока; он реализован в `anim2()`, `anim3()` и `anim4()`.

Для фона в конце добавлено небольшое дополнительное затухание, которое выполняется в `anim5()`.
