...
Если вам нужно введение в Lua, посмотрите наше [руководство Lua in Defold](/ru/manuals/lua).
...
После установки и запуска редактора создайте новый проект и подготовьте его к работе. Создайте [новый проект](/ru/manuals/project-setup/#creating-a-new-project) на основе шаблона "Empty Project".
...
Changed Files
: Содержит файлы, добавленные, изменённые, переименованные или удалённые локально по сравнению с текущим коммитом Git. Можно просмотреть различия для одного изменённого или переименованного текстового файла за раз, а также откатить выбранные локальные изменения. Для синхронизации с удалённым репозиторием используйте внешний Git-клиент или командную строку.
...
Добавьте ссылку на релиз в список [game.project dependencies](/ru/manuals/libraries/#setting-up-library-dependencies). После добавления Spine extension редактор нужно перезапустить, чтобы активировать включённую в него интеграцию.
...
2. Создайте новый файл *Atlas* для текстур земли (щёлкните правой кнопкой по подходящей папке, например по *main* в *Assets pane*, и выберите <kbd>New ▸ Atlas File</kbd>). Назовите файл *level.atlas*.
  ...
  <div class='sidenote' markdown='1'>
  *Atlas* — это файл, который объединяет несколько отдельных изображений в одно большое. Это экономит место и повышает производительность. Подробнее об Atlas и других возможностях 2D-графики читайте в [документации по 2D graphics](/ru/manuals/2dgraphics).
  </div>
...
Пока этого описания, вероятно, достаточно. Однако гораздо более глубокий разбор есть в [руководстве Building blocks](/ru/manuals/building-blocks). Позже к нему полезно вернуться, чтобы лучше понять, как работает Defold.
...
Defold добавляет поверх Lua набор API, а также систему _message passing_, которая позволяет программировать взаимодействие между game object. Подробнее читайте в [руководстве Message passing](/ru/manuals/message-passing).
</div>
...
<div class='sidenote' markdown='1'>
Коллизия "Kinematic" означает, что столкновения должны регистрироваться, но physics engine не будет автоматически разрешать их и симулировать объекты. Physics engine поддерживает несколько разных типов collision object. Подробнее о них читайте в [документации по Physics](/ru/manuals/physics).
</div>
...
Чтобы сделать это и корректно обрабатывать столкновения, потребуется немного векторной математики. Подробное объяснение того, как решать kinematic collisions, есть в [документации Physics](/ru/manuals/physics-resolving-collisions/).
</div>
...
3. Сохраните файл.
4. Создайте новый *Game Object* file с именем *platform.go* в папке *level*. (Щёлкните правой кнопкой по *level* в *Assets pane*, затем выберите <kbd>New ▸ Game Object File</kbd>.)
...
> Скачать завершённую версию проекта можно [здесь](/tutorials/images/runner/sample-runner.zip)
