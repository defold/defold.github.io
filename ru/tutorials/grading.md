...
Предполагается, что вы уже немного знакомы с редактором Defold и в общих чертах понимаете GL shaders и render pipeline движка. Если нет, сначала стоит посмотреть [руководство по Shader](/ru/manuals/shader/) и [руководство по Render](/ru/manuals/render/).
...
Обычно это именно то, что нужно. Но если вы хотите, например, сделать всю игру черно-белой, модифицировать отдельный шейдер для каждого типа компонента неудобно. В Defold есть несколько встроенных материалов и наборов vertex/fragment-программ, и поддерживать это вручную быстро становится тяжело.

Гораздо гибче разделить рендеринг на два шага:

![Render target](/tutorials/images/grading/render_target.png)
...
3. Откройте `grade.render` и укажите для свойства *Script* значение `/main/grade.render_script`.
   ...
   ![grade.render](/tutorials/images/grading/grade_render.png)

4. Откройте `game.project` и в поле *Render* укажите `/main/grade.render`.

   ![game.project](/tutorials/images/grading/game_project.png)
...
6. Добавьте *Tag* с именем `grade`.
   ...
   ![grade.material](/tutorials/images/grading/grade_material.png)

7. Откройте *`main.collection`*, выберите model-компонент объекта `grade` и задайте для него материал `/main/grade.material`.

   ![model properties](/tutorials/images/grading/model_properties.png)
...
Добавим в render script новый predicate для quad-модели. Откройте `grade.render_script` и измените `init()`:
...
![desaturated game](/tutorials/images/grading/desaturated_game.png)
...
![color cube](/tutorials/images/grading/color_cube.png)
...
![render target grading](/tutorials/images/grading/render_target_grading.png)
...
![lookup texture](/tutorials/images/grading/lut.png)
...
Здесь `B` — значение синего в диапазоне от `0` до `1`, а `N` — число ячеек. В нашем случае номер ячейки будет в диапазоне `0`--`15`.

Например, RGB-цвет `(0.63, 0.83, 0.4)` окажется в ячейке с номером `6`, потому что именно там хранятся значения с синим каналом около `0.4`. Дальше по значениям red и green можно вычислить окончательные texture coordinates:

![lookup table](/tutorials/images/grading/lut_lookup.png)

Важно читать значения из *центров* пикселей, чтобы sampling не захватывал соседние ячейки.
...
В выбранной точке мы оказываемся между четырьмя пикселями, и итог зависит от фильтрации sampler:

![lookup table filtering](/tutorials/images/grading/lut_filtering.png)

- Если filtering = `NEAREST`, GL вернет цвет ближайшего пикселя. Для lookup texture `4 bit` это приведет к заметной квантованности цвета.
- Если filtering = `LINEAR`, GL вернет интерполированное значение, смешав соседние пиксели.

Именно линейная фильтрация позволяет получить хорошую точность даже из сравнительно маленькой lookup table.
...
3. Установите *`Filter min`* = `FILTER_MODE_MIN_LINEAR` и *`Filter mag`* = `FILTER_MODE_MAG_LINEAR`.
    ...
    ![lookup table sampler](/tutorials/images/grading/material_lut_sampler.png)

4. Скачайте текстуру lookup table `lut16.png` и добавьте ее в проект.

    ![16 colors lookup table](/tutorials/images/grading/lut16.png)

5. Откройте *`main.collection`* и укажите texture `lut` для модели.

    ![quad model lookup table](/tutorials/images/grading/quad_lut.png)
...
![world original look](/tutorials/images/grading/world_original.png)

Но под поверхностью есть проблема. Посмотрите, что происходит, если добавить sprite с тестовым синим градиентом:

![blue banding](/tutorials/images/grading/blue_banding.png)

Появляются заметные полосы. Почему?

## Интерполяция синего канала

Проблема в том, что GL не интерполирует синий канал между lookup cells автоматически. Мы заранее выбираем одну конкретную ячейку по синему значению, и вся информация между соседними ячейками теряется.

Например, если синий канал находится где-то между `0.400` и `0.466`, мы все равно будем читать цвет только из ячейки `6`, где синий равен `0.400`.

Чтобы повысить точность синего канала, нужно сделать интерполяцию самостоятельно: брать цвет из двух соседних ячеек и смешивать их.

Тогда нужно читать из:
...
Здесь `C_frac` — дробная часть синего канала после масштабирования в диапазон `0`--`15`. В GLSL для этого есть функции `fract()` и `mix()`. Финальный вариант `grade.fp` выглядит так:
...
![blue no banding](/tutorials/images/grading/blue_no_banding.png)
...
![world in Affinity](/tutorials/images/grading/world_graded_affinity.png)
...
![world graded](/tutorials/images/grading/world_graded.png)
