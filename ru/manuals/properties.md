...
Для GUI-нод сама нода передаётся первым параметром либо специализированной функции свойства, либо универсальным функциям `gui.get()` и `gui.set()`:
...
| *rotation* | Локальное вращение игрового объекта, выраженное в виде `quaternion` | `quaternion` | `get+set` |
| *euler*    | Локальное вращение игрового объекта в углах Эйлера | `vector3` | `get+set` |
| *scale*    | Локальное неоднородное масштабирование игрового объекта, выраженное в виде вектора, каждый компонент которого содержит множитель по соответствующей оси. Чтобы удвоить размер по осям X и Y, не изменяя Z, используйте `vmath.vector3(2.0, 2.0, 1.0)`. | `vector3` | `get+set` |
| *scale.xy* | Локальное неоднородное масштабирование игрового объекта по осям X и Y. Используйте это свойство или `go.set_scale_xy()`, если масштабирование по Z не требуется. | `vector3` | `get+set` |

<div class='sidenote' markdown='1'>
Существуют также специальные функции для работы с преобразованием игрового объекта: `go.get_position()`, `go.set_position()`, `go.get_rotation()`, `go.set_rotation()`, `go.get_scale()`, `go.set_scale()` и `go.set_scale_xy()`.
</div>
...
| свойство   | описание                                                                 | тип       |                  |
| ---------- | ------------------------------------------------------------------------ | --------- | ---------------- |
| *size*     | Немасштабированный размер спрайта — его размер, взятый из исходного атласа. | `vector3` | `get` |
| *image*    | Хэш пути к текстуре спрайта.                                             | `hash`    | `get` |
| *scale*    | Неравномерный масштаб спрайта.                                           | `vector3` | `get+set` |
| *scale.xy* | Неравномерный масштаб спрайта вдоль осей X и Y.                          | `vector3` | `get+set` |
| *material* | Материал, используемый спрайтом.                                         | `hash`    | `get+set` |
| *cursor*   | Положение (от 0 до 1) курсора воспроизведения.                           | `number`  | `get+set` |
| *playback_rate* | Частота кадров анимации flipbook.                                   | `number`  | `get+set` |
...
| *texture0*--*texture15* | Хэши путей текстур модели. | `hash` | `get+set`|
...
| *material* | Материал, используемый моделью. | `hash` | `get+set` |

*СВОЙСТВА КОМПОНЕНТА LABEL*

| Свойство | Описание                    | Тип         |                  |
| ---------- | -------------------------------------- | --------------- | ---------------- |
| *text* | Текстовое содержимое метки. Доступно начиная с Defold 1.13.2. | `string` | `get+set` |
| *scale* | Масштаб метки                                                | `vector3` | `get+set` |
| *scale.xy* | Масштаб надписи (label) вдоль осей X и Y. | `vector3` | `get+set` |
...
| *material* | Материал, используемый меткой. | `hash` | `get+set` |
| *font* | Шрифт, используемый меткой. | `hash` | `get+set` |

## Свойства GUI-ноды

Для свойств GUI-нод существуют специализированные функции чтения и записи, например `gui.get_position()` и `gui.set_position()`. Перечисленные ниже встроенные свойства также можно читать и изменять универсальными функциями `gui.get(node, property)` и `gui.set(node, property, value)`. Для других значений ноды всё ещё могут требоваться специализированные функции. Константы материала GUI-нод также используют универсальные функции. Чтобы обратиться к отдельному компоненту векторного свойства, добавьте его имя, например `gui.set(node, "color.x", 1)`.

Универсальные и специализированные функции не всегда используют одинаковые типы значений. Для полных свойств `position`, `scale`, `size` и `euler` функция `gui.get()` возвращает `vector4`, а соответствующие специализированные функции --- `vector3`. `gui.set()` принимает для этих свойств и `vector3`, и `vector4`. Универсальное свойство `rotation` использует кватернион; для задания вращения в градусах используйте `euler`.

* `position` (или `gui.PROP_POSITION`)
* `rotation` (или `gui.PROP_ROTATION`)
* `euler` (или `gui.PROP_EULER`)
...
* `leading` (или `gui.PROP_LEADING`)
* `tracking` (или `gui.PROP_TRACKING`)
...
| *rotation* | Вращение ноды. Функция чтения возвращает кватернион; функция записи принимает кватернион или углы Эйлера в виде вектора. | `quaternion`, `vector3` или `vector4` | `gui.get_rotation()` `gui.set_rotation()` |
| *euler* | Вращение ноды в углах Эйлера, выраженных в градусах. | `vector3` | `gui.get_euler()` `gui.set_euler()` |
...
| *leading* | Масштаб межстрочного интервала текстовой ноды. | `number` | `gui.get_leading()` `gui.set_leading()` |
| *tracking* | Масштаб межбуквенного интервала текстовой ноды. | `number` | `gui.get_tracking()` `gui.set_tracking()` |
| *slice9* | Расстояния от краёв ноды Slice 9. | `vector4` | `gui.get_slice9()` `gui.set_slice9()` |