...
Подробнее см. в [руководстве «Написание кода»](/ru/manuals/writing-code).
...
Подробнее см. в [руководстве по HTTP API редактора](/ru/manuals/editor-http-api/#reading-console-output).

### Сохраняемые журналы {#persisted-logs}

Defold также может сохранять журнал игры, если включить параметр `Write Log File` в `game.project`. См. [Журналы игры и системы](/ru/manuals/debugging-game-and-system-logs/). Журналирование в файл полезно для упакованных приложений и тестирования на целевых устройствах, где консоль редактора недоступна.
...
Подробнее см. в [руководстве по сервису движка](/ru/manuals/engine-service/#automation-bridge-extension).
...
Редактор может создать и предоставить HTML5-сборку с помощью актуальной команды `build-html5`, как описано в [руководстве по HTTP API редактора](/ru/manuals/editor-http-api/#building-html5). Bob также может создать HTML5-бандл без редактора.
...
| Метод | Назначение |
| --- | --- |
| [Предпросмотр редактора](/ru/manuals/editor-http-api/#rendering-scene-previews) | Макет загруженного ресурса, например уровня или GUI, композиция атласа, инспекция тайловой карты, композиция статической сцены, проверка рендеринга и шейдеров редактора либо создание миниатюр для документации |
| [Снимок экрана среды выполнения](/ru/manuals/engine-service) | Отрисованное состояние запущенной сборки в контролируемом сценарии |
...
[Руководство по Bob](/ru/manuals/bob) описывает платформы, файлы настроек, бандлы, кэши, нативные расширения и отчёты о сборке.
...
Один и тот же формат должен быть пригоден для разработчика, локального скрипта, сервиса CI или [ИИ-агента для программирования](/ru/manuals/ai-agents). Это сохраняет проверку детерминированной, даже когда диагностика или исправление делегированы.