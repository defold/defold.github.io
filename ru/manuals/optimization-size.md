...
## Оптимизация размера движка
Быстрый способ уменьшить размер движка — исключить из него функциональность, которая вам не нужна. Это можно сделать с помощью [файла манифеста приложения](https://defold.com/ru/manuals/app-manifest/), в котором можно отключить ненужные компоненты движка. Примеры:

* Physics — если вы не используете физику Box2D или Bullet3D, рекомендуется исключить соответствующие модули
* GUI, эффекты частиц и тайловые карты — эти компоненты можно исключить по отдельности с помощью [переключателей компонентов в App Manifest](/ru/manuals/app-manifest/#exclude-gui). Удалите ссылки на компоненты и вызовы API каждой исключаемой функции. Исключение эффектов частиц также исключает поддержку нод частиц в GUI-сценах.
* Форматированный текст — отключите [Use Rich Text](/ru/manuals/app-manifest/#use-rich-text), если меткам и GUI-тексту нужен только обычный текст. Это исключает разбор текстовой разметки и стилевые эффекты, сохраняя обычную отрисовку текста.
...
Defold поддерживает Wave-файлы с 8- и 16-битным PCM. Ogg Vorbis и Ogg Opus используют собственные сжатые форматы, поэтому требование к разрядности PCM к ним не относится. Декодер Opus по умолчанию не включён; прежде чем использовать ресурсы `.opus`, включите **Include Sound Decoder: Opus** в [манифесте приложения](/ru/manuals/app-manifest/#sound).
...
Также вы можете использовать [дополнительные файлы APK Expansion](https://developer.android.com/google/play/expansion-files) в сочетании с [контентом Live Update](/ru/manuals/live-update), используя [расширение APKX](https://defold.com/assets/apkx/).