...
- Примеры использования пользовательских вершинных атрибутов
- Инстансинг
- anchor: vertex-and-fragment-constants
  title: Вершинные и фрагментные константы
- Константы материала GUI-ноды
- Сэмплеры
- Настройки сэмплера
- Буферы констант
---
...
: Семантический тип указывает смысл того, *что* представляет атрибут и/или *как* он должен отображаться в редакторе. Например, указание `SEMANTIC_TYPE_COLOR` отобразит виджет выбора цвета, хотя данные по-прежнему передаются как есть из движка в шейдер.
  ...
  - `SEMANTIC_TYPE_TEXTURE_TRANSFORM_2D` — генерирует для атрибута матрицу преобразования текстуры 3x3 на вершину. Для particle-компонентов движок предоставляет матрицу, которая преобразует координаты в пространство атласа для свойства image компонента. Для sprite-компонентов движок предоставляет матрицу для каждого изображения, используемого компонентом (при использовании multi-texturing). Для model-компонентов предоставляется единичная матрица.
...
Value
: Значение атрибута. Может быть переопределено для каждого компонента, в противном случае используется значение по умолчанию. Внимание: для *default* атрибутов (position, texture coordinates и page indices) это значение игнорируется.
...
## Вершинные и фрагментные константы {#vertex-and-fragment-constants}
...
: Обратная мировая матрица. Используется для преобразования из мирового пространства обратно в локальное пространство объекта.

`CONSTANT_TYPE_VIEW_INVERSE`
: Обратная матрица вида. Используется для преобразования из пространства камеры обратно в мировое пространство.

`CONSTANT_TYPE_PROJECTION_INVERSE`
: Обратная матрица проекции. Используется для преобразования из пространства отсечения обратно в пространство камеры.

`CONSTANT_TYPE_VIEWPROJ_INVERSE`
: Обратная матрица комбинированных матриц вида и проекции. Используется для преобразования из пространства отсечения обратно в мировое пространство.

`CONSTANT_TYPE_WORLDVIEW_INVERSE`
: Обратная матрица комбинированных мировой матрицы и матрицы вида. Используется для преобразования из пространства камеры обратно в локальное пространство объекта.

`CONSTANT_TYPE_WORLDVIEWPROJ_INVERSE`
: Обратная матрица комбинированных мировой матрицы, матрицы вида и матрицы проекции. Используется для преобразования из пространства отсечения обратно в локальное пространство объекта. Эти обратные константы позволяют не вычислять обратную матрицу в шейдере.
...
`CONSTANT_TYPE_TIME`
: Предоставляемый движком `vector4`, где `.x` — время, прошедшее с запуска движка, `.y` — дельта времени с предыдущего кадра, а `.z` и `.w` пока равны нулю. Движок автоматически обновляет это значение; вызывать `go.set()` не требуется. Пример приведён в [руководстве по Shadertoy](/tutorials/shadertoy/#animation).
...
`CONSTANT_TYPE_USER`
: Константа типа vector4, которую вы можете использовать для любых собственных данных, которые вы хотите передать в программу шейдера. Вы можете установить начальное значение константы в месте ее определения, но она изменяема через функции [go.set()](/ref/go/#go.set) / [go.animate()](/ref/go/#go.animate). Также вы можете получить ее значение через [go.get()](/ref/go/#go.get). Изменение константы материала одного экземпляра компонента [ломает пакетирование для рендеринга и приведет к дополнительным вызовам отрисовки](/ru/manuals/render/#draw-calls-and-batching).
