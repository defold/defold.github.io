...
Defold предлагает централизованную обработку физических событий через функцию `physics.set_event_listener()`. Эта функция позволяет задать собственный обработчик, который будет принимать все события физических взаимодействий в одном месте, что упрощает код и повышает эффективность.
...
Если обработчик установлен, [physics messages](/ru/manuals/physics-messages) больше не будут отправляться для того физического мира, где установлен этот обработчик.
</div>
