...
Идентификаторы в структуре задаются разработчиком и должны быть уникальными в пределах своего контекста имён. В данном примере для игрового объекта был выбран идентификатор --- "bean", для компонента Sprite --- "body", а для скрипта, управляющего персонажем --- "controller". Идентификаторы, используемые в строковых URL-адресах, не должны содержать `:` или `#`: синтаксис URL резервирует `:` как разделитель сокета, а `#` --- как разделитель игрового объекта и компонента. Другие знаки пунктуации парсер URL не отклоняет.
...
Defold предлагает два удобных сокращения, которые можно использовать для отправки сообщений без указания полного URL:
...
Тогда каждый объект "bean" отвечает за отправку менеджеру сообщений о состоянии: "contact", если обнаружен враг, или "ouch!", если получен урон. Чтобы это работало, скрипт "controller" объекта "bean" использует абсолютную адресацию для отправки сообщений компоненту "controller" объекта "manager".
...
Движок хранит все идентификаторы в виде хэш-значений. Все функции, которым в качестве аргумента необходимо передать компонент или игровой объект, принимают строку, хэш или URL-объект. Выше мы рассматривали, как использовать строки при адресации.
...
Такой идентификатор можно использовать взамен строкового идентификатора, или сформировать его самостоятельно. При этом следует иметь в виду, что хэшированный идентификатор соответствует пути объекта, то есть абсолютному адресу:
...
URL --- это объект, обычно записываемый в виде строки специального формата. Типовой URL состоит из трех частей:
