...
- anchor: dynamic-loading-of-factory-resources
  title: Динамическая загрузка ресурсов фабрики
...
`[position]`
: (Опционально) Мировая позиция нового игрового объекта. Это должен быть `vector3`. Если позицию не указать, игровой объект будет порожден в позиции игрового объекта, вызывающего `factory.create()`.
...
## Динамическая загрузка ресурсов фабрики {#dynamic-loading-of-factory-resources}
...
Настройка проекта *Max Instances* в разделе *Collection* задаёт верхнюю границу количества игровых объектов в каждой коллекции (игровом мире). Во время сборки Defold может выделить меньшую ёмкость, если способен определить, что это безопасно. Все одновременно существующие игровые объекты в мире учитываются в этой ёмкости независимо от того, размещены ли они в редакторе или порождены во время выполнения.

![Max instances](/manuals/images/factory/factory_max_instances.png)

Фактически выделяемая ёмкость зависит от анализа во время сборки:

* Если сборка способна определить фиксированное количество игровых объектов (обычно когда в коллекции нет Factory или Collection Factory), ёмкость равна количеству скомпилированных объектов, но не превышает *Max Instances*.
* Коллекция, содержащая Factory или Collection Factory, использует *Max Instances* как ёмкость игровых объектов. Для прототипа со статической ссылкой сборка определяет типы компонентов, которые способна породить фабрика. Эти типы используют соответствующие максимальные значения из настроек проекта, а незатронутые типы компонентов по-прежнему могут использовать точные значения.
* Включение *Dynamic Prototype* отключает анализ количества компонентов для родительской коллекции, поэтому она использует *Max Instances* и настроенные максимальные значения для каждого типа компонентов.

Планируйте *Max Instances* исходя из наибольшего количества игровых объектов, которые могут одновременно существовать в динамическом мире. О том, как вычисляются остальные ограничения компонентов, см. в разделе [Оптимизация максимального количества компонентов](/ru/manuals/project-settings/#component-max-count-optimizations).
