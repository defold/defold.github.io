...
Bob распространяется в виде Java _JAR_ архива, содержащего все необходимое для сборки. Последнюю версию *bob.jar* можно найти на [странице релизов GitHub](https://github.com/defold/defold/releases). Выберите релиз, затем загрузите *bob/bob.jar*. Для запуска потребуется OpenJDK 25.

Совместимые зеркала OpenJDK 25:
* [OpenJDK 25 от Microsoft](https://learn.microsoft.com/en-us/java/openjdk/download#openjdk-25)
* [OpenJDK 25 от Adoptium Working Group](https://github.com/adoptium/temurin25-binaries/releases) / [Adoptium.net](https://adoptium.net/)
...
`--texture-compression` --- переключатель без значения. Укажите его, чтобы включить сжатие, выбранное профилями текстур; не указывайте, чтобы отключить сжатие текстур. Устаревшая форма `--texture-compression=true` по-прежнему принимается. Устаревшая форма `--texture-compression=false` игнорируется и выводит предупреждение; вместо неё просто не указывайте переключатель.
...
По умолчанию Bob ищет проект в текущем каталоге и собирает в *build/default* ресурсы, достижимые из `game.project`. Ресурсы без ссылок не компилируются. Если код во время выполнения загружает необработанные файлы по пути, включите их с помощью [настройки проекта Custom Resources](/ru/manuals/project-settings/#custom-resources); для остальных ресурсов Defold нужна ссылка, достижимая из корня сборки.
