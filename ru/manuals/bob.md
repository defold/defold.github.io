...
Bob распространяется в виде Java _JAR_ архива, содержащего все необходимое для сборки. Последнюю версию *bob.jar* можно найти на [странице релизов GitHub](https://github.com/defold/defold/releases). Выберите релиз, затем загрузите *bob/bob.jar*. Если вы используете Defold 1.12.0 или новее, для запуска потребуется OpenJDK 25. Для более старых версий Defold потребуется OpenJDK 21.

Совместимые зеркала OpenJDK 25 (для Defold 1.12.0):
...
`x86_64-darwin` (Defold 1.3.5 и старее)
`x86_64-macos` (Defold 1.3.6 и новее)
: macOS 64 бита

`arm64-macos` (Defold 1.5.0 и старее)
...
`armv7-darwin` (Defold 1.3.5 и старее)
`armv7-ios` (Defold 1.3.6 и новее)
