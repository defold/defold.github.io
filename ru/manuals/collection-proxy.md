...
- anchor: changing-an-excluded-proxys-collection
  title: Изменение коллекции исключённого прокси
...
2. В свойстве *Collection* укажите ссылку на коллекцию, которую необходимо динамически загрузить в среду выполнения позднее. Это статическая зависимость времени сборки: указанная коллекция и её зависимости компилируются. Если флажок *Exclude* не установлен, они включаются в основной бандл. Если *Exclude* установлен, ресурсы, на которые ссылаются только исключённые прокси, могут не включаться в основной бандл для Live Update, а незагруженный прокси во время выполнения можно перенаправить на другую скомпилированную коллекцию, как описано ниже.

![add proxy component](/manuals/images/collection-proxy/create_proxy.png)

(Содержимое можно исключить из сборки и загрузить вместо него код, отметив опцию *Exclude* и используя [Live-обновление](/ru/manuals/live-update/).)
...
`"enable"`
: Это сообщение указывает компоненту Collection Proxy, что все игровые объекты и компоненты должны быть активированы. Например, все компоненты Sprite начинают отрисовываться при активации по сообщению `"enable"`.

## Изменение коллекции исключённого прокси {#changing-an-excluded-proxys-collection}

Функция [`collectionproxy.set_collection()`](/ref/collectionproxy/#collectionproxy.set_collection) позволяет перенаправить исключённый и незагруженный прокси на скомпилированную коллекцию. Это удобно после монтирования пакета Live Update. У прокси должен быть установлен флажок *Exclude*, и он не должен быть загружен или находиться в процессе загрузки. Путь должен оканчиваться на `.collectionc`. При загрузке прокси коллекция и все её зависимости должны быть доступны системе ресурсов.

Проверяйте возвращаемое значение перед загрузкой прокси. Инициализируйте и активируйте новый мир только после получения `proxy_loaded`:
...
Чтобы восстановить коллекцию, назначенную в редакторе, вызовите `collectionproxy.set_collection("#level_proxy", nil)`, пока прокси не загружен и не находится в процессе загрузки. Инструкции по скачиванию и монтированию контента см. в [руководстве по скриптованию Live Update](/ru/manuals/live-update-scripting/), а коды ошибок `collectionproxy.RESULT_*` --- в справочнике API.
