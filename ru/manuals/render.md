...
- anchor: multisampled-render-targets
  title: Рендер-таргеты с мультисэмплингом
- Обработчики текстур
...
Чтобы включить отсечение для вызова отрисовки, передайте матрицу вида-проекции в параметре `frustum` функции `render.draw()`:
...
При рендеринге с помощью компонента камеры функция `render.set_camera()` может автоматически использовать матрицу вида-проекции камеры для последующих вызовов отрисовки:
...
При использовании любого из этих способов эмиттеры Particle FX отсекаются по их ограничивающим рамкам.

Усечение реализовано в движке по типу компонента. Текущий статус:
...
Начиная с Defold 1.13.0, примитивы компонентов используют порядок вершин против часовой стрелки, при этом нормаль примитива направлена к камере. Спрайты, GUI-ноды, тайловые карты (тайлгриды) и эффекты частиц используют тот же порядок, что и остальные типы компонентов, поэтому для всех компонентов можно применять одинаковые настройки отсечения граней.

Это может повлиять на проекты, в которых отсечение граней настроено для компонентов, отличных от моделей. Если компонент неожиданно отсекается, убедитесь, что задние грани выбраны с помощью `render.set_cull_face(graphics.FACE_TYPE_BACK)`, либо удалите вызов `render.set_cull_face()`, чтобы использовать режим `graphics.FACE_TYPE_BACK` по умолчанию.
</div>
...
## Ресурсы рендеринга
Чтобы передать определенные ресурсы движка в рендер-скрипт, вы можете добавить их в таблицу `Render Resources` в файле `.render` проекта:
...
### Рендер-таргеты с мультисэмплингом {#multisampled-render-targets}

Рендер-таргеты поддерживают сглаживание методом мультисэмплинга (MSAA). Оно сглаживает края геометрии во внеэкранном проходе рендеринга. Число сэмплов таргета не зависит от [Display ▸ Samples](/ru/manuals/project-settings/#samples), который управляет сглаживанием окна.

Для ресурса `.render_target` задайте в редакторе **Sample Count** равным `1`, `2`, `4`, `8` или `16`. Значение `1` отключает мультисэмплинг. Добавьте ресурс в таблицу **Render Resources** файла `.render` и используйте назначенное ему имя в `render.set_render_target()`, как в примере выше.

Также можно создать таргет в функции `init()` render-скрипта. Поместите `sample_count` во внешнюю таблицу параметров, рядом с присоединёнными буферами:
...
В этом примере таргет содержит только цветовой буфер. Все присоединённые к таргету буферы цвета, глубины и трафарета используют одно число сэмплов. Если проход требует проверки глубины, добавьте буфер глубины и обычные настройки проверки глубины.

Для следующего фрагмента `update()` назначьте материалам сцены тег `scene`, а материалу полноэкранного прямоугольника — тег `present`. Материал прямоугольника должен читать текстуру из текстурного блока `0`. Задайте подходящие матрицы вида и проекции для каждого прохода:
...
Переключение с таргета завершает проход и автоматически объединяет сэмплы его цветовых буферов в итоговое изображение. `render.enable_texture()` привязывает полученную цветовую текстуру, поэтому прямоугольник использует обычный текстурный сэмплер. Отдельная команда объединения не требуется.

Запрашиваемое число сэмплов по умолчанию равно `1` и должно быть положительным целым числом. Графические бэкенды уменьшают неподдерживаемые значения до поддерживаемой степени двойки, при необходимости до `1`, и записывают предупреждение в журнал при изменении значения. Чем больше сэмплов, тем больше памяти требуется для присоединённых буферов.

При использовании ресурса рендер-таргета можно узнать его фактическое число сэмплов из `.script` игрового объекта с помощью `resource.get_render_target_info()`. Например, после добавления `/render/offscreen.render_target` в **Render Resources**:
...
Используйте это фактическое значение при проверке поддержки устройства, не предполагая, что запрошенное число сэмплов было доступно. Полные таблицы параметров и результатов приведены в документации [`render.render_target()`](/ref/beta/render/#render.render_target:parameters) и [`resource.get_render_target_info()`](/ref/beta/resource/#resource.get_render_target_info:path).
...
`"draw_line"`
: Нарисовать отладочную линию. Используется для визуализации `ray_casts`, векторов и много другого. Линии рисуются с помощью вызова `render.draw_debug3d()`.
...
Каждый вызов `render.draw()` определяет порядок сортировки соответствующих элементов мира. По умолчанию используется `render.SORT_BACK_TO_FRONT`; для отрисовки от ближних элементов к дальним используйте `render.SORT_FRONT_TO_BACK`, а для сохранения порядка вставки — `render.SORT_NONE`:
...
Выбранный порядок определяет, какие элементы будут соседними, и поэтому может влиять на пакетирование. В упорядоченном списке каждый объект объединяется с предыдущим в один вызов отрисовки, если выполнены следующие условия:
...
Это означает, что если два компонента Sprite в одной collection proxy после выбранной сортировки оказались соседними и используют одну текстуру, материал и константы, они будут объединены в один вызов отрисовки.
