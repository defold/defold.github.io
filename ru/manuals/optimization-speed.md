...
* Упростите шейдеры — изучите [оптимизации GLSL в статье Khronos](https://www.khronos.org/opengl/wiki/GLSL_Optimizations). Вы также можете изменить стандартные шейдеры Defold (они находятся в `builtins/materials`) и выбрать меньшую точность там, где шейдеру не требуется `highp`. В кросс-компилированных шейдерах GLSL ES по умолчанию для значений с плавающей точкой используется `mediump`, а для целых чисел — `highp`; эти значения по умолчанию можно изменить в настройках проекта Shader. Явно заданные квалификаторы отдельных переменных имеют приоритет. См. [документацию по точности шейдеров](/ru/manuals/shader/#precision).
