...
- нельзя использовать некоторые функции, выполнение которых занимает длительное время, в контекстах, где редактор ожидает немедленного ответа от скрипта. См. раздел [Режимы выполнения](#execution-modes) для подробностей.
...
- `editor.get(node_id, property)` — получить значение узла в редакторе. Узлы в редакторе — это различные сущности, такие как скриптовые или коллекционные файлы, игровые объекты в коллекциях, json-файлы, загруженные как ресурсы и т.д. `node_id` — это userdata, переданная редактором скрипту. Вместо `node_id` можно также передать путь к ресурсу, например `"/main/game.script"`. `property` — строка. Поддерживаются следующие свойства:
  ...
  - `"parent"` — родительский узел редактора для узла Outline, у которого есть родитель
  ...
  - для GUI-файлов: списки узлов, включая `layers`, `fonts`, `materials`, `textures`, `particlefxs`, `nodes` и `layouts`
...
- `editor.properties(node_id)` — возвращает отсортированный, зависящий от контекста список имён свойств, которые можно прочитать из узла, например `pprint(editor.properties("/game.project"))`. Чтобы определить, можно ли также изменить, сбросить, дополнить или переупорядочить указанное свойство, используйте функции `editor.can_*`.
...
- `editor.ui.*` — различные функции, связанные с пользовательским интерфейсом. См. [UI manual](/ru/manuals/editor-scripts-ui).
...
Если модуль скрипта редактора определяет `get_commands()`, эта функция вызывается при перезагрузке расширений. Возвращённые команды могут появляться в меню и в контекстных меню Assets, Outline, Scene и Code в зависимости от поля `locations`. Например:
```lua
local M = {}
...
- `locations` (обязательный) — массив, определяющий, где должна быть доступна команда. Поддерживаются значения `"Edit"`, `"View"`, `"Project"`, `"Debug"` и `"Help"` для соответствующих меню; `"Bundle"` для подменю **Project → Bundle**; а также `"Assets"`, `"Outline"`, `"Scene"` и `"Code"` для соответствующих контекстных меню.
- `query` — способ для команды запросить у редактора необходимую информацию и определить, с какими данными она работает. Для каждого ключа в таблице `query` будет соответствующий ключ в таблице `opts`, который обратные вызовы `active` и `run` получают в качестве аргумента. Поддерживаемые ключи:
  ...
  - `active_view` — команда доступна, когда активное представление редактора соответствует запрошенному типу. Активное представление передаётся в обработчик команды как `opts.active_view`.
    - `type` — тип активного представления, с которым работает команда: `"code"`, `"scene"`, `"html"` или `"form"`.
    - Активное представление поддерживает свойства `"type"`, `"resource"` и `"dirty"`. Используйте `editor.get(view, "resource")`, чтобы получить ресурс, открытый в представлении, и `editor.get(view, "dirty")`, чтобы проверить наличие несохранённых изменений.
...
Команды в пунктах меню, например `"View"`, могут запрашивать текущее активное представление редактора. Это полезно, когда команда должна работать с файлом или сценой, которые пользователь сейчас просматривает:
...
Помимо свойств из Outline, GUI-файлы определяют несколько свойств со списками узлов:
- `layers` — список узлов слоёв редактора (с поддержкой переупорядочивания);
- `fonts` — список узлов шрифтов;
- `materials` — список узлов материалов;
- `textures` — список узлов текстур;
- `particlefxs` — список узлов Particle FX;
- `nodes` — список узлов GUI;
- `layouts` — список узлов layout GUI.
...
Редактор поддерживает подмножество [Language Server Protocol](https://microsoft.github.io/language-server-protocol/): диагностику (предупреждения линтера), автодополнение, информацию при наведении, символы документа в панели Structure, переход к определению, поиск ссылок, переименование символов и форматирование документа или диапазона. Наведите курсор на символ, чтобы увидеть информацию от языкового сервера. Установив курсор на символ, нажмите <kbd>F2</kbd>, чтобы переименовать его, <kbd>F12</kbd>, чтобы перейти к определению, или <kbd>Shift+F12</kbd>, чтобы найти ссылки. Эти действия также доступны в меню <kbd>Edit</kbd>. О команде форматирования и настройке форматирования при сохранении см. в разделе [Форматирование кода](/ru/manuals/writing-code/#formatting-code).

Встроенный языковой сервер Lua содержит аннотации типов Defold для API среды выполнения и скриптов редактора. В файлах `.editor_script` автодополнение и диагностика распознают функции `editor.*`, а также типы их аргументов и возвращаемых значений. См. раздел [Автодополнение кода](/ru/manuals/writing-code/#code-completion).

Чтобы зарегистрировать дополнительный языковой сервер, определите функцию `get_language_servers` в вашем скрипте редактора следующим образом:
...
- вложенные: каждый ключ предпочтения представляет собой строку с точечной нотацией, где первый сегмент определяет скрипт редактора, а остальные — группы и отдельные предпочтения внутри него.
