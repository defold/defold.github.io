...
* Подробнее о конвейере рендеринга см. в [документации по Render](/ru/manuals/render).
* Подробное объяснение шейдерных программ см. в [документации по Shader](/ru/manuals/shader).
...
![Compute file](/manuals/images/compute/compute_file.png)
...
![Compute render file](/manuals/images/compute/compute_render_file.png)
...
В массив включены только поддерживаемые возможности; это не таблица, в которой константы возможностей используются как ключи. Всегда выполняйте эту проверку перед использованием compute-шейдеров, если игра может запускаться с разными графическими адаптерами или на устройствах с разным уровнем поддержки драйверов. Поддержка OpenGL и OpenGL ES зависит от версии API и драйвера. Vulkan и Metal через MoltenVK поддерживают compute-шейдеры начиная с версии 1.0. Используйте [манифест приложения](/ru/manuals/app-manifest), чтобы выбрать Vulkan на платформах, где он ещё не является графическим backend по умолчанию.