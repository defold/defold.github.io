...
В этом руководстве объясняется, как создавать интерактивные диалоги и открывать ресурсы в редакторе с помощью скриптов редактора на Lua. Чтобы начать работу со скриптами редактора, см. [руководство по скриптам редактора](/ru/manuals/editor-scripts). Полный справочник по API редактора находится [здесь](/ref/stable/editor-lua/).
...
## Открытие ресурсов {#opening-resources}

Вызовите `editor.ui.open_resource()` из функции `run` команды, чтобы открыть ресурс проекта. Путь начинается с `/`. Если представление не указано, выбирается основное представление ресурса:

```lua
editor.ui.open_resource("/main/main.script")
```

Представления `code` и `text` принимают положение курсора или выделение в качестве третьего аргумента. Номера строк и столбцов начинаются с `1`; если столбец не указан, используется `1`. При передаче этих аргументов указывайте представление:
...
Чтобы выделить диапазон, вместо этого передайте позиции курсора `from` и `to`:
...
Настроенное представление ресурса может открываться в редакторе или во внешнем приложении. Встроенные представления Code и Text поддерживают аргументы положения курсора и выделения. Поддерживаемые имена представлений перечислены в документации [`editor.ui.open_resource()`](/ref/beta/editor/#editor.ui.open_resource:resource_path-view-args).
...
Редактор определяет следующие компоненты отображения данных:
...
- **`image`** — изображение, загруженное по пути к ресурсу проекта, начинающемуся с `/`, или по внешнему URL. Необязательные props `width` и `height` вписывают изображение в заданные размеры с сохранением пропорций.
...
Например, UI может отображать как изображение проекта, так и изображение из интернета:
