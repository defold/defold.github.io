...
- От MonoBehaviour к Lua-модулям
...
Defold ориентирован на производительность даже на слабых устройствах. Он использует небольшую компонентную модель, в которой многие игровые взаимодействия обрабатываются кодом и передачей сообщений.

Defold значительно меньше Unity. Размер движка с пустым проектом составляет от 1 до 3 МБ на всех платформах. Вы можете исключить дополнительные части движка и вынести часть игрового контента в [Live Update](/ru/manuals/live-update), чтобы загружать его отдельно позже. Сравнение размеров и другие причины выбрать Defold описаны на странице [Why Defold](https://defold.com/why/).
...
Если нужны подробности, прочитайте краткое [руководство по установке](/ru/manuals/install/).
...
![Сравнение экранов приветствия](/manuals/images/unity/unity_defold_start.png)
...
![Сравнение Templates](/manuals/images/unity/unity_defold_templates.png)
...
![Hello World Step 1](/manuals/images/unity/helloworld_1.png)
...
![Hello World Step 2](/manuals/images/unity/helloworld_2.png)
4. Щелкните правой кнопкой мыши по созданному game object `go`, выберите `Add Component`, а затем `Label`.
![Hello World Step 3](/manuals/images/unity/helloworld_3.png)
...
![Hello World Step 4](/manuals/images/unity/helloworld_4.png)
...
![Hello World Step 5](/manuals/images/unity/helloworld_5.png)
...
Здесь мы покажем редактор Defold с точки зрения того, что пользователь Unity, вероятно, хочет узнать в первую очередь, но мы настоятельно рекомендуем затем прочитать полное [руководство по обзору редактора](/ru/manuals/editor).
...
![Сравнение редакторов](/manuals/images/unity/defold_unity_editor.png)
...
![Панель инструментов Defold](/manuals/images/unity/defold_2d.png)

Вы также можете настроить `Grid Settings` на панели инструментов, чтобы использовать плоскость `Y`, как в Unity:

![Настройки 3D в Defold](/manuals/images/unity/defold_3d.png)

### Обзор панелей Defold

Редактор Defold разделен на 6 основных панелей.

![Editor 2](/manuals/images/editor/editor_overview.png)
...
| [Debugger](/ru/manuals/debugging/) | Visual Studio Debugger | Отладчик полностью интегрирован в Defold из коробки. Также есть отдельная вкладка для отслеживания, включения и отключения breakpoints. |
...
![Строительные блоки](/manuals/images/unity/blocks.png)

Подробности смотрите в полном руководстве о [строительных блоках Defold](/ru/manuals/building-blocks/).
...
В Unity под "component" обычно понимается `MonoBehaviour`, прикрепленный к `GameObject`. Вы можете создать собственный компонент, унаследовавшись от `MonoBehaviour`, или использовать встроенные компоненты Unity, например Light, физические компоненты и другие.

В Defold Component относится только к тому, что примерно соответствует встроенным компонентам Unity. Defold не считает script аналогом MonoBehaviour и не требует явно "помечать" его для прикрепления к game object, кроме создания обработчиков событий или callback'ов.

Пользовательское игровое поведение обычно не добавляют как множество отдельных script components на один game object. Вместо этого его часто реализуют в Lua-модулях, которые использует один основной `.script`, или в более крупном system script, управляющем многими объектами. Подробнее это описано ниже в разделе "Написание кода".

Подробнее о [компонентах Defold](/ru/manuals/components/).
...
| [Collision Object](/ru/manuals/physics-objects) | Rigidbody + Collider | В Defold physics object и collision shapes объединены в одном компоненте. |
| [Collision Shapes](/ru/manuals/physics-shapes/) | BoxCollider / SphereCollider / CapsuleCollider | В Defold shapes (box, sphere, capsule) настраиваются внутри компонента Collision Object. Оба движка поддерживают collision shapes из tilemaps и convex hull data. |
| [Camera](/ru/manuals/camera/) | Camera | В Unity камера имеет больше встроенных настроек рендеринга и пост-обработки, тогда как в Defold это отдано пользователю через render script. |
| [GUI](/ru/manuals/gui/) | UI Toolkit / Unity UI / uGUI Canvas | GUI в Defold — мощный компонент для построения полноценных UI и templates. В Unity нет одного эквивалентного UI-компонента, там есть несколько UI-фреймворков. Для Defold также есть расширение для [Extension](https://github.com/britzl/extension-imgui). |
| [GUI Script](/ru/manuals/gui-script/) | Unity UI / uGUI scripts | GUI в Defold можно контролировать через GUI scripts, используя отдельный `gui` API. |
| [Model](/ru/manuals/model/) | MeshRenderer + Material | В Defold компонент Model объединяет 3D-модель, texture'ы и material с shaders. |
| [Mesh](/ru/manuals/mesh/) | MeshRenderer / MeshFilter / Procedural Mesh | В Defold Mesh — это компонент для управления набором вершин через код. Он похож на Defold Model, но еще более низкоуровневый. |
| [ParticleFX](/ru/manuals/particlefx/) | Particle System | Редактор частиц Defold поддерживает 2D/3D particle effects со множеством свойств и позволяет анимировать их во времени с помощью curves в Curve Editor. У него нет Trails и Collisions. |
| [Script](/ru/manuals/script/) | Script | Подробнее различия программирования объясняются ниже. |

#### Расширения и пользовательские компоненты

У Defold также есть официальные компоненты [Spine](/extension-spine/) и [Rive](/extension-rive/), доступные через расширения.
...
| Defold | Unity | Differences |
|---|---|---|
| [Atlas](/ru/manuals/atlas/) | Sprite Atlas / Texture2D | В Defold также есть [расширение для Texture Packer](https://defold.com/extension-texturepacker/). |
| [Tile source](/ru/manuals/tilesource/) | Tile Palette + Asset | В Defold tile source может использоваться как texture для tilemap, но также и для sprite'ов или particle effects. |
| [Font](/ru/manuals/font/) | Font | Используется компонентом Label в Defold или text nodes в GUI, аналогично Text/TextMeshPro в Unity. |
| [Material](/ru/manuals/material/) | Material | В Defold shader'ы называются vertex program и fragment program. |
...
Подробнее о Collections читайте в [руководстве по Building Blocks](/ru/manuals/building-blocks/#collections).
...
Эквивалентом скриптов `MonoBehaviour` в Defold является компонент Script, но между ними есть несколько важных различий.
...
В Defold Native Extensions можно писать на нескольких других языках: C, C++, C#, Objective-C, Java или JS в зависимости от целевой платформы. Если вам очень комфортно с C#, технически можно построить большую часть логики игры внутри C#-расширения и вызывать ее из небольшого bootstrap Lua-скрипта, однако это требует хорошего знания API и не рекомендуется новичкам.

Подробнее о расширениях в [руководстве по Native Extensions](/ru/manuals/extensions/).

### От MonoBehaviour к Lua-модулям

В Unity используется открытая модель скриптинга. Поскольку `MonoBehaviour` — основной способ добавлять поведение в редакторе, многие Unity-проекты начинаются с одного управляющего скрипта на каждый важный GameObject: `PlayerController`, `EnemyController`, `BulletController`, `GameManager`, `EnemyManager` и так далее.

Defold более конкретен в своей стандартной архитектуре. У game object может быть `.script`, но обычно не нужно создавать отдельный script для каждого Game Object: один script в Defold может управлять сотнями или тысячами других объектов и их компонентов, даже если у этих объектов вообще нет собственных скриптов, благодаря мощной адресации и передаче сообщений. Создание скриптов по одному на каждый Game Object редко необходимо и может привести к излишней сложности.

Для переиспользуемого игрового поведения разработчики Unity часто приходят к композиции: небольшим скриптам `MonoBehaviour`, таким как `Health.cs`, `Attack.cs` или `EnemyFinder.cs`, прикрепленным к одному GameObject. В Defold обычно оставляют один прикрепленный `.script` как хост или координатор, а переиспользуемую логику помещают в обычные Lua-модули.

В Unity такая композиция может выглядеть так:
...
В Defold те же обязанности часто распределяются между одним прикрепленным script и переиспользуемыми модулями:
...
Прикрепленный `.script` становится хостом или координатором. Lua-модули содержат переиспользуемую логику, подобно тому как небольшие скрипты `MonoBehaviour` в Unity часто отвечают за одну задачу.
...
Важное отличие не в том, что Defold запрещает модульную архитектуру, а в том, где происходит композиция и как игровой код взаимодействует:

| Unity | Defold |
|---|---|
| Прикрепить несколько скриптов `MonoBehaviour` в Inspector | Прикрепить один `.script` и скомпоновать Lua-модули в коде |
| Использовать `GetComponent<T>()` или сериализованные поля для связи поведений | Хранить экземпляры модулей в `self` и использовать адреса/сообщения между объектами |
| У каждого component могут быть собственные lifecycle methods | Host script маршрутизирует `init()`, `update()`, `on_message()`, `final()` и т.д. |
| Возможны разные архитектурные стили | Распространенная практика — messaging-oriented подход и явная композиция в коде |

Сначала это может казаться непривычным, особенно если вы привыкли настраивать поведение, добавляя components в Inspector. В Defold многое из того, что в Unity можно настроить визуально, вместо этого создается, связывается, включается, выключается или обновляется через код. Система сообщений Defold помогает уменьшать связанность логики: отправитель посылает данные на адрес, а получатель сам решает, что с ними делать.

Этот подход рекомендуется, но не навязывается. Вы всё равно можете писать scripts так, как вам удобно, в том числе прикреплять несколько scripts к одному game object или использовать стиль, более близкий к объектно-ориентированному программированию. Для этого даже есть библиотеки, например [defold-oop](https://github.com/xiyoo0812/defold-oop) или [lua-class](https://github.com/d954mas/lua-class).

Для множества однотипных объектов, таких как пули, враги, частицы, тайлы или простые интерактивные элементы, часто лучше управлять ими из system или manager script, а не давать каждому объекту отдельный script. Используйте per-object scripts, когда у объекта есть собственное значимое состояние и поведение. Используйте modules, когда нужна переиспользуемая логика. Используйте system scripts, когда один script может эффективно управлять множеством объектов.
...
- [Script manual](/ru/manuals/script/)
- [Writing code](/ru/manuals/writing-code/)
- [Debugging](/ru/manuals/debugging/)
...
Вы также можете настроить параметры редактора Defold так, чтобы текстовые файлы по умолчанию открывались в VS Code (или любом другом внешнем редакторе). Подробности см. в [Editor Preferences](/ru/manuals/editor-preferences/).
...
Подробнее о shader'ах читайте в [руководстве по Shaders](/ru/manuals/shader).
...
Подробнее о materials читайте в [руководстве по Materials](/ru/manuals/material).
...
Подробнее читайте в [руководстве по передаче сообщений](/ru/manuals/message-passing/).
...
Подробнее читайте в [руководстве по Addressing](/ru/manuals/addressing/).
...
Подробнее — в [руководстве по Factory](/ru/manuals/factory/).
...
Подробнее — в [руководстве по Collection Factory](/ru/manuals/collection-factory/).
...
Подробнее — в [руководстве по управлению ресурсами](/ru/manuals/resource/).
...
Подробнее — в [руководстве по Collection Proxy](/ru/manuals/collection-proxy/).
...
| `on_input` | Input Methods | В Defold ввод поступает, когда [для скрипта установлен input focus](/ru/manuals/input/#input-focus). Обрабатывается первым в update loop. |
| `fixed_update()` | `FixedUpdate()` | Вызывается с фиксированным timestep. Чтобы включить это в Defold, нужно задать `Use Fixed Timestep` — [подробности](https://defold.com/ru/manuals/project-settings/#use-fixed-timestep). Начиная с 1.12.0, выполняется перед `update()`. |
...
Подробнее читайте в [руководстве по жизненному циклу приложения](/ru/manuals/application-lifecycle/).
...
Подробнее — в [руководстве по GUI](/ru/manuals/gui/).
