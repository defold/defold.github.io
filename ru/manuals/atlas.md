...
![Многостраничный атлас](/manuals/images/atlas/multipage_atlas.png)
...
Используйте [`resource.create_texture(path, params)`](https://defold.com/ref/resource/#resource.create_texture:path-table), чтобы создать новый ресурс текстуры:
...
После создания текстуры можно использовать [`resource.set_texture(path, params, buffer)`](https://defold.com/ref/resource/#resource.set_texture:path-table-buffer), чтобы установить пиксели текстуры:
...
Полученную текстуру можно напрямую использовать в [компоненте модели](/ru/manuals/model/) через `go.set()`:
...
Если текстура должна использоваться в [компоненте спрайта](/ru/manuals/sprite/), её необходимо подключить к атласу. Используйте [`resource.create_atlas(path, params)`](https://defold.com/ref/resource/#resource.create_atlas:path-table), чтобы создать атлас:
...
Элементы `frames` представляют собой индексы в таблице `geometries`, нумерация которых начинается с 1. Список может повторно использовать геометрии, менять их порядок или пропускать их — это невозможно выразить с помощью устаревших полей диапазона `frame_start` и `frame_end`. Функция `resource.get_atlas()` возвращает `frames`; используйте такое же представление при передаче данных атласа в `resource.set_atlas()` или `resource.create_atlas()`. Поля диапазона по-прежнему принимаются функциями установки и создания для совместимости, но в новом коде следует использовать `frames`.