...
### Текстовые свойства {#text-properties}

Начиная с Defold 1.13.2, строковое значение по умолчанию определяет текстовое свойство. Текстовые свойства поддерживают UTF-8 и символы перевода строки и редактируются в многострочном поле редактора:
...
Выберите компонент Script в игровом объекте или коллекции, чтобы переопределить его текстовые свойства, как и другие свойства скрипта. Символы NUL внутри строк не допускаются ни в значениях по умолчанию, ни в переопределениях.

Другие скрипты могут читать и записывать текстовое свойство через URL компонента Script. Например, разместите приведённый выше скрипт и метку в игровом объекте `speaker` в коллекции, задав компонентам идентификаторы `script` и `label`. Обновите их из `init()` другого скрипта:
...
Изменение свойства скрипта не обновляет метку автоматически; последняя строка явно копирует новое значение в свойство `text` метки.
...
Пользовательские свойства скрипта также можно читать с помощью `go.get()` и записывать с помощью `go.set()`. Числовые свойства, включая векторы и кватернионы, можно анимировать с помощью `go.animate()`. Текстовые свойства можно читать и записывать, но нельзя анимировать:
...
## Свойства ресурсов {#resource-properties}
