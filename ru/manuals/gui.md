...
: Максимальное количество динамических текстур, отслеживаемых этим GUI-компонентом; по умолчанию — `128`. Сюда входят текстуры, созданные с помощью [`gui.new_texture()`](/ref/gui/#gui.new_texture:texture_id-width-height-type-buffer-flip), и внешние текстуры, назначенные GUI с помощью `go.set(..., "textures", ...)` или `gui.set(msg.url(), "textures", ...)`. В проектах, где заменяется много внешних текстур, может потребоваться увеличить этот предел.
