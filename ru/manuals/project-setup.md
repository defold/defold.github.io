...
- anchor: creating-a-new-project
  title: Создание нового локального проекта
...
## Создание нового локального проекта {#creating-a-new-project}
...
Локальный проект не интегрирован ни с одной системой контроля версий, поэтому файлы хранятся только на жёстком диске, а истории для отмены изменений нет. Файлы, удалённые через панель *Assets* редактора, перемещаются в системную Корзину, если это поддерживается, но могут быть удалены безвозвратно, если такая операция недоступна или завершается ошибкой. Корзина не защищает от произвольных изменений и не хранит историю версий, поэтому рекомендуется использовать систему контроля версий, например Git, для отслеживания изменений в файлах. Кроме того, это облегчает совместную работу над проектом с другими людьми. Загрузить локальный проект на GitHub можно всего за несколько шагов:
