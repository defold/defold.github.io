...
Позволяет управлять функциональностью rig и model, либо выбрать None, чтобы полностью исключить модели и риги. См. [документацию по `Model`](https://defold.com/ru/manuals/model/#model-component).
...
Исключает из движка ресурсы, компоненты и поддержку Lua для GUI. Включайте этот параметр только в том случае, если проект не использует GUI-сцены или GUI-скрипты. Компоненты Label остаются доступными. По умолчанию параметр отключён.

## Exclude Particle FX

Исключает ресурсы и компоненты эффектов частиц, а также модуль Lua `particlefx`. Это также исключает поддержку нод частиц в GUI-сценах; GUI-сцены без нод частиц продолжают поддерживаться. Прежде чем включать этот параметр, удалите ссылки на эффекты частиц и вызовы их API. По умолчанию он отключён.

## Exclude Tilemaps

Исключает ресурсы и компоненты тайловых карт, а также модуль Lua `tilemap`. Включайте этот параметр только в том случае, если проект не использует компоненты Tilemap или их API. Источники тайлов, используемые другими компонентами, остаются доступными. По умолчанию параметр отключён.

## Exclude Live Update

Исключает из движка [функциональность Live Update](/ru/manuals/live-update).
...
Использует устаревшую Android Support Library вместо Android X. [Подробнее](https://defold.com/ru/manuals/android/#using-androidx).
...
Если включено (`true`), включает полную систему раскладки текста с поддержкой формирования глифов, в том числе для языков с письмом справа налево. Включите этот параметр вместе с `font.runtime_generation` в *game.project*, чтобы использовать генерацию SDF-шрифтов во время выполнения из ресурсов TrueType (`.ttf`) или OpenType (`.otf`). Генерация из ресурсов `.otf` во время выполнения поддерживается начиная с Defold 1.13.2. Подробнее см. в [руководстве по шрифтам](/ru/manuals/font/#enabling-runtime-fonts).

## Use Rich Text

Включает разбор форматированной текстовой разметки и стилевые эффекты для меток и GUI-текста. По умолчанию этот параметр включён. Отключите его, чтобы уменьшить размер движка, если проекту нужен только обычный текст. Метки и GUI-текст продолжат поддерживаться, но разметка будет отображаться как обычный текст без применения форматирования или эффектов.
