...
: Динамические объекты симулируются физическим движком. Движок определяет все столкновения и применяет результирующие силы. Динамические объекты хорошо подходят для объектов, которые должны вести себя реалистично. Чаще всего на них воздействуют косвенно: [прикладывая силы](/ref/physics/#apply_force) или изменяя угловое [затухание](/ref/stable/physics/#angular_damping) и [скорость](/ref/stable/physics/#linear_velocity), а также линейное [затухание](/ref/stable/physics/#linear_damping) и угловую [скорость](/ref/stable/physics/#angular_velocity). Также можно напрямую изменять позицию и ориентацию динамического объекта, если в *game.project* включена настройка [Allow Dynamic Transforms](/ru/manuals/project-settings/#allow-dynamic-transforms).

Кинематические объекты
: Кинематические объекты только регистрируют столкновения с другими физическими объектами, но физический движок не производит никаких автоматических симуляций. Решение о разрешении столкновений, либо их игнорировании, остается за вами ([узнайте больше](/ru/manuals/physics-resolving-collisions)). Кинематические объекты весьма хороши для игрового персонажа или для объектов, управляемых скриптами, которые требуют тонкого контроля над физическими реакциями, как например игровой персонаж.

Триггеры
: Триггеры --- это объекты, которые регистрируют простые столкновения. Триггеры --- это облегченные версии объектов столкновения. Они похожи на [рейкасты](/ru/manuals/physics-ray-casts) в том смысле, что считывают состояние физического мира, а не взаимодействуют с ним. Они хорошо подходят для объектов, которым нужно только зарегистрировать попадание (например, пуле), а также для игровой логики, где нужно запускать особые действия, когда объект достигает определенной точки. Триггеры вычислительно дешевле, чем кинематические объекты, и по возможности их стоит предпочитать.
...
2. Сделайте <kbd>клик ПКМ</kbd> на новом компоненте и выберите <kbd>Add Shape</kbd>, затем выберите форму: <kbd>Box</kbd>, <kbd>Capsule</kbd>, <kbd>Sphere</kbd>, <kbd>Hull</kbd> или <kbd>Mesh</kbd> в проектах с 3D-физикой, <kbd>Box</kbd> или <kbd>Circle</kbd> в проектах с 2D-физикой. Формы Hull и Mesh доступны начиная с Defold 1.13.2 и используют именованную сетку из сцены glTF или GLB. В компонент можно добавить несколько форм. Также можно использовать тайловую карту или ресурс `.convexshape` через свойство *Collision Shape*.
...
Компонент столкновения может содержать несколько встроенных форм, включая выпуклые оболочки и треугольные сетки в 3D-физике, либо использовать ресурс тайловой карты или выпуклой формы. За подробностями о различных формах и о том, как их добавлять к объекту столкновения, обращайтесь к [руководству по формам столкновений](/ru/manuals/physics-shapes).
...
Collision Shape
: Ресурс тайловой карты или `.convexshape`. Чтобы использовать сетку glTF или GLB, добавьте в компонент форму Hull или Mesh и задайте её свойства *Scene* и *Mesh*. За подробностями обращайтесь к [руководству по формам столкновения](/ru/manuals/physics-shapes).

Type
: Тип объекта столкновений: `Dynamic`, `Kinematic`, `Static` или `Trigger`. Если объект выставлен как `Dynamic`, _необходимо_ установить свойство *Mass* в ненулевое значение. Для объектов `Dynamic` или `Static` также требуется проверить, что значения *Friction* и *Restitution* также выставлены в приемлемые значения под используемый сценарий.
