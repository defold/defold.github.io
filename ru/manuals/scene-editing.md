...
- anchor: manipulating-objects
  title: Выделение и манипулирование объектами
- Выделение объектов
- Фильтры видимости
- Настройки сетки
...
Исходный вид камеры зависит от ресурса. 3D-ресурсы, такие как модели и сцены glTF, по умолчанию используют **перспективную** проекцию, а 2D-ресурсы, такие как спрайты, тайловые карты и GUI-сцены, — **ортографическую**. Ориентацию камеры, проекцию и сетку можно изменить через панель инструментов сцены.
...
## Сохранение видов сцены {#remembered-scene-views}

Редактор запоминает состояние камеры для каждого ресурса сцены при закрытии его вкладки или выходе из редактора. При повторном открытии того же ресурса его вид восстанавливается, поэтому разные коллекции или модели могут сохранять разные положения, ориентации и проекции камеры.

Фильтры видимости также запоминаются отдельно для каждой сцены. Если скрыть модели или направляющие компонентов в одной сцене, в другой можно использовать другие фильтры. Это настройки вида в редакторе; они не изменяют камеру игры или видимость во время выполнения.

Для ресурсов без сохранённого состояния камеры модели, сетки и ресурсы glTF открываются в перспективной проекции. Вид объектов столкновения выбирается на основе настройки 2D/3D-физики проекта; для коллекций и игровых объектов исходный вид определяется по геометрии сцены.
...
## Выделение и манипулирование объектами {#manipulating-objects}
...
Редактор хранит отдельные настройки сетки для 2D- и 3D-видов. Задавайте размер, плоскость и внешний вид, пока нужный режим активен; при переключении восстанавливаются настройки сетки соответствующего режима. **Reset to Defaults** сбрасывает настройки активного режима.
...
- Кнопка **Reset to Defaults**
  Возвращает настройки сетки активного режима к исходным значениям.
