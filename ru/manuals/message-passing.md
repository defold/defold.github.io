...
Содержимое этого примера находится в двух отдельных файлах. Один файл относится к основной загрузочной коллекции, а другой - к коллекции с идентификатором "level". Однако в Defold имена файлов _не имеют значения_. Значение имеет идентификатор, который вы присваиваете экземплярам.
</div>
...
Как мы ранее убедились, механизм отправки сообщения очень прост. Нужно вызвать функцию `msg.post()`, которая помещает сообщение в очередь сообщений. Затем, в каждом кадре, движок просматривает очередь и доставляет каждое сообщение по целевому адресу. Некоторые системные сообщения (такие как `"enable"`, `"disable"`, `"set_parent"` и др.) обрабатываются кодом движка. Движок также выдает некоторые системные сообщения (например, `"collision_response"` при физических столкновениях), которые доставляются вашим объектам. Для пользовательских сообщений, посылаемых компонентам скриптов, движок просто вызывает специальную Lua-функцию Defold с именем `on_message()`.
...
Когда отправленное сообщение в конечном итоге доставляется, вызывается функция `on_message()` получателя. Довольно часто код реакции отправляет новые сообщения, которые добавляются в очередь сообщений.
...
function init(self)
    ...
    -- и поддерживаем её в течение нескольких шагов update().
    print("INIT")
    msg.post("#", "msg")
    self.updates = 0
    self.count = 0
