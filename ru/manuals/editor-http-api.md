...
Установленные версии Defold можно найти через `installations.json`, как описано в [руководстве по редактору](/ru/manuals/editor/#editor-installation-metadata). Поле `launcherPath` содержит запускаемый исполняемый файл. Чтобы непосредственно открыть проект, передайте путь к `game.project` как первый позиционный аргумент.
...
Редактор — графическое настольное приложение. Запускайте его в интерактивном пользовательском сеансе с доступом к дисплею. Если графический сеанс недоступен, например в headless-CI, либо для создания автономных бандлов, используйте [Bob](/ru/manuals/bob). Открытый редактор также поддерживает автоматизацию только компиляции через `/command/compile`.
...
![Домашняя страница локального сервера редактора](/manuals/images/automation/editor_server.png)
...
Список документированных путей команд редактора:
...
В Defold 1.13.2 и более поздних версиях каждая команда имеет собственный путь в документе OpenAPI. В предыдущих версиях команды описываются через путь `/command/{command}` и перечисление имён команд.
...
Чтобы вызвать команду редактора, отправьте запрос `POST` на её документированный путь, например:
...
Чтобы скомпилировать проект без запуска:
...
Эти конвейеры команд отображают тело ответа. В скриптах автоматизации также проверяйте HTTP-статус и `success`, используя подход из раздела [Сборка HTML5](#building-html5).

<div class='sidenote' markdown='1'>
Начиная с Defold 1.13.2, `/command/build` — устаревший псевдоним `/command/run`, сохранённый для совместимости и не включённый в OpenAPI. В новых интеграциях используйте `/command/run`.
</div>

Успешная компиляция возвращает HTTP-статус `200` со структурированным результатом:
...
Ответ зависит от команды. В Defold 1.13.2 и более поздних версиях команды `compile`, `run`, `clean-build`, `build-html5`, `debugger-start` и `hot-reload` ожидают завершения операции и возвращают структурированный результат с `success` и `issues`, как показано выше. При успехе возвращается HTTP `200`, при ошибке сборки или проверки — `422`.

Другие команды, например `debugger-break`, по-прежнему могут возвращать `202`. Изучайте операцию в актуальной схеме OpenAPI и обрабатывайте фактический HTTP-статус ответа:
...
Если в актуальном документе OpenAPI указан путь `/command/build-html5`, вызовите команду по этому пути. В скрипте оболочки сохраняйте HTTP-статус отдельно от тела ответа и прекращайте выполнение при неудачном запросе или сборке:
...
В Defold 1.13.2 и более поздних версиях этот запрос ожидает завершения сборки и возвращает структурированный результат. Пример выводит тело ответа, включая проблемы сборки, и продолжает работу только при HTTP `200` и `success: true`. После успешной сборки редактор открывает игру в браузере и предоставляет её по адресу:

```text
http://127.0.0.1:<editor-port>/html5/
```

Завершение сборки не означает, что игра уже загрузилась в браузере. Дождитесь готовности canvas и приложения перед отправкой ввода или проверкой игрового процесса. Подробнее см. в разделе [Браузерные тесты для HTML5](/ru/manuals/automated-testing/#browser-tests-for-html5).
...
О формате результатов тестов и классификации ошибок см. в разделе [Автоматизированное тестирование и проверка](/ru/manuals/automated-testing/#structured-test-results).
...
![Отрисованный редактором предпросмотр основной коллекции](/manuals/images/automation/main-preview.png)
...
![Отрисованный редактором предпросмотр модели куба](/manuals/images/automation/cube-preview.png)
...
<div class='important' markdown='1'>
Предпросмотр редактора — не снимок экрана запущенной игры. Он не проверяет динамически созданные объекты, постобработку среды выполнения или платформенный рендеринг. Когда эти элементы необходимы, используйте [снимок экрана среды выполнения](/ru/manuals/automated-testing/#editor-previews-and-runtime-screenshots).
</div>
...
Вычисляемый код может использовать [Editor API](https://defold.com/ref/editor-lua/) и окружение скриптов редактора. Он не может использовать API среды выполнения игры, например `go.*`, для управления запущенной игрой. Для игрового процесса используйте тест среды выполнения, отладчик, браузерный тест или [API автоматизации среды выполнения](/ru/manuals/engine-service/#automation-bridge-extension).
...
Скрипты редактора могут определять дополнительные маршруты с помощью [`get_http_server_routes()`](/ru/manuals/editor-scripts/#http-server). Необязательная таблица операции OpenAPI предоставляет маршрут через тот же документ `/openapi.json`, что и встроенные операции.
...
Сервер редактора принадлежит процессу редактора. У запущенной игры другой порт и другие обязанности, описанные в [руководстве по сервису движка и HTTP API среды выполнения](/ru/manuals/engine-service).