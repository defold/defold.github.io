...
Каждая строка должна начинаться с `/` и сопоставляется с путями проекта относительно его корня. Шаблон соответствует пути, если он равен этому пути или является одной из его родительских папок. Сопоставление чувствительно к регистру.

### Подстановочные знаки

Шаблоны могут содержать подстановочные знаки:

* `*` соответствует любому количеству символов, кроме `/`
* `?` соответствует ровно одному символу, кроме `/`
* `**` соответствует любому количеству целых папок, поэтому `/**/name` соответствует `name` на любой глубине, а `/folder/**` соответствует самой папке и всему ее содержимому

Все остальные символы сопоставляются буквально. Пример:
...
Это исключит папку `tiled` в каждой непосредственной подпапке `/levels` (например, `/levels/01/tiled`), все папки с именем `generated` на любой глубине, включая `/generated` в корне проекта, а также файлы вроде `/assets/temp_01.png`.
