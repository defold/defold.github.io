...
`tint`
: Цветовой оттенок спрайта (`vector4`). Тип `vector4` используется для представления оттенка, где `x`, `y`, `z` и `w` соответствуют красному, зеленому, синему и альфа-каналу.
...
## Многотекстурные спрайты {#multi-textured-sprites}
