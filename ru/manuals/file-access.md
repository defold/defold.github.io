...
- anchor: how-to-access-files-bundled-with-the-application
  title: Как получить доступ к файлам, включённым в приложение
...
* Вы можете использовать стандартные функции [`io.*`](https://defold.com/ref/io/) для чтения и записи файлов. Эти функции дают вам очень тонкий контроль над процессом ввода-вывода.
...
* Вы можете использовать [`os.rename()`](https://defold.com/ref/os/#os.rename:oldname-newname) и [`os.remove()`](https://defold.com/ref/os/#os.remove:filename) для переименования и удаления файлов.

* Вы можете использовать [`sys.save()`](https://defold.com/ref/sys/#sys.save:filename-table) и [`sys.load()`](https://defold.com/ref/sys/#sys.load:filename) для чтения и записи Lua таблиц. Дополнительные функции [`sys.*`](https://defold.com/ref/sys/) помогают с платформонезависимым разрешением путей к файлам.
...
Расширения также могут добавлять эти файлы через `ext.properties`. Их пути объединяются с пользовательскими ресурсами проекта как в сборках редактора, так и в архивах Bob. См. раздел [Пользовательские ресурсы расширений](/ru/manuals/extensions/#custom-resources).
