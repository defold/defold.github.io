...
Выберите свою code signing identity, укажите mobile provisioning file и вариант сборки (Debug или Release). При необходимости можно снять флажок `Sign application`, чтобы пропустить процесс подписи и выполнить его вручную позже. Установите флажок `Simulator`, чтобы вместо сборки для устройства создать сборку `arm64_sim-ios` для симулятора iOS.

<div class='important' markdown='1'>
Сборки для симулятора работают только в симуляторе iOS на Mac с Apple Silicon. Они всегда подписываются ad-hoc, поэтому при установленном флажке `Simulator` параметры подписи, установки и запуска недоступны. Установите сборку с помощью `xcrun simctl`, как описано ниже.
</div>
...
Иконку приложения, launch screen storyboard и другие параметры вы задаете в файле настроек проекта *game.project* в [разделе iOS](/ru/manuals/project-settings/#ios).
...
{% include shared/ru/apple-privacy-manifest.md %}
...
Если вы считаете, что ваш проект освобожден от этих требований, установите ключ [`ITSAppUsesNonExemptEncryption`](https://developer.apple.com/documentation/bundleresources/information-property-list/itsappusesnonexemptencryption) в `False` в `Info.plist` проекта. Подробнее см. в [Application Manifests](/ru/manuals/extensions-manifest-merge-tool).

## FAQ
{% include shared/ru/ios-faq.md %}