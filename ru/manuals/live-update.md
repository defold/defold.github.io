...
Сейчас Defold может хранить ресурсы тремя способами. Выберите способ в выпадающем списке *Mode* в окне настроек:

`Zip`
: Эта опция указывает Defold создать Zip-архив со всеми исключенными ресурсами. Архив сохраняется в месте, указанном в *Export path*, и может быть смонтирован во время выполнения с помощью URI `zip:` и `liveupdate.add_mount()`.

`Folder`
: Эта опция указывает Defold создать папку со всеми исключенными ресурсами. Она удобна для обработки файлов перед загрузкой или упаковкой. Папку с отдельными скомпилированными файлами, расположенными по ожидаемым путям ресурсов, можно смонтировать во время выполнения с помощью URI `file:`.

`Amazon`
: Эта опция указывает Defold автоматически загружать исключенные ресурсы в Amazon Web Service (AWS) S3 bucket. Укажите имя профиля AWS в *Credential profile*, выберите подходящий *Bucket* и задайте имя *Prefix*. Подробнее о настройке учетной записи AWS можно прочитать в этом [руководстве по AWS](/ru/manuals/live-update-aws).
...
При публикации контента Live Update Defold автоматически удаляет записи, относящиеся только к Live Update, из включённого в бандл `game.dmanifest`, тогда как опубликованный `liveupdate.game.dmanifest` сохраняет полный список ресурсов. Это уменьшает размер бандла и потребление памяти во время выполнения. Прежняя настройка `liveupdate.exclude_entries_from_main_manifest` удалена; оставшаяся в проекте запись игнорируется.

В архивном сценарии `collectionproxy.get_resources()` возвращает `{}`, пока соответствующий архив не смонтирован. После монтирования функция возвращает хеши ресурсов для этого proxy.
...
Рекомендуем хранить метаданные для каждого mount, чтобы приложение могло решить, оставлять ли пакет смонтированным. Проверяйте их после добавления mount, в том числе когда приложение снова добавляет нужные mounts при запуске.
Можно добавить в Zip файл `metadata.json` и прочитать его через `sys.load_resource("/metadata.json")` после монтирования. _Используйте уникальный путь ресурса для данных каждого mount, иначе будет возвращён файл из mount с наивысшим приоритетом._
...
Mounts активны только в текущей сессии движка. После перезапуска приложение должно снова вызвать `liveupdate.add_mount()` для каждого нужного пакета. Если выбор должен сохраняться между сессиями, сохраните местоположение, имя и приоритет пакета в собственных постоянных данных приложения.

<div class='sidenote' markdown='1'>
Монтирование Zip-архива или папки не копирует и не перемещает содержимое. Оно должно оставаться по указанному пути всё время, пока используется mount.
</div>

## Скриптование с Live Update

Чтобы действительно использовать контент Live Update, нужно скачать и смонтировать данные в игру.
Подробнее о том, как [использовать Live Update в скриптах, читайте здесь](/ru/manuals/live-update-scripting).
...
: Разработчик может скачивать контент в любой файл или папку, обычно внутри директории приложения. Путь к папке поддержки можно узнать с помощью `print(sys.get_save_file("", ""))`. Чтобы принудительно скачать пакет заново, удалите его и соответствующую запись из состояния, которым управляет приложение. Списка mount'ов, управляемого движком, больше нет; mounts не сохраняются после перезапуска.
