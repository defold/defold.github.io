...
- anchor: physics-updates
  title: Обновления физики
- Предостережения и типовые проблемы
---

# Физика

Defold включает [Box2D](https://box2d.org/) для симуляции 2D-физики и Bullet для 3D-физики. Настройка [Physics 2D в манифесте приложения](/ru/manuals/app-manifest/#physics-2d) позволяет выбрать **Box2D Version 3**, **Box2D (Legacy Defold version)** или **None**. По умолчанию используется прежняя реализация, а Box2D 3 включается явно. Смена реализации может изменить результаты симуляции и потребовать перенастройки зависящих от версии [параметров проекта Box2D](/ru/manuals/project-settings/#box2d).

Описанные в руководствах компонентный подход к объектам столкновений и модуль `physics` работают с любой реализацией Box2D; выбор **None** исключает 2D-физику. Defold также предоставляет низкоуровневые API [`b2d`](/ref/stable/b2d/), `b2d.body`, `b2d.fixture`, `b2d.shape`, `b2d.joint`, `b2d.chain` и `b2d.world` для прямой работы с 2D-телами, формами, сочленениями, цепями и мирами. Не все низкоуровневые функции доступны в обеих реализациях Box2D; сверяйте сгенерированную документацию каждой функции API с реализацией, выбранной в манифесте приложения.
...
: Если вы испытываете проблемы с тем, что столкновения не обрабатываются или не детектируются, то убедитесь, что вам знаком весь материал из секции по [отладке физики в руководстве по отладке](/ru/manuals/debugging-game-logic/#debugging-problems-with-physics).