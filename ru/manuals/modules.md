...
В таком случае любой скрипт может потребовать этот файл через `require` и использовать функцию:
...
Во-вторых, Lua кэширует файлы, загруженные через `require`. Когда файл требуется в первый раз, он помещается в таблицу [`package.loaded`](/ref/package/#package.loaded), чтобы его можно было быстрее прочитать при последующих вызовах `require`. Можно заставить файл быть повторно считанным с диска, установив запись файла в значение `nil`: `package.loaded["my_module"] = nil`.
