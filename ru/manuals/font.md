...
Начиная с Defold 1.13.2, и прежняя, и полная системы раскладки текста поддерживают контуры TrueType и OpenType CFF1/CFF2, включая генерацию во время выполнения из ресурсов `.ttf` и `.otf`.

О стилизации отдельных фрагментов текста и работе со ссылками и встроенными спрайтами см. в [руководстве по форматированной текстовой разметке](/ru/manuals/font-richtext).
...
При использовании runtime-шрифтов файлы `.ttf` и `.otf` включаются в бандл как есть, а растеризация выполняется по требованию во время выполнения. Это уменьшает как потребление памяти во время выполнения, так и размер бандла.
...
См. раздел [Включение runtime-шрифтов](/ru/manuals/font#enabling-runtime-fonts)

Редактор использует систему рендеринга шрифтов движка для предпросмотра шрифтов и текста в сценах. Формирование глифов и раскладка справа налево требуют [runtime-шрифтов](#enabling-runtime-fonts) и параметра **Use full text layout system** в App Manifest. Для офлайн-шрифтов предпросмотр учитывает настройки шрифта **Characters** и **All Chars**.

## Коллекция шрифтов

Формат файла `.fontc` также называется коллекцией шрифтов. В офлайн-режиме с ним связан только один шрифт.
При использовании runtime-шрифтов с коллекцией шрифтов можно связать более одного файла шрифта (`.ttf` или `.otf`).
...
Возможно использовать генерацию шрифтов типа SDF (Distance Field) во время выполнения при использовании шрифтов TrueType (`.ttf`) или OpenType (`.otf`). Генерация из ресурсов `.otf` во время выполнения поддерживается начиная с Defold 1.13.2.
...
* Добавьте [App Manifest](/ru/manuals/app-manifest) и включите опцию `Use full text layout system`.
...
Параметр `font.runtime_generation` влияет на все шрифты `.ttf` и `.otf` в проекте.
</div>
