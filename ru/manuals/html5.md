...
- anchor: webgl-context-version
  title: Версия контекста WebGL
- anchor: download-verification
...
HTML5-бандлы поддерживают две архитектуры WebAssembly:

* `wasm-web` — обычный однопоточный движок WebAssembly.
* `wasm_pthread-web` — движок WebAssembly, способный использовать потоки.

Можно включить одну из архитектур или обе. Если включены обе, загрузчик выбирает `wasm_pthread-web`, когда браузер и среда хостинга её поддерживают, и в противном случае переходит на `wasm-web`. Канонические имена целей приведены в [руководстве по Bob](/ru/manuals/bob/#usage).

<div class='important' markdown='1'>
Многопоточному движку требуется `SharedArrayBuffer` на защищённой странице с [изоляцией между источниками](https://developer.mozilla.org/en-US/docs/Web/API/Window/crossOriginIsolated). Размещайте бандл по HTTPS (или на localhost) и настройте на сервере совместимые заголовки изоляции между источниками, обычно:
...
Ресурсы из других источников, загружаемые страницей, также должны использовать совместимые заголовки CORS или Cross-Origin-Resource-Policy. Бандл, содержащий только `wasm_pthread-web`, не запустится, если эти требования не выполнены; включите `wasm-web` как резервный вариант, если игра может размещаться на сайте без поддержки изоляции между источниками.
</div>

HTML5-бандлам Defold требуется современный браузер с поддержкой WebAssembly. Internet Explorer 11 не поддерживается.
...
## Версия контекста WebGL {#webgl-context-version}

Выберите запрашиваемый графический контекст с помощью [`graphics.webgl_version_hint`](/ru/manuals/project-settings/#webgl-version-hint). По умолчанию используется WebGL 2; запросите WebGL 1, чтобы тестировать или использовать этот контекст в браузерах, поддерживающих обе версии.

## Проверка загрузок {#download-verification}

По умолчанию загрузчик HTML5 проверяет размеры загруженных файлов движка и архива. При неудачной проверке загрузка повторяется, прежде чем загрузчик сообщит об ошибке:

* При сетевых ошибках, неуспешных HTTP-статусах и несовпадении размера загружаемого файла JavaScript или WebAssembly движка используется предел повторных попыток `html5.retry_count`.
* Проверка файлов архива имеет собственный предел повторных попыток при несовпадении размера или SHA-1. При каждой повторной проверке части файла загружаются заново, причём для каждой загрузки доступны обычные повторные попытки при сетевых ошибках.

Параметр `html5.retry_time` задаёт задержку между повторными попытками в обоих случаях.

Если ваш сервер, прокси или CDN намеренно изменяет отдаваемые файлы и их размеры, отключите проверку размера в *game.project*:
...
Отключение **Verify Downloaded File Size** не отключает проверку SHA-1, если она включена в бандл. См. [настройки проекта для HTML5](/ru/manuals/project-settings/#verify-downloaded-file-size).
...
* Поддержка геймпадов - [См. документацию по геймпадам](/ru/manuals/input-gamepads/#gamepads-in-html5), где описаны особые моменты и действия, которые могут понадобиться на HTML5.
...
Подробнее о каждой опции см. в [руководстве по настройкам проекта](/ru/manuals/project-settings/#html5).
...
Откройте *game.project* как текст и посмотрите на `[section_name]` и имя нужного поля. Затем можно использовать токен вида `{% raw %}{{section_name.field}}{% endraw %}` или `{% raw %}{{{section_name.field}}}{% endraw %}`.
...
: Вычисленный префикс пути к архиву, используемый загрузчиком, на основе `html5.archive_location_prefix`.

DEFOLD_ARCHIVE_LOCATION_SUFFIX
: Вычисленный суффикс, добавляемый к URL архива, на основе `html5.archive_location_suffix`.

DEFOLD_HAS_ARCHIVE_ORIGIN
: `true`, если префикс архива задаёт источник HTTP или HTTPS, в том числе URL без указания протокола, например `//cdn.example.com/archive`. Равен `false` для относительных префиксов архива. Доступен начиная с Defold 1.13.2.

DEFOLD_ARCHIVE_ORIGIN
: Источник архива, включая схему, хост и необязательный порт, либо пустая строка, если источник не задан. Префикс без указания протокола даёт источник без указания протокола. Используется для подсказки preconnect и доступен начиная с Defold 1.13.2.

DEFOLD_HAS_WASM_ENGINE
: `true`, если бандл включает движок WebAssembly: `wasm-web` или `wasm_pthread-web`.

DEFOLD_HAS_WASM_PTHREAD_ENGINE
: `true`, если бандл включает `wasm_pthread-web`. Используйте его, чтобы избежать предварительной загрузки неверного варианта движка, когда загрузчик выбирает архитектуру во время выполнения.
...
<div class='important' markdown='1'>
Важно, чтобы этот встроенный блок находился перед загрузкой основного скрипта приложения. Поскольку он включает HTML-теги, этот макрос должен использоваться в тройных фигурных скобках `{% raw %}{{{TOKEN}}}{% endraw %}`, чтобы последовательности символов не экранировались.
</div>
...
При создании собственного шаблона параметры загрузчика движка можно изменить, присвоив значения глобальному объекту `CUSTOM_PARAMETERS`. Встроенный шаблон предоставляет для таких настроек намеренно пустой блок `<script id="engine-setup">`.
<div class='important' markdown='1'>
Блок `engine-setup` должен находиться после скрипта, загружающего `dmloader.js`, но перед блоком `engine-start`, который вызывает `EngineLoader.load()`.
</div>
Например:
...
`CUSTOM_PARAMETERS` может содержать, в частности, следующие поля:
...
Можно указать дополнительные аргументы при настройке и загрузке движка. Во время выполнения их можно получить через `sys.get_config_string()`. Присвойте аргументы непосредственно `CUSTOM_PARAMETERS.engine_arguments` в блоке `engine-setup` файла `index.html`:
...
Присваивание нового массива заменяет все аргументы движка, настроенные в *game.project*. Чтобы сохранить эти аргументы и добавить ещё один, используйте `CUSTOM_PARAMETERS.engine_arguments.push("--config=example.foo3=bar3")`.

Также можно добавить `--config=example.foo1=bar1, --config=example.foo2=bar2` в поле *Engine Arguments* в HTML5-разделе *game.project*. Разделённые запятыми значения добавляются в `CUSTOM_PARAMETERS.engine_arguments` в сгенерированном `dmloader.js`.
...
* [Использование памяти](/ru/manuals/optimization-memory)
* [Размер движка](/ru/manuals/optimization-size)
* [Размер игры](/ru/manuals/optimization-size)

## FAQ
{% include shared/ru/html5-faq.md %}