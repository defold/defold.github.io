...
Можно указать дополнительные аргументы движка во время его настройки и загрузки. Эти дополнительные аргументы потом можно получить во время выполнения через `sys.get_config_string()`. Чтобы добавить пары ключ-значение, нужно изменить поле `engine_arguments` объекта `extra_params`, который передается движку при загрузке в `index.html`:
...
Также можно добавить `--config=foo1=bar1, --config=foo2=bar2` в поле *Engine Arguments* в HTML5-разделе *game.project*, и оно будет внедрено в сгенерированный `index.html`.
