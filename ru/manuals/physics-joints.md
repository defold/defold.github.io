...
Сочленение соединяет два объекта столкновения с помощью ограничения. Defold поддерживает сочленения как в 2D-, так и в 3D-физике, используя разные API.

Это руководство описывает 2D-сочленения, предоставляемые модулем `physics`. Начиная с Defold 1.13.2, в 3D-проектах можно создавать ограничения и управлять ими через [`bullet3d.constraint`](/ref/beta/bullet3d.constraint/), включая шарниры, слайдеры и пружины. Используйте этот API с твёрдыми телами, полученными через [`bullet3d.get_rigid_body()`](/ref/beta/bullet3d/#bullet3d.get_rigid_body).

В 2D API `physics` поддерживаются следующие типы сочленений:
...
Описанные здесь 2D-сочленения создаются программно вызовом [`physics.create_joint()`](/ref/physics/#physics.create_joint:joint_type-collisionobject_a-joint_id-position_a-collisionobject_b-position_b-[properties]):
