...
Звуковые компоненты могут быть инстанцированы только на месте в игровом объекте. Создайте новый игровой объект, кликните на нем ПКМ, выберите <kbd>Add Component ▸ Sound</kbd> и нажмите *OK*.
...
*Sound*
: Звуковой файл проекта в формате _Wave_, _Ogg Vorbis_ или _Ogg Opus_. Defold поддерживает Wave-файлы с 8- и 16-битным PCM. Воспроизведение Ogg Opus необязательно и требует включить **Include Sound Decoder: Opus** в [манифесте приложения](/ru/manuals/app-manifest/#sound); по умолчанию декодер Opus не включён.
...
- Усиление, установленное в группе "master". Может быть изменено с помощью `sound.set_group_gain(hash("master"), gain)`.

Когда в [настройках проекта Sound](/ru/manuals/project-settings/#sound) включён параметр **Use Linear Gain** (по умолчанию), выходное усиление равно произведению этих четырёх усилений. Усиление `1.0` соответствует единичному усилению (0 dB). Если линейное усиление отключено, Defold применяет при микшировании нелинейную кривую, поэтому прямое перемножение четырёх уровней и приведённое ниже преобразование в децибелы не описывают итоговый уровень сигнала.
...
Когда параметр **Use Linear Gain** включён, положительное значение усиления преобразуется в децибелы по стандартной формуле:
...
В Defold 1.10.2 было исправлено давнее ослабление примерно на 3 dB в звуковом микшере. Если микс старого проекта компенсировал это ослабление и после обновления стал звучать громче, перенастройте общий параметр **Sound ▸ Gain** или усиление соответствующих групп.
...
Также возможно использование [потокового воспроизведения звуков](/ru/manuals/sound-streaming).