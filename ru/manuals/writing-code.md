...
Встроенный языковой сервер Lua содержит аннотации типов для API Defold. Автодополнение, информация при наведении и диагностика распознают типы Defold, такие как хеши, URL, векторы и кватернионы, а также аргументы функций и возвращаемые значения. Редактор предоставляет аннотации для игровых скриптов и API `editor.*`, используемых в файлах `.editor_script`. При работе во встроенном редакторе кода Defold отдельная библиотека аннотаций для встроенных API не требуется.

Для API сторонних расширений могут потребоваться собственные аннотации.

### Форматирование кода {#formatting-code}

Выберите <kbd>Edit ▸ Format Document/Selection</kbd> или нажмите <kbd>Alt</kbd> + <kbd>Shift</kbd> + <kbd>F</kbd>, чтобы запустить форматтер языкового сервера. При наличии выделения редактор форматирует выделенные строки, в противном случае — весь документ. Для форматирования требуется языковой сервер, поддерживающий соответствующую операцию.

Чтобы форматировать изменённые открытые файлы при сохранении, включите **Format on save** в <kbd>Preferences ▸ Code</kbd>. По умолчанию этот параметр отключён; для него требуется языковой сервер с поддержкой форматирования документа. См. [настройки кода](/ru/manuals/editor-preferences/#code).

### Переход к символу {#jump-to-symbol}

Встроенный редактор кода может отображать список символов в текущем файле кода с возможностью поиска, например функций, объектов и переменных. Выберите <kbd>View ▸ Jump to Symbol…</kbd> или нажмите <kbd>Ctrl</kbd> + <kbd>Shift</kbd> + <kbd>O</kbd> в Windows и Linux либо <kbd>⌘ Cmd</kbd> + <kbd>Shift</kbd> + <kbd>O</kbd> в macOS.

Начните вводить текст для нечёткого поиска по символам. Используйте клавиши со стрелками, чтобы перемещаться по результатам и предварительно просматривать расположение символов в редакторе, затем нажмите <kbd>Enter</kbd>, чтобы перейти к выбранному символу. Нажмите <kbd>Esc</kbd>, чтобы закрыть диалоговое окно и вернуться к прежнему положению курсора и прокрутки.
