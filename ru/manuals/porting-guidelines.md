...
## Ввод
Обязательно адаптируйте игру под методы ввода конкретной платформы. Подумайте о добавлении поддержки [геймпадов](/ru/manuals/input-gamepads), если платформа их поддерживает. Также убедитесь, что в игре есть меню паузы: если контроллер внезапно отключится, игра должна быть поставлена на паузу.
...
Только для iOS: обязательно укажите [Localizations](/ru/manuals/project-settings/#localizations) в `game.project`, поскольку `sys.get_info()` никогда не вернёт язык, которого нет в этом списке.
</div>
...
Обязательно [генерируйте отладочные символы](/ru/manuals/debugging-native-code/#symbolicate-a-callstack) для каждой выпущенной версии, чтобы иметь возможность разбирать падения приложения. Храните их вместе со сборкой приложения.

## Оптимизация приложения

Прочитайте [руководство по оптимизации](/ru/manuals/optimization), чтобы узнать, как оптимизировать приложение по производительности, размеру, потреблению памяти и батареи.



## Производительность
Всегда тестируйте на целевом железе. Проверьте производительность игры и при необходимости оптимизируйте её. Используйте [профилировщик](/ru/manuals/profiling), чтобы находить узкие места в коде.


## Разрешение экрана и частота обновления
Для платформ с фиксированной ориентацией и разрешением экрана: убедитесь, что игра корректно работает при целевом разрешении и соотношении сторон. Для платформ с переменным разрешением и соотношением сторон: убедитесь, что игра корректно работает при разных разрешениях и соотношениях сторон. Учитывайте, какой [view projection](/ru/manuals/render/#default-view-projection) используется в рендер-скрипте и камере.
...
## Мобильные телефоны с вырезом и камерами в отверстии экрана
Использование небольших вырезов в экране для фронтальной камеры и датчиков (notch или hole punch) стало очень распространённым. При переносе игры на мобильные платформы размещайте критически важную информацию внутри безопасной области платформы.

Defold имеет встроенную поддержку безопасной области на Android и iOS. Параметр `gui.safe_area_mode` в *game.project* определяет, какие противоположные отступы безопасной области влияют на корректировку GUI. Значение по умолчанию `none` игнорирует отступы; `long` применяет левый и правый отступы в альбомной ориентации и верхний и нижний в портретной; `short` применяет противоположную пару; `both` применяет все четыре края. GUI-скрипт может переопределить общий режим проекта для своей сцены с помощью [`gui.set_safe_area_mode()`](/ref/gui/#gui.set_safe_area_mode). Для пользовательской логики GUI или рендеринга функция [`window.get_safe_area()`](/ref/window/#window.get_safe_area) возвращает безопасный прямоугольник и отдельные отступы каждого края. Платформы без встроенных отступов безопасной области возвращают всё окно и нулевые отступы.

[Расширение Safe Area](/extension-safearea) остаётся альтернативой для старых проектов или процессов, которым требуется поведение сверх встроенных API; для стандартной обработки безопасной области Android и iOS оно не нужно.


## Рекомендации для конкретных платформ

### Android
Обязательно храните свой [keystore](/ru/manuals/android/#creating-a-keystore) в надёжном месте, чтобы вы могли обновлять игру.
