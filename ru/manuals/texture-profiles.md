...
Если две [настройки путей](#path-settings) совпадают с одним и тем же файлом и путь использует разные профили с разными платформами, будут использованы **оба** профиля и будет сгенерировано **две** текстуры.
</div>
...
Список доступных компрессоров см. в разделе [Компрессоры](#compressors).
...
<div class='sidenote' markdown='1'>
Defold поддерживает устанавливаемые компрессоры в пайплайне компрессии текстур. Это позволяет реализовывать алгоритмы сжатия текстур в расширениях, например WEBP или полностью пользовательские варианты.
</div>
...
| Пресет    | Время сжатия | Относительное время |
| --------- | ------------ | ------------------- |
| `LOW`     | 0m0.143s     | 0.5x                |
