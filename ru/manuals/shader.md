...
Спецификации OpenGL ES 2.0 (OpenGL для встраиваемых систем) и OpenGL ES Shading Language можно найти по адресу [Khronos OpenGL Registry](https://www.khronos.org/registry/gles/).
...
: Позиции вершин формы модели хранятся относительно начала координат модели. Это называется "пространство модели". Однако мир игры это "пространство мира", где позиция, ориентация и масштаб каждой вершины выражены относительно начала координат пространства мира. Разделяя их таким образом, игровой движок способен двигать, вращать и масштабировать каждую модель без разрушения изначальных значений вершин, хранимых в компоненте модели.
  ...
  Когда модель помещается в игровой мир, локальные координаты вершин модели должны быть переведены в координаты мирового пространства. Этот перевод делается посредством *мировой матрицы преобразования*, которая говорит, какой сдвиг, поворот и масштабирование должны быть применены к вершинам модели, чтобы корректно разместить в системе координат игрового мира.
...
: Константы шейдеров остаются постоянными на протяжении всего вызова отрисовки рендера. Константы добавляются к файлу материала в секции *Constants* и затем объявляются как `uniform` переменные  в программе шейдера. Uniforms Сэмплеры добавляются в секции *Samplers* материала и затем объявляются как `uniform` переменные  в программе шейдера. Матрицы необходимые для выполнения вершинных преобразований в вершинном шейдере доступны в качестве констант:
  ...
  - `CONSTANT_TYPE_VIEW` --- это *матрица вида* которая делает отображение из пространства мира в пространство камеры.
  ...
  - `CONSTANT_TYPE_WORLDVIEW`, `CONSTANT_TYPE_VIEWPROJ` и `CONSTANT_TYPE_WORLDVIEWPROJ` предоставляют соответствующие комбинированные матрицы.
  - `CONSTANT_TYPE_WORLD_INVERSE`, `CONSTANT_TYPE_VIEW_INVERSE`, `CONSTANT_TYPE_PROJECTION_INVERSE`, `CONSTANT_TYPE_VIEWPROJ_INVERSE`, `CONSTANT_TYPE_WORLDVIEW_INVERSE` и `CONSTANT_TYPE_WORLDVIEWPROJ_INVERSE` предоставляют обратные матрицы, не требуя их вычисления в шейдере.
  - `CONSTANT_TYPE_TIME` — предоставляемый движком `vec4`: время с момента запуска движка в `.x`, дельта времени кадра в `.y` и нули в `.z` и `.w`.
...
При кросс-компиляции шейдеров в GLSL ES Defold создаёт глобальные квалификаторы точности по умолчанию. Для значений с плавающей точкой по умолчанию используется `mediump`, для целых чисел — `highp`. Их можно изменить с помощью [настроек проекта](/ru/manuals/project-settings/#shader) **GLSL ES Default Precision Float** (`shader.glsl_es_default_precision_float`) и **GLSL ES Default Precision Int** (`shader.glsl_es_default_precision_int`); обе принимают `mediump` или `highp`.

Явный квалификатор переменной, входа или выхода имеет приоритет над созданным глобальным значением по умолчанию. Во фрагментных шейдерах OpenGL ES 2.0 и WebGL 1.0 точность `highp` поддерживается не всеми устройствами. Если в качестве глобального значения по умолчанию выбрано `highp`, Defold проверяет `GL_FRAGMENT_PRECISION_HIGH` и на устройствах без поддержки использует `mediump`.
