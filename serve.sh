...
PAGEFIND_PYTHON="python3"

if [ -x ".venv/bin/python" ]; then
    PAGEFIND_PYTHON=".venv/bin/python"
elif [ -x "venv/bin/python" ]; then
    PAGEFIND_PYTHON="venv/bin/python"
fi

PAGEFIND_VERSION=$("$PAGEFIND_PYTHON" -m pagefind --version 2>/dev/null | awk '{print $2}')
...
if [ "$PAGEFIND_VERSION" != "$REQUIRED_PAGEFIND_VERSION" ]; then
    ...
    echo "$PAGEFIND_PYTHON -m pip install \"pagefind[extended]==$REQUIRED_PAGEFIND_VERSION\""
    exit 1
fi
...
bundle exec jekyll build --drafts --future --incremental && "$PAGEFIND_PYTHON" -m pagefind
