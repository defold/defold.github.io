...
Powodem jest to, że gra będzie rysowana na siatce, w której każdy segment ma 16x16 pikseli, więc ekran gry nie utnie żadnych częściowych segmentów. Plik `game.project` zawiera wszystkie ważne ustawienia projektu - więcej przeczytasz o nich w [instrukcji ustawień projektu](/pl/manuals/project-settings/).
...
Więcej szczegółów możesz też przeczytać w instrukcji o [importowaniu zasobów graficznych](/pl/manuals/importing-graphics/).

<input type="checkbox"/>`Gotowe!`

## Dodawanie Tile Source

Defold udostępnia wbudowany komponent [Tilemap](/pl/manuals/tilemap/), którego użyjesz do utworzenia planszy złożonej z *kafelków* wyrównanych do siatki. Mapa kafelków pozwala ustawiać i odczytywać pojedyncze kafelki, co idealnie pasuje do tej gry. Ponieważ mapy kafelków pobierają grafikę z [Tilesource](/pl/manuals/tilesource/), musisz utworzyć taki zasób:
