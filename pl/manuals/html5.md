...
- anchor: webgl-context-version
  title: Wersja kontekstu WebGL
- anchor: download-verification
  title: Weryfikacja pobierania
...
![Ustawienia projektu](/manuals/images/html5/html5_project_settings.png)
...
![Utwórz pakiet HTML5](/manuals/images/html5/html5_bundle.png)
...
Możesz dołączyć jedną z tych architektur albo obie. Gdy dołączone są obie, loader wybiera `wasm_pthread-web`, jeśli przeglądarka i środowisko hostingu go obsługują, a w przeciwnym razie używa `wasm-web`. Kanoniczne nazwy celów znajdziesz w [instrukcji Boba](/pl/manuals/bob/#usage).
...
## Wersja kontekstu WebGL {#webgl-context-version}

Wybierz żądany kontekst graficzny za pomocą [`graphics.webgl_version_hint`](/pl/manuals/project-settings/#webgl-version-hint). Domyślnie jest to WebGL 2; wybierz WebGL 1, aby testować lub obsługiwać ten kontekst w przeglądarkach obsługujących obie wersje.

## Weryfikacja pobierania {#download-verification}

Moduł ładujący HTML5 domyślnie sprawdza rozmiary pobranych plików silnika i archiwum. Nieudana weryfikacja powoduje ponowienie pobierania, zanim moduł zgłosi błąd:

* Błędy sieci, nieprawidłowe statusy HTTP oraz niezgodności rozmiaru podczas pobierania plików JavaScript lub WebAssembly silnika korzystają z limitu ponowień `html5.retry_count`.
* Weryfikacja plików archiwum ma własny limit ponowień w przypadku niezgodności rozmiaru lub SHA-1. Każde ponowienie weryfikacji ponownie pobiera części pliku, a dla każdego pobrania dostępne są zwykłe ponowienia po błędach sieci.

Ustawienie `html5.retry_time` określa opóźnienie między ponowieniami w obu przypadkach.

Jeśli serwer, proxy lub CDN celowo przekształca udostępniane pliki i zmienia ich rozmiary, wyłącz weryfikację rozmiaru w pliku *game.project*:
...
Wyłączenie **Verify Downloaded File Size** pozostawia włączoną weryfikację SHA-1, jeśli została dołączona do pakietu. Zobacz [ustawienia projektu HTML5](/pl/manuals/project-settings/#verify-downloaded-file-size).
...
* Obsługa gamepadów (Gamepad support) - [dokumentacja gamepadów](/pl/manuals/input-gamepads/#gamepads-in-html5) zawiera szczególne uwagi i kroki, które mogą być potrzebne w HTML5.
...
![Sekcja HTML5](/manuals/images/html5/html5_section.png)

Więcej informacji o każdej opcji znajdziesz w [instrukcji ustawień projektu](/pl/manuals/project-settings/#html5).
...
![Sekcja Display](/manuals/images/html5/html5_display.png)

Otwórz *game.project* jako tekst i sprawdź `[section_name]` oraz nazwę pola, którego chcesz użyć. Następnie możesz użyć go jako tokenu: `{% raw %}{{section_name.field}}{% endraw %}` albo `{% raw %}{{{section_name.field}}}{% endraw %}`.

![Sekcja Display](/manuals/images/html5/html5_game_project.png)
...
: Wynikowy prefiks ścieżki archiwum używany przez moduł ładujący, oparty na `html5.archive_location_prefix`.

DEFOLD_ARCHIVE_LOCATION_SUFFIX
: Wynikowy przyrostek dodawany do adresów URL archiwum, oparty na `html5.archive_location_suffix`.

DEFOLD_HAS_ARCHIVE_ORIGIN
: `true`, gdy prefiks archiwum określa źródło HTTP lub HTTPS, w tym adres URL względny wobec protokołu, taki jak `//cdn.example.com/archive`. Dla względnych prefiksów archiwum ma wartość `false`. Dostępny od wersji Defold 1.13.2.

DEFOLD_ARCHIVE_ORIGIN
: Źródło archiwum obejmujące schemat, host i opcjonalny port albo pusty ciąg, jeśli nie określono źródła. Prefiks względny wobec protokołu daje źródło względne wobec protokołu. Używany do wskazówki preconnect; dostępny od wersji Defold 1.13.2.

DEFOLD_HAS_WASM_ENGINE
: `true`, jeśli pakiet zawiera silnik WebAssembly: `wasm-web` lub `wasm_pthread-web`.

DEFOLD_HAS_WASM_PTHREAD_ENGINE
: `true`, jeśli pakiet zawiera `wasm_pthread-web`. Użyj tego tokenu, aby uniknąć wstępnego ładowania niewłaściwego wariantu silnika, gdy moduł ładujący wybiera architekturę w czasie działania.
...
<div class='important' markdown='1'>
Ważne jest, aby ten blok wstawiany bezpośrednio pojawił się przed załadowaniem głównego skryptu aplikacji. Ponieważ zawiera znaczniki HTML, to makro powinno używać potrójnych nawiasów klamrowych `{% raw %}{{{TOKEN}}}{% endraw %}`, aby sekwencje znaków nie zostały escapowane.
</div>
...
* [Zużycie pamięci](/pl/manuals/optimization-memory)
* [Rozmiar silnika](/pl/manuals/optimization-size)
* [Rozmiar gry](/pl/manuals/optimization-size)

## FAQ
{% include shared/pl/html5-faq.md %}