...
Więcej informacji zawiera [instrukcja pisania kodu](/pl/manuals/writing-code).
...
Więcej informacji znajduje się w [instrukcji Editor HTTP API](/pl/manuals/editor-http-api/#reading-console-output).

### Utrwalone logi {#persisted-logs}

Defold może też zapisywać log gry po włączeniu `Write Log File` w `game.project`. Zobacz [Logi gry i systemu](/pl/manuals/debugging-game-and-system-logs/). Rejestrowanie do pliku jest przydatne w przypadku spakowanych aplikacji i testowania urządzeń docelowych, na których konsola edytora jest niedostępna.
...
Więcej informacji zawiera [instrukcja usługi silnika](/pl/manuals/engine-service/#automation-bridge-extension).
...
Edytor może utworzyć i udostępnić kompilację HTML5 za pomocą bieżącego polecenia `build-html5`, jak opisano w [instrukcji Editor HTTP API](/pl/manuals/editor-http-api/#building-html5). Bob również może utworzyć pakiet HTML5 bez edytora.
...
| Metoda | Zastosowanie |
| --- | --- |
| [Podgląd edytora](/pl/manuals/editor-http-api/#rendering-scene-previews) | Układ wczytanego zasobu, np. poziomu lub GUI, kompozycja atlasu, sprawdzanie mapy kafelków, kompozycja statycznej sceny, poprawność renderowania i shaderów edytora albo tworzenie miniatur dokumentacji |
| [Zrzut ekranu z uruchomionej aplikacji](/pl/manuals/engine-service) | Wyrenderowany stan uruchomionej kompilacji w kontrolowanym scenariuszu |
...
[Instrukcja Bob](/pl/manuals/bob) opisuje platformy, pliki ustawień, pakiety, pamięci podręczne, rozszerzenia natywne i raporty z budowania.
...
Ten sam format powinien być przydatny dla programisty, lokalnego skryptu, usługi CI lub [agenta AI wspomagającego programowanie](/pl/manuals/ai-agents). Dzięki temu weryfikacja pozostaje deterministyczna nawet wtedy, gdy diagnozowanie lub naprawa zostaną oddelegowane.