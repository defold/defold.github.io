...
Szczegóły dotyczące importowania danych 3D do komponentu *Model* dla potrzeb animacji znajdziesz w [instrukcji modeli 3D](/pl/manuals/model).
...
Oprócz wywoływania `model.play_anim()` do sterowania animacją, komponenty *Model* wystawiają właściwość "cursor", którą można animować za pomocą `go.animate()` (więcej w sekcji [animacje właściwości](/pl/manuals/property-animation)).
