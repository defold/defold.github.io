...
![Ustawienia projektu](/manuals/images/project-settings/settings.jpg)
...
Wartości z *game.project* można odczytać w czasie działania za pomocą funkcji [`sys.get_config_string(key)`](/ref/sys/#sys.get_config_string), [`sys.get_config_number(key)`](/ref/sys/#sys.get_config_number), [`sys.get_config_int(key)`](/ref/sys/#sys.get_config_int) i [`sys.get_config_boolean(key)`](/ref/sys/#sys.get_config_boolean). Przykłady:
...
#### Dependencies
Lista adresów URL do projektów będących *Library URL*. Więcej informacji znajdziesz w [instrukcji Libraries](/pl/manuals/libraries/).
...
#### Custom Resources
`custom_resources`
{% include shared/pl/custom-resources.md %}

Ładowanie zasobów niestandardowych opisano dokładniej w [instrukcji File Access](/pl/manuals/file-access/#how-to-access-files-bundled-with-the-application).

Ścieżki dostarczane przez rozszerzenia za pomocą `custom_resources.default` w pliku `ext.properties` są łączone z tym ustawieniem. Przykład znajdziesz w sekcji [zasoby niestandardowe rozszerzeń](/pl/manuals/extensions/#custom-resources).

#### Bundle Resources
`bundle_resources`
{% include shared/pl/bundle-resources.md %}

Ładowanie zasobów bundla opisano dokładniej w [instrukcji File Access](/pl/manuals/file-access/#how-to-access-files-bundled-with-the-application).
...
#### Include Dirs
Lista katalogów rozdzielonych spacjami, które mają być współdzielone z projektu przez mechanizm bibliotek. Więcej informacji znajdziesz w [instrukcji Libraries](/pl/manuals/libraries/).
...
To ustawienie dotyczy okna. Pozaekranowe [cele renderowania z wielokrotnym próbkowaniem](/pl/manuals/render/#multisampled-render-targets) mają własną liczbę próbek.
...
#### Display Profiles
Określa plik profili wyświetlania, którego należy użyć. Domyślnie `/builtins/render/default.display_profilesc`. Więcej informacji znajdziesz w [instrukcji GUI Layouts](/pl/manuals/gui-layouts/#creating-display-profiles).
...
`graphics.vulkan_version_major` określa główną wersję kontekstu/API Vulkan. Dotyczy tylko wybranego backendu graficznego Vulkan. Wartość domyślna to `1`.
...
`shader.glsl_es_default_precision_float` ustawia domyślny globalny kwalifikator precyzji dla wartości zmiennoprzecinkowych w shaderach GLSL ES po kompilacji krzyżowej. Prawidłowe wartości to `mediump` i `highp`; wartość domyślna to `mediump`.

#### GLSL ES Default Precision Int
`shader.glsl_es_default_precision_int` ustawia domyślny globalny kwalifikator precyzji dla wartości całkowitych w shaderach GLSL ES po kompilacji krzyżowej. Prawidłowe wartości to `mediump` i `highp`; wartość domyślna to `highp`.
...
#### Launch Screen
Plik storyboard .storyboard. Więcej informacji o tworzeniu storyboardu znajdziesz w [instrukcji iOS](/pl/manuals/ios/#creating-a-storyboard).
...
#### Info.plist
Jeśli ustawiono, podczas bundlowania aplikacji wskazany plik *`Info.plist`* zostanie użyty zamiast wbudowanego manifestu bazowego iOS. Wbudowany manifest zawiera wpisy sieci lokalnej i Bonjour wymagane do wykrywania celów edytora w buildach innych niż release. Jeśli dostarczasz własny manifest i potrzebujesz wykrywania celów, profilowania, szybkiego przeładowania lub przesyłania logów na urządzeniu, zachowaj te wpisy zgodnie z opisem w [instrukcji iOS](/pl/manuals/ios/#creating-an-ios-application-bundle).
...
Wybierz `/builtins/manifests/android/dmengine.keep`, aby bezpośrednio użyć domyślnych reguł silnika Defold. Rozszerzenia dostarczają własne [reguły zachowywania klas](/pl/manuals/extensions/#r8-keep-rules-for-android), które są łączone z tym plikiem.
...
Włączanie R8 i zachowywanie mapowania zaciemnionych nazw razem z pakietem wydania opisano w [instrukcji Androida](/pl/manuals/android/#shrinking-java-code-with-r8).
...
#### App Icon
Plik obrazu .ico używany jako ikona aplikacji w Windows. Więcej informacji o tworzeniu plików .ico znajdziesz w [instrukcji Windows](/pl/manuals/windows).

---

### HTML5

Więcej informacji o wielu z tych opcji znajdziesz w [instrukcji platformy HTML5](/pl/manuals/html5/).
...
`liveupdate.enabled` włącza system Live update w czasie działania. Domyślnie włączone. Sposób wykluczania, pobierania i montowania zasobów opisano w [instrukcji Live update](/pl/manuals/live-update/).
...
#### _App Manifest_
Jeśli ustawiono, użyj manifestu aplikacji do dostosowania builda silnika. Pozwala to usunąć nieużywane części silnika i zmniejszyć rozmiar końcowego pliku binarnego. Jak wykluczać nieużywane funkcje opisano w [instrukcji Application Manifest](/pl/manuals/app-manifest).

---

### Profiler

Ustawienie **Profiler** w manifeście aplikacji określa, czy kod profilera jest dołączany do buildów debug i release. Poniższe ustawienia sterują zachowaniem kodu profilera obecnego w wybranym buildzie. Szczegóły znajdziesz w [instrukcji profilowania](/pl/manuals/profiling/).
...
#### Max Sample Count
`profiler.max_sample_count` to maksymalna liczba próbek profilera rejestrowanych dla każdego wątku w jednej klatce. Wartość domyślna to `4096`, a minimalna `128`. Zwiększ ją tylko wtedy, gdy prawidłowy profil przekracza limit; najpierw sprawdź kod profilowania rozszerzeń natywnych pod kątem niedopasowanych wywołań rozpoczęcia i zakończenia zakresu.
...
Własne wartości, tak samo jak każdą inną wartość konfiguracyjną, można odczytywać za pomocą odpowiedniej funkcji opisanej w sekcji [Dostęp w czasie działania](#runtime-access):
...
{% include shared/pl/component-max-count-optimizations.md %}

## Własne ustawienia projektu

Można definiować własne ustawienia dla głównego projektu albo dla [native extension](/pl/manuals/extensions/). Własne ustawienia dla głównego projektu należy zdefiniować w pliku `game.properties` w katalogu głównym projektu. Pliki o nazwie `ext.properties` są wykrywane w dowolnym miejscu projektu oraz w pobranych zależnościach bibliotecznych; nie wymagają sąsiadującego pliku `ext.manifest`. Wszystkie wykryte metadane rozszerzeń są łączone, po czym stosowany jest główny plik `game.properties`, który może je nadpisać.
...
Zarówno Bob, jak i edytor odczytują te pliki metadanych. Edytor używa ich do tworzenia odpowiednich pól, opcji, walidacji i podpowiedzi w widoku *game.project*.