...
Zasób Buffer może być użyty jako dane wejściowe dla [komponentu Mesh](/pl/manuals/mesh). Zasoby Buffer można też tworzyć w czasie działania przy użyciu `buffer.create()` oraz [powiązanych funkcji API](/ref/buffer/#buffer.create:element_count-declaration).