...
: Przeciągnij obrazy z panelu *Assets* do widoku edytora.
  ...
  ![Tworzenie atlasu, dodawanie obrazów](/manuals/images/atlas/add.png)
  ...
  ![Dodane obrazy](/manuals/images/atlas/single_images.png)
...
: Kliknij prawym przyciskiem myszy główny wpis atlasu w panelu *Outline*.
  ...
  ![Tworzenie atlasu, dodawanie obrazów](/manuals/images/atlas/add_animation.png)
  ...
  ![Grupa animacji](/manuals/images/atlas/animation_group.png)
...
![Atlas wielostronicowy](/manuals/images/atlas/multipage_atlas.png)
...
![Właściwości atlasu](/manuals/images/atlas/atlas_properties.png)
...
![Właściwości obrazu](/manuals/images/atlas/image_properties.png)
...
Jeśli tekstura ma być używana na [komponencie sprite'a](/pl/manuals/sprite/), najpierw musi zostać użyta przez atlas. Użyj [`resource.create_atlas(path, params)`](https://defold.com/ref/stable/resource/#resource.create_atlas:path-table), aby utworzyć atlas:
...
Wpisy `frames` są indeksami numerowanymi od 1, które wskazują elementy tabeli `geometries`. Lista może ponownie wykorzystywać geometrie, zmieniać ich kolejność lub je pomijać, czego nie da się przedstawić za pomocą przestarzałych pól przedziału `frame_start` i `frame_end`. Funkcja `resource.get_atlas()` zwraca `frames`; używaj tej samej reprezentacji podczas przekazywania danych atlasu do `resource.set_atlas()` lub `resource.create_atlas()`. Ze względu na zgodność setter i funkcja tworząca nadal przyjmują pola przedziału, ale nowy kod powinien używać `frames`.