...
如果您需要Lua编程入门，请查看我们的[Lua in Defold手册](/zh/manuals/lua)。
...
![编辑器概览](/tutorials/images/runner/1/editor2_overview.png)
...
Changed Files
: 包含与当前 Git 提交相比在本地添加、修改、重命名或删除的文件。对于修改或重命名的文本文件，您可以一次查看一个文件的差异；也可以在此还原选中的本地更改。请使用外部 Git 客户端或命令行与远程仓库同步。
...
![构建](/tutorials/images/runner/1/build_and_launch.png)
...
![带有未保存更改的文件](/tutorials/images/runner/1/file_changed.png)
</div>
...
![右键单击并复制发布链接](/tutorials/images/runner/extension-spine-releases.png)

将发布链接添加到您的[game.project dependencies](/zh/manuals/libraries/#setting-up-library-dependencies)列表中。添加Spine扩展后，您还需要重新启动编辑器以激活Spine扩展中包含的编辑器集成。
...
2. 创建一个新的*Atlas*文件来保存地面纹理（在*Assets pane*中右键单击合适的文件夹，例如*main*文件夹，然后选择<kbd>New ▸ Atlas File</kbd>）。将图集文件命名为*level.atlas*。
  ...
  <div class='sidenote' markdown='1'>
  *Atlas*是一个将一组单独的图像合并为一个更大图像文件的文件。这样做的原因是为了节省空间并获得性能。您可以在[2D graphics documentation](/zh/manuals/2dgraphics)中阅读更多关于Atlas和其他2D图形功能的信息。
  </div>
...
3. 通过右键单击*Outline*中的图集根目录并选择<kbd>Add Images</kbd>，将地面图像添加到新图集中。选择导入的图像，然后单击*OK*。图集中的每个图像现在可以作为单帧动画（静态图像）访问，用于精灵、粒子效果和其他视觉元素。保存文件。
  ...
  ![创建新图集](/tutorials/images/runner/1/new_atlas.png)

  ![将图像添加到图集](/tutorials/images/runner/1/add_images_to_atlas.png)
...
5. 在每个对象中，添加一个精灵组件（在*Outline*视图中右键单击游戏对象，然后选择<kbd>Add Component</kbd>，然后选择*Sprite*并单击*OK*），将精灵组件的*Image*属性设置为您刚刚创建的图集，并将精灵的默认动画设置为两个地面图像之一。将_精灵组件_（不是游戏对象）的X位置设置为190，Y位置设置为40。由于图像的宽度为380像素，我们将其横向移动一半像素，游戏对象的枢轴将位于精灵图像的最左边缘。
  ...
  ![创建地面集合](/tutorials/images/runner/1/ground_collection.png)

6. 我们使用的图形有点太大，所以将每个游戏对象缩放到60%（X和Y缩放0.6，结果为228像素宽的地面片段）。

  ![缩放地面](/tutorials/images/runner/1/scale_ground.png)
...
9. 启动游戏（<kbd>Project ▸ Build</kbd>）以查看一切是否就位。
  ...
  ![静止的地面](/tutorials/images/runner/1/still_ground.png)
...
目前，这些描述可能已经足够了。但是，可以在[Building blocks manual](/zh/manuals/building-blocks)中找到对这些内容更全面的介绍。稍后访问该手册以获得对Defold工作原理的更深入理解是个好主意。
...
Defold在Lua之上添加了一组API，以及一个_消息传递_系统，允许您编程游戏对象之间的通信。有关其工作原理的详细信息，请参见[Message passing manual](/zh/manuals/message-passing)。
</div>
...
![地面控制器](/tutorials/images/runner/1/ground_controller.png)
...
![英雄图集](/tutorials/images/runner/2/hero_atlas.png)
...
![英雄脊椎场景](/tutorials/images/runner/2/hero_spinescene.png)

<div class='sidenote' markdown='1'>
文件*hero.spinejson*已以Spine JSON格式导出。您需要Spine动画软件才能创建此类文件。如果您想使用其他动画软件，可以将动画导出为精灵表，并将其用作*Tile Source*或*Atlas*资源中的翻书动画。有关更多信息，请参见[Animation](/zh/manuals/animation)手册。
</div>
...
![脊椎模型属性](/tutorials/images/runner/2/spinemodel_properties.png)
...
<div class='sidenote' markdown='1'>
"Kinematic"碰撞意味着我们希望碰撞被注册，但物理引擎不会自动解决碰撞并模拟对象。物理引擎支持多种不同类型的碰撞对象。您可以在[Physics documentation](/zh/manuals/physics)中阅读更多相关信息。
</div>
...
![英雄游戏对象](/tutorials/images/runner/2/hero_game_object.png)

<div class='sidenote' markdown='1'>
我们自己处理碰撞的原因是，如果我们将角色的碰撞对象类型设置为动态，引擎将对所涉及的物体执行牛顿模拟。对于这样的游戏，这样的模拟远非最佳，因此我们完全控制而不是与物理引擎的各种力量作斗争。

现在，要做到这一点并正确处理碰撞需要一点向量数学。在[Physics documentation](/zh/manuals/physics-resolving-collisions/)中给出了如何解决运动学碰撞的详细解释。
</div>
...
![输入绑定](/tutorials/images/runner/2/input_bindings.png)
...
![脚本属性](/tutorials/images/runner/2/script_property.png)
...
![添加地面代码](/tutorials/images/runner/insert_ground_code.png)
...
![地面碰撞](/tutorials/images/runner/2/ground_collision.png)
...
![平台](/tutorials/images/runner/2/platform_long.png)
...
![运行游戏](/tutorials/images/runner/2/run_game.png)
...
![插入英雄代码](/tutorials/images/runner/insert_hero_code.png)
...
8. 保存文件。
    ...
    ![平台尖刺](/tutorials/images/runner/3/danger_edges.png)

9. 打开*hero.go*，标记*Collision Object*，并将"danger"名称添加到*Mask*属性中。然后保存文件。

    ![英雄碰撞](/tutorials/images/runner/3/hero_collision.png)
...
3. 打开*platform.script*并修改它，以便当平台到达最左边缘时，不仅删除平台，还向关卡控制器发送一条消息要求删除平台：
    ...
    ![插入平台代码](/tutorials/images/runner/insert_platform_code.png)
...
![插入英雄代码](/tutorials/images/runner/insert_hero_code_2.png)
...
10. 将脚本文件作为*Script*组件添加到硬币对象（在*Outline*中右键单击根目录，然后选择<kbd>Add Component from File</kbd>）。
    ...
    ![硬币游戏对象](/tutorials/images/runner/3/coin.png)
...
![硬币工厂](/tutorials/images/runner/3/coin_factory.png)
...
![插入控制器代码](/tutorials/images/runner/insert_controller_code.png)
...
> 在[此处](/tutorials/images/runner/sample-runner.zip)下载项目的完整版本
