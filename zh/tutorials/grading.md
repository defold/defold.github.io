...
假设您已经熟悉Defold编辑器，并且对GL着色器和Defold渲染管线有基本了解。如果您需要学习这些主题，请查看[我们的着色器手册](/zh/manuals/shader/)和[渲染手册](/zh/manuals/render/)。
...
您可以用修改像素数据或以编程方式创建全新像素颜色的着色器程序替换组件的着色器程序。[Shadertoy教程](/zh/tutorials/shadertoy)教您如何做到这一点。
...
![渲染目标](/tutorials/images/grading/render_target.png)
...
1. 打开 *`main.collection`* 并创建一个名为 "`grade`" 的新游戏对象。
2. 向 "`grade`" 游戏对象添加一个 Model 组件。
3. 将模型组件的 *Mesh* 属性设置为 `builtins/assets/meshes` 中的 *`quad.gltf`* 文件。
...
7. 打开*`main.collection`*，选择游戏对象"`grade`"中的模型组件，并将其*材质*属性设置为"`/main/grade.material`"。
   ...
   ![模型属性](/tutorials/images/grading/model_properties.png)
...
![去饱和游戏](/tutorials/images/grading/desaturated_game.png)

## 色彩分级

颜色表示为三个分量值，其中每个分量决定颜色中红色、绿色或蓝色的数量。从黑色、红色、绿色、蓝色、黄色和粉色到白色的完整颜色光谱可以适合立方体形状：

![颜色立方体](/tutorials/images/grading/color_cube.png)
...
![渲染目标分级](/tutorials/images/grading/render_target_grading.png)
...
![查找纹理](/tutorials/images/grading/lut.png)
...
![查找表](/tutorials/images/grading/lut_lookup.png)
...
![查找表过滤](/tutorials/images/grading/lut_filtering.png)
...
3. 将*`Filter min`*属性设置为`FILTER_MODE_MIN_LINEAR`，将*`Filter mag`*属性设置为`FILTER_MODE_MAG_LINEAR`。
    ...
    ![查找表采样器](/tutorials/images/grading/material_lut_sampler.png)

4. 下载以下查找表纹理（*`lut16.png`*）并将其添加到您的项目中。

    ![16色查找表](/tutorials/images/grading/lut16.png)

5. 打开*`main.collection`*并将*`lut`*纹理属性设置为下载的查找表纹理。

    ![四边形模型查找表](/tutorials/images/grading/quad_lut.png)
...
![世界原始外观](/tutorials/images/grading/world_original.png)

到目前为止，看起来我们做的一切都是正确的，但表面下潜伏着一个问题。看看当我们添加一个带有渐变测试纹理的精灵时会发生什么：

![蓝色条带](/tutorials/images/grading/blue_banding.png)
...
![蓝色无条带](/tutorials/images/grading/blue_no_banding.png)
...
![Affinity中的世界](/tutorials/images/grading/world_graded_affinity.png)
...
![分级后的世界](/tutorials/images/grading/world_graded.png)
