...
它与属于 Defold 编辑器、用于控制打开项目的[编辑器服务器](/zh/manuals/editor-http-api)相互独立。
...
![Defold 调试构建中的引擎服务端口信息](/manuals/images/automation/engine-service.png)
...
![访问引擎服务器](/manuals/images/automation/engine-server.png)
...
运行时脚本、物理、输入、动态创建的对象和特定于平台的渲染都需要运行中的引擎，并应通过[自动化运行时测试](/zh/manuals/automated-testing)进行验证。
