...
### 组件z值
所有游戏对象和组件都使用`vector3`对象表示的位置定位在3D空间中。当您以2D查看游戏的图形内容时，X和Y值确定对象在"宽度"和"高度"轴上的位置，而Z位置确定对象在"深度"轴上的位置。Z位置允许您控制重叠对象的可见性：Z值为1的精灵将出现在Z位置0的精灵前面。默认情况下，Defold使用允许Z值在-1和1之间的坐标系：
