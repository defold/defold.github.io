...
- 引导集合
- 加载集合
- 更改被排除代理的集合 {changing-an-excluded-proxys-collection}
- 定位到新世界
- 卸载世界
- 时间步
- 注意事项和常见问题
...
2. 将 *Collection* 属性设置为您希望稍后动态加载到运行时中的集合的引用。这是一项构建时静态依赖：引用的集合及其依赖项会被编译。除非勾选 *Exclude*，否则它们会包含在主包中。勾选 *Exclude* 后，仅通过被排除代理引用的资源可以在 Live Update 流程中从主包省略，并且可以在运行时将未加载的代理重定向到另一个已编译集合，如下文所述。
...
[`collectionproxy.set_collection()`](/ref/collectionproxy/#collectionproxy.set_collection) 可以将一个被排除且未加载的代理重定向到已编译的集合，这在挂载 Live Update 包后很有用。代理必须已勾选 *Exclude*，并且不能处于已加载或正在加载的状态。路径必须以 `.collectionc` 结尾。加载代理时，该集合及其所有依赖项必须可供资源系统使用。

加载代理前请检查返回值。只在收到 `proxy_loaded` 后才初始化并启用新世界：
...
在代理既未加载也未正在加载时，调用 `collectionproxy.set_collection("#level_proxy", nil)` 可恢复编辑器中分配的集合。下载和挂载内容请参阅 [Live Update 脚本手册](/zh/manuals/live-update-scripting/)；`collectionproxy.RESULT_*` 失败代码请参阅 API 参考。
