...
* [编辑器脚本](/zh/manuals/editor-scripts)可以提供项目专用的资源操作和工具。
* [编辑器 HTTP API](/zh/manuals/editor-http-api)提供编辑器命令、构建结果、控制台输出、引用搜索、预览、偏好设置和编辑器脚本路由。
* [引擎服务和运行时自动化 API](/zh/manuals/engine-service)提供实时调试引擎状态、输入、截图和扩展定义的操作。
* [Bob](/zh/manuals/bob)提供命令行构建、报告、存档和打包功能。
...
智能体应遵循与其他任何自动化相同的[检查、更改、验证、评估循环](/zh/manuals/automation/#the-automation-loop)。
...
有关自动化测试和验证的良好实践，请参阅[本手册](/zh/manuals/automated-testing)。

## 多模态评估

具有图像输入能力的智能体可以检查[编辑器预览](/zh/manuals/editor-http-api/#rendering-scene-previews)、运行时截图、视觉差异和浏览器捕获内容。

多模态评估适用于标签被截断、控件重叠、选择状态不明确、构图或内容超出安全区域等语义问题。请事先定义预期视口和标准。

请在[本手册](/zh/manuals/automated-testing)中进一步了解编辑器预览和运行时截图及视觉检查。
