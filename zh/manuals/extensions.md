...
Defold通过基于云的构建解决方案，为零配置原生扩展提供了入口点。任何开发并添加到游戏项目中的原生扩展（无论是直接添加还是通过[库项目](/zh/manuals/libraries/)添加），都将成为普通项目内容的一部分。无需构建特殊版本的引擎并分发给团队成员，这一过程会自动处理——任何构建和运行项目的团队成员都将获得一个包含所有原生扩展的项目特定引擎可执行文件。

![Cloud build](/manuals/images/extensions/cloud_build.png)

Defold免费提供云构建服务器，没有任何使用限制。服务器托管在欧洲，发送本地代码的URL在[编辑器首选项窗口](/zh/manuals/editor-preferences/#extensions)中配置，或通过[bob](/zh/manuals/bob/#usage)的`--build-server`命令行选项配置。如果您希望设置自己的服务器，请[遵循这些说明](/zh/manuals/extender-local-setup)。
...
```
*ext.manifest*
: 扩展文件夹_必须_包含一个*ext.manifest*文件。此文件是配置文件，包含构建单个扩展时使用的标志和定义。文件格式定义可在[扩展清单手册](https://defold.com/zh/manuals/extensions-ext-manifests/)中找到。
...
* `android` - 此文件夹接受要合并到主应用程序中的清单存根文件（[如此处所述](/zh/manuals/extensions-manifest-merge-tool)）。
  * 该文件夹还可以包含一个`build.gradle`文件，其中包含要[由Gradle解析的依赖项](/zh/manuals/extensions-gradle)。
  * 包含 Java 代码的扩展应提供 [R8 保留规则文件](#r8-keep-rules-for-android)（`.keep`），以保留运行时需要的类。
* `ios` - 此文件夹接受要合并到主应用程序中的清单存根文件（[如此处所述](/zh/manuals/extensions-manifest-merge-tool)）。
  * 该文件夹还可以包含一个`Podfile`文件，其中包含要[由Cocoapods解析的依赖项](/zh/manuals/extensions-cocoapods)。
* `osx` - 此文件夹接受要合并到主应用程序中的清单存根文件（[如此处所述](/zh/manuals/extensions-manifest-merge-tool)）。
* `web` - 此文件夹接受要合并到主应用程序中的清单存根文件（[如此处所述](/zh/manuals/extensions-manifest-merge-tool)）。
...
[启用 R8](/zh/manuals/android/#enabling-r8) 后，这些规则会与项目选定的保留规则文件合并。
...
例如，将 JSON 文件放在 `/myextension/data/settings.json`。该路径相对于项目根目录，包含扩展文件夹。将扩展作为库共享时，请在库的 [Include Dirs](/zh/manuals/libraries/#setting-up-library-sharing) 中包含 `myextension`，使使用该库的项目能获得扩展及其数据。
...
有关自定义资源与捆绑资源的区别，请参阅[文件访问](/zh/manuals/file-access/#custom-resources)。

## 共享扩展

扩展在项目中被视为与其他资产一样，可以以相同方式共享。如果原生扩展文件夹作为库文件夹添加，它可以作为项目依赖项共享给其他人使用。有关更多信息，请参考[库项目手册](/zh/manuals/libraries/)。
