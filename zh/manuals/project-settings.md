...
可以在运行时使用 [`sys.get_config_string(key)`](/ref/sys/#sys.get_config_string)、[`sys.get_config_number(key)`](/ref/sys/#sys.get_config_number)、[`sys.get_config_int(key)`](/ref/sys/#sys.get_config_int) 和 [`sys.get_config_boolean(key)`](/ref/sys/#sys.get_config_boolean) 从 *game.project* 中读取值。例如：
...
扩展通过 `ext.properties` 中的 `custom_resources.default` 提供的路径会与此设置合并。示例请参阅[扩展的自定义资源](/zh/manuals/extensions/#custom-resources)。

#### Bundle Resources
`bundle_resources`
{% include shared/zh/bundle-resources.md %}

加载打包资源在[文件访问手册](/zh/manuals/file-access/#how-to-access-files-bundled-with-the-application)中有更详细的介绍。
...
此设置控制窗口的抗锯齿。离屏的[多重采样渲染目标](/zh/manuals/render/#multisampled-render-targets)有自己的采样数。
...
`graphics.vulkan_version_major` 是 Vulkan 上下文/API 主版本提示。仅在选择 Vulkan 图形后端时适用。默认值为 `1`。
...
`shader.glsl_es_default_precision_float` 设置交叉编译的 GLSL ES 着色器中浮点值的默认全局精度限定符。有效值为 `mediump` 和 `highp`；默认为 `mediump`。

#### GLSL ES Default Precision Int
`shader.glsl_es_default_precision_int` 设置交叉编译的 GLSL ES 着色器中整数值的默认全局精度限定符。有效值为 `mediump` 和 `highp`；默认为 `highp`。
...
GUI 脚本可以使用 [`gui.set_safe_area_mode()`](/ref/gui/#gui.set_safe_area_mode) 为其场景覆盖此模式。有关平台支持和自定义布局，请参阅[安全区域指南](/zh/manuals/porting-guidelines/#mobile-phones-and-notch-and-hole-punch-cameras)。
...
如果指定，打包应用时将使用此 *`Info.plist`* 文件代替内置 iOS 基础清单。内置清单包含非 Release 构建中编辑器目标发现所需的本地网络和 Bonjour 条目。如果提供自定义清单，并且需要在设备上进行目标发现、性能分析、热重载或日志流传输，请按 [iOS 手册](/zh/manuals/ios/#creating-an-ios-application-bundle)所述保留这些条目。
...
选择 `/builtins/manifests/android/dmengine.keep` 可直接使用 Defold 的默认规则。扩展提供自己的[保留规则](/zh/manuals/extensions/#r8-keep-rules-for-android)，这些规则会与此文件合并。

仅当需要添加项目特定规则时，才将内置文件复制到项目中。请在副本中保留内置规则：选择自定义文件会替换完整的项目规则集。

有关如何启用 R8 并在发布包中保留其混淆映射，请参阅 [Android 手册](/zh/manuals/android/#shrinking-java-code-with-r8)。
...
启动期间下载失败后的重试次数，包括网络错误、失败的 HTTP 状态，以及引擎 JavaScript 或 WebAssembly 文件的大小不匹配。初次请求不计入重试次数。归档文件验证有自己的重试上限；请参阅[下载验证](/zh/manuals/html5/#download-verification)和 `Retry Time`。
...
`html5.verify_downloaded_file_size` 检查下载的引擎和归档文件是否符合预期大小。默认启用（`true`）。仅当服务器、代理或 CDN 有意重写文件并改变其大小时，才将其设置为 `false`。验证失败会触发下载重试，耗尽重试后启动失败。引擎下载与归档文件验证的重试上限不同；请参阅[下载验证](/zh/manuals/html5/#download-verification)。
...
`liveupdate.enabled` 在运行时启用实时更新系统。默认启用。有关如何排除、下载和挂载资源，请参阅[实时更新手册](/zh/manuals/live-update/)。
...
App Manifest 的 **Profiler** 设置控制是否将分析器代码链接到 Debug 和 Release 构建中。以下设置控制所选构建中已存在的分析器代码的运行时行为。详情请参阅[性能分析手册](/zh/manuals/profiling/)。
...
`profiler.max_sample_count` 是每个线程每帧记录的分析器样本数上限。默认值为 `4096`，最小值为 `128`。仅当正常性能分析确实超出限制时才提高此值；请先检查原生扩展性能分析代码中是否存在不匹配的 scope begin/end 调用。
...
与其他配置值一样，自定义值也可使用[运行时访问](#runtime-access)中介绍的相应函数读取：
...
可以为主项目或[原生扩展](/zh/manuals/extensions/)定义自定义设置。主项目的自定义设置必须在项目根目录的 `game.properties` 文件中定义。项目和获取的库依赖项中任何位置名为 `ext.properties` 的文件都会被发现，不要求旁边存在 `ext.manifest`。所有发现的扩展元数据会先合并，然后应用根目录的 `game.properties` 文件，并可覆盖这些元数据。
...
Bob 和编辑器都会解析这些元数据文件。编辑器使用它们在 *game.project* 查看器中创建相应字段、选项、验证和帮助工具提示。