...
可以在配置和加载引擎时指定额外的引擎参数。这些额外的引擎参数可以在运行时使用`sys.get_config_string()`检索。要添加键值对，请修改传递给`index.html`中加载的引擎的`extra_params`对象的`engine_arguments`字段：
...
您也可以将`--config=foo1=bar1, --config=foo2=bar2`添加到*game.project*的HTML5部分的 *Engine Arguments* 字段中，它将被注入到生成的`index.html`文件中。
