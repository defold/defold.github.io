...
您可以包含其中一种架构，也可以同时包含两种。同时包含时，如果浏览器和托管环境支持，加载器会选择 `wasm_pthread-web`，否则回退到 `wasm-web`。规范的目标名称请参阅 [Bob 手册](/zh/manuals/bob/#usage)。
...
通过 [`graphics.webgl_version_hint`](/zh/manuals/project-settings/#webgl-version-hint) 选择请求的图形上下文。默认使用 WebGL 2；请求 WebGL 1 可在同时支持这两个版本的浏览器中测试或面向该上下文。
...
禁用 **Verify Downloaded File Size** 后，包中包含的 SHA-1 验证仍然启用。请参阅 [HTML5 项目设置](/zh/manuals/project-settings/#verify-downloaded-file-size)。
...
Defold为`index.html`提供了深色和浅色主题。默认情况下设置浅色主题，但可以通过更改`Custom CSS`文件来更改。在`Scale Mode`字段中还有四种预定义的缩放模式可供选择。
...
如果创建自定义模板，您可以通过在全局 `CUSTOM_PARAMETERS` 对象中赋值来更改引擎加载器的参数。内置模板为这些自定义项提供了一个特意留空的 `<script id="engine-setup">` 块。
<div class='important' markdown='1'>
请将 `engine-setup` 块放在加载 `dmloader.js` 的脚本之后，并放在调用 `EngineLoader.load()` 的 `engine-start` 块之前。
</div>
例如：
```html
...
可以在配置和加载引擎时指定额外的引擎参数。这些额外的引擎参数可以在运行时使用 `sys.get_config_string()` 获取。请在 `index.html` 的 `engine-setup` 块中将参数直接分配给 `CUSTOM_PARAMETERS.engine_arguments`：
...
分配新数组会替换 *game.project* 中配置的所有引擎参数。要保留这些参数并添加另一个参数，请改用 `CUSTOM_PARAMETERS.engine_arguments.push("--config=example.foo3=bar3")`。

您也可以将 `--config=example.foo1=bar1, --config=example.foo2=bar2` 添加到 *game.project* 的 HTML5 部分的 *Engine Arguments* 字段中。这些用逗号分隔的值会添加到生成的 `dmloader.js` 中的 `CUSTOM_PARAMETERS.engine_arguments`。
