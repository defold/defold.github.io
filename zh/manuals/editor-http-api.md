...
编辑器是图形桌面应用程序。请在可以访问显示器的交互式用户会话中启动它。当图形会话不可用（例如在无头 CI 中），或只需执行编译自动化和创建独立包时，请使用 [Bob](/zh/manuals/bob)。
...
开始浏览器测试前，请等待该 URL 可用。更多详情请参阅 [HTML5 的浏览器测试](/zh/manuals/automated-testing/#browser-tests-for-html5)。
