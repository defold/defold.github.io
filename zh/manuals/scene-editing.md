...
初始摄像机视图取决于资源。模型和 glTF 场景等 3D 资源默认使用**透视**视图，而精灵、瓦片地图和 GUI 场景等 2D 资源默认使用**正交**视图。您可以通过场景工具栏更改摄像机方向、投影和网格。
...
## 记忆场景视图 {#remembered-scene-views}

关闭场景资源的标签页或退出编辑器时，编辑器会记住每个场景资源的摄像机状态。重新打开同一资源时会恢复其视图，因此不同集合或模型可以保留不同的摄像机位置、方向和投影。

可见性筛选器也会按场景分别保存。在一个场景中隐藏模型或组件辅助线，不要求另一个场景使用相同的筛选器。这些属于编辑器视图设置，不会更改游戏的摄像机或运行时可见性。

对于没有已保存摄像机状态的资源，模型、网格和 glTF 资源会以透视视图打开。碰撞对象根据项目的 2D/3D 物理设置选择视图；集合和游戏对象则根据其场景几何体选择初始视图。
...
编辑器分别保存 2D 和 3D 视图的网格设置。在所需模式下设置大小、平面和外观；切换模式时会恢复该模式的网格设置。**Reset to Defaults** 会重置当前模式的设置。
