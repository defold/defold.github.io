...
启用后（`true`），会包含完整文本布局系统，用于文本塑形，包括从右到左书写的语言。同时启用此选项和 *game.project* 中的 `font.runtime_generation`，即可从 TrueType（`.ttf`）或 OpenType（`.otf`）资源在运行时生成 SDF 字体。自 Defold 1.13.2 起支持从 `.otf` 资源进行运行时生成。更多信息请参阅[字体手册](/zh/manuals/font/#enabling-runtime-fonts)。
