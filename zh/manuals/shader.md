...
: 着色器常量在渲染绘制调用的持续时间内保持不变。常量被添加到材质文件的*常量*部分，然后在着色器程序中声明为 `uniform`。采样器 uniform 被添加到材质的*采样器*部分，然后在着色器程序中声明为 `uniform`。在顶点着色器中执行顶点变换所需的矩阵可作为常量使用：
  ...
  - `CONSTANT_TYPE_WORLDVIEW`、`CONSTANT_TYPE_VIEWPROJ` 和 `CONSTANT_TYPE_WORLDVIEWPROJ` 提供相应的组合矩阵。
  - `CONSTANT_TYPE_WORLD_INVERSE`、`CONSTANT_TYPE_VIEW_INVERSE`、`CONSTANT_TYPE_PROJECTION_INVERSE`、`CONSTANT_TYPE_VIEWPROJ_INVERSE`、`CONSTANT_TYPE_WORLDVIEW_INVERSE` 和 `CONSTANT_TYPE_WORLDVIEWPROJ_INVERSE` 提供逆矩阵，无需着色器自行计算。
  - `CONSTANT_TYPE_TIME` 是引擎提供的 `vec4`：`.x` 为引擎启动后经过的时间，`.y` 为帧增量时间，`.z` 和 `.w` 为零。
...
Defold 在将着色器交叉编译为 GLSL ES 时生成全局默认精度限定符。浮点值默认为 `mediump`，整数默认为 `highp`。可通过[项目设置](/zh/manuals/project-settings/#shader)中的 **GLSL ES Default Precision Float**（`shader.glsl_es_default_precision_float`）和 **GLSL ES Default Precision Int**（`shader.glsl_es_default_precision_int`）更改；两者都接受 `mediump` 或 `highp`。

变量、输入或输出上的显式限定符优先于生成的全局默认值。在 OpenGL ES 2.0 和 WebGL 1.0 片段着色器中，并非所有设备都支持 `highp`。选择 `highp` 作为全局默认值时，Defold 会使用 `GL_FRAGMENT_PRECISION_HIGH` 保护它，并在不支持的设备上回退到 `mediump`。
