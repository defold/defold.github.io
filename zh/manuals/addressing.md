...
设置中的标识符由开发者定义，并且在各自的命名上下文中必须唯一。在这里，我们选择给游戏对象命名为"bean"，其精灵组件命名为"body"，而控制角色的脚本组件命名为"controller"。用在字符串 URL 地址中的标识符不应包含 `:` 或 `#`，因为 URL 语法将 `:` 保留为 socket 分隔符，将 `#` 保留为游戏对象/组件分隔符。除此之外，URL 解析器不会拒绝标点符号。
