...
目前 Defold 有三种方式可以存储资源。在设置窗口的 *模式* 下拉菜单中选择方法：

`Zip`
: 此选项告诉 Defold 创建一个包含排除资源的 Zip 归档文件。归档文件保存在 *导出路径* 指定的位置，并可在运行时通过 `zip:` URI 和 `liveupdate.add_mount()` 挂载。

`Folder`  
: 此选项告诉 Defold 创建一个包含所有排除资源的文件夹，适合在上传或打包前进行后处理。若文件夹中的各个编译文件按引擎预期的资源路径组织，则可在运行时通过 `file:` URI 挂载。

`Amazon`
: 此选项告诉 Defold 自动将排除的资源上传到 Amazon Web Service (AWS) S3 存储桶。填写您的 AWS *凭证配置文件* 名称，选择适当的 *存储桶* 并提供 *前缀* 名称。您可以在此 [aws 指南](/zh/manuals/live-update-aws) 中阅读有关如何设置 AWS 账户的更多信息。
...
打包时，任何被排除的资源都会留在应用程序包之外。通过勾选 *发布 Live update 内容* 复选框，您告诉 Defold 根据 Live Update 设置（见上文）上传被排除的资源到 Amazon，或创建 Zip 归档文件。发布的 Live Update 内容仍包含 `liveupdate.game.dmanifest`，其中包含远程交付所需的完整资源列表。

发布基于归档的 Live Update 内容时，默认启用 *Strip Live Update Entries from Main Manifest*（`liveupdate.exclude_entries_from_main_manifest`）。启用此设置后，仅用于 Live Update 的资源会从打包后的 `game.dmanifest` 中移除，从而减小包体大小和运行时内存占用。只有在需要旧行为，即被排除条目仍保留在打包后的 `game.dmanifest` 中时，才应禁用它。

启用默认设置后，`collectionproxy.get_resources()` 会在相关归档挂载前返回 `{}`。挂载后，它会返回该代理的资源哈希。
...
可以使用 Play Asset Delivery 下载并挂载 Live Update 内容。请在[官方手册](https://defold.com/extension-pad/)中了解更多信息。
...
建议为每个挂载点保存元数据，以便应用决定内容包是否应保持挂载。添加挂载点后进行验证，包括应用在启动时重新添加所需挂载点的情况。
一种方法是在 Zip 中添加 `metadata.json` 文件，并在挂载后通过 `sys.load_resource("/metadata.json")` 读取。_请为每个挂载点的自定义数据使用唯一的资源路径，否则将返回最高优先级挂载点中的文件。_
...
挂载点仅在当前引擎会话中有效。重启后，应用必须为每个所需的内容包再次调用 `liveupdate.add_mount()`。如果这些选择需要跨会话保留，请在应用自行管理的持久数据中保存内容包的位置、挂载名称和优先级。

<div class='sidenote' markdown='1'>
挂载 Zip 归档或文件夹不会复制或移动其内容。使用挂载点期间，内容必须保留在指定位置。
</div>

## 使用 Live Update 进行脚本编写

要实际使用 live update 内容，您需要将数据下载并挂载到您的游戏中。
阅读更多关于[如何使用 live update 进行脚本编写的内容](/zh/manuals/live-update-scripting)。
...
强制重新下载资源
: 开发者可以将内容下载到任意文件或文件夹，通常位于应用程序路径下。可以使用 `print(sys.get_save_file("", ""))` 找到应用程序支持文件夹。要强制重新下载，请删除已下载的内容包及应用自行管理状态中的对应记录。引擎不再管理挂载列表；挂载点不会在重启后保留。
