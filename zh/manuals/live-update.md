...
发布 Live Update 内容时，Defold 会自动从打包后的 `game.dmanifest` 中移除仅用于 Live Update 的条目，而发布的 `liveupdate.game.dmanifest` 仍保留完整的资源列表。这会减少包体大小和运行时内存占用。原有的 `liveupdate.exclude_entries_from_main_manifest` 设置已被移除；项目中仍然存在的该条目会被忽略。

在基于归档的工作流程中，`collectionproxy.get_resources()` 会在相关归档挂载前返回 `{}`。挂载后，它会返回该代理的资源哈希。
