...
Defold 比 Unity 小得多。空项目的引擎体积在所有平台上约为 1-3 MB。你可以裁剪掉引擎的额外部分，并把部分游戏内容放入 [Live Update](/zh/manuals/live-update)，以后再单独下载。体积对比以及选择 Defold 的其他原因，可参阅 [Why Defold 页面](https://defold.com/why/)。
...
如需更多细节，请阅读这篇简短的[安装手册](/zh/manuals/install/)。
...
这里会从 Unity 用户最先想了解的角度介绍 Defold Editor，不过我们也建议之后阅读完整的[编辑器概览手册](/zh/manuals/editor)。
...
| [Debugger](/zh/manuals/debugging/) | Visual Studio Debugger | Debugger 开箱即用并完全集成在 Defold 中。还有一个额外标签页用于跟踪、启用和禁用断点。 |
...
更多细节请参阅完整的 [Defold 构建块手册](/zh/manuals/building-blocks/)。
...
更多内容请阅读 [Defold Components 手册](/zh/manuals/components/)。

下表列出类似的 Unity 组件，便于快速查找，并附有各 Defold 组件手册链接：

| Defold | Unity | 差异 |
|---|---|---|
| [Sprite](/zh/manuals/sprite/) | Sprite Renderer | 在 Defold 中，只能通过代码修改 tint（颜色属性）。 |
| [Tilemap](/zh/manuals/tilemap/) | Tilemap / Grid | Defold 内置 Tilemap Editor，支持方形网格（也有例如 [Hexagon](https://github.com/selimanac/defold-hexagon/) 的扩展），但没有内置自动铺瓦规则。[Tiled](https://defold.com/assets/tiled/)、[TileSetter](https://defold.com/assets/tilesetter/) 或 [Sprite Fusion](https://defold.com/assets/spritefusion/) 等工具可导出到 Defold。 |
| [Label](/zh/manuals/label/) | Text / TextMeshPro | 自 Defold 1.13.2 起，Label 组件和 GUI 文本节点支持内置[富文本标记](/zh/manuals/font-richtext/)，可使用颜色、渐变、轮廓和动画效果。此外也有独立的 [RichText 扩展](https://defold.com/assets/richtext/)。 |
| [Sound](/zh/manuals/sound/) | AudioSource | Defold 只有全局声音源（非空间音频）。Defold 有官方 [FMOD 扩展](https://github.com/defold/extension-fmod)。 |
| [Factory](/zh/manuals/factory/) | Prefab Instantiate() | 在 Defold 中，Factory 是带有特定原型（prefab）的组件。 |
| [Collection Factory](/zh/manuals/collection-factory/) | -（无直接组件对应物） | Defold 中的 Collection Factory 组件可以一次生成多个带父子关系的 Game Objects。 |
| [Collision Object](/zh/manuals/physics-objects) | Rigidbody + Collider | 在 Defold 中，物理对象和碰撞形状组合在一个组件中。 |
| [Collision Shapes](/zh/manuals/physics-shapes/)  | BoxCollider / SphereCollider / CapsuleCollider | 在 Defold 中，形状（box、sphere、capsule）配置在 Collision Object 组件内。两者都支持来自 tilemap 和凸包数据的碰撞形状。 |
| [Camera](/zh/manuals/camera/) | Camera | Unity 的相机有更多内置渲染和后处理设置，而 Defold 通过渲染脚本把控制权交给用户。 |
| [GUI](/zh/manuals/gui/) | UI Toolkit / Unity UI / uGUI Canvas | Defold GUI 是用于构建完整 UI 和模板的强大组件。Unity 没有一个等价的单一 UI 组件，而是有多个 UI 框架。Defold 也有 [ImGui 扩展](https://github.com/britzl/extension-imgui)。 |
| [GUI Script](/zh/manuals/gui-script/) | Unity UI / uGUI scripts | Defold GUI 可通过 GUI scripts 使用专用 `gui` API 控制。 |
| [Model](/zh/manuals/model/) | MeshRenderer + Material | 在 Defold 中，Model 组件包含 3D 模型文件、纹理，以及带着色器的材质。 |
| [Mesh](/zh/manuals/mesh/) | MeshRenderer / MeshFilter / Procedural Mesh | 在 Defold 中，Mesh 是一个用于通过代码管理顶点集的组件。它类似 Defold Model，但更底层。 |
| [ParticleFX](/zh/manuals/particlefx/) | Particle System | Defold 的粒子编辑器支持带有许多属性的 2D/3D 粒子效果，并可在 Curve Editor 中用曲线随时间动画化。它没有 Trails 或 Collisions。 |
| [Script](/zh/manuals/script/) | Script | 编程差异会在下文详细解释。 |

#### 扩展和自定义组件

Defold 还通过扩展提供官方 [Spine](/extension-spine/) 和 [Rive](/extension-rive/) 组件。
...
| Defold | Unity | 差异 |
|---|---|---|
| [Atlas](/zh/manuals/atlas/) | Sprite Atlas / Texture2D | Defold 也有 [Texture Packer 扩展](https://defold.com/extension-texturepacker/)。 |
| [Tile source](/zh/manuals/tilesource/) | Tile Palette + Asset | 在 Defold 中，tile source 可作为 tilemap 的纹理，也可用于 sprites 或 particles。 |
| [Font](/zh/manuals/font/) | Font | 由 Defold Label 组件或 GUI 中的文本节点使用，类似 Unity 中的 Text/TextMeshPro。 |
| [Material](/zh/manuals/material/) | Material | 在 Defold 中，着色器被称为 vertex program 和 fragment program。 |
...
Unity scenes 默认共享同一个全局游戏状态和同一个物理模拟，实际上就是同一个 *world*（*game world*）。在 Defold 中，你有两个选择：
...
更多集合信息请阅读 [Building Blocks 手册](/zh/manuals/building-blocks/#collections)。
...
更多扩展内容请阅读 [Defold Native Extensions 手册](/zh/manuals/extensions/)。
...
- [Script 手册](/zh/manuals/script/)
- [Writing code](/zh/manuals/writing-code/)
- [Debugging](/zh/manuals/debugging/)
...
你也可以配置 Defold Editor 偏好设置，让文本文件默认在 VS Code（或其他外部编辑器）中打开。详情见 [Editor Preferences](/zh/manuals/editor-preferences/)。
...
更多着色器内容请阅读 [Shaders 手册](/zh/manuals/shader)。

#### 材质

Defold 使用 `Material` 概念连接 `.fp` 和 `.vp` 着色器、采样器（纹理），以及 Vertex Attributes 或 Constants 等其他内容。

更多材质内容请阅读 [Materials 手册](/zh/manuals/material)。
...
更多细节请阅读 [Message Passing 手册](/zh/manuals/message-passing/)。
...
更多细节请阅读 [Addressing 手册](/zh/manuals/addressing/)。
...
更多细节请阅读 [Factory 手册](/zh/manuals/factory/)。
...
更多细节请阅读 [Collection Factory 手册](/zh/manuals/collection-factory/)。
...
更多细节请阅读 [Resource Management 手册](/zh/manuals/resource/)。
...
更多细节请阅读 [Collection Proxy 手册](/zh/manuals/collection-proxy/)。
...
| `on_input` | Input Methods | 当[脚本设置了输入焦点](/zh/manuals/input/#input-focus)时，Defold 会接收输入。它在更新循环中最先处理。 |
| `fixed_update()` | `FixedUpdate()` | 以固定时间步调用。要在 Defold 中启用它，必须设置 `Use Fixed Timestep`，详见[项目设置](https://defold.com/zh/manuals/project-settings/#use-fixed-timestep)。从 1.12.0 开始，它在 `update()` 前运行。 |
...
更多细节请阅读 [Application Lifecycle 手册](/zh/manuals/application-lifecycle/)。
