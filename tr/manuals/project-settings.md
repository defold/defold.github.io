...
- anchor: runtime-access
  title: Çalışma sırasında erişim
- Bölümler ve ayarlar
- Project
- Bootstrap
- Library
...
![Proje ayarları](/manuals/images/project-settings/settings.jpg)
...
#### Dependencies
Projenin *Library URL* adreslerinin listesi. Daha fazla bilgi için [Kütüphaneler kılavuzuna](/tr/manuals/libraries/) bakın.
...
#### Custom Resources
`custom_resources`
{% include shared/tr/custom-resources.md %}

Özel kaynakların (custom resource) yüklenmesi, [Dosya erişimi kılavuzunda](/tr/manuals/file-access/#how-to-access-files-bundled-with-the-application) daha ayrıntılı açıklanır.

Eklentilerin `custom_resources.default` üzerinden `ext.properties` dosyasında sağladığı yollar, bu ayarla birleştirilir. Bir örnek için [eklentilerin özel kaynaklarına](/tr/manuals/extensions/#custom-resources) bakın.

#### Bundle Resources
`bundle_resources`
{% include shared/tr/bundle-resources.md %}

Dağıtım paketine eklenen kaynakların yüklenmesi, [Dosya erişimi kılavuzunda](/tr/manuals/file-access/#how-to-access-files-bundled-with-the-application) daha ayrıntılı açıklanır.
...
#### Include Dirs
Projenizden kütüphane paylaşımı yoluyla paylaşılacak dizinlerin boşlukla ayrılmış listesi. Daha fazla bilgi için [Kütüphaneler kılavuzuna](/tr/manuals/libraries/) bakın.
...
Bu ayar pencereyi denetler. Ekran dışı [çok örneklemeli işleme hedeflerinin](/tr/manuals/render/#multisampled-render-targets) kendi örnek sayıları vardır.
...
#### Display Profiles
Kullanılacak ekran profilleri dosyasını belirtir; varsayılan olarak `/builtins/render/default.display_profilesc`. Daha fazla bilgi için [GUI yerleşimleri kılavuzuna](/tr/manuals/gui-layouts/#creating-display-profiles) bakın.
...
Bir GUI betiği, [`gui.set_safe_area_mode()`](/ref/gui/#gui.set_safe_area_mode) ile kendi sahnesi için modu geçersiz kılabilir. Platform desteği ve özel yerleşimler için [güvenli alan yönergelerine](/tr/manuals/porting-guidelines/#mobile-phones-and-notch-and-hole-punch-cameras) bakın.
...
#### Launch Screen
Storyboard dosyası (.storyboard). Böyle bir dosyanın nasıl oluşturulacağını [iOS kılavuzundan](/tr/manuals/ios/#creating-a-storyboard) öğrenebilirsiniz.
...
#### Info.plist
Belirtilirse uygulamanız paketlenirken yerleşik iOS temel bildirimi yerine bu *`Info.plist`* dosyası kullanılır. Yerleşik bildirim, yayıma yönelik olmayan derlemelerde düzenleyicinin hedef keşfi için gereken yerel ağ ve Bonjour girdilerini içerir. Özel bir bildirim sağlıyorsanız ve bir cihazda hedef keşfine, profil çıkarmaya, çalışma sırasında yeniden yüklemeye (hot reload) veya günlük akışına ihtiyaç duyuyorsanız bu girdileri [iOS kılavuzunda](/tr/manuals/ios/#creating-an-ios-application-bundle) açıklandığı şekilde koruyun.
...
Defold'un varsayılan kurallarını doğrudan kullanmak için `/builtins/manifests/android/dmengine.keep` dosyasını seçin. Eklentiler kendi [koruma kurallarını](/tr/manuals/extensions/#r8-keep-rules-for-android) sağlar; bu kurallar bu dosyayla birleştirilir.
...
R8'i etkinleştirmek ve karartma eşlemesini yayıma yönelik bir dağıtım paketiyle birlikte saklamak için [Android kılavuzuna](/tr/manuals/android/#shrinking-java-code-with-r8) bakın.
...
#### App Icon
Windows'ta uygulama simgesi olarak kullanılacak görüntü dosyası (.ico). .ico dosyası oluşturma hakkında daha fazla bilgi için [Windows kılavuzuna](/tr/manuals/windows) bakın.

---

### HTML5

Bu seçeneklerin çoğu hakkında daha fazla bilgi için [HTML5 platformu kılavuzuna](/tr/manuals/html5/) bakın.
...
#### Retry Count
Başlangıç sırasında başarısız bir indirmeden sonraki yeniden deneme sayısı; ağ hatalarını, başarısız HTTP durumlarını ve motorun JavaScript veya WebAssembly dosyasındaki boyut uyuşmazlıklarını kapsar. İlk istek ayrı sayılır. Arşiv dosyası doğrulamasının kendi yeniden deneme sınırı vardır; [indirme doğrulamasına](/tr/manuals/html5/#download-verification) ve `Retry Time` ayarına bakın.

#### Retry Time
İndirme başarısız olduğunda dosyayı indirme girişimleri arasında beklenecek saniye sayısı (bkz. `Retry Count`).

#### Verify Downloaded File Size
`html5.verify_downloaded_file_size`, indirilen motor ve arşiv dosyalarını beklenen boyutlarıyla karşılaştırır. Varsayılan olarak etkindir (`true`). Yalnızca bir sunucu, ara sunucu veya CDN dosyaları bilerek yeniden yazıp boyutlarını değiştiriyorsa `false` olarak ayarlayın. Doğrulama başarısız olursa başlatma başarısızlıkla sonuçlanmadan önce indirme yeniden denenir. Motor indirmeleriyle arşiv dosyası doğrulamasının yeniden deneme sınırları farklıdır; [indirme doğrulamasına](/tr/manuals/html5/#download-verification) bakın.
...
`liveupdate.enabled`, çalışma sırasında Live Update sistemini etkinleştirir. Varsayılan olarak etkindir. Kaynakları paket dışında bırakma, indirme ve bağlama hakkında [Live Update kılavuzuna](/tr/manuals/live-update/) bakın.
