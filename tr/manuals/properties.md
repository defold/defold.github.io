...
* Lua betiklerinde tanımlanan, kullanıcı tanımlı betik bileşeni (script component) özellikleri (ayrıntılar için [Betik özellikleri belgelerine](/tr/manuals/script-properties) bakın)
* GUI düğümü özellikleri
* Gölgelendiricilerde (shader) ve materyal (material) dosyalarında tanımlanan gölgelendirici sabitleri (ayrıntılar için [Materyal belgelerine](/tr/manuals/material) bakın)
...
`go.get()`, `go.set()` ve `go.animate()` işlevleri ilk parametre olarak bir başvuru, ikinci parametre olarak bir özellik tanımlayıcısı alır. Başvuru, oyun nesnesini veya bileşeni tanımlar ve bir dize, karma (hash) değeri veya URL olabilir. URL'ler [adresleme kılavuzunda](/tr/manuals/addressing) ayrıntılı olarak açıklanır. Özellik tanımlayıcısı, özelliği adlandıran bir dize veya karma değeridir:
...
| ---------- | -------------------------------------- | --------------- | ---------------- |
| *position* | Oyun nesnesinin yerel konumu. | `vector3`      | `get+set` |
| *rotation* | Oyun nesnesinin `quaternion` olarak ifade edilen yerel dönmesi.  | `quaternion` | `get+set` |
| *euler*    | Oyun nesnesinin Euler açılarıyla ifade edilen yerel dönmesi. | `vector3` | `get+set` |
| *scale*    | Oyun nesnesinin yerel, eşit olmayan ölçeği; her bileşeni ilgili eksen boyunca bir çarpan içeren bir vektör olarak ifade edilir. Z'yi değiştirmeden X ve Y boyutlarını iki katına çıkarmak için `vmath.vector3(2.0, 2.0, 1.0)` kullanın. | `vector3` | `get+set` |
| *scale.xy*    | Oyun nesnesinin X ve Y eksenleri boyunca yerel, eşit olmayan ölçeği. Z ekseninde ölçekleme amaçlanmadığında bu özelliği veya `go.set_scale_xy()` işlevini kullanın. | `vector3` | `get+set` |
...
| özellik   | açıklama                            | tür            |                  |
| ---------- | -------------------------------------- | --------------- | ---------------- |
| *size*     | Sprite bileşeninin ölçeklenmemiş boyutu---kaynak atlastan alınan boyutu. | `vector3` | `get` |
| *image* | Sprite bileşeninin doku yolunun karma değeri. | `hash` | `get`|
| *scale* | Sprite bileşeninin eşit olmayan ölçeği. | `vector3` | `get+set`|
| *scale.xy* | Sprite bileşeninin X ve Y eksenleri boyunca eşit olmayan ölçeği. | `vector3` | `get+set`|
| *material* | Sprite bileşeninin kullandığı materyal. | `hash` | `get+set`|
| *cursor* | Oynatma imlecinin konumu (0--1 arasında). | `number` | `get+set`|
| *playback_rate* | Kare dizisi animasyonunun (flipbook animation) kare hızı. | `number` | `get+set`|

*Çarpışma nesnesi bileşeni özellikleri*

| özellik   | açıklama                            | tür            |                  |
| ---------- | -------------------------------------- | --------------- | ---------------- |
| *mass*     | Çarpışma nesnesinin kütlesi. | `number` | `get` |
| *linear_velocity* | Çarpışma nesnesinin geçerli doğrusal hızı. | `vector3` | `get` |
| *angular_velocity* | Çarpışma nesnesinin geçerli açısal hızı. | `vector3` | `get` |
| *linear_damping* | Çarpışma nesnesinin doğrusal sönümlemesi. | `vector3` | `get+set` |
| *angular_damping* | Çarpışma nesnesinin açısal sönümlemesi. | `vector3` | `get+set` |

*Model (3B) bileşeni özellikleri*

| özellik   | açıklama                            | tür            |                  |
| ---------- | -------------------------------------- | --------------- | ---------------- |
| *animation* | Geçerli animasyon.                | `hash`          | `get`     |
| *texture0*--*texture15* | Modelin doku yollarının karma değerleri. | `hash` | `get+set`|
| *cursor*  | Oynatma imlecinin konumu (0--1 arasında). | `number`   | `get+set` |
| *playback_rate* | Animasyonun oynatma hızı. Animasyonun oynatma hızı için bir çarpandır. | `number` | `get+set` |
| *material* | Modelin kullandığı materyal. | `hash` | `get+set`|

*Etiket bileşeni özellikleri*

| özellik   | açıklama                            | tür            |                  |
| ---------- | -------------------------------------- | --------------- | ---------------- |
| *text* | Etiketin (label) metin içeriği. Defold 1.13.2 sürümünden itibaren kullanılabilir. | `string` | `get+set` |
| *scale* | Etiketin ölçeği. | `vector3` | `get+set` |
| *scale.xy* | Etiketin X ve Y eksenleri boyunca ölçeği. | `vector3` | `get+set`|
| *color*     | Etiketin rengi. | `vector4` | `get+set` |
| *outline* | Etiketin dış çizgi rengi. | `vector4` | `get+set` |
| *shadow* | Etiketin gölge rengi. | `vector4` | `get+set` |
| *size* | Etiketin boyutu. Satır kaydırma etkinse boyut, metni sınırlar. | `vector3` | `get+set` |
| *material* | Etiketin kullandığı materyal. | `hash` | `get+set`|
| *font* | Etiketin kullandığı yazı tipi. | `hash` | `get+set`|
