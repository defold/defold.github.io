...
- anchor: r8-keep-rules-for-android
  title: Android için R8 koruma kuralları
...
- anchor: build-server-logs
  title: Derleme sunucusu günlükleri
- Eklenti örnekleri
---
...
Defold, bulut tabanlı bir derleme çözümüyle yerel kod eklentilerini kullanmaya kurulum gerektirmeden başlamanızı sağlar. Geliştirilip bir oyun projesine doğrudan veya bir [kütüphane projesi](/tr/manuals/libraries/) aracılığıyla eklenen her yerel kod eklentisi, olağan proje içeriğinin bir parçası olur. Motorun özel sürümlerini derleyip ekip üyelerine dağıtmanız gerekmez; bu işlem otomatik olarak yapılır---projeyi derleyip çalıştıran her ekip üyesi, motorun tüm yerel kod eklentilerini içeren, projeye özgü yürütülebilir dosyasını alır.

![Bulutta derleme](/manuals/images/extensions/cloud_build.png)

Defold, bulut derleme sunucusunu herhangi bir kullanım kısıtlaması olmadan ücretsiz sunar. Sunucu Avrupa'da barındırılır ve yerel kodun gönderildiği URL, [düzenleyici tercihleri penceresinde](/tr/manuals/editor-preferences/#extensions) veya [bob](/tr/manuals/bob/#usage) aracının `--build-server` komut satırı seçeneğiyle yapılandırılır. Kendi sunucunuzu kurmak istiyorsanız lütfen [bu yönergeleri izleyin](/tr/manuals/extender-local-setup).
...
```
*ext.manifest*
: Eklenti klasörü bir *ext.manifest* dosyası _içermek zorundadır_. Bu dosya, tek bir eklenti derlenirken kullanılan bayrakları ve tanımları içeren bir yapılandırma dosyasıdır. Dosya biçiminin tanımını [Eklenti bildirimi kılavuzunda](https://defold.com/tr/manuals/extensions-ext-manifests/) bulabilirsiniz.
...
: Bu isteğe bağlı klasör, eklentinin bağımlı olduğu derlenmiş kütüphaneleri içerir. Kütüphane dosyalarının, kütüphanelerinizin desteklediği mimarilere bağlı olarak `platform` veya `architecture-platform` biçiminde adlandırılan alt klasörlere yerleştirilmesi önerilir.
  ...
  {% include shared/tr/platforms.md %}
...
* `android` - Bu klasör, ana uygulamayla birleştirilecek bir bildirim parçası dosyası kabul eder ([burada açıklandığı gibi](/tr/manuals/extensions-manifest-merge-tool)).
  * Klasör, [Gradle tarafından çözümlenecek](/tr/manuals/extensions-gradle) bağımlılıkları içeren bir `build.gradle` dosyası da içerebilir.
...
* `ios` - Bu klasör, ana uygulamayla birleştirilecek bir bildirim parçası dosyası kabul eder ([burada açıklandığı gibi](/tr/manuals/extensions-manifest-merge-tool)).
  * Klasör, [Cocoapods tarafından çözümlenecek](/tr/manuals/extensions-cocoapods) bağımlılıkları içeren bir `Podfile` dosyası da içerebilir.
* `osx` - Bu klasör, ana uygulamayla birleştirilecek bir bildirim parçası dosyası kabul eder ([burada açıklandığı gibi](/tr/manuals/extensions-manifest-merge-tool)).
* `web` - Bu klasör, ana uygulamayla birleştirilecek bir bildirim parçası dosyası kabul eder ([burada açıklandığı gibi](/tr/manuals/extensions-manifest-merge-tool)).
...
Bu kurallar, [R8 etkinleştirildiğinde](/tr/manuals/android/#enabling-r8) projenin seçilen koruma dosyasıyla birleştirilir.
...
Örneğin `/myextension/data/settings.json` konumuna bir JSON dosyası yerleştirin. Yol, eklenti klasörünü de içerir ve projenin kök dizinine göredir. Eklentiyi kütüphane olarak paylaşırken, kütüphaneyi kullanan projelerin eklentiyi ve verilerini alması için kütüphanenin [Include Dirs](/tr/manuals/libraries/#setting-up-library-sharing) ayarına `myextension` ekleyin.
...
![Bildirim](/manuals/images/extensions/manifest.png)
...
![C++ dosyası](/manuals/images/extensions/cppfile.png)
