...
- anchor: editor-scripts-in-libraries
  title: Kütüphanelerdeki düzenleyici betikleri
- anchor: preferences
  title: Tercihler
- anchor: execution-modes
  title: Yürütme modları
- Eylemler
...
- `editor.ui.*` — kullanıcı arayüzüyle ilgili çeşitli işlevler; [Kullanıcı arayüzü kılavuzuna](/tr/manuals/editor-scripts-ui) bakın.
...
- `on_bundle_started(opts)` — bir dağıtım paketi (bundle) oluşturduğunuzda veya oyunun HTML5 sürümünü derlediğinizde yürütülür. `on_build_started` için olduğu gibi, bu kancanın tetiklediği değişiklikler dağıtım paketine yansır ve hatalar paketlemeyi durdurur. `opts` şu anahtarlara sahip olur:
  ...
  - `platform` — oyunun hangi platform için paketlendiği. Olası platform değerlerinin listesi için [Bob kılavuzuna](/tr/manuals/bob) bakın.
...
- `languages` (zorunlu) — sunucunun ilgilendiği, [burada](https://code.visualstudio.com/docs/languages/identifiers#known-language-identifiers) tanımlanan dillerin listesi (dosya uzantıları da kullanılabilir);
