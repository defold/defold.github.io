...
- Androide özgü boyut optimizasyonları
---
...
Uygulamanızın boyutunu hangi öğelerin oluşturduğunu daha iyi anlamak için paketleme sırasında [bir derleme raporu oluşturabilirsiniz](/tr/manuals/bundling/#build-reports). Oyun boyutunun büyük bölümünü seslerin ve grafiklerin oluşturması oldukça yaygındır.
...
## Motor boyutunu optimize etme
Motor boyutunu küçültmenin hızlı bir yolu, motorda kullanmadığınız işlevleri kaldırmaktır. Bu işlem, ihtiyaç duymadığınız motor bileşenlerini kaldırmanıza olanak tanıyan [uygulama bildirimi dosyasında](https://defold.com/tr/manuals/app-manifest/) yapılır. Örnekler:

* Fizik - Oyununuz Box2D veya Bullet3D fiziğini kullanmıyorsa fizik motorlarını kaldırmanız kuvvetle önerilir
* GUI, parçacık efektleri ve karo haritaları - Bu bileşenler, [App Manifest bileşen anahtarlarıyla](/tr/manuals/app-manifest/#exclude-gui) ayrı ayrı dışarıda bırakılabilir. Dışarıda bıraktığınız her özelliğe ait bileşen başvurularını ve API çağrılarını kaldırın. Parçacık efektlerini dışarıda bırakmak, GUI sahnelerindeki parçacık düğümlerinin desteğini de kaldırır.
* Zengin metin - Etiketlerin ve GUI metinlerinin yalnızca düz metne ihtiyacı varsa [Use Rich Text](/tr/manuals/app-manifest/#use-rich-text) ayarını devre dışı bırakın. Bu işlem, normal metin işlemeyi (rendering) korurken zengin metin ayrıştırmasını ve stil efektlerini kaldırır.
...
Defold, 8 bit ve 16 bit PCM Wave dosyalarını destekler. Ogg Vorbis ve Ogg Opus, bir PCM bit derinliği gereksinimi yerine kendi sıkıştırılmış biçimlerini kullanır. Opus kod çözücüsü varsayılan olarak dahil edilmez; `.opus` kaynaklarını (resources) kullanmadan önce [App Manifest](/tr/manuals/app-manifest/#sound) içinde **Include Sound Decoder: Opus** ayarını etkinleştirin.
...
### Yazı tiplerini optimize etme
Kullanacağınız simgeleri belirleyip All Chars onay kutusunu kullanmak yerine bunları [Characters](/tr/manuals/font/#properties) alanında belirtirseniz yazı tiplerinizin boyutu daha küçük olur.
...
[APK genişletme dosyalarını](https://developer.android.com/google/play/expansion-files) ve [Live Update içeriğini](/tr/manuals/live-update), [Asset Portal'daki APKX uzantısı](https://defold.com/assets/apkx/) sayesinde birlikte de kullanabilirsiniz.