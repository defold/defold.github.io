...
Bob varsayılan olarak geçerli dizinde bir proje arar ve `game.project` dosyasından erişilebilen kaynakları (resource) derleyerek *build/default* dizinine yerleştirir. Başvurusu olmayan kaynaklar derlenmez. Kod, çalışma sırasında ham dosyaları yollarını kullanarak yüklüyorsa bunları [Custom Resources proje ayarı](/tr/manuals/project-settings/#custom-resources) aracılığıyla projeye dahil edin; diğer Defold kaynakları için bir derleme kökünden erişilebilen bir başvuru gerekir.
