...
Lua programlamaya giriş yapmak istiyorsanız [Defold'da Lua kılavuzumuza](/tr/manuals/lua) göz atın.
...
{% include shared/tr/install.md %}

Düzenleyici kurulup başlatıldıktan sonra yeni bir proje oluşturup hazırlama zamanı gelmiştir. "Empty Project" şablonundan [yeni bir proje](/tr/manuals/project-setup/#creating-a-new-project) oluşturun.
...
![Düzenleyiciye genel bakış](/tutorials/images/runner/1/editor2_overview.png)
...
![Derleme](/tutorials/images/runner/1/build_and_launch.png)
...
![Kaydedilmemiş değişiklikler içeren dosya](/tutorials/images/runner/1/file_changed.png)
</div>
...
![Sağ tıklayıp sürüm bağlantısını kopyalama](/tutorials/images/runner/extension-spine-releases.png)

Sürüm bağlantısını [game.project bağımlılıkları](/tr/manuals/libraries/#setting-up-library-dependencies) listenize ekleyin. Spine eklentisi eklendikten sonra, eklentiyle birlikte gelen düzenleyici entegrasyonunu etkinleştirmek için düzenleyiciyi yeniden başlatmanız da gerekir.
...
2. Zemin dokularını tutmak için yeni bir *Atlas* dosyası oluşturun (*Assets* bölmesinde uygun bir klasöre, örneğin *main* klasörüne sağ tıklayıp <kbd>New ▸ Atlas File</kbd> seçeneğini seçin). Atlas dosyasına *level.atlas* adını verin.
  ...
  <div class='sidenote' markdown='1'>
  Bir *atlas* (Atlas), ayrı görüntüleri tek ve daha büyük bir görüntü dosyasında birleştiren bir dosyadır. Bunun amacı yerden tasarruf etmek ve performansı artırmaktır. Atlaslar ve diğer 2B grafik özellikleri hakkında daha fazla bilgiyi [2B grafik belgelerinde](/tr/manuals/2dgraphics) bulabilirsiniz.
  </div>
...
3. *Outline* görünümünde atlasın köküne sağ tıklayıp <kbd>Add Images</kbd> seçeneğini seçerek zemin görüntülerini yeni atlasa ekleyin. İçe aktardığınız görüntüleri seçip *OK* düğmesine tıklayın. Artık atlastaki her görüntüye sprite bileşenlerinde, parçacık efektlerinde ve diğer görsel öğelerde kullanılacak tek karelik bir animasyon (durağan görüntü) olarak erişilebilir. Dosyayı kaydedin.
  ...
  ![Yeni atlas oluşturma](/tutorials/images/runner/1/new_atlas.png)

  ![Atlasa görüntü ekleme](/tutorials/images/runner/1/add_images_to_atlas.png)
...
5. Her nesneye bir sprite bileşeni ekleyin (*Outline* görünümünde oyun nesnesine sağ tıklayıp <kbd>Add Component</kbd> seçeneğini seçin, ardından *Sprite* seçeneğini seçip *OK* düğmesine tıklayın), sprite bileşeninin *Image* özelliğini az önce oluşturduğunuz atlasa ayarlayın ve sprite bileşeninin varsayılan animasyonunu iki zemin görüntüsünden birine ayarlayın. _Sprite bileşeninin_ (oyun nesnesinin değil) X konumunu 190, Y konumunu 40 yapın. Görüntünün genişliği 380 piksel olduğundan ve onu bunun yarısı kadar piksel yana kaydırdığımızdan, oyun nesnesinin dayanak noktası sprite görüntüsünün en sol kenarında olacaktır.
  ...
  ![Zemin koleksiyonu oluşturma](/tutorials/images/runner/1/ground_collection.png)
...
6. Kullandığımız grafikler biraz fazla büyük olduğundan her oyun nesnesini %60'a ölçekleyin (X ve Y için 0.6 ölçeği, 228 piksel genişliğinde zemin parçaları verir).
  ...
  ![Zemini ölçekleme](/tutorials/images/runner/1/scale_ground.png)
...
9. Her şeyin yerinde olduğunu görmek için oyunu başlatın (<kbd>Project ▸ Build</kbd>).
  ...
  ![Durağan zemin](/tutorials/images/runner/1/still_ground.png)
...
Şimdilik bu açıklamalar muhtemelen yeterlidir. Yine de bu konuların çok daha kapsamlı bir incelemesini [Yapı taşları kılavuzunda](/tr/manuals/building-blocks) bulabilirsiniz. Defold'un nasıl çalıştığını daha derinden anlamak için daha sonra bu kılavuzu okumanız iyi olacaktır.
...
Defold, Lua'nın üzerine bir dizi API ve oyun nesneleri arasındaki iletişimi programlamanızı sağlayan bir _ileti aktarımı_ (message passing) sistemi ekler. Bunun nasıl çalıştığına ilişkin ayrıntılar için [İleti aktarımı kılavuzuna](/tr/manuals/message-passing) bakın.
</div>
...
![Zemin denetleyicisi](/tutorials/images/runner/1/ground_controller.png)
...
![Kahraman atlası](/tutorials/images/runner/2/hero_atlas.png)
...
![Spine modeli özellikleri](/tutorials/images/runner/2/spinemodel_properties.png)
...
<div class='sidenote' markdown='1'>
Kinematik çarpışma ("Kinematic"), çarpışmaların algılanmasını istediğimiz, ancak fizik motorunun çarpışmaları otomatik olarak çözmeyeceği ve nesnelerin simülasyonunu yapmayacağı anlamına gelir. Fizik motoru birkaç farklı çarpışma nesnesi türünü destekler. Bunlar hakkında daha fazla bilgiyi [Fizik belgelerinde](/tr/manuals/physics) bulabilirsiniz.
</div>
...
![Kahraman oyun nesnesi](/tutorials/images/runner/2/hero_game_object.png)
...
Bunu yapmak ve çarpışmaları doğru biçimde ele almak için biraz vektör matematiği gerekir. Kinematik çarpışmaların nasıl çözüleceğine ilişkin ayrıntılı bir açıklama [Fizik belgelerinde](/tr/manuals/physics-resolving-collisions/) bulunur.
</div>
...
![Girdi eşlemeleri](/tutorials/images/runner/2/input_bindings.png)
...
![Betik özelliği](/tutorials/images/runner/2/script_property.png)
...
![Zemin kodunu ekleme](/tutorials/images/runner/insert_ground_code.png)
...
![Zemin çarpışması](/tutorials/images/runner/2/ground_collision.png)
...
![Platform](/tutorials/images/runner/2/platform_long.png)
...
![Oyunu çalıştırma](/tutorials/images/runner/2/run_game.png)
...
![Kahraman kodunu ekleme](/tutorials/images/runner/insert_hero_code.png)
...
8. Dosyayı kaydedin.
    ...
    ![Platform dikenleri](/tutorials/images/runner/3/danger_edges.png)
...
9. *hero.go* dosyasını açın, *Collision Object* bileşenini seçip *Mask* özelliğine "danger" adını ekleyin. Ardından dosyayı kaydedin.
    ...
    ![Kahraman çarpışması](/tutorials/images/runner/3/hero_collision.png)
...
3. *platform.script* dosyasını açıp en sol kenara ulaşan bir platformu yalnızca silmek yerine, bölüm denetleyicisine platformun kaldırılmasını isteyen bir ileti gönderecek şekilde değiştirin:
    ...
    ![Platform kodunu ekleme](/tutorials/images/runner/insert_platform_code.png)
...
![Kahraman kodunu ekleme](/tutorials/images/runner/insert_hero_code_2.png)
...
10. Betik dosyasını para nesnesine bir *Script* bileşeni olarak ekleyin (*Outline* görünümündeki köke sağ tıklayıp <kbd>Add Component from File</kbd> seçeneğini seçin).
    ...
    ![Para oyun nesnesi](/tutorials/images/runner/3/coin.png)
...
![Para fabrikası](/tutorials/images/runner/3/coin_factory.png)
...
![Denetleyici kodunu ekleme](/tutorials/images/runner/insert_controller_code.png)
...
> Projenin tamamlanmış sürümünü [buradan](/tutorials/images/runner/sample-runner.zip) indirin
