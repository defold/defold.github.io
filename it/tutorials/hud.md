---
brief: In questo progetto di esempio impari a creare effetti per il conteggio del punteggio.
layout: tutorial
locale: it
title: Esempio di codice per un HUD
---

# HUD - progetto di esempio {#hud-sample-project}

<iframe width="560" height="315" src="https://www.youtube.com/embed/NoPHHG2kbOk" frameborder="0" allowfullscreen></iframe>

In questo progetto di esempio, che puoi [aprire dall'editor](/it/manuals/project-setup/) o [scaricare da GitHub](https://github.com/defold/sample-hud), mostriamo alcuni effetti per il conteggio del punteggio. I punteggi appaiono in posizioni casuali sullo schermo, simulando un gioco in cui il giocatore ottiene punti in posizioni diverse.

I punteggi fluttuano per un po' dopo essere apparsi. Per ottenere questo effetto, li rendiamo trasparenti e poi ne facciamo apparire gradualmente il colore. Li animiamo anche verso l'alto. Questo avviene in `on_message()` qui sotto.

Poi si spostano verso il punteggio totale nella parte superiore dello schermo, dove vengono sommati.
Inoltre, mentre salgono, diventano leggermente più trasparenti. Questo avviene in `float_done()`.

Quando raggiungono il punteggio in alto, i loro valori vengono aggiunti a un punteggio obiettivo, verso il quale il punteggio totale aumenta gradualmente. Questo avviene in `swoosh_done()`.

A ogni aggiornamento, lo script controlla se il punteggio obiettivo è aumentato e se il punteggio totale deve crescere. In tal caso, il punteggio totale viene aumentato di un piccolo incremento.
La scala del punteggio totale viene quindi animata per creare un effetto di rimbalzo. Questo avviene in `update()`.

Ogni volta che il totale aumenta, generiamo alcune stelle più piccole e le animiamo in modo che si allontanino dal punteggio totale. Le stelle vengono generate, animate ed eliminate in `spawn_stars()`, `fade_out_star()` e `delete_star()`.
...
In main.script riceviamo l'input dal tocco o dal mouse, quindi inviamo un messaggio allo script GUI che crea nuovi punteggi usando la posizione del tocco.
