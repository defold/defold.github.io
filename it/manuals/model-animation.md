...
Per i dettagli su come importare dati 3D in un modello da animare, consulta la [documentazione sui modelli](/it/manuals/model).

  ![Animazione in Blender](/manuals/images/animation/blender_animation.png)
  ![Oscillazione in ciclo](/manuals/images/animation/suzanne.gif)
...
Oltre a usare `model.play_anim()` per far avanzare l'animazione di un modello, i componenti *Model* espongono una proprietà `cursor` che può essere modificata con `go.animate()` (maggiori informazioni sulle [animazioni delle proprietà](/it/manuals/property-animation)):
