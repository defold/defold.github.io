...
- anchor: editor-scripts
  title: Script delleditor
- anchor: editor-script-runtime
  title: Runtime degli script delleditor
- anchor: anatomy-of-editor_script
  title: Struttura di .editor_script
- anchor: editor-api
  title: API delleditor
- anchor: commands
  title: Comandi
- anchor: use-commands-to-change-the-in-memory-editor-state
  title: Usare i comandi per modificare lo stato delleditor in memoria
- anchor: use-commands-with-the-active-editor-view
  title: Usare i comandi con la vista attiva delleditor
- anchor: use-shell-commands
  title: Usare i comandi della shell
- anchor: lifecycle-hooks
  title: Hook del ciclo di vita
- anchor: language-servers
  title: Server di linguaggio
- anchor: http-server
  title: Server HTTP
- anchor: editor-scripts-in-libraries
  title: Script delleditor nelle librerie
- anchor: preferences
  title: Preferenze
...
- anchor: non-undoable-actions
  title: Azioni non annullabili
- anchor: mixing-actions-and-side-effects
  title: Combinare azioni ed effetti collaterali
---
...
- `editor.ui.*` — varie funzioni relative all'interfaccia utente; consulta il [manuale dell'interfaccia utente](/it/manuals/editor-scripts-ui).
- `editor.prefs.*` — funzioni per interagire con le preferenze dell'editor; consulta [preferenze](#preferences).

Puoi trovare [qui](/ref/editor/) il riferimento completo dell'API dell'editor.
...
- `on_bundle_started(opts)` — eseguito quando crei un bundle o una build HTML5 del gioco. Come per `on_build_started`, le modifiche attivate da questo hook saranno presenti nel bundle e gli errori ne interromperanno la creazione. `opts` contiene queste chiavi:
  ...
  - `platform` — piattaforma per cui viene creato il bundle del gioco. Consulta l'elenco dei possibili valori della piattaforma nel [manuale di Bob](/it/manuals/bob).
...
- `languages` (obbligatorio) — un elenco dei linguaggi a cui il server è interessato, come definiti [qui](https://code.visualstudio.com/docs/languages/identifiers#known-language-identifiers) (sono accettate anche le estensioni dei file);
