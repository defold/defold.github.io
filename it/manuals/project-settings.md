...
- anchor: runtime-access
  title: Accesso a runtime
- anchor: sections-and-settings
  title: Sezioni e impostazioni
- Project
- Bootstrap
- Library
- Engine
...
- anchor: setting-config-values-on-engine-startup
  title: Impostare i valori di configurazione allavvio del motore
- anchor: custom-project-settings
  title: Impostazioni personalizzate del progetto
---
...
![Impostazioni del progetto](/manuals/images/project-settings/settings.jpg)
...
#### Dependencies
Un elenco degli URL *Library URL* del progetto. Per ulteriori informazioni, consulta il [manuale delle librerie](/it/manuals/libraries/).
...
#### Custom Resources
`custom_resources`
{% include shared/it/custom-resources.md %}

Il caricamento delle risorse personalizzate è descritto più in dettaglio nel [manuale sull'accesso ai file](/it/manuals/file-access/#how-to-access-files-bundled-with-the-application).

I percorsi forniti dalle estensioni tramite `custom_resources.default` in `ext.properties` vengono combinati con questa impostazione. Consulta [Risorse personalizzate delle estensioni](/it/manuals/extensions/#custom-resources) per un esempio.

#### Bundle Resources
`bundle_resources`
{% include shared/it/bundle-resources.md %}

Il caricamento delle risorse del bundle è descritto più in dettaglio nel [manuale sull'accesso ai file](/it/manuals/file-access/#how-to-access-files-bundled-with-the-application).
...
#### Include Dirs
Un elenco separato da spazi delle directory del progetto da condividere tramite la condivisione delle librerie. Per ulteriori informazioni, consulta il [manuale delle librerie](/it/manuals/libraries/).
...
Questa impostazione controlla la finestra. I [render target multicampionati](/it/manuals/render/#multisampled-render-targets) fuori schermo hanno un proprio numero di campioni.
...
#### Display Profiles
Specifica il file dei profili di visualizzazione da usare; il valore predefinito è `/builtins/render/default.display_profilesc`. Per approfondire, consulta il [manuale dei layout GUI](/it/manuals/gui-layouts/#creating-display-profiles).
...
#### WebGL Version Hint
`graphics.webgl_version_hint` seleziona la versione del contesto WebGL da richiedere per HTML5. I valori validi sono `1` (WebGL 1) e `2` (WebGL 2, il valore predefinito). Impostalo su `1` per usare o provare WebGL 1 anche in un browser che supporta WebGL 2. Mantieni disabilitato [Exclude GLES 2.0](#exclude-gles-20) quando usi WebGL 1, affinché vengano inclusi gli shader necessari.
...
Uno script GUI può sovrascrivere la modalità per la propria scena con [`gui.set_safe_area_mode()`](/ref/gui/#gui.set_safe_area_mode). Consulta le [indicazioni sull'area sicura](/it/manuals/porting-guidelines/#mobile-phones-and-notch-and-hole-punch-cameras) per il supporto delle piattaforme e i layout personalizzati.
...
#### Launch Screen
Il file storyboard (.storyboard). Per sapere come crearne uno, consulta il [manuale iOS](/it/manuals/ios/#creating-a-storyboard).
...
Seleziona `/builtins/manifests/android/dmengine.keep` per usare direttamente le regole predefinite di Defold. Le estensioni forniscono le proprie [regole di conservazione](/it/manuals/extensions/#r8-keep-rules-for-android), che vengono combinate con questo file.
...
Consulta il [manuale Android](/it/manuals/android/#shrinking-java-code-with-r8) per abilitare R8 e conservare la mappatura dell'offuscamento insieme a un bundle di release.
...
#### App Icon
Il file immagine (.ico) da usare come icona dell'applicazione su Windows. Per sapere come creare un file .ico, consulta il [manuale Windows](/it/manuals/windows).

---

### HTML5

Consulta il [manuale della piattaforma HTML5](/it/manuals/html5/) per ulteriori informazioni su molte di queste opzioni.
...
#### Retry Count
Il numero di nuovi tentativi dopo un download fallito durante l'avvio, inclusi errori di rete, stati HTTP di errore e dimensioni non corrispondenti nel file JavaScript o WebAssembly del motore. La richiesta iniziale è separata. La verifica dei file di archivio ha un proprio limite di tentativi; consulta [Verifica dei download](/it/manuals/html5/#download-verification) e `Retry Time`.
...
`html5.verify_downloaded_file_size` confronta i file scaricati del motore e dell'archivio con le dimensioni previste. Abilitata per impostazione predefinita (`true`). Impostala su `false` solo se un server, un proxy o una CDN riscrive intenzionalmente i file e ne cambia le dimensioni. Una verifica fallita causa nuovi tentativi di download prima che l'avvio fallisca. I limiti di tentativi sono diversi per i download del motore e la verifica dei file di archivio; consulta [Verifica dei download](/it/manuals/html5/#download-verification).
...
`liveupdate.enabled` abilita il sistema Live update a runtime. Abilitata per impostazione predefinita. Consulta il [manuale Live update](/it/manuals/live-update/) per sapere come escludere, scaricare e montare le risorse.
