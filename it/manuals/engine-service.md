...
È distinto dal [server dell'editor](/it/manuals/editor-http-api), che appartiene all'editor Defold e controlla il progetto aperto.
...
![Informazioni sulla porta del servizio del motore in una build di debug Defold](/manuals/images/automation/engine-service.png)
...
![Accesso al server del motore](/manuals/images/automation/engine-server.png)
...
La route `/post` è utilizzata da operazioni di sviluppo come hot reload, riavvio, ridimensionamento e controllo dei processi. Il relativo corpo è un messaggio binario Protobuf del tipo indicato nella route; non è un'API per messaggi JSON. Il messaggio Protobuf serializzato non può superare i 1024 byte; in caso contrario, viene restituito `400 Too large message`.
...
Script a runtime, fisica, input, oggetti creati dinamicamente e rendering della piattaforma richiedono un motore in esecuzione e dovrebbero essere verificati tramite [test automatici a runtime](/it/manuals/automated-testing).
