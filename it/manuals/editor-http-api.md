...
![La pagina iniziale del server locale dell'editor](/manuals/images/automation/editor_server.png)
...
Una build completata non significa che il gioco abbia finito di caricarsi nel browser. Attendi che il canvas e l'applicazione siano pronti prima di inviare input o verificare il gameplay. Per ulteriori dettagli, consulta [Test nel browser per HTML5](/it/manuals/automated-testing/#browser-tests-for-html5).
...
![Anteprima del modello del cubo renderizzata dall'editor](/manuals/images/automation/cube-preview.png)
...
<div class='important' markdown='1'>
Un'anteprima dell'editor non è una schermata del gioco in esecuzione. Non verifica gli oggetti creati dinamicamente, la post-elaborazione a runtime o il rendering specifico della piattaforma. Utilizza una [schermata a runtime](/it/manuals/automated-testing/#editor-previews-and-runtime-screenshots) quando sono necessari questi elementi.
</div>
...
Il codice valutato può utilizzare l'[API dell'editor](https://defold.com/ref/editor-lua/) e l'ambiente di scripting dell'editor. Non può utilizzare le API di runtime del gioco come `go.*` per manipolare un gioco in esecuzione. Per il gameplay, utilizza un test a runtime, il debugger, un test nel browser o un'[API di automazione a runtime](/it/manuals/engine-service/#automation-bridge-extension).
...
Il server dell'editor appartiene al processo dell'editor. Un gioco in esecuzione dispone di una porta diversa e di responsabilità differenti, descritte nel [manuale del servizio del motore e dell'API HTTP a runtime](/it/manuals/engine-service).