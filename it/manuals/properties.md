...
- anchor: game-object-and-component-properties
  title: Proprietà degli oggetti di gioco e dei componenti
- anchor: gui-node-properties
  title: Proprietà dei nodi GUI
---
...
* Proprietà dei componenti script definite dall'utente negli script Lua (consulta la [documentazione sulle proprietà degli script](/it/manuals/script-properties) per i dettagli)
* Proprietà dei nodi GUI
* Costanti degli shader definite negli shader e nei file dei materiali (consulta la [documentazione sui materiali](/it/manuals/material) per i dettagli)
...
Le funzioni `go.get()`, `go.set()` e `go.animate()` accettano un riferimento come primo parametro e un identificatore di proprietà come secondo. Il riferimento identifica l'oggetto di gioco o il componente e può essere una stringa, un hash o un URL. Gli URL sono spiegati in dettaglio nel [manuale sull'indirizzamento](/it/manuals/addressing). L'identificatore della proprietà è una stringa o un hash che ne specifica il nome:
...
| ---------- | -------------------------------------- | --------------- | ---------------- |
| *position* | La posizione locale dell'oggetto di gioco. | `vector3`      | `get+set` |
| *rotation* | La rotazione locale dell'oggetto di gioco, espressa come `quaternion`.  | `quaternion` | `get+set` |
| *euler*    | La rotazione locale dell'oggetto di gioco, espressa in angoli di Eulero. | `vector3` | `get+set` |
| *scale*    | La scala locale non uniforme dell'oggetto di gioco, espressa come un vettore in cui ogni componente contiene un moltiplicatore lungo il rispettivo asse. Per raddoppiare le dimensioni lungo X e Y senza modificare Z, usa `vmath.vector3(2.0, 2.0, 1.0)`. | `vector3` | `get+set` |
| *scale.xy*    | La scala locale non uniforme dell'oggetto di gioco lungo gli assi X e Y. Usa questa proprietà o `go.set_scale_xy()` quando non vuoi modificare la scala lungo Z. | `vector3` | `get+set` |
...
| proprietà   | descrizione                            | tipo            |                  |
| ---------- | -------------------------------------- | --------------- | ---------------- |
| *size*     | Le dimensioni dello sprite prima dell'applicazione della scala, così come sono definite nell'atlas di origine. | `vector3` | `get` |
| *image* | L'hash del percorso della texture dello sprite. | `hash` | `get`|
| *scale* | La scala non uniforme dello sprite. | `vector3` | `get+set`|
| *scale.xy* | La scala non uniforme dello sprite lungo gli assi X e Y. | `vector3` | `get+set`|
| *material* | Il materiale usato dallo sprite. | `hash` | `get+set`|
| *cursor* | La posizione (tra 0--1) del cursore di riproduzione. | `number` | `get+set`|
| *playback_rate* | La frequenza dei fotogrammi dell'animazione flipbook. | `number` | `get+set`|

*PROPRIETÀ DEL COMPONENTE OGGETTO DI COLLISIONE*

| proprietà   | descrizione                            | tipo            |                  |
| ---------- | -------------------------------------- | --------------- | ---------------- |
| *mass*     | La massa dell'oggetto di collisione. | `number` | `get` |
| *linear_velocity* | La velocità lineare attuale dell'oggetto di collisione. | `vector3` | `get` |
| *angular_velocity* | La velocità angolare attuale dell'oggetto di collisione. | `vector3` | `get` |
| *linear_damping* | Lo smorzamento lineare dell'oggetto di collisione. | `vector3` | `get+set` |
| *angular_damping* | Lo smorzamento angolare dell'oggetto di collisione. | `vector3` | `get+set` |

*PROPRIETÀ DEL COMPONENTE MODELLO (3D)*

| proprietà   | descrizione                            | tipo            |                  |
| ---------- | -------------------------------------- | --------------- | ---------------- |
| *animation* | L'animazione attuale.                | `hash`          | `get`     |
| *texture0*--*texture15* | Gli hash dei percorsi delle texture del modello. | `hash` | `get+set`|
| *cursor*  | La posizione (tra 0--1) del cursore di riproduzione. | `number`   | `get+set` |
| *playback_rate* | La velocità di riproduzione dell'animazione. Un moltiplicatore della velocità di riproduzione dell'animazione. | `number` | `get+set` |
| *material* | Il materiale usato dal modello. | `hash` | `get+set`|

*PROPRIETÀ DEL COMPONENTE ETICHETTA (LABEL)*

| proprietà   | descrizione                            | tipo            |                  |
| ---------- | -------------------------------------- | --------------- | ---------------- |
| *text* | Il contenuto testuale dell'etichetta. Disponibile da Defold 1.13.2. | `string` | `get+set` |
| *scale* | La scala dell'etichetta. | `vector3` | `get+set` |
| *scale.xy* | La scala dell'etichetta lungo gli assi X e Y. | `vector3` | `get+set`|
| *color*     | Il colore dell'etichetta. | `vector4` | `get+set` |
| *outline* | Il colore del contorno dell'etichetta. | `vector4` | `get+set` |
| *shadow* | Il colore dell'ombra dell'etichetta. | `vector4` | `get+set` |
| *size* | Le dimensioni dell'etichetta. Le dimensioni limitano il testo se il ritorno a capo è abilitato. | `vector3` | `get+set` |
| *material* | Il materiale usato dall'etichetta. | `hash` | `get+set`|
| *font* | Il font usato dall'etichetta. | `hash` | `get+set`|
