...
- anchor: store-materials
  title: Materiali per lo store
- anchor: app-icon
  title: Icona dellapp
- anchor: store-banners-and-images
  title: Banner e immagini dello store
- anchor: save-games
  title: Salvataggi del gioco
- anchor: save-games-on-desktop-mobile-and-web
  title: Salvataggi su desktop, dispositivi mobili e web
- anchor: save-games-on-console
  title: Salvataggi su console
- anchor: build-artifacts
  title: Artefatti di build
- anchor: application-optimizations
  title: Ottimizzazione dellapplicazione
- anchor: performance
  title: Prestazioni
- anchor: screen-resolution-and-refresh-rate
  title: Risoluzione dello schermo e frequenza di aggiornamento
- anchor: mobile-phones-and-notch-and-hole-punch-cameras
  title: Telefoni cellulari con notch o fotocamere in un foro nello schermo
- anchor: platform-specific-guidelines
  title: Linee guida specifiche per piattaforma
- Android
- anchor: consoles
  title: Console
- Nintendo Switch
...
## Input
Assicurati di adattare il gioco ai metodi di input della piattaforma. Valuta l'aggiunta del supporto per i [gamepad](/it/manuals/input-gamepads) se la piattaforma li supporta! Assicurati inoltre che il gioco disponga di un menu di pausa: se un controller si disconnette all'improvviso, il gioco dovrebbe essere messo in pausa!
...
Solo iOS: assicurati di specificare [Localizations](/it/manuals/project-settings/#localizations) in `game.project`, poiché `sys.get_info()` non restituirà mai una lingua che non sia presente in questo elenco.
</div>
...
Assicurati di [generare i simboli di debug](/it/manuals/debugging-native-code/#symbolicate-a-callstack) per ogni versione pubblicata, in modo da poter analizzare i crash. Conservali insieme al bundle dell'applicazione.

## Ottimizzazione dell'applicazione {#application-optimizations}

Leggi il [manuale sull'ottimizzazione](/it/manuals/optimization) per sapere come ottimizzare l'applicazione in termini di prestazioni, dimensioni, uso della memoria e consumo della batteria.



## Prestazioni {#performance}
Esegui sempre i test sull'hardware di destinazione! Controlla le prestazioni del gioco e ottimizzalo se necessario. Usa il [profilatore](/it/manuals/profiling) per individuare i colli di bottiglia nel codice.
...
### Android
Assicurati di conservare il tuo [keystore](/it/manuals/android/#creating-a-keystore) in un luogo sicuro, così da poter aggiornare il gioco.
