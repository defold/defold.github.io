...
- anchor: script-execution-callbacks-and-self
  title: Esecuzione degli script, callback e self
- anchor: reactive-logic
  title: Logica reattiva
- anchor: preprocessing
  title: Preelaborazione
- anchor: editor-support
  title: Supporto dell’editor
---

# Script {#scripts}

I componenti script consentono di creare la logica di gioco usando il [linguaggio di programmazione Lua](/it/manuals/lua).
...
Script di rendering
: Estensione _.render_script_. Vengono eseguiti dalla pipeline di rendering e contengono la logica necessaria per disegnare tutta la grafica dell’applicazione o del gioco a ogni fotogramma. Lo script di rendering occupa una posizione particolare nel ciclo di vita del gioco. Puoi trovare i dettagli nella [documentazione sul ciclo di vita dell’applicazione](/it/manuals/application-lifecycle). Gli script di rendering hanno accesso alle funzioni [Render](/ref/render) e a tutte le funzioni delle librerie Defold, tranne quelle di [GO](/ref/go) e [GUI](/ref/gui).
...
Quando vengono inviati messaggi al componente script tramite [`msg.post()`](/ref/msg#msg.post), il motore chiama questa funzione del componente ricevente. Scopri [di più sullo scambio di messaggi](/it/manuals/message-passing).
...
Se questo componente ha acquisito il focus dell’input (vedi [`acquire_input_focus`](/ref/go/#acquire_input_focus)), il motore chiama questa funzione quando viene rilevato un input. Scopri [di più sulla gestione dell’input](/it/manuals/input).
...
#### `on_reload(self)`
Questa funzione viene chiamata quando lo script viene ricaricato tramite la funzione di hot reload dell’editor (<kbd>Edit ▸ Reload Resource</kbd>). È molto utile per il debug, i test e la messa a punto. Scopri [di più sull’hot reload](/it/manuals/hot-reload).
...
![Completamento automatico](/manuals/images/script/completion.png)