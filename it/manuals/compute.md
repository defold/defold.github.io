...
![File di calcolo](/manuals/images/compute/compute_file.png)

Il nuovo programma di calcolo si aprirà nel *Compute Editor*.

![Editor dei programmi di calcolo](/manuals/images/compute/compute.png)
...
![File di rendering con un programma di calcolo](/manuals/images/compute/compute_render_file.png)
...
L'array include soltanto le funzionalità supportate; non è una tabella con le costanti delle funzionalità come chiavi. Esegui sempre questa verifica prima di usare gli shader di calcolo quando il gioco può essere eseguito con adattatori grafici diversi o su dispositivi con un supporto dei driver variabile. Il supporto di OpenGL e OpenGL ES dipende dalla versione dell'API e dal driver. Vulkan e Metal tramite MoltenVK supportano gli shader di calcolo dalla versione 1.0. Usa un [manifest dell'applicazione](/it/manuals/app-manifest) per selezionare Vulkan sulle piattaforme in cui non è già il backend grafico predefinito.