...
Per ulteriori dettagli, consulta il [manuale Scrivere codice](/it/manuals/writing-code).
...
Per ulteriori informazioni, consulta il [manuale dell'API HTTP dell'editor](/it/manuals/editor-http-api/#reading-console-output).

### Log persistenti {#persisted-logs}

Defold può anche salvare il log del gioco abilitando `Write Log File` in `game.project`. Consulta [Log di gioco e di sistema](/it/manuals/debugging-game-and-system-logs/). La registrazione su file è utile per le applicazioni distribuite in pacchetti e per testare dispositivi di destinazione sui quali la console dell'editor non è disponibile.
...
Per ulteriori dettagli, consulta il [manuale del servizio del motore](/it/manuals/engine-service/#automation-bridge-extension).
...
L'editor può creare e servire una build HTML5 tramite il comando corrente `build-html5`, come descritto nel [manuale dell'API HTTP dell'editor](/it/manuals/editor-http-api/#building-html5). Bob può inoltre creare un bundle HTML5 senza l'editor.
...
Il [manuale di Bob](/it/manuals/bob) descrive piattaforme, file di impostazioni, bundle, cache, estensioni native e report di build.
...
Lo stesso formato dovrebbe poter essere utilizzato da uno sviluppatore, uno script locale, un servizio CI o un [agente di programmazione IA](/it/manuals/ai-agents). In questo modo, la verifica resta deterministica anche quando la diagnosi o la correzione viene delegata.