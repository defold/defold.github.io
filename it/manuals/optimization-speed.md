...
- anchor: optimizing-runtime-speed
  title: Ottimizzare la velocità di esecuzione
- anchor: reduce-script-execution-time
  title: Riduci il tempo di esecuzione degli script
- anchor: use-reactive-code-patterns
  title: Usa schemi di programmazione reattiva
- anchor: reduce-garbage-collection
  title: Riduci gli interventi del garbage collector
- anchor: prehash-message-and-action-ids
  title: Precalcola gli hash degli ID di messaggi e azioni
- anchor: prefer-and-cache-urls
  title: Preferisci gli URL e memorizzali nella cache
- anchor: reduce-time-it-takes-to-render-a-frame
  title: Riduci il tempo necessario per il rendering di un fotogramma
- anchor: reduce-scene-graph-complexity
  title: Riduci la complessità del grafo della scena
- anchor: frustum-culling
  title: Esclusione degli elementi fuori dal frustum
- anchor: platform-specific-optimizations
  title: Ottimizzazioni specifiche per piattaforma
- Android Device Performance Framework
---

# Ottimizzare la velocità di esecuzione {#optimizing-runtime-speed}
Prima di provare a ottimizzare un gioco per ottenere una frequenza dei fotogrammi elevata e stabile, devi sapere dove si trovano i colli di bottiglia. Quale operazione richiede effettivamente più tempo in un fotogramma del tuo gioco? Il rendering? La logica di gioco? Il grafo della scena? Per scoprirlo, è consigliabile usare gli strumenti di profilazione integrati. Usa il [profilatore a schermo o web](/it/manuals/profiling/) per campionare le prestazioni del gioco e poi decidere se ottimizzare qualcosa e su cosa intervenire. Quando avrai capito meglio quali operazioni richiedono tempo, potrai iniziare ad affrontare i problemi.
