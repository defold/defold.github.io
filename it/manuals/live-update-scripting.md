...
toc:
- anchor: scripting-live-update
  title: Utilizzo di Live Update tramite script
- anchor: get-mounts
  title: Ottenere i punti di montaggio
- anchor: scripting-with-excluded-collection-proxies
  title: Utilizzo dei proxy di collezione esclusi tramite script
---
...
Dopo aver montato un pacchetto, puoi anche reindirizzare un proxy escluso e non caricato a un'altra collezione compilata con `collectionproxy.set_collection()`. Consulta [Modificare la collezione di un proxy escluso](/it/manuals/collection-proxy/#changing-an-excluded-proxys-collection) per le restrizioni e la sequenza di caricamento.
