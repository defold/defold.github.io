...
- anchor: when-an-ai-agent-is-useful
  title: Quando è utile un agente IA
- anchor: model-neutral-defold-interfaces
  title: Interfacce Defold indipendenti dal modello
- anchor: integration-layers
  title: Livelli di integrazione
- Model Context Protocol
- anchor: community-mcp-integrations
  title: Integrazioni MCP della comunità
- anchor: project-instructions
  title: Istruzioni del progetto
- anchor: documentation-discovery
  title: Individuazione della documentazione
- anchor: bounded-change-and-verification-loops
  title: Cicli circoscritti di modifica e verifica
- anchor: multimodal-evaluation
  title: Valutazione multimodale
- anchor: security-isolation-and-good-practices
  title: Sicurezza, isolamento e buone pratiche
---
...
* Gli [script dell'editor](/it/manuals/editor-scripts) possono fornire operazioni sulle risorse e strumenti specifici del progetto.
* L'[API HTTP dell'editor](/it/manuals/editor-http-api) fornisce comandi dell'editor, risultati di build, output della console, ricerca nella documentazione di riferimento, anteprime, preferenze e route degli script dell'editor.
* Il [servizio del motore e le API di automazione a runtime](/it/manuals/engine-service) forniscono lo stato del motore di debug in esecuzione, input, schermate e operazioni definite dalle estensioni.
* [Bob](/it/manuals/bob) fornisce build da riga di comando, report, archivi e bundle.
...
Gli agenti dovrebbero seguire lo stesso [ciclo di ispezione, modifica, verifica e valutazione](/it/manuals/automation/#the-automation-loop) di qualsiasi altra automazione.
...
Le buone pratiche per i test automatici e la verifica sono descritte in [questo manuale](/it/manuals/automated-testing).

## Valutazione multimodale {#multimodal-evaluation}

Un agente con input di immagini può ispezionare le [anteprime dell'editor](/it/manuals/editor-http-api/#rendering-scene-previews), le schermate a runtime, le differenze visive e le acquisizioni dal browser.
...
Per ulteriori informazioni sulle anteprime dell'editor, sulle schermate a runtime e sull'ispezione visiva, consulta [questo manuale](/it/manuals/automated-testing).
