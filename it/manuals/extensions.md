...
: Questa cartella facoltativa contiene le eventuali librerie compilate da cui dipende l'estensione. I file delle librerie vanno collocati in sottocartelle denominate secondo lo schema `platform` o `architecture-platform`, a seconda delle architetture supportate dalle librerie.
  ...
  {% include shared/it/platforms.md %}
...
* `android` - Questa cartella accetta un frammento di file manifest da unire a quello dell'applicazione principale ([come descritto qui](/it/manuals/extensions-manifest-merge-tool)).
  * La cartella può contenere anche un file `build.gradle` con dipendenze da [risolvere tramite Gradle](/it/manuals/extensions-gradle).
...
* `ios` - Questa cartella accetta un frammento di file manifest da unire a quello dell'applicazione principale ([come descritto qui](/it/manuals/extensions-manifest-merge-tool)).
  * La cartella può contenere anche un file `Podfile` con dipendenze da [risolvere tramite Cocoapods](/it/manuals/extensions-cocoapods).
* `osx` - Questa cartella accetta un frammento di file manifest da unire a quello dell'applicazione principale ([come descritto qui](/it/manuals/extensions-manifest-merge-tool)).
* `web` - Questa cartella accetta un frammento di file manifest da unire a quello dell'applicazione principale ([come descritto qui](/it/manuals/extensions-manifest-merge-tool)).
...
Queste regole vengono combinate con il file di conservazione selezionato nel progetto quando [R8 è abilitato](/it/manuals/android/#enabling-r8).
...
Per esempio, inserisci un file JSON in `/myextension/data/settings.json`. Il percorso è relativo alla radice del progetto e include la cartella dell'estensione. Quando condividi l'estensione come libreria, includi `myextension` in [Include Dirs](/it/manuals/libraries/#setting-up-library-sharing) della libreria, affinché i progetti che la usano ricevano l'estensione e i suoi dati.
...
![File manifest](/manuals/images/extensions/manifest.png)
...
![File C++](/manuals/images/extensions/cppfile.png)
