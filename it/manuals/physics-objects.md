...
toc:
- anchor: collision-objects
  title: Oggetti di collisione
- anchor: adding-a-collision-object-component
  title: Aggiungere un componente oggetto di collisione
- anchor: adding-a-collision-shape
  title: Aggiungere una forma di collisione
- anchor: collision-object-properties
  title: Proprietà delloggetto di collisione
- anchor: runtime-properties
  title: Proprietà a runtime
---
...
2. <kbd>Fai clic con il pulsante destro</kbd> sul nuovo componente e seleziona <kbd>Add Shape</kbd>, poi scegli una forma: <kbd>Box</kbd>, <kbd>Capsule</kbd>, <kbd>Sphere</kbd>, <kbd>Hull</kbd> o <kbd>Mesh</kbd> nei progetti con fisica 3D, <kbd>Box</kbd> o <kbd>Circle</kbd> nei progetti con fisica 2D. Le forme Hull e Mesh sono disponibili da Defold 1.13.2 e usano una mesh con nome da una scena glTF o GLB. Puoi aggiungere più forme al componente. Puoi anche usare una tilemap o una risorsa `.convexshape` tramite la proprietà *Collision Shape*.
...
![Oggetto di collisione fisica](/manuals/images/physics/collision_object.png)


## Aggiungere una forma di collisione {#adding-a-collision-shape}

Un componente di collisione può contenere più forme incorporate, inclusi inviluppi convessi e mesh triangolari nella fisica 3D, oppure usare una tilemap o una risorsa di forma convessa. Per saperne di più sulle varie forme e su come aggiungerle a un componente di collisione, consulta il [manuale delle forme di collisione](/it/manuals/physics-shapes).
...
Collision Shape
: Una tilemap o una risorsa `.convexshape`. Per usare una mesh glTF o GLB, aggiungi al componente una forma Hull o Mesh e imposta invece le proprietà *Scene* e *Mesh* di quella forma. Per ulteriori informazioni, consulta il [manuale delle forme di collisione](/it/manuals/physics-shapes).
...
Group
: Il nome del gruppo di collisione a cui deve appartenere l'oggetto. Puoi avere 16 gruppi diversi e nominarli come ritieni opportuno per il tuo gioco. Per esempio `players`, `bullets`, `enemies` e `world`. Se *Collision Shape* è impostato su una mappa di tile, questo campo non viene usato e i nomi dei gruppi vengono ricavati dalla sorgente di tile. [Scopri di più sui gruppi di collisione](/it/manuals/physics-groups).

Mask
: Gli altri _gruppi_ con cui questo oggetto deve collidere. Puoi indicare un gruppo oppure specificarne più di uno in un elenco separato da virgole. Se lasci il campo *Mask* vuoto, l'oggetto non colliderà con nulla. [Scopri di più sui gruppi di collisione](/it/manuals/physics-groups).
